...
const datatableState={meta:null,data:null,busy:false,stage:"overview",from:"",to:"",shiftId:"",search:"",sla:"ALL",status:"ALL",door:"",actor:"",sort:"start_desc",page:1,limit:25,problemOnly:false,rejectedOnly:false,searchTimer:0,columns:new Set(JSON.parse(localStorage.getItem("wvf_dt_columns")||'["company","plate","actor"]'))};
...
async function init() {
  ...
  if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js?v=20260812-r93",{updateViaCache:"none"}).catch(() => undefined);
...
function renderNavigation() {
  ...
  if (role !== "INBOUND") items.push(["datatable","▤","Datatable"]);
...
async function refreshLiveData(){
  ...
  if(state.view==="datatable"){loadDatatable(false);return}
...
async function rejectReceiving(vehicle){
  ...
  receivingState.busyIds.add(autoId);renderCurrentView();const idempotencyKey=createIdempotencyKey();
  try{
    ...
    mergeVehicleUpdate(response.vehicle);const data=await api("/api/vehicles/active");applyVehicleData(data);playFeedbackSound(response.duplicate?"duplicate":"success");renderCurrentView();
...
async function runQueueCall(vehicle,payload){
  const autoId=String(vehicle.auto_id);receivingState.busyIds.add(autoId);renderOperations();
...
function datatableStageLabels(key=datatableState.stage){return({overview:["Gate In","สถานะล่าสุด"],gate_to_document:["Gate In","ยื่นเอกสาร"],document_review:["ยื่นเอกสาร","ตรวจเสร็จ"],ready:["พร้อมตรวจรับ","เริ่มตรวจรับ"],receiving:["เริ่มตรวจรับ","รับสินค้าเสร็จ"],return:["เริ่มรอคืนเอกสาร","คืนเอกสาร"],gate_out:["เริ่มรอออก","Gate Out"],total:["Gate In","Gate Out"]})[key]||["เริ่ม","สิ้นสุด"]}
...
function renderDatatableLayout(){
  const meta=datatableState.meta||{},stages=meta.stages||[],stageMeta=datatableStageMeta(),shiftOptions=(meta.shifts||[]).map(s=>`<option value="${escapeHtml(s.shift_id)}">${escapeHtml(s.shift_name)}</option>`).join(""),doorOptions=(meta.doors||[]).map(d=>`<option value="${escapeHtml(d.door_code)}">${escapeHtml(d.door_code)}</option>`).join(""),actorOptions=(meta.actors||[]).map(a=>`<option value="${escapeHtml(a.name)}">${escapeHtml(a.name)}</option>`).join("");
  $("pageContent").innerHTML=`<section class="dt-page">
    <header class="dt-page-head"><div class="dt-head-main"><div class="dt-head-title"><h2>ศูนย์ข้อมูล Datatable</h2><span>ติดตามทุกช่วงเวลา ดูค่าเตือนจากที่ Admin ตั้งไว้ และสั่งการจากตารางได้ทันที</span></div><div class="dt-head-strip"><div class="dt-head-pill"><small>ช่วงงานที่เลือก</small><b id="dtStageInfo">${escapeHtml(stageMeta.label)}</b><span id="dtStageInfoNote">${escapeHtml(stageMeta.description||"สรุปข้อมูลทุกช่วง")}</span></div><div class="dt-head-pill"><small>ข้อมูลที่กำลังแสดง</small><b id="dtLoadedInfo">กำลังโหลด</b><span id="dtLoadedInfoNote">เตรียมข้อมูลจาก Datatable</span></div></div></div><div class="dt-range-controls"><label><span>จาก</span><input id="dtFrom" type="date" value="${escapeHtml(datatableState.from)}"></label><label><span>ถึง</span><input id="dtTo" type="date" value="${escapeHtml(datatableState.to)}"></label><label><span>กะ</span><select id="dtShift"><option value="">ทั้งหมด</option>${shiftOptions}</select></label><button id="dtRefresh" class="quiet-button" type="button">รีเฟรช</button><button id="dtFullscreen" class="outline-button" type="button">เต็มหน้าจอ</button><button id="dtExport" class="primary" type="button">ส่งออก</button></div></header>
    <section id="dtSummary" class="dt-summary"></section>
    <nav id="dtStageTabs" class="dt-stage-tabs">${stages.map(s=>`<button type="button" data-dt-stage="${s.key}" class="${datatableState.stage===s.key?"active":""}"><span>${escapeHtml(s.label)}</span></button>`).join("")}</nav>
    <section class="dt-workspace"><main class="dt-main"><div class="dt-filters"><div class="dt-search"><input id="dtSearch" value="${escapeHtml(datatableState.search)}" placeholder="ค้นหาเลขนัดหมาย / บริษัท / ทะเบียน / คนขับ"><span aria-hidden="true">⌕</span></div><select id="dtSla"><option value="ALL">SLA: ทุกระดับ</option>${["NORMAL","WATCH","WARNING","URGENT","CRITICAL"].map(v=>`<option value="${v}">${alertLevelLabel(v)}</option>`).join("")}</select><select id="dtStatus"><option value="ALL">สถานะ: ทั้งหมด</option><option value="ACTIVE">กำลังดำเนินการ</option><option value="CLOSED">เสร็จสิ้น</option><option value="REJECTED">ปฏิเสธรับสินค้า</option>${Object.keys(DATATABLE_STATUS_OPTIONS).map(v=>`<option value="${v}">${DATATABLE_STATUS_OPTIONS[v]}</option>`).join("")}</select><select id="dtDoor"><option value="">ประตู: ทั้งหมด</option>${doorOptions}</select><select id="dtActor"><option value="">ผู้ดำเนินการ: ทั้งหมด</option>${actorOptions}</select></div>
      <div class="dt-tools"><select id="dtSort"><option value="start_desc">เวลาเริ่ม ใหม่→เก่า</option><option value="start_asc">เวลาเริ่ม เก่า→ใหม่</option><option value="duration_desc">ใช้เวลานาน→สั้น</option><option value="duration_asc">ใช้เวลาสั้น→นาน</option><option value="appointment_asc">เลขนัดหมาย น้อย→มาก</option><option value="appointment_desc">เลขนัดหมาย มาก→น้อย</option><option value="company_asc">บริษัท A→Z</option></select><button id="dtReset" class="quiet-button" type="button">รีเซ็ตตัวกรอง</button><button id="dtProblem" class="outline-button ${datatableState.problemOnly?"is-active":""}" type="button">เฉพาะมีปัญหา</button><button id="dtColumns" class="outline-button" type="button">เลือกคอลัมน์</button></div>
      ...
      <aside class="dt-side"><section class="dt-side-card"><header><b>เมื่อกดแต่ละเมนูจะแสดงอะไร</b></header><div id="dtStageGuide" class="dt-stage-guide"></div></section><section class="dt-side-card dt-rule-card"><header><div><b>เกณฑ์ปัจจุบันที่ Admin ตั้งไว้</b><small>ใช้ค่าจริงจากระบบ</small></div></header><div id="dtRules"></div></section><section class="dt-side-card"><header><b>Quick Access / สั่งการด่วน</b></header><div class="dt-quick"><button data-dt-quick="critical">ดูเฉพาะวิกฤต</button><button data-dt-quick="rejected">เปิดรายการปฏิเสธ</button><button data-dt-quick="receiving">ติดตามงานรับสินค้า</button><button data-dt-quick="export">ส่งออกรายวันนี้</button></div></section><section class="dt-side-card"><header><b>กิจกรรมล่าสุด</b><span id="dtActivityTime"></span></header><div id="dtActivity" class="dt-activity"><div class="loading">กำลังโหลด</div></div></section></aside></section>
  </section>`;
  ...
  renderDatatableSide();bindDatatableEvents();
}
...
function bindDatatableEvents(){
  ...
  $("dtFrom")?.addEventListener("change",()=>{datatableState.from=$("dtFrom").value;datatableState.page=1;loadDatatable(true)});$("dtTo")?.addEventListener("change",()=>{datatableState.to=$("dtTo").value;datatableState.page=1;loadDatatable(true)});$("dtShift")?.addEventListener("change",()=>{datatableState.shiftId=$("dtShift").value;datatableState.page=1;loadDatatable(true)});
  ...
  $("dtProblem")?.addEventListener("click",()=>{datatableState.problemOnly=!datatableState.problemOnly;datatableState.page=1;$("dtProblem").classList.toggle("is-active",datatableState.problemOnly);loadDatatable(true)});$("dtReset")?.addEventListener("click",resetDatatableFilters);$("dtRefresh")?.addEventListener("click",()=>loadDatatable(true));$("dtFullscreen")?.addEventListener("click",toggleFullscreen);$("dtExport")?.addEventListener("click",downloadDatatableExport);$("dtColumns")?.addEventListener("click",chooseDatatableColumns);
  ...
  $("dtTable")?.addEventListener("click",event=>{const detail=event.target.closest("[data-dt-detail]");if(detail)return showDatatableDetail(detail.dataset.dtDetail);const command=event.target.closest("[data-dt-command]");if(command)return runDatatableCommand(command.dataset.dtCommand)});
...
async function loadDatatable(reset=false){if(state.view!=="datatable"||datatableState.busy)return;if(reset)datatableState.page=1;datatableState.busy=true;const table=$("dtTable");if(table&&!datatableState.data)table.innerHTML=`<div class="loading">กำลังโหลดข้อมูล</div>`;try{const data=await api(`/api/datatable?${datatableParams().toString()}`);if(state.view!=="datatable")return;datatableState.data=data;if(datatableState.page>Number(data.pages||1)){datatableState.page=Number(data.pages||1);datatableState.busy=false;return loadDatatable(false)}renderDatatableData(data)}catch(error){if(table)table.innerHTML=`<div class="empty-state"><b>โหลดข้อมูลไม่สำเร็จ</b><span>${escapeHtml(error.message)}</span></div>`}finally{datatableState.busy=false}}
function renderDatatableData(data){renderDatatableSummary(data.summary||{});renderDatatableHeaderStatus(data);renderDatatableRows(data.items||[]);renderDatatablePager(data);renderDatatableActivity(data.activity||[],data.generatedAt);}
function renderDatatableSummary(summary){const el=$("dtSummary");if(!el)return;const cards=[["total","รถทั้งหมด",summary.total||0,"dt-blue"],["active","กำลังดำเนินการ",summary.active||0,"dt-orange"],["warnings","มีการแจ้งเตือน",summary.warnings||0,"dt-red"],["rejected","ปฏิเสธรับสินค้า",summary.rejected||0,"dt-rose"],["completed","เสร็จสิ้น",summary.completed||0,"dt-green"]];el.innerHTML=cards.map(([key,label,value,tone])=>`<button type="button" data-dt-summary="${key}" class="dt-summary-card ${tone}"><small>${label}</small><b>${Number(value).toLocaleString("th-TH")}</b><span>รายการ</span></button>`).join("");el.querySelectorAll("[data-dt-summary]").forEach(button=>button.addEventListener("click",()=>{const key=button.dataset.dtSummary;if(key==="total"){datatableState.status="ALL";datatableState.rejectedOnly=false;datatableState.problemOnly=false;datatableState.sla="ALL"}else if(key==="active"){datatableState.status="ACTIVE";datatableState.rejectedOnly=false;datatableState.problemOnly=false;datatableState.sla="ALL"}else if(key==="warnings"){datatableState.status="ALL";datatableState.problemOnly=true;datatableState.rejectedOnly=false;datatableState.sla="ALL"}else if(key==="rejected"){datatableState.stage="overview";datatableState.status="ALL";datatableState.rejectedOnly=true;datatableState.problemOnly=false;datatableState.sla="ALL";document.querySelectorAll("[data-dt-stage]").forEach(b=>b.classList.toggle("active",b.dataset.dtStage==="overview"));renderDatatableSide()}else if(key==="completed"){datatableState.status="CLOSED";datatableState.rejectedOnly=false;datatableState.problemOnly=false;datatableState.sla="ALL"}datatableState.page=1;if($("dtStatus"))$("dtStatus").value=datatableState.status;if($("dtSla"))$("dtSla").value=datatableState.sla;$("dtProblem")?.classList.toggle("is-active",datatableState.problemOnly);loadDatatable(true)}))}
function renderDatatableHeaderStatus(data){const total=Math.max(0,Number(data.total||0)),pages=Math.max(1,Number(data.pages||1)),page=Math.min(pages,Number(data.page||datatableState.page)),limit=Number(data.limit||datatableState.limit),from=total?(page-1)*limit+1:0,to=Math.min(total,page*limit),stageMeta=datatableStageMeta();if($("dtLoadedInfo"))$("dtLoadedInfo").textContent=total?`${from}-${to} / ${total.toLocaleString("th-TH")}`:"0 รายการ";if($("dtLoadedInfoNote"))$("dtLoadedInfoNote").textContent=datatableState.problemOnly?"กำลังกรองเฉพาะรายการมีปัญหา":datatableState.rejectedOnly?"กำลังกรองเฉพาะงานปฏิเสธ":`ช่วงวันที่ ${datatableState.from} ถึง ${datatableState.to}`;if($("dtStageInfo"))$("dtStageInfo").textContent=stageMeta.label||"ภาพรวม";if($("dtStageInfoNote"))$("dtStageInfoNote").textContent=stageMeta.description||"สรุปข้อมูลทุกช่วง";}
function datatableColumnOn(name){return datatableState.columns.has(name)}
function renderDatatableRows(items){const table=$("dtTable");if(!table)return;const overview=datatableState.stage==="overview",labels=datatableStageLabels();const optional=[datatableColumnOn("company")?`<span>บริษัท</span>`:"",datatableColumnOn("plate")?`<span>ทะเบียน</span>`:""].join("");const actorHead=datatableColumnOn("actor")?`<span>ผู้ดำเนินการ</span>`:"";const head=overview?`<div class="dt-row dt-head"><span>เลขนัดหมาย</span>${optional}<span>สถานะ</span><span>เวลารวม</span><span>SLA</span><span>ประตู</span>${actorHead}<span>สั่งการ</span><span>รายละเอียด</span></div>`:`<div class="dt-row dt-head"><span>เลขนัดหมาย</span>${optional}<span>${escapeHtml(labels[0])}</span><span>${escapeHtml(labels[1])}</span><span>ใช้เวลา</span><span>SLA</span>${actorHead}<span>สั่งการ</span><span>รายละเอียด</span></div>`;
  const body=items.length?items.map(row=>{const company=datatableColumnOn("company")?`<span class="dt-company" data-label="บริษัท">${escapeHtml(row.companyName||"-")}</span>`:"",plate=datatableColumnOn("plate")?`<span data-label="ทะเบียน">${escapeHtml(joinText(row.vehiclePlate,row.province))}</span>`:"",actor=datatableColumnOn("actor")?`<span data-label="ผู้ดำเนินการ">${escapeHtml(row.stageActor||"ระบบ")}</span>`:"",sla=`<span class="dt-sla" style="--dt-alert:${safeColor(row.stageAlertColor)}" data-label="SLA"><i></i><b>${alertLevelLabel(row.stageAlertLevel)}</b></span>`,command=`<button class="dt-command ${alertLevelRank(row.stageAlertLevel)>=3?"urgent":""}" type="button" data-dt-command="${escapeHtml(row.autoId)}">${datatableCommandLabel(row)}</button>`,detail=`<button class="dt-detail" type="button" data-dt-detail="${escapeHtml(row.autoId)}">เปิดข้อมูล</button>`;if(overview)return`<div class="dt-row"><b data-label="เลขนัดหมาย">${escapeHtml(row.appointmentNo||row.autoId)}</b>${company}${plate}<span data-label="สถานะ"><em class="dt-status ${statusTone(row.currentStatus)}">${statusLabel(row.currentStatus)}</em></span><b data-label="เวลารวม">${formatDuration(row.totalInSiteSeconds)}</b>${sla}<span data-label="ประตู">${escapeHtml(row.doorCode||"-")}</span>${actor}<span data-label="สั่งการ">${command}</span><span data-label="รายละเอียด">${detail}</span></div>`;return`<div class="dt-row"><b data-label="เลขนัดหมาย">${escapeHtml(row.appointmentNo||row.autoId)}</b>${company}${plate}<span data-label="${escapeHtml(labels[0])}">${formatDateShort(row.stageStartAt)}</span><span data-label="${escapeHtml(labels[1])}">${formatDateShort(row.stageEndAt)}</span><b data-label="ใช้เวลา">${row.stageSeconds==null?"-":formatDuration(row.stageSeconds)}</b>${sla}${actor}<span data-label="สั่งการ">${command}</span><span data-label="รายละเอียด">${detail}</span></div>`}).join(""):`<div class="empty-state"><b>ไม่พบข้อมูลตามตัวกรอง</b><span>ลองเปลี่ยนช่วงเวลา สถานะ หรือระดับ SLA</span></div>`;table.innerHTML=head+body;table.dataset.optionalColumns=String((datatableColumnOn("company")?1:0)+(datatableColumnOn("plate")?1:0)+(datatableColumnOn("actor")?1:0));table.classList.toggle("is-overview",overview);const headRow=table.querySelector(".dt-head");if(headRow)table.style.setProperty("--dt-columns",String(Math.max(1,headRow.children.length-1)))}
function formatDateShort(seconds){if(!seconds)return"-";const parts=new Intl.DateTimeFormat("en-GB",{timeZone:cfg.timezone,day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false}).formatToParts(new Date(Number(seconds)*1000)),p=Object.fromEntries(parts.map(x=>[x.type,x.value]));return`${p.day}/${p.month} ${p.hour}:${p.minute}:${p.second}`}
function datatableCommandLabel(row){if(row.currentStatus==="WAITING_DOCUMENT_CHECK"&&state.user?.accessRights==="ADMIN")return"ตรวจเอกสาร";if(["READY_FOR_RECEIVING","RECEIVING_IN_PROGRESS"].includes(row.currentStatus))return alertLevelRank(row.stageAlertLevel)>=3?"เร่งดำเนินการ":"ดำเนินการ";if(String(row.currentStatus||"").startsWith("REJECTED_"))return"ติดตาม";if(row.currentStatus==="CLOSED")return"ดูข้อมูล";return"ติดตาม"}
...
function renderDatatableSide(){const guide=$("dtStageGuide"),rules=$("dtRules"),stageMeta=datatableStageMeta();if($("dtStageInfo"))$("dtStageInfo").textContent=stageMeta.label||"ภาพรวม";if($("dtStageInfoNote"))$("dtStageInfoNote").textContent=stageMeta.description||"สรุปข้อมูลทุกช่วง";if(guide){guide.innerHTML=(datatableState.meta?.stages||[]).map(stage=>`<button type="button" data-guide-stage="${stage.key}" class="${stage.key===datatableState.stage?"active":""}"><b>${escapeHtml(stage.label)}</b><span>${escapeHtml(stage.description||"")}</span></button>`).join("");guide.querySelectorAll("[data-guide-stage]").forEach(button=>button.addEventListener("click",()=>document.querySelector(`[data-dt-stage="${button.dataset.guideStage}"]`)?.click()))}if(rules)rules.innerHTML=renderDatatableRules(datatableState.meta?.rules||[])}
function datatableRuleLabel(code){return({GATE_TO_DOCUMENT:"Gate In→ยื่นเอกสาร",DOCUMENT_REVIEW:"ตรวจเอกสาร",DOCUMENT_TO_RECEIVING_START:"พร้อมตรวจรับ (ขั้นตอนเดิม)",DOCUMENT_CHECKED_TO_RECEIVING_START:"พร้อมตรวจรับ",RECEIVING_DURATION:"ตรวจรับสินค้า",RECEIVING_TO_RETURN:"คืนเอกสาร",RETURN_TO_GATE_OUT:"Gate Out",TOTAL_IN_SITE:"เวลารวม"})[code]||code}
function renderDatatableRules(rules){const order=["NORMAL","WATCH","WARNING","URGENT","CRITICAL"],groups={};for(const rule of rules){(groups[rule.stage_code]??=[]).push(rule)}return Object.entries(groups).map(([code,items])=>`<article class="dt-rule-row ${datatableStageMeta().ruleCodes?.includes(code)?"active":""}"><b>${escapeHtml(datatableRuleLabel(code))}</b><div>${order.map(level=>{const r=items.find(x=>x.level_code===level);return r?`<span style="--dt-rule:${safeColor(r.color)}"><i></i><em>${alertLevelLabel(level)}</em><strong>${Math.round(Number(r.start_seconds||0)/60)}น.</strong></span>`:""}).join("")}</div></article>`).join("")||`<div class="empty-state">ยังไม่มีเกณฑ์แจ้งเตือน</div>`}
function renderDatatableActivity(items,generatedAt){const el=$("dtActivity");if(!el)return;const allowed=new Set(["GATE_IN","DOCUMENT_SUBMITTED","DOCUMENT_CHECKED","RECEIVING_STARTED","RECEIVING_COMPLETED","RECEIVING_REJECTED","DOCUMENT_RETURNED","GATE_OUT"]),rows=items.filter(item=>allowed.has(item.event_code)).slice(0,7);el.innerHTML=rows.map(item=>`<div><time>${formatDateShort(item.event_time)}</time><span><b>${datatableEventLabel(item.event_code)}</b><small>${escapeHtml(item.appointment_no||item.auto_id)} · ${escapeHtml(item.actor||"ระบบ")}</small></span></div>`).join("")||`<div class="empty-state">ยังไม่มีกิจกรรมในช่วงนี้</div>`;if($("dtActivityTime"))$("dtActivityTime").textContent=generatedAt?`อัปเดต ${formatDateShort(generatedAt)}`:""}
function datatableEventLabel(code){return({GATE_IN:"Gate In",DOCUMENT_SUBMITTED:"ยื่นเอกสาร",DOCUMENT_CHECKED:"ตรวจเอกสารเสร็จ",RECEIVING_STARTED:"เริ่มตรวจรับสินค้า",RECEIVING_COMPLETED:"รับสินค้าเสร็จ",RECEIVING_REJECTED:"ปฏิเสธรับสินค้า",DOCUMENT_RETURNED:"คืนเอกสารแล้ว",GATE_OUT:"Gate Out"})[code]||code}
...
async function chooseDatatableColumns(){if(!window.Swal)return;const options=[["company","บริษัท"],["plate","ทะเบียนรถ"],["actor","ผู้ดำเนินการ"]],html=`<div class="dt-column-chooser">${options.map(([key,label])=>`<label><input type="checkbox" data-dt-column="${key}" ${datatableColumnOn(key)?"checked":""}><span>${label}</span></label>`).join("")}</div>`;const result=await Swal.fire({title:"เลือกคอลัมน์เสริม",html,showCancelButton:true,confirmButtonText:"ใช้คอลัมน์นี้",cancelButtonText:"ยกเลิก",customClass:swalClasses(),buttonsStyling:false,width:380,preConfirm:()=>[...document.querySelectorAll("[data-dt-column]:checked")].map(el=>el.dataset.dtColumn)});if(!result.isConfirmed)return;datatableState.columns=new Set(result.value||[]);localStorage.setItem("wvf_dt_columns",JSON.stringify([...datatableState.columns]));renderDatatableRows(datatableState.data?.items||[])}
async function runDatatableCommand(autoId){const row=datatableState.data?.items?.find(item=>String(item.autoId)===String(autoId));if(!row)return;try{const active=await api("/api/vehicles/active");applyVehicleData(active);const vehicle=state.vehicles.find(v=>String(v.auto_id)===String(autoId));if(!vehicle)return showDatatableDetail(autoId);if(vehicle.current_status==="WAITING_DOCUMENT_CHECK"&&state.user?.accessRights==="ADMIN"){await confirmDocumentChecked(vehicle);return loadDatatable(true)}if(["READY_FOR_RECEIVING","RECEIVING_IN_PROGRESS"].includes(vehicle.current_status)){return showDatatableReceivingActions(vehicle)}return openDatatableTrack(autoId)}catch(error){await showNotice("error",error.message||"ไม่สามารถสั่งการรายการนี้ได้")}}
async function showDatatableReceivingActions(vehicle){if(!window.Swal)return;const progress=vehicle.current_status==="RECEIVING_IN_PROGRESS",called=Number(vehicle.queue_called_at||0)>0,buttons=progress?[["complete","รับสินค้าเสร็จ"],["reject","ปฏิเสธรับสินค้า"],["track","เปิด Track"]]:[[called?"recall":"call",called?"เรียกซ้ำ / เปลี่ยนประตู":"เรียกรถ"],["start","เริ่มตรวจรับ"],["reject","ปฏิเสธรับสินค้า"],["track","เปิด Track"]];const result=await Swal.fire({title:`สั่งการ ${escapeHtml(vehicle.appointment_no||vehicle.auto_id)}`,html:`<div class="dt-action-menu">${buttons.map(([key,label])=>`<button type="button" data-dt-action="${key}">${escapeHtml(label)}</button>`).join("")}</div>`,showConfirmButton:false,showCancelButton:true,cancelButtonText:"ปิด",customClass:swalClasses(),buttonsStyling:false,width:430,didOpen:()=>document.querySelectorAll("[data-dt-action]").forEach(btn=>btn.addEventListener("click",()=>{Swal.close();const action=btn.dataset.dtAction;if(action==="call")callVehicle(vehicle,false).then(()=>loadDatatable(true));else if(action==="recall")callVehicle(vehicle,true).then(()=>loadDatatable(true));else if(action==="start")startReceiving(vehicle).then(()=>loadDatatable(true));else if(action==="complete")completeReceiving(vehicle).then(()=>loadDatatable(true));else if(action==="reject")rejectReceiving(vehicle).then(()=>loadDatatable(true));else if(action==="track")openDatatableTrack(vehicle.auto_id)}))});return result}
async function openDatatableTrack(autoId){try{const result=await api("/api/track/link",{method:"POST",body:{search:autoId}}),token=String(result?.token||"").trim();if(!token)throw new Error("ไม่สามารถสร้างลิงก์ Track ได้");window.open(new URL(`./track.html?t=${encodeURIComponent(token)}&v=20260812-r93`,location.href).href,"_blank","noopener")}catch(error){await showNotice("error",error.message||"เปิด Track ไม่สำเร็จ")}}
async function showDatatableDetail(autoId){try{if(window.Swal)Swal.fire({title:"กำลังโหลดรายละเอียด",allowOutsideClick:false,allowEscapeKey:false,didOpen:()=>Swal.showLoading(),showConfirmButton:false,customClass:swalClasses(),width:420});const data=await api(`/api/datatable/detail?autoId=${encodeURIComponent(autoId)}`),v=data.vehicle||{},events=data.events||[],d=data.durations||{},rej=data.rejection;const stages=[["Gate In→ยื่นเอกสาร",d.gateToDocumentSeconds],["ตรวจเอกสาร",d.documentReviewSeconds],["พร้อม→เริ่มตรวจรับ",d.readyToReceivingSeconds],["เรียก→เริ่มตรวจรับ",d.calledToReceivingSeconds],["ตรวจรับสินค้า",d.receivingSeconds],["รอคืนเอกสาร",d.receivingToReturnSeconds],["รอ Gate Out",d.returnToGateOutSeconds],["เวลารวม",d.totalInSiteSeconds]];const html=`<div class="dt-detail-modal"><section class="dt-detail-identity"><div><small>เลขนัดหมาย</small><b>${escapeHtml(v.appointmentNo||v.autoId||autoId)}</b><span>${escapeHtml(v.companyName||"ไม่ระบุบริษัท")}</span></div><div><small>ทะเบียน</small><b>${escapeHtml(joinText(v.vehiclePlate,v.province))}</b><span>${escapeHtml(statusLabel(v.currentStatus))}</span></div></section><section class="dt-detail-durations">${stages.map(([label,value])=>`<div><small>${label}</small><b>${value==null?"-":formatDuration(value)}</b></div>`).join("")}</section>${rej?`<section class="dt-rejection-detail"><b>ปฏิเสธรับสินค้า</b><span>เหตุผล: ${escapeHtml(rej.reason||"-")}</span><span>หัวหน้างาน: ${escapeHtml(rej.supervisor||"-")}</span><span>${Number(rej.require_document_return)?"ต้องคืนเอกสาร":"ไม่ต้องคืนเอกสาร"}</span></section>`:""}<section class="dt-timeline"><h3>Timeline</h3>${events.map(e=>`<div><i></i><time>${formatDate(e.occurred_at)}</time><span><b>${datatableEventLabel(e.event_type)}</b><small>${escapeHtml(e.actor||"ระบบ")}</small></span></div>`).join("")}</section></div>`;if(window.Swal)await Swal.fire({title:"ข้อมูลนัดหมาย",html,confirmButtonText:"ปิด",customClass:{...swalClasses(),popup:"wfv-swal dt-detail-swal"},buttonsStyling:false,width:760})}catch(error){if(window.Swal)Swal.close();await showNotice("error",error.message||"โหลดรายละเอียดไม่สำเร็จ")}}
async function downloadDatatableExport(){if(datatableState.busy)return;try{if(window.Swal)Swal.fire({title:"กำลังเตรียมไฟล์ Datatable",allowOutsideClick:false,allowEscapeKey:false,didOpen:()=>Swal.showLoading(),showConfirmButton:false,customClass:swalClasses(),width:390});const data=await api(`/api/datatable/export?${datatableParams({page:false}).toString()}`),items=data.items||[],headers=["Auto ID","เลขนัดหมาย","บริษัท","คนขับ","ทะเบียน","จังหวัด","สถานะ","กะ","ประตู","Gate In","ยื่นเอกสาร","ตรวจเอกสารเสร็จ","เรียกครั้งแรก","เรียกล่าสุด","จำนวนเรียก","เริ่มตรวจรับ","รับสินค้าเสร็จ","ปฏิเสธรับสินค้า","เหตุผลปฏิเสธ","หัวหน้างานรับทราบ","คืนเอกสาร","Gate Out","Gate In→ยื่น (วินาที)","ตรวจเอกสาร (วินาที)","พร้อม→เริ่มตรวจรับ (วินาที)","เรียก→เริ่มตรวจรับ (วินาที)","ตรวจรับสินค้า (วินาที)","รอคืนเอกสาร (วินาที)","รอ Gate Out (วินาที)","เวลารวม (วินาที)","SLA ช่วงที่เลือก","ระดับ SLA","ผู้ดำเนินการช่วงที่เลือก"],rows=items.map(i=>[i.autoId,i.appointmentNo,i.companyName,i.driverName,i.vehiclePlate,i.province,statusLabel(i.currentStatus),i.shiftName,i.doorCode,formatDate(i.gateInAt),formatDate(i.documentSubmittedAt),formatDate(i.documentCheckedAt),formatDate(i.firstCalledAt),formatDate(i.lastCalledAt),i.queueCallCount,formatDate(i.receivingStartedAt),formatDate(i.receivingCompletedAt),formatDate(i.rejectedAt),i.rejectionReason,i.rejectionSupervisor,formatDate(i.documentReturnedAt),formatDate(i.gateOutAt),i.gateToDocumentSeconds,i.documentReviewSeconds,i.readyToReceivingSeconds,i.calledToReceivingSeconds,i.receivingSeconds,i.receivingToReturnSeconds,i.returnToGateOutSeconds,i.totalInSiteSeconds,i.stageRuleCode,alertLevelLabel(i.stageAlertLevel),i.stageActor]);const csv="\uFEFF"+[headers,...rows].map(row=>row.map(csvCell).join(",")).join("\r\n"),blob=new Blob([csv],{type:"text/csv;charset=utf-8"}),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=`Datatable_${datatableState.from}_${datatableState.to}_${datatableState.stage}.csv`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),2000);if(window.Swal)await Swal.fire({icon:"success",title:"สร้างไฟล์ Datatable แล้ว",text:`${items.length.toLocaleString("th-TH")} รายการ${data.truncated?" (จำกัด 5,000 รายการ)":""}`,timer:1800,showConfirmButton:false,customClass:swalClasses(),width:420})}catch(error){if(window.Swal)Swal.close();await showNotice("error",error.message||"ส่งออก Datatable ไม่สำเร็จ")}}

function renderDashboard() {
  if(!["ADMIN","USER"].includes(state.user?.accessRights))return navigate("inbound");
  $("pageContent").innerHTML=`<section class="opsdash-head"><div class="opsdash-title"><button id="dashboardMenuButton" class="dashboard-menu-button" aria-label="เปิดเมนู" aria-expanded="false">☰</button><div><span>ภาพรวมคลังสินค้า</span><h2>ศูนย์ควบคุมการปฏิบัติงาน</h2><p id="dashboardRangeLabel">กำลังโหลดข้อมูล</p></div><button type="button" class="dashboard-global-info" aria-label="ศูนย์รวมคำอธิบาย Dashboard">i</button></div><div class="opsdash-filter">${["ADMIN","USER"].includes(state.user?.accessRights)?`<button id="dashboardQueueButton" class="dashboard-queue-button" type="button" aria-label="เปิดจอคิว"><span aria-hidden="true">▣</span><b>จอคิว</b></button>`:""}<div class="dashboard-theme-wrap"><button id="dashboardThemeButton" class="dashboard-theme-button" type="button" aria-label="เลือกธีม" aria-expanded="false"><i aria-hidden="true"></i><span>ธีม</span></button><div id="dashboardThemeMenu" class="dashboard-theme-menu" hidden><b>เลือกธีม</b><button type="button" data-dashboard-theme="blue"><i></i><span>ฟ้าเรียบง่าย</span></button><button type="button" data-dashboard-theme="green"><i></i><span>เขียวสบายตา</span></button><button type="button" data-dashboard-theme="dark"><i></i><span>เข้มทันสมัย</span></button><button type="button" data-dashboard-theme="warm"><i></i><span>อบอุ่นนุ่มนวล</span></button></div></div><button id="dashboardCalendarButton" class="calendar-button" aria-label="เลือกวันที่"><span class="calendar-icon" aria-hidden="true"><i></i><i></i><strong>31</strong></span><b id="dashboardDateButtonLabel" data-mobile-label="วันที่">เลือกวันที่</b></button><div class="range-buttons"><button data-dashboard-range="today" data-mobile-label="1วัน" aria-label="แสดงข้อมูลรายวัน">วัน</button><button data-dashboard-range="7d" data-mobile-label="7วัน" aria-label="แสดงข้อมูล 7 วัน">7 วัน</button><button data-dashboard-range="30d" data-mobile-label="30วัน" aria-label="แสดงข้อมูล 30 วัน">30 วัน</button></div><select id="dashboardShift" aria-label="เลือกกะ"><option value="">ทุกกะ</option></select><button id="dashboardFullscreen" class="outline-button">เต็มหน้าจอ</button><button id="dashboardMoreButton" class="dashboard-more-button" type="button" aria-label="ตัวเลือก Dashboard เพิ่มเติม" aria-expanded="false">⋮</button></div><div id="dashboardMobileMenu" class="dashboard-mobile-menu" hidden>${["ADMIN","USER"].includes(state.user?.accessRights)?`<button id="dashboardMobileQueue" type="button"><b>▣</b><span>เปิดจอคิว</span></button>`:""}<button id="dashboardMobileFullscreen" type="button"><b>⛶</b><span>เต็มหน้าจอ</span></button><button id="dashboardMobileInfo" type="button"><b>i</b><span>ศูนย์รวมคำอธิบาย</span></button><button id="dashboardMobileTheme" type="button"><b>◉</b><span>เลือกธีม</span></button></div></section><section id="dashboardCalendarPopover" class="dashboard-calendar-popover" hidden></section><div id="dashboardBody" class="loading">กำลังสรุปข้อมูล</div>`;
...
function selectDashboardDate(date){dashboardState.date=date;dashboardState.calendarMonth=date.slice(0,7);dashboardState.lastLoadedAt=0;const popover=$("dashboardCalendarPopover");if(popover)popover.hidden=true;loadDashboard(true,true)}
...
function renderAdminShell(){
  const tabs=[["users","ผู้ใช้งาน"],["workflow","ขั้นตอนงาน"],["doors","ประตู"],["shifts","กะทำงาน"],["alerts","เวลาแจ้งเตือน"],["rejections","ปฏิเสธรับสินค้า"],["recall","เรียกรถซ้ำ"],["data","การใช้ข้อมูล"],["export","ส่งออกข้อมูล"],["tracking","ติดตามคนขับ"],["voice","เสียงประกาศ"],["queue","จอคิว"]];
...
function clearSession() { stopCamera(); inboundLiveState.version="";inboundLiveState.checking=false;inboundLiveState.failures=0;inboundLiveState.nextAllowedAt=0;alertSoundState.initialized=false;alertSoundState.levels.clear();sessionStorage.removeItem("wvf_token"); state.token=""; state.user=null; $("appView").hidden=true; $("loginView").hidden=false; $("loginPassword").value=""; window.scrollTo(0, 0); }
