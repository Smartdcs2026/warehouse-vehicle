...
const FRONTEND_BUILD="2026.08.18-round178-appointment-production-default-off";
...
function renderAdminAppointmentData(){
  const cfg=adminState.data?.appointmentModule||{moduleEnabled:false,uploadEnabled:false,useImportedData:false,timeReference:"PERIOD",searchWindowHours:36,adapterTimeoutMs:250,display:{}},schema=adminState.data?.appointmentSchema||{ready:false,tablesReady:0,tablesTotal:5,indexReady:false,latest:null};
  const pages=[["inbound","Inbound"],["receiving","Receiving"],["datatable","Datatable"],["dashboard","Dashboard"],["queue","จอคิว"],["track","Track"]],latest=schema.latest;
  ...
  $("adminPanel").innerHTML=`<div class="admin-section-head clean-admin-head"><div><h3>ข้อมูลนัดหมาย</h3><p>เตรียมโมดูลแผนนัดหมายสำหรับระบบหลัก โดยค่าเริ่มต้นปิดทั้งหมดและไม่ Query ข้อมูลงาน</p></div></div><section class="system-health-summary ${schema.ready?"pass":"warn"}"><div><small>ฐานข้อมูลนัดหมาย</small><b>${schema.ready?"พร้อม":"ยังไม่พร้อม"}</b><span>ตาราง ${Number(schema.tablesReady||0)}/${Number(schema.tablesTotal||5)} · Index ${schema.indexReady?"พร้อม":"ยังไม่มี"}</span></div><div><small>ชุดข้อมูลล่าสุด</small><b>${latest?`${escapeHtml(latest.snapshotDate||"")} ${escapeHtml(latest.snapshotTime||"")}`:"ยังไม่มีข้อมูล"}</b><span>${latest?.fileName?escapeHtml(latest.fileName):"รอนำเข้าไฟล์นัดหมาย"}</span></div></section><form id="appointmentModuleForm" class="display-settings-form"><section class="display-setting-card"><div><span class="display-setting-icon dashboard-icon">A</span><div><b>เปิดโมดูลข้อมูลนัดหมาย</b><small>เมื่อปิด ระบบหลักทำงานแบบเดิมและไม่ค้นข้อมูลนัดหมาย</small></div></div><label class="clean-toggle"><input id="appointmentModuleEnabled" type="checkbox" ${cfg.moduleEnabled?"checked":""}><span></span></label></section><section class="display-setting-card"><div><span class="display-setting-icon datatable-icon">↑</span><div><b>เปิดการอัปโหลดไฟล์</b><small>ใช้ได้เฉพาะ ADMIN และ USER เมื่อเชื่อมหน้าการนำเข้าแล้ว</small></div></div><label class="clean-toggle"><input id="appointmentUploadEnabled" type="checkbox" ${cfg.uploadEnabled?"checked":""}><span></span></label></section><section class="display-setting-card"><div><span class="display-setting-icon dashboard-icon">↔</span><div><b>ใช้ข้อมูลนัดหมายกับงานจริง</b><small>เมื่อปิด จะไม่ Query Appointment ในเส้นทางงานรถ</small></div></div><label class="clean-toggle"><input id="appointmentUseImportedData" type="checkbox" ${cfg.useImportedData?"checked":""}><span></span></label></section><div class="appointment-rule-grid"><label><span>เวลาอ้างอิง</span><select id="appointmentTimeReference"><option value="PERIOD">PERIOD</option><option value="FROM">FROM</option></select><small>ใช้คำนวณมาก่อน / ช้า</small></label><label><span>ช่วงค้นหานัดหมาย</span><input id="appointmentSearchWindow" type="number" min="1" max="168" value="${Number(cfg.searchWindowHours||36)}"><small>ชั่วโมง</small></label><label><span>เวลารอข้อมูลสูงสุด</span><input id="appointmentAdapterTimeout" type="number" min="50" max="2000" step="10" value="${Number(cfg.adapterTimeoutMs||250)}"><small>มิลลิวินาที · หากเกินให้ Gate In ทำงานต่อ</small></label></div><div class="admin-section-head clean-admin-head"><div><h3>การแสดงข้อมูลแต่ละหน้า</h3><p>เมื่อปิดช่องใด หน้านั้นจะไม่แสดงข้อมูลส่วนนั้น</p></div></div>${pageCards}<div class="display-setting-note"><b>ค่าเริ่มต้น Production</b><span>โมดูล ปิด · อัปโหลด ปิด · ใช้ข้อมูลกับงานจริง ปิด ดังนั้นการ Deploy รอบนี้ยังไม่เปลี่ยนขั้นตอนงานเดิม</span></div><div class="admin-form-actions"><button class="primary" type="submit">บันทึกการตั้งค่า</button></div></form>`;
  $("appointmentTimeReference").value=cfg.timeReference==="FROM"?"FROM":"PERIOD";
  const sync=()=>{const on=$("appointmentModuleEnabled").checked;$("appointmentUploadEnabled").disabled=!on;$("appointmentUseImportedData").disabled=!on;document.querySelectorAll("#appointmentModuleForm select,#appointmentModuleForm input[type=number],[data-appointment-page] input").forEach(el=>el.disabled=!on);if(!on){$("appointmentUploadEnabled").checked=false;$("appointmentUseImportedData").checked=false}};$("appointmentModuleEnabled")?.addEventListener("change",sync);sync();
  $("appointmentModuleForm")?.addEventListener("submit",async event=>{event.preventDefault();if(adminState.busy)return;const display={};document.querySelectorAll("[data-appointment-page]").forEach(card=>{const key=card.dataset.appointmentPage,read=f=>card.querySelector(`[data-ap-field="${f}"]`)?.checked===true;display[key]={enabled:read("enabled"),timing:read("timing"),vendor:read("vendor"),carrier:read("carrier"),po:read("po")}});const body={moduleEnabled:$("appointmentModuleEnabled").checked,uploadEnabled:$("appointmentUploadEnabled").checked,useImportedData:$("appointmentUseImportedData").checked,timeReference:$("appointmentTimeReference").value,searchWindowHours:Number($("appointmentSearchWindow").value),adapterTimeoutMs:Number($("appointmentAdapterTimeout").value),display};adminState.busy=true;try{const result=await api("/api/admin/appointment-module",{method:"POST",body});adminState.data=await api("/api/admin/settings");renderAdminShell();await Swal.fire({icon:"success",title:result.message||"บันทึกแล้ว",timer:1600,showConfirmButton:false,customClass:swalClasses(),width:380})}catch(error){await showNotice("error",error.message||"บันทึกข้อมูลนัดหมายไม่สำเร็จ")}finally{adminState.busy=false}})
}
