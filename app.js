...
function renderInboundRows() {
  ...
  }).join(""):`<div class="empty-state"><b>${isCheck?"ไม่มีเอกสารรอตรวจ":"ไม่พบข้อมูลในสถานะนี้"}</b><span>${isCheck?"เมื่อมีงานใหม่ รายการจะขึ้นที่นี่":"เลือกสถานะอื่นหรือรอข้อมูลใหม่"}</span></div>`;
...
function syncInboundStageSummary(){
  const counts=statusCounts(),documentCheckOn=state.documentCheckEnabled||Number(counts.WAITING_DOCUMENT_CHECK||0)>0;
  const values={ALL:state.vehicles.length,WAITING_DOCUMENT_SUBMISSION:counts.WAITING_DOCUMENT_SUBMISSION||0,WAITING_DOCUMENT_CHECK:counts.WAITING_DOCUMENT_CHECK||0,READY_FOR_RECEIVING:counts.READY_FOR_RECEIVING||0};
  const metricValues={metricWaiting:values.WAITING_DOCUMENT_SUBMISSION,metricDocumentCheck:values.WAITING_DOCUMENT_CHECK,metricReady:values.READY_FOR_RECEIVING,metricTotal:values.ALL};
