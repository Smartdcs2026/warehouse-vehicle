...
function renderDatatableLayout(){
  ...
  $("pageContent").innerHTML=`<section class="dt-page dt-clean-page dt-r98-page">
    <section class="dt-r98-toolbar">
      <div class="dt-r98-headline"><b>Datatable</b><small id="dtPanelStatusSub">กำลังเชื่อมต่อข้อมูล</small></div>
      <label class="dt-r98-field"><span>วันที่เริ่ม</span><input id="dtFrom" type="date" value="${escapeHtml(datatableState.from)}"></label>
      <label class="dt-r98-field"><span>วันที่สิ้นสุด</span><input id="dtTo" type="date" value="${escapeHtml(datatableState.to)}"></label>
      <label class="dt-r98-field dt-r98-shift"><span>กะ</span><select id="dtShift"><option value="">ทุกกะ</option>${shiftOptions}</select></label>
      <div class="dt-r98-actions"><button id="dtRefresh" class="quiet-button" type="button">รีเฟรช</button><button id="dtFullscreen" class="outline-button" type="button">เต็มจอ</button><button id="dtExport" class="primary" type="button">ส่งออก</button></div>
      <div class="dt-r98-status"><i></i><div><b id="dtPanelStatus">พร้อมใช้งาน</b><small id="dtPanelMiniNote">กำลังอัปเดตข้อมูล</small></div></div>
      <div id="dtQuickCounts" class="dt-r98-quickcounts"></div>
      <span id="dtShiftHint" class="dt-shift-hint dt-r98-shift-hint" hidden></span>
    </section>
    <nav id="dtStageTabs" class="dt-stage-tabs dt-stage-tabs-r98">${stages.map(s=>`<button type="button" data-dt-stage="${s.key}" class="${datatableState.stage===s.key?"active":""} ${datatableStageToneClass(s.key)}"><span>${escapeHtml(s.label)}</span></button>`).join("")}</nav>
    <section class="dt-current-rule dt-current-rule-r98"><div><small>เกณฑ์เวลาของช่วงนี้</small><b id="dtRuleStageName">${escapeHtml(datatableStageMeta().label)}</b></div><div id="dtCurrentRules" class="dt-current-rule-levels"></div><div class="dt-current-rule-actions"><button id="dtAllRules" class="quiet-button" type="button">เกณฑ์ทั้งหมด</button><button id="dtActivityButton" class="quiet-button" type="button">กิจกรรม</button></div></section>
...
function renderDatatableSummary(summary){
  const el=$("dtQuickCounts");
  if(!el)return;
  const cards=[["total","ทั้งหมด",summary.total||0,"dt-blue"],["active","กำลังดำเนินการ",summary.active||0,"dt-orange"],["warnings","มีแจ้งเตือน",summary.warnings||0,"dt-red"],["rejected","ปฏิเสธแล้ว",summary.rejected||0,"dt-rose"],["completed","เสร็จสิ้น",summary.completed||0,"dt-green"]];
  el.innerHTML=cards.map(([key,label,value,tone])=>`<button type="button" data-dt-summary="${key}" class="dt-r98-chip ${tone}"><small>${label}</small><b>${Number(value).toLocaleString("th-TH")}</b></button>`).join("");
  el.querySelectorAll("[data-dt-summary]").forEach(button=>button.addEventListener("click",()=>{
    const key=button.dataset.dtSummary;
    if(key==="total"){datatableState.status="ALL";datatableState.rejectedOnly=false;datatableState.problemOnly=false;datatableState.sla="ALL"}
    else if(key==="active"){datatableState.status="ACTIVE";datatableState.rejectedOnly=false;datatableState.problemOnly=false;datatableState.sla="ALL"}
    else if(key==="warnings"){datatableState.status="ALL";datatableState.problemOnly=true;datatableState.rejectedOnly=false;datatableState.sla="ALL"}
    else if(key==="rejected"){datatableState.stage="overview";datatableState.status="ALL";datatableState.rejectedOnly=true;datatableState.problemOnly=false;datatableState.sla="ALL";document.querySelectorAll("[data-dt-stage]").forEach(b=>b.classList.toggle("active",b.dataset.dtStage==="overview"));renderDatatableSide()}
    else if(key==="completed"){datatableState.status="CLOSED";datatableState.rejectedOnly=false;datatableState.problemOnly=false;datatableState.sla="ALL"}
    datatableState.page=1;
    if($("dtStatus"))$("dtStatus").value=datatableState.status;
    if($("dtSla"))$("dtSla").value=datatableState.sla;
    $("dtProblem")?.classList.toggle("is-active",datatableState.problemOnly);
    loadDatatable(true);
  }))
}
function renderDatatableHeaderStatus(data){
  const total=Number(data.total||0),button=$("dtRefresh"),stamp=formatDateShort(data.generatedAt);
  if(button)button.title=`${total.toLocaleString("th-TH")} รายการ · อัปเดต ${stamp}`;
  if($("dtPanelStatus"))$("dtPanelStatus").textContent="พร้อมใช้งาน";
  if($("dtPanelStatusSub"))$("dtPanelStatusSub").textContent=`อัปเดต ${stamp}`;
  if($("dtPanelMiniNote"))$("dtPanelMiniNote").textContent=`${total.toLocaleString("th-TH")} รายการ`;
}
