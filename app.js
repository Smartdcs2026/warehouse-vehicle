...
function renderAdminVoice(){
  ...
  <form id="voiceSettingsForm" class="admin-form-grid voice-admin-form">
    ...
    <label class="admin-toggle-row"><span><b>อ่านหมายเลขประตู</b><small>อ่านเฉพาะรายการที่มีการเลือกประตู</small></span><input id="voiceReadDoor" type="checkbox" ${v.readDoor!==false?"checked":""}></label>
    ...
    <div class="admin-form-actions"><button id="voiceSaveButton" class="primary" type="submit">บันทึกการตั้งค่าเสียง</button><button id="voiceTestButton" class="outline-button" type="button">ทดลองเสียง R07</button></div>
  </form>
...
async function ensureQueueVoiceEngine(){if(window.SmartQueueVoice)return window.SmartQueueVoice;await new Promise((resolve,reject)=>{const existing=document.querySelector('script[data-queue-voice-engine]');if(existing){existing.addEventListener("load",resolve,{once:true});existing.addEventListener("error",reject,{once:true});return}const script=document.createElement("script");script.src="./voice-engine.js?v=20260808-r74";script.dataset.queueVoiceEngine="1";script.onload=resolve;script.onerror=()=>reject(new Error("โหลดระบบเสียงไม่สำเร็จ"));document.head.appendChild(script)});if(!window.SmartQueueVoice)throw new Error("ระบบเสียงยังไม่พร้อม");return window.SmartQueueVoice}
async function testAdminQueueVoice(settings){const button=$("voiceTestButton"),original=button?.textContent||"ทดลองเสียง";if(button){button.disabled=true;button.textContent="กำลังเตรียมเสียง"}try{const engine=await ensureQueueVoiceEngine();engine.configure({...settings,enabled:true,assetBasePath:"/voice/queue/th-TH/standard-01/",apiBaseUrl:cfg.apiBaseUrl,repeatCount:1});await engine.unlockAndPrepare();await engine.announceNow({appointmentNo:"2006988",doorCode:"R07"});await showNotice("success","ทดสอบเสียงแล้ว")}catch(error){await showNotice("error",error.message||"ทดลองเสียงไม่สำเร็จ")}finally{if(button){button.disabled=false;button.textContent=original}}}

function renderAdminQueue(){
  const panel=$("adminPanel");if(!panel)return;
  const queueUrl=new URL("./queue.html?v=20260808-r74",location.href).href;
  panel.innerHTML=`<div class="admin-section-head clean-admin-head"><div><h3>จอแสดงสถานะคิว</h3><p>ใช้สำหรับจอส่วนกลาง อ่านข้อมูลอย่างเดียว ไม่เปลี่ยนสถานะงาน</p></div><a class="primary admin-queue-open" href="./queue.html?v=20260808-r74" target="_blank" rel="noopener">เปิดจอคิว</a></div>
