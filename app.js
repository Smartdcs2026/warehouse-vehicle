...
const dashboardState = { range:"today", shiftId:"", data:null, busy:false };
...
function renderDashboard() {
  ...
  $("pageContent").innerHTML=`<section class="dashboard-head"><div><span class="eyebrow">ภาพรวมคลังสินค้า</span><h2>Dashboard การปฏิบัติงาน</h2><p id="dashboardRangeLabel">กำลังโหลดข้อมูล</p></div><div class="dashboard-filter"><div class="range-buttons"><button data-dashboard-range="today">วันนี้</button><button data-dashboard-range="7d">7 วัน</button><button data-dashboard-range="30d">30 วัน</button></div><select id="dashboardShift"><option value="">ทุกกะ</option></select><button id="dashboardFullscreen" class="outline-button">เต็มหน้าจอ</button></div></section><div id="dashboardBody" class="loading">กำลังสรุปข้อมูล</div>`;
  ...
  $("dashboardFullscreen").addEventListener("click",toggleFullscreen);loadDashboard(true);
}
...
function renderDashboardData(data){
  if(state.view!=="dashboard"||!$("dashboardBody"))return;const m=data.metrics||{},maxHour=Math.max(1,...(data.hours||[]).map(row=>Number(row.total)||0)),maxCompany=Math.max(1,...(data.companies||[]).map(row=>Number(row.total)||0)),stages=[["Gate In → ยื่นเอกสาร",data.stageAverages?.gate_to_doc],["ยื่นเอกสาร → เริ่มตรวจรับ",data.stageAverages?.doc_to_start],["ระยะเวลาตรวจรับ",data.stageAverages?.receiving],["รับเสร็จ → รับเอกสารคืน",data.stageAverages?.complete_to_return],["รับเอกสารคืน → Gate Out",data.stageAverages?.return_to_out]];
  ...
  $("dashboardBody").innerHTML=`<section class="dashboard-kpis">${dashboardKpi("Gate In",m.total,"คันในช่วงที่เลือก","#416FC3")}${dashboardKpi("ปิดงาน",m.closed,"คัน","#59A63E")}${dashboardKpi("ยังอยู่ในพื้นที่",m.activeNow,"คันขณะนี้","#D5007F")}${dashboardKpi("เกินระดับเตือน",m.warningNow,"คันขณะนี้","#D9304F")}${dashboardKpi("เวลาเฉลี่ย",formatDuration(m.avgTotalSeconds),"ตั้งแต่ Gate In ถึง Gate Out","#337497")}${dashboardKpi("P90",formatDuration(m.p90TotalSeconds),"90% เสร็จภายในเวลานี้","#F7AA12")}</section><section class="dashboard-bento"><article class="dashboard-panel span-2"><header><h3>รถเข้าแต่ละชั่วโมง</h3><span>เวลาไทย</span></header><div class="hour-chart">${Array.from({length:24},(_,hour)=>{const row=(data.hours||[]).find(item=>Number(item.hour)===hour),value=Number(row?.total||0);return`<div title="${String(hour).padStart(2,"0")}:00 · ${value} คัน"><i style="height:${Math.max(3,value/maxHour*100)}%"></i><small>${hour%3===0?String(hour).padStart(2,"0"):""}</small></div>`}).join("")}</div></article><article class="dashboard-panel"><header><h3>สถานะงาน</h3><span>${(data.statuses||[]).reduce((sum,row)=>sum+Number(row.total||0),0)} คัน</span></header>${dashboardBars((data.statuses||[]).map(row=>({label:statusLabel(row.label),total:row.total})))}</article><article class="dashboard-panel"><header><h3>จำนวนตามกะ</h3><span>ช่วงที่เลือก</span></header>${dashboardBars(data.shifts||[])}</article><article class="dashboard-panel span-2"><header><h3>เวลาเฉลี่ยแต่ละช่วง</h3><span>HH:mm:ss</span></header><div class="stage-time-grid">${stages.map(([label,value])=>`<div><span>${label}</span><b>${value==null?"–":formatDuration(value)}</b></div>`).join("")}</div></article><article class="dashboard-panel span-2"><header><h3>บริษัทที่มีรถเข้ามาก</h3><span>8 อันดับแรก</span></header><div class="company-bars">${(data.companies||[]).map(row=>`<div><span>${escapeHtml(row.label)}</span><i><em style="width:${Number(row.total)/maxCompany*100}%"></em></i><b>${Number(row.total)} คัน</b><small>${row.avg_seconds?`เฉลี่ย ${formatDuration(row.avg_seconds)}`:""}</small></div>`).join("")||`<div class="empty-state">ไม่มีข้อมูล</div>`}</div></article></section><section class="dashboard-panel recent-table"><header><h3>รถที่ปิดงานล่าสุด</h3><span>10 รายการ</span></header><div class="dashboard-table-head"><span>เลขนัดหมาย</span><span>บริษัท</span><span>ทะเบียนรถ</span><span>Gate Out</span><span>เวลารวม</span></div>${(data.recent||[]).map(row=>`<div class="dashboard-table-row"><b>${escapeHtml(row.appointment_no||row.auto_id)}</b><span>${escapeHtml(row.company_name||"ไม่ระบุ")}</span><span>${escapeHtml(joinText(row.vehicle_plate,row.province))}</span><span>${formatDate(row.gate_out_at)}</span><b>${formatDuration(row.total_seconds)}</b></div>`).join("")||`<div class="empty-state">ยังไม่มีงานที่ปิด</div>`}</section>`;
}
