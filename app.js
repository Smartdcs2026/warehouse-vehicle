...
async function startReceiving(vehicle){
  ...
  if(usesDoor&&!availableDoors.length){
    await showNotice("warning","ยังไม่มีประตูที่เปิดใช้งาน กรุณาติดต่อผู้ดูแล");
    return;
  }
  const currentDoor=String(vehicle.door_code||"").toUpperCase(),currentMatch=currentDoor.match(/^(SS|RR|SR|RS|S|R)(\d{1,3})$/),defaultPrefix=currentMatch?.[1]||"S",defaultNumber=currentMatch?.[2]||"";
  if(window.Swal){
    const prefixOptions=["S","R","SS","RR","SR","RS"].map(prefix=>`<option value="${prefix}" ${prefix===defaultPrefix?"selected":""}>${prefix}</option>`).join("");
    ...
    if(usesDoor){
      ...
      const cleanPrefix=String(prefix||"S").trim().toUpperCase();if(!["S","R","SS","RR","SR","RS"].includes(cleanPrefix)){showNotice("warning","กรุณาเลือกตัวอักษรหน้าประตูให้ถูกต้อง");return}
...
function openPublicQueue(){window.open(new URL("./queue.html?v=20260808-r49",location.href).href,"_blank","noopener")}
...
function renderAdminQueue(){
  const panel=$("adminPanel");if(!panel)return;
  const queueUrl=new URL("./queue.html?v=20260808-r75",location.href).href;
  panel.innerHTML=`<div class="admin-section-head clean-admin-head"><div><h3>จอแสดงสถานะคิว</h3><p>ใช้สำหรับจอส่วนกลาง อ่านข้อมูลอย่างเดียว ไม่เปลี่ยนสถานะงาน</p></div><a class="primary admin-queue-open" href="./queue.html?v=20260808-r75" target="_blank" rel="noopener">เปิดจอคิว</a></div>
