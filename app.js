...
const FRONTEND_BUILD="2026.08.18-round179b-production-preview-modal-guard";
...
async function appointmentPreviewSummaryModal(total,{blocked,canRecord,hasRowChanges,advancesSnapshot}){
  ...
  const result=await window.Swal.fire({
    ...
    customClass:{...swalClasses(),container:"appointment-preview-swal-container"},
    ...
    allowEscapeKey:true,
    didOpen:()=>{const c=window.Swal.getContainer?.();if(c){c.style.zIndex="2147483000";c.style.pointerEvents="auto"}}
  });
  return result;
}
async function appointmentPreviewChanges(){const r=appointmentUploadState.result;if(!r||appointmentIssueCount()>0||Number(r.totalAppointments||0)===0)return;let snapshot;try{snapshot=appointmentSnapshotForRequest()}catch(error){await showNotice("warning",error.message);return}const button=$("appointmentPreviewChanges");button.disabled=true;$("appointmentConfirmImport").disabled=true;try{const items=(r.appointments||[]).map(appointmentImportItem),batchSize=Math.max(10,Math.min(20,Number(appointmentUploadState.config?.batchSize)||20)),total={inserted:0,updated:0,unchanged:0,olderSkipped:0,conflicts:0};for(let offset=0;offset<items.length;offset+=batchSize){const batch=items.slice(offset,offset+batchSize);appointmentSetBusy(true,`กำลังตรวจ ${Math.min(offset+batch.length,items.length).toLocaleString()} / ${items.length.toLocaleString()}`);const part=await api("/api/appointment/import/preview",{method:"POST",body:{...snapshot,items:batch},timeoutMs:20000});if(part.blockedOlder)throw new Error(part.message||"ชุดข้อมูลนี้เก่ากว่าข้อมูลปัจจุบัน");appointmentAddCounts(total,part.counts)}appointmentSetBusy(false);const latestKey=Number(appointmentUploadState.config?.schema?.latest?.snapshotKey||0),incomingKey=appointmentSnapshotNumericKey(snapshot),advancesSnapshot=incomingKey>latestKey;const blocked=total.olderSkipped>0||total.conflicts>0,hasRowChanges=total.inserted+total.updated>0,canRecord=hasRowChanges||advancesSnapshot;appointmentUploadState.preview={fileHash:r.fileHash,snapshotKey:`${snapshot.snapshotDate}|${snapshot.snapshotTime}`,counts:total,advancesSnapshot,acknowledged:false};const modal=await appointmentPreviewSummaryModal(total,{blocked,canRecord,hasRowChanges,advancesSnapshot});appointmentUploadState.preview.acknowledged=true;$("appointmentConfirmImport").disabled=blocked||!canRecord||!modal?.isConfirmed}catch(error){appointmentSetBusy(false);appointmentUploadState.preview=null;$("appointmentConfirmImport").disabled=true;await showNotice("error",error.message)}finally{button.disabled=false}}
