...
function renderAdminShell(){
  const tabs=[["users","ผู้ใช้งาน"],["workflow","ขั้นตอนงาน"],["doors","ประตู"],["shifts","กะทำงาน"],["alerts","เวลาแจ้งเตือน"],["recall","เรียกรถซ้ำ"],["data","การใช้ข้อมูล"],["export","ส่งออกข้อมูล"],["tracking","ติดตามคนขับ"],["voice","เสียงประกาศ"],["queue","จอคิว"]];
...
function renderAdminPanel(){if(adminState.tab==="users")renderAdminUsers();else if(adminState.tab==="workflow")renderAdminWorkflow();else if(adminState.tab==="doors")renderAdminDoors();else if(adminState.tab==="shifts")renderAdminShifts();else if(adminState.tab==="alerts")renderAdminAlerts();else if(adminState.tab==="recall")renderAdminQueueRecall();else if(adminState.tab==="data")renderAdminDataUsage();else if(adminState.tab==="export")renderAdminMonthlyExport();else if(adminState.tab==="tracking")renderAdminTracking();else if(adminState.tab==="voice")renderAdminVoice();else renderAdminQueue()}
...
function renderAdminVoice(){
  const panel=$("adminPanel");if(!panel)return;
  const v=adminState.data.voice||{enabled:false,volume:80,repeatCount:1,repeatDelaySeconds:7,readDoor:true,readDoorLeadingZero:false,playDing:true,playThanks:true,packId:"th-TH-standard-01",availablePacks:[{id:"th-TH-standard-01",label:"เสียงไทย ชุดมาตรฐาน 01"},{id:"th-TH-female-01",label:"เสียงผู้หญิง ใสหวาน 01"}]};
  const packs=Array.isArray(v.availablePacks)&&v.availablePacks.length?v.availablePacks:[{id:"th-TH-standard-01",label:"เสียงไทย ชุดมาตรฐาน 01"},{id:"th-TH-female-01",label:"เสียงผู้หญิง ใสหวาน 01"}];
  ...
  panel.innerHTML=`<div class="admin-section-head clean-admin-head"><div><h3>เสียงประกาศคิว</h3><p>เลือกเสียงที่ใช้กับจอคิวได้จากหน้านี้ การเปลี่ยนเสียงไม่กระทบเวลาหรือขั้นตอนงาน</p></div><span class="tracking-saved-pill"><span>เสียงที่ใช้อยู่</span><b>${escapeHtml(v.packLabel||packs.find(x=>x.id===v.packId)?.label||"ชุดมาตรฐาน")}</b></span></div>
  ...
  <form id="voiceSettingsForm" class="admin-form-grid voice-admin-form">
    ...
    <label><span>เลือกเสียงประกาศ</span><select id="voicePack">${packOptions}</select><small class="field-help">เลือกแล้วกดทดลองฟังก่อนบันทึกได้</small></label>
    ...
    <label class="admin-toggle-row"><span><b>อ่านหมายเลขประตู</b><small>อ่านเฉพาะเมื่อระบบใช้ประตูและรายการมีหมายเลขประตู</small></span><input id="voiceReadDoor" type="checkbox" ${v.readDoor!==false?"checked":""}></label>
    ...
    <div class="admin-form-actions"><button id="voiceSaveButton" class="primary" type="submit">บันทึกเสียงที่เลือก</button><button id="voiceTestButton" class="outline-button" type="button">ทดลองแบบมีประตู R07</button><button id="voiceTestNoDoorButton" class="outline-button" type="button">ทดลองแบบไม่มีประตู</button></div>
  </form>
  <section class="admin-queue-guide"><article><b>เปลี่ยนเสียงได้ทันที</b><p>หลังบันทึก จอคิวจะรับค่าชุดใหม่อัตโนมัติ โดยไม่เปลี่ยนสถานะงาน</p></article><article><b>ชุดเดิมยังอยู่</b><p>เสียงมาตรฐานและเสียงผู้หญิงเก็บแยกกัน จึงสลับกลับได้จากหน้านี้</p></article><article><b>จอคิวต้องเปิดเสียงครั้งแรก</b><p>หลังเปิดหน้าจอ ให้กด “เปิดเสียง” หนึ่งครั้งเพื่อเตรียมลำโพง</p></article></section>`;
  ...
  const testSettings=()=>{const selected=packs.find(item=>item.id===$("voicePack").value)||packs[0];return{enabled:true,packId:selected.id,assetBasePath:voicePackAssetPath(selected.id),volume:Number($("voiceVolume").value),repeatCount:1,repeatDelaySeconds:Number($("voiceRepeatDelay").value),playDing:$("voicePlayDing").checked,readDoor:$("voiceReadDoor").checked,readDoorLeadingZero:$("voiceDoorZero").checked,playThanks:$("voicePlayThanks").checked}};
  ...
  $("voicePack")?.addEventListener("change",updateAdminVoiceReadyStatus);
  updateAdminVoiceReadyStatus();
}
function voicePackAssetPath(packId){return packId==="th-TH-female-01"?"/voice/queue/th-TH/female-01/":"/voice/queue/th-TH/standard-01/"}
async function updateAdminVoiceReadyStatus(){const box=$("voiceReadyStatus");if(!box)return;const packId=$("voicePack")?.value||adminState.data.voice?.packId||"th-TH-standard-01";try{const base=String(cfg.apiBaseUrl||"").replace(/\/$/,"");const response=await fetch(base+"/api/voice/status?packId="+encodeURIComponent(packId),{cache:"no-store"}),data=await response.json();if(!response.ok||!data.ready)throw new Error(data.message||"ยังไม่พร้อม");box.classList.add("ready");box.innerHTML=`<b>${escapeHtml(data.packLabel||"ชุดเสียง")} พร้อมใช้งาน</b><span>ทดลองฟังหรือบันทึกเพื่อให้จอคิวใช้ชุดนี้ได้</span>`}catch(error){box.classList.remove("ready");box.innerHTML=`<b>ชุดเสียงนี้ยังไม่พร้อม</b><span>${escapeHtml(error.message||"กรุณาตรวจสอบชุดเสียง")}</span>`}}
async function ensureQueueVoiceEngine(){if(window.SmartQueueVoice)return window.SmartQueueVoice;await new Promise((resolve,reject)=>{const existing=document.querySelector('script[data-queue-voice-engine]');if(existing){existing.addEventListener("load",resolve,{once:true});existing.addEventListener("error",reject,{once:true});return}const script=document.createElement("script");script.src="./voice-engine.js?v=20260809-r79";script.dataset.queueVoiceEngine="1";script.onload=resolve;script.onerror=()=>reject(new Error("โหลดระบบเสียงไม่สำเร็จ"));document.head.appendChild(script)});if(!window.SmartQueueVoice)throw new Error("ระบบเสียงยังไม่พร้อม");return window.SmartQueueVoice}
async function testAdminQueueVoice(settings,item={appointmentNo:"2006988",doorCode:"R07",useDoor:true},buttonId="voiceTestButton"){const button=$(buttonId),original=button?.textContent||"ทดลองเสียง";if(button){button.disabled=true;button.textContent="กำลังเตรียมเสียง"}try{const engine=await ensureQueueVoiceEngine();engine.configure({...settings,enabled:true,apiBaseUrl:cfg.apiBaseUrl,repeatCount:1});await engine.unlockAndPrepare();await engine.announceNow(item);await showNotice("success",item?.useDoor===false?"ทดสอบเสียงแบบไม่มีประตูแล้ว":"ทดสอบเสียงแบบมีประตูแล้ว")}catch(error){await showNotice("error",error.message||"ทดลองเสียงไม่สำเร็จ")}finally{if(button){button.disabled=false;button.textContent=original}}}
...
function applyVehicleData(data){const items=data.items||[];if(typeof data.trackingEnabled!=="undefined")state.trackingEnabled=Boolean(data.trackingEnabled);if(data.queueRecall&&typeof data.queueRecall==="object")state.queueRecall=data.queueRecall;if(Array.isArray(data.activeDoors))state.activeDoors=data.activeDoors.map(code=>String(code||"").trim().toUpperCase()).filter(Boolean);let shouldSound=false;if(alertSoundState.initialized){for(const vehicle of items){const key=String(vehicle.auto_id),previous=alertSoundState.levels.get(key),current=String(vehicle.alert_level||"NORMAL");if(Number(vehicle.alert_sound_enabled)&&previous&&alertLevelRank(current)>alertLevelRank(previous))shouldSound=true}}state.vehicles=items;alertSoundState.levels=new Map(items.map(vehicle=>[String(vehicle.auto_id),String(vehicle.alert_level||"NORMAL")]));if(alertSoundState.initialized&&shouldSound)playFeedbackSound("warning");alertSoundState.initialized=true}
