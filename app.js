...
async function confirmInboundSubmit(autoId,source){
  ...
  if(!window.Swal){
    ...
    try{const idempotencyKey=createIdempotencyKey(),result=await api("/api/workflow/inbound-scan",{method:"POST",headers:{"x-idempotency-key":idempotencyKey},body:{autoId:value,idempotencyKey,source}});mergeVehicleUpdate(result.vehicle);playFeedbackSound(result.duplicate?"duplicate":"success");announceInboundResult(result,vehicle,value);showKioskMessage(result.message||"บันทึกเรียบร้อย",true);await refreshInboundKioskData().catch(()=>restoreInboundMainDisplay())}
    catch(error){const duplicate=Boolean(error.data?.duplicate);playFeedbackSound(duplicate?"duplicate":"error");if(duplicate)announceInboundResult(error.data,vehicle,value);showKioskMessage(error.message,duplicate)}
  ...
  try{
    ...
    const duplicate=Boolean(result.duplicate);playFeedbackSound(duplicate?"duplicate":"success");announceInboundResult(result,vehicle,value);
  ...
  }catch(error){const duplicate=Boolean(error.data?.duplicate),errorVehicle=error.data?.vehicle||vehicle;playFeedbackSound(duplicate?"duplicate":"error");if(duplicate)announceInboundResult(error.data,errorVehicle,value);await Swal.fire({icon:duplicate?"warning":"error",title:duplicate?"ตรวจพบการสแกนซ้ำ":"บันทึกไม่สำเร็จ",html:`<p class="swal-message${duplicate?" duplicate-message":""}">${escapeHtml(error.message)}</p>${vehicleDetailsHtml(errorVehicle,value)}`,timer:automatic?(duplicate?4200:5000):undefined,timerProgressBar:automatic,showConfirmButton:!automatic,confirmButtonText:"ตกลง",allowOutsideClick:!automatic,allowEscapeKey:!automatic,customClass:swalClasses(),buttonsStyling:false,width:420})}
  finally{submitState.busy=false;restoreInboundMainDisplay()}
}
