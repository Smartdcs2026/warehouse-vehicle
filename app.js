...
const dashboardState = { range:"today", shiftId:"", tab:"command", data:null, busy:false };
...
async function loadDashboard(showLoading=true){
  if(dashboardState.busy||state.view!=="dashboard")return;dashboardState.busy=true;if(showLoading&&$("dashboardBody"))$("dashboardBody").innerHTML=`<div class="loading">กำลังสรุปข้อมูล</div>`;
  try{const query=`?range=${encodeURIComponent(dashboardState.range)}${dashboardState.shiftId?`&shiftId=${encodeURIComponent(dashboardState.shiftId)}`:""}`,data=await api(`/api/dashboard/summary${query}`);dashboardState.data=data;renderDashboardData(data)}catch(error){if($("dashboardBody"))$("dashboardBody").innerHTML=`<div class="empty-state"><b>โหลด Dashboard ไม่สำเร็จ</b><span>${escapeHtml(error.message)}</span><button id="retryDashboard" class="primary">ลองใหม่</button></div>`;$(`retryDashboard`)?.addEventListener("click",()=>loadDashboard(true))}finally{dashboardState.busy=false}
}
...
function renderDashboardData(data){
  ...
  const shift=$("dashboardShift");if(shift){shift.innerHTML=`<option value="">ทุกกะ</option>${(data.shiftOptions||[]).map(item=>`<option value="${escapeHtml(item.shift_id)}">${escapeHtml(item.shift_name)}</option>`).join("")}`;shift.value=dashboardState.shiftId;shift.onchange=()=>{dashboardState.shiftId=shift.value;loadDashboard(true)}}
  $("dashboardBody").innerHTML=`<section class="dashboard-kpis">${dashboardKpi("รถเข้า",m.total,"คันในช่วงที่เลือก","#2563EB")}${dashboardKpi("ปิดงาน",m.closed,`${Number(m.completionRate||0).toFixed(1)}% ของรถเข้า`,"#059669")}${dashboardKpi("รถในพื้นที่",m.activeNow,"คันขณะนี้","#7C3AED")}${dashboardKpi("ต้องเร่งดำเนินการ",m.warningNow,"ระดับเตือนขึ้นไป","#DC2626")}${dashboardKpi("เวลาเฉลี่ย",formatDuration(m.avgTotalSeconds),"Gate In ถึง Gate Out","#0F766E")}${dashboardKpi("P90",formatDuration(m.p90TotalSeconds),"90% ปิดงานภายใน","#D97706")}</section><nav class="dashboard-view-tabs">${[["command","สั่งการ"],["performance","ประสิทธิภาพ"],["capacity","ประตูและกะ"],["exceptions","ข้อยกเว้น"]].map(([id,label])=>`<button data-dashboard-tab="${id}" class="${dashboardState.tab===id?"active":""}">${label}</button>`).join("")}</nav><div id="dashboardTabContent" class="dashboard-tab-content">${dashboardTabContent(data)}</div>`;
...
function dashboardKpi(label,value,note,color){return`<article class="dashboard-kpi" style="--kpi-color:${color}"><small>${label}</small><b>${value??0}</b><span>${note}</span></article>`}
...
function dashboardTabContent(data){if(dashboardState.tab==="performance")return dashboardPerformance(data);if(dashboardState.tab==="capacity")return dashboardCapacity(data);if(dashboardState.tab==="exceptions")return dashboardExceptions(data);return dashboardCommand(data)}
...
function dashboardPerformance(data){const stages=[["Gate In → ยื่นเอกสาร",data.stageAverages?.gate_to_doc],["ยื่นเอกสาร → เริ่มตรวจรับ",data.stageAverages?.doc_to_start],["ระยะเวลาตรวจรับ",data.stageAverages?.receiving],["รับเสร็จ → รับเอกสารคืน",data.stageAverages?.complete_to_return],["รับเอกสารคืน → Gate Out",data.stageAverages?.return_to_out]];return`<section class="executive-grid"><article class="dashboard-panel span-2"><header><h3>เวลาเฉลี่ยแต่ละช่วง</h3><span>ชี้จุดสูญเสียเวลา</span></header><div class="stage-time-grid">${stages.map(([label,value])=>`<div><span>${label}</span><b>${value==null?"–":formatDuration(value)}</b></div>`).join("")}</div></article><article class="dashboard-panel"><header><h3>การกระจายเวลาปิดงาน</h3><span>จำนวนรถ</span></header>${dashboardBars(data.cycleBands||[])}</article><article class="dashboard-panel"><header><h3>ผลลัพธ์แต่ละกะ</h3><span>ปิดงาน / รถเข้า</span></header>${performanceRows(data.shiftPerformance)}</article><article class="dashboard-panel recent-table span-4"><header><h3>งานที่ปิดล่าสุด</h3><span>ใช้ตรวจสอบความเร็วหน้างาน</span></header>${recentTable(data.recent)}</article></section>`}
function dashboardCapacity(data){const maxHour=Math.max(1,...(data.hours||[]).map(row=>Number(row.total)||0));return`<section class="executive-grid"><article class="dashboard-panel span-2"><header><h3>ภาระงานตามประตู</h3><span>อ้างอิงรหัสประตู ไม่ใช้ชื่อบริษัท</span></header>${doorRows(data.doors)}</article><article class="dashboard-panel"><header><h3>จำนวนรถตามกะ</h3><span>ช่วงที่เลือก</span></header>${dashboardBars(data.shifts||[])}</article><article class="dashboard-panel"><header><h3>คุณภาพข้อมูล</h3><span>รายการที่ต้องตรวจสอบ</span></header>${qualityRows(data.dataQuality)}</article><article class="dashboard-panel span-4"><header><h3>ช่วงเวลาที่รถเข้าหนาแน่น</h3><span>ใช้วางแผนกำลังคน</span></header>${hourChart(data.hours,maxHour)}</article></section>`}
function dashboardExceptions(data){return`<section class="executive-grid exceptions-grid"><article class="dashboard-panel"><header><h3>ระดับการแจ้งเตือน</h3><span>รถในพื้นที่</span></header>${dashboardBars(alertRows(data.alertLevels))}</article><article class="dashboard-panel"><header><h3>คอขวดปัจจุบัน</h3><span>ใช้สั่งเร่งงาน</span></header><div class="command-callout"><small>ขั้นตอน</small><b>${escapeHtml(data.bottleneck?.label||"ไม่มี")}</b><span>${Number(data.bottleneck?.queue||0)} คัน · ${formatDuration(data.bottleneck?.avg)}</span></div></article><article class="dashboard-panel"><header><h3>ข้อมูลไม่ครบ</h3><span>ช่วงที่เลือก</span></header>${qualityRows(data.dataQuality)}</article><article class="dashboard-panel action-panel span-4"><header><h3>รายการข้อยกเว้น</h3><span>ใช้เลขนัดหมายและทะเบียนรถเป็นหลัก</span></header>${actionTable(data.actionItems)}</article></section>`}
