...
function renderInbound() {
  ...
  const tabs=[
    ["ALL","ทั้งหมด",state.vehicles.length],
  ...
  $("pageContent").innerHTML = `<section class="inbound-workspace inbound-workspace-r88"><aside class="inbound-scan-station${state.trackingEnabled?"":" tracking-disabled"}"><div class="scanner compact-scanner"><div class="scan-panel-head"><h2>สแกน QR Code</h2></div><div id="scanFrame" class="scan-frame"><video id="qrVideo" class="qr-video" playsinline muted hidden></video><canvas id="qrCanvas" hidden></canvas><div id="scanPlaceholder" class="scan-placeholder"><span class="scan-corners" aria-hidden="true"><i></i><i></i><i></i><i></i></span></div><div id="scanBeam" class="scan-beam" hidden></div></div><div class="scan-actions"><button id="startCamera" class="primary">เปิดกล้องสแกน</button><button id="stopCamera" class="outline-button" hidden>ปิดกล้อง</button></div></div><div class="scan-input-block compact-input-block"><h2>บันทึกด้วยตนเอง</h2><div class="auto-input"><input id="autoSearch" autocomplete="off" autocapitalize="characters" spellcheck="false" enterkeyhint="done" placeholder="กรอก Auto ID / เลขนัดหมาย"><button id="autoButton" class="primary">บันทึก</button></div><small class="input-hint">สแกนแล้วช่องจะเคลียร์อัตโนมัติ</small></div>${state.trackingEnabled?`<section id="inboundDriverQr" class="inbound-driver-qr" aria-live="polite"><header><div><small>สำหรับคนขับรถ</small><h2>ติดตามสถานะรถ</h2></div><span id="driverQrTimer" class="driver-qr-timer" hidden></span></header><div id="driverQrBody" class="driver-qr-body"><div class="driver-qr-idle"><b>รอการยื่นเอกสาร</b><span>QR จะขึ้นอัตโนมัติหลังบันทึกสำเร็จ</span></div></div></section>`:""}</aside><section class="inbound-main-stack"><section class="inbound-controlbar"><div class="inbound-kiosk-title inbound-kiosk-title-merged"><span class="inbound-mini-logo" aria-hidden="true"><span class="spectrum-mark"><i></i><i></i><i></i><i></i><i></i><i></i></span></span><div class="inbound-title-copy"><small>แผนก Inbound</small><b>ยื่นเอกสารและตรวจเอกสาร</b><span class="inbound-title-meta"><time id="inboundHeaderClock">${formatDate(unixNow())}</time><em id="inboundTrackingStatus">${documentCheckOn?"ตรวจเอกสารก่อนส่งเข้ารับสินค้า":"ขั้นตอนเดิม: ยื่นเอกสารแล้วพร้อมตรวจรับ"}</em></span></div></div><div class="inbound-page-actions inbound-page-actions-merged"><span id="inboundSyncStatus" class="inbound-sync-status ${syncClass}">${syncLabel}</span><button id="fullscreenButton" class="quiet-button">เต็มหน้าจอ</button>${kioskLogout}</div></section><section class="inbound-metrics inbound-metrics-top inbound-metrics-r88">${inboundMetric("metricWaiting","รอยื่นเอกสาร",counts.WAITING_DOCUMENT_SUBMISSION,"metric-orange","doc-pen")}${checkMetric}${inboundMetric("metricReady","พร้อมตรวจรับ",counts.READY_FOR_RECEIVING,"metric-green","check-circle")}${inboundMetric("metricTotal","รถในพื้นที่",state.vehicles.length,"metric-magenta","truck")}</section><section class="list-card inbound-list-card inbound-list-r88"><header class="inbound-list-head-r88"><div><h2>${documentCheckOn?"รายการ Inbound":"รถที่ยังอยู่ในพื้นที่"}</h2><span id="inboundListCount"></span></div><nav class="inbound-stage-tabs" aria-label="กรองสถานะ">${tabs.map(([key,label,count])=>`<button type="button" data-inbound-filter="${key}" class="${inboundListState.filter===key?"active":""}"><span>${label}</span><b>${Number(count)||0}</b></button>`).join("")}</nav></header><div id="inboundTableHead" class="inbound-table-head" aria-hidden="true"></div><div id="inboundRows"></div><footer class="inbound-workflow-note ${documentCheckOn?"":"is-disabled"}"><b>${documentCheckOn?"ลำดับงาน: ยื่นเอกสาร → ตรวจเอกสารเสร็จ → พร้อมตรวจรับ":"ขั้นตอนตรวจเอกสารก่อนตรวจรับถูกปิดโดย Admin"}</b><span>${documentCheckOn?"รถจะยังไม่แสดงในหน้า งานรับสินค้า จนกว่า Inbound จะยืนยันตรวจเอกสารเสร็จ":"เมื่อยื่นเอกสารสำเร็จ รถจะเข้าสู่พร้อมตรวจรับตามขั้นตอนเดิม"}</span></footer></section></section></section>`;
  renderInboundRows();
...
function inboundFilteredItems(){const key=inboundListState.filter||"ALL";return key==="ALL"?state.vehicles:state.vehicles.filter(v=>v.current_status===key)}
function renderInboundRows() {
  const items=inboundFilteredItems(),isCheck=inboundListState.filter==="WAITING_DOCUMENT_CHECK";
  const head=$("inboundTableHead");if(head){head.classList.toggle("document-check-head",isCheck);head.innerHTML=isCheck?`<span>เลขนัดหมาย</span><span>บริษัท / คนขับ</span><span>Gate In</span><span>ยื่นเอกสาร</span><span>เวลาตรวจเอกสาร</span><span>ระดับ</span><span>ดำเนินการ</span>`:`<span>เลขนัดหมาย</span><span>บริษัท</span><span>ทะเบียนรถ</span><span>ประตู</span><span>สถานะ</span><span>ระดับเตือน / เวลาค้าง</span>`;}
  const rows=$("inboundRows");if(!rows)return;
  rows.classList.toggle("document-check-list",isCheck);
  ...
  }).join(""):`<div class="empty-state"><b>${isCheck?"ไม่มีเอกสารรอตรวจ":"ไม่พบข้อมูลในสถานะนี้"}</b><span>${isCheck?"เมื่อมีการยื่นเอกสาร รายการจะเข้ามาที่ส่วนนี้อัตโนมัติ":"เลือกสถานะอื่นหรือรอข้อมูลใหม่"}</span></div>`;
...
async function confirmDocumentChecked(vehicle){
  ...
  let confirmed=true;
  if(window.Swal){const result=await Swal.fire({title:"ยืนยันตรวจเอกสารเสร็จ",html:`<div class="document-check-confirm"><div><small>เลขนัดหมาย</small><b>${escapeHtml(vehicle.appointment_no||vehicle.auto_id)}</b></div><div><small>บริษัท</small><b>${escapeHtml(vehicle.company_name||"ไม่ระบุ")}</b></div><div class="document-check-confirm-times"><span><small>Gate In</small><b>${formatDate(gateIn)}</b></span><span><small>ยื่นเอกสาร</small><b>${formatDate(submitted)}</b></span><span><small>ใช้เวลาตรวจแล้ว</small><b>${formatDuration(reviewSeconds)}</b></span></div><p>หลังยืนยัน รถจะเปลี่ยนเป็น <strong>พร้อมตรวจรับ</strong> และจึงแสดงในหน้า งานรับสินค้า</p></div>`,showCancelButton:true,confirmButtonText:"ยืนยันเสร็จ",cancelButtonText:"ยกเลิก",reverseButtons:true,focusCancel:true,customClass:{...swalClasses(),confirmButton:"wfv-swal-confirm document-check-confirm-button"},buttonsStyling:false,width:470});confirmed=result.isConfirmed}else confirmed=window.confirm(`ยืนยันตรวจเอกสารเสร็จ ${vehicle.appointment_no||vehicle.auto_id}`);
  if(!confirmed)return;
  submitState.busy=true;const idempotencyKey=createIdempotencyKey();
  try{if(window.Swal)Swal.fire({title:"กำลังบันทึกเวลาตรวจเอกสาร",allowOutsideClick:false,allowEscapeKey:false,didOpen:()=>Swal.showLoading(),showConfirmButton:false,customClass:swalClasses(),width:360});const result=await api("/api/workflow/document-check",{method:"POST",headers:{"x-idempotency-key":idempotencyKey},body:{autoId:vehicle.auto_id,idempotencyKey,source:"manual"}});mergeVehicleUpdate(result.vehicle);playFeedbackSound(result.duplicate?"duplicate":"success");await refreshInboundKioskData();if(window.Swal)await Swal.fire({icon:"success",title:"ตรวจเอกสารเสร็จแล้ว",text:"รถพร้อมเข้าสู่ขั้นตอนตรวจรับสินค้า",timer:1800,showConfirmButton:false,customClass:swalClasses(),width:380})}
...
function trackingPageUrl(token){return new URL(`./track.html?t=${encodeURIComponent(token)}&v=20260811-r88`,location.href).href}
...
function updateInboundMetrics(){const counts=statusCounts(),values={metricWaiting:counts.WAITING_DOCUMENT_SUBMISSION,metricDocumentCheck:counts.WAITING_DOCUMENT_CHECK,metricReady:counts.READY_FOR_RECEIVING,metricTotal:state.vehicles.length};Object.entries(values).forEach(([id,value])=>{if($(id))$(id).textContent=Number(value)||0})}
...
async function createDriverTrackingLink(){
  ...
  try{
    ...
    const vehicle=result.vehicle||{},url=new URL(`./track.html?t=${encodeURIComponent(token)}&v=20260811-r88`,location.href).href;
    if(resultBox){
