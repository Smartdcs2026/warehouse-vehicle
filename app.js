...
const dashboardState = { range:"today", shiftId:"", tab:"overview", data:null, busy:false, lastLoadedAt:0, error:"" };
...
function renderDashboard() {
  ...
  $("pageContent").innerHTML=`<section class="opsdash-head"><div class="opsdash-title"><button id="dashboardMenuButton" class="dashboard-menu-button" aria-label="เปิดเมนู" aria-expanded="false">☰</button><div><span>ภาพรวมคลังสินค้า</span><h2>ศูนย์ควบคุมการปฏิบัติงาน</h2><p id="dashboardRangeLabel">กำลังโหลดข้อมูล</p></div></div><div class="opsdash-filter"><div class="range-buttons"><button data-dashboard-range="today">วันนี้</button><button data-dashboard-range="7d">7 วัน</button><button data-dashboard-range="30d">30 วัน</button></div><select id="dashboardShift" aria-label="เลือกกะ"><option value="">ทุกกะ</option></select><button id="dashboardFullscreen" class="outline-button">เต็มหน้าจอ</button></div></section><div id="dashboardBody" class="loading">กำลังสรุปข้อมูล</div>`;
...
async function loadDashboard(showLoading=true,force=false){
  ...
  try{const query=`?range=${encodeURIComponent(dashboardState.range)}${dashboardState.shiftId?`&shiftId=${encodeURIComponent(dashboardState.shiftId)}`:""}`,data=await api(`/api/dashboard/summary${query}`);dashboardState.data=data;dashboardState.error="";dashboardState.lastLoadedAt=Date.now();renderDashboardData(data)}catch(error){dashboardState.error=error.message;if(dashboardState.data){renderDashboardData(dashboardState.data)}else if($("dashboardBody")){ $("dashboardBody").innerHTML=`<div class="empty-state"><b>โหลด Dashboard ไม่สำเร็จ</b><span>${escapeHtml(error.message)}</span><button id="retryDashboard" class="primary">ลองใหม่</button></div>`;$("retryDashboard")?.addEventListener("click",()=>loadDashboard(true,true))}}finally{dashboardState.busy=false}
}
...
function renderDashboardData(data){
  ...
  const ctx=data.shiftContext||{},shiftHours=ctx.startMinute==null?"ยังไม่กำหนดเวลา":`${minuteToTime(ctx.startMinute)}–${minuteToTime(ctx.endMinute)}${ctx.crossesMidnight?" · ข้ามวัน":""}`,fresh=dashboardState.error?`<span class="opsdash-stale">ข้อมูลล่าสุด · กำลังเชื่อมต่อใหม่</span>`:`<span class="opsdash-live">● สด</span>`,businessDay=formatDate(data.businessDate||data.from).split(" ")[0];
  $("dashboardBody").innerHTML=`<section class="opsdash-context"><div class="context-date"><span class="context-icon">▣</span><span><small>วันปฏิบัติงาน</small><b>${businessDay}</b></span></div><div><small>${ctx.selected?"กะที่เลือก":"กะปัจจุบัน"}</small><b style="--shift-color:${safeColor(ctx.color)}">${escapeHtml(ctx.shiftName||"ทุกกะ")} · ${shiftHours}</b></div><div><small>ช่วงข้อมูลแบบรุ่นรถเข้า</small><b>${formatDate(data.from)} – ${formatDate(data.to-1)}</b></div>${fresh}</section><section class="opsdash-kpis">${dashboardKpi("รถเข้า",m.total,deltaNote(data.comparison?.delta?.total,"ช่วงก่อนหน้า"),"#2563EB","▣")}${dashboardKpi("ปิดงาน",m.closed,`${Number(m.completionRate||0).toFixed(1)}% ของรถเข้า`,"#059669","✓")}${dashboardKpi("รถในพื้นที่",m.activeNow,"คันขณะนี้","#7C3AED","▰")}${dashboardKpi("ต้องเร่งดำเนินการ",m.warningNow,"ระดับเตือนขึ้นไป","#DC2626","!")}${dashboardKpi("เวลาเฉลี่ย",formatDuration(m.avgTotalSeconds),deltaNote(data.comparison?.delta?.avgTotalSeconds,"ช่วงก่อนหน้า",true),"#0F766E","◷")}${dashboardKpi("P90",formatDuration(m.p90TotalSeconds),"90% ปิดงานภายใน","#D97706","⌛")}</section><nav class="opsdash-tabs">${[["overview","ภาพรวม"],["comparison","เปรียบเทียบ"],["performance","ประสิทธิภาพ"],["capacity","ประตูและกะ"],["exceptions","ข้อยกเว้น"]].map(([id,label])=>`<button data-dashboard-tab="${id}" class="${dashboardState.tab===id?"active":""}">${label}</button>`).join("")}</nav><div id="dashboardTabContent" class="opsdash-content">${dashboardTabContent(data)}</div>`;
...
function dashboardTabContent(data){if(dashboardState.tab==="comparison")return dashboardComparison(data);if(dashboardState.tab==="performance")return dashboardPerformance(data);if(dashboardState.tab==="capacity")return dashboardCapacity(data);if(dashboardState.tab==="exceptions")return dashboardExceptions(data);return dashboardOverview(data)}
...
function dashboardOverview(data){return`<section class="opsdash-grid overview-grid control-center-grid">${panel("รถเข้าแต่ละชั่วโมง","คัน",hourChart(data.hours,Math.max(1,...(data.hours||[]).map(row=>Number(row.total)||0))),"hourly-panel")}${panel("คิวงานตามขั้นตอน","คัน",stageQueueChart(data.activeStages||{}),"stage-panel")}${panel("เปรียบเทียบรายวัน","7 วันล่าสุด",dailyComboChart(data.comparison?.daily||[]),"daily-panel")}${panel("เปรียบเทียบตามกะ","จำนวนรถ · ปิดงาน · เวลาเฉลี่ย",shiftCompareChart(data.shiftPerformance||[]),"shift-panel")}${panel("แนวโน้มเวลาเฉลี่ย","รายชั่วโมง",cycleTrendChart(data.hours||[],data.comparison?.targetTotalSeconds),"cycle-panel")}${panel("การใช้ประตู","24 ชั่วโมงล่าสุด",doorHeatmap(data.comparison?.doorHeatmap||[]),"heat-panel")}${panel("การกระจายระดับเตือน","รถในพื้นที่",alertDonut(data.alertLevels),"alert-panel")}${panel("รายการต้องดำเนินการเร่งด่วน","เรียงตามระดับเตือนและเวลารวม",actionTable(data.actionItems),"urgent-panel")}</section>`}
