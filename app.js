...
const FRONTEND_BUILD="2026.08.20-round204.1.1-retention-stability";
...
const appointmentCenterState={status:null,tab:(()=>{try{return localStorage.getItem("wvf_appointment_center_tab")||"overview"}catch{return"overview"}})(),volumeDays:1,volumeBusy:false,volumeData:null};
...
function appointmentCenterAllowedTabs(){
  return[["overview","ภาพรวม"],["timing","แผนเทียบเวลา"],["volume","แผนเทียบจำนวน"],["vendor","บริษัทผู้ส่งสินค้า"],["carrier","ผู้ขนส่ง"],["po","เลข PO"],["files","ไฟล์นำเข้า"]];
}
...
function appointmentRenderCenterTab(){
  ...
  if(tab==="volume"){
    body.innerHTML=appointmentPlanVolumeHtml();appointmentBindPlanVolume(true);return;
  }
  if(["vendor","carrier","po"].includes(tab)){
    appointmentPartnerAnalyticsState.type=tab;appointmentPartnerAnalyticsState.loaded=false;body.innerHTML=appointmentPartnerAnalyticsAdminHtml(false);appointmentBindPartnerAnalytics();void appointmentLoadPartnerAnalytics();return;
  }
...
function appointmentPlanActualAdminHtml(showProcess=true){
  return`<section class="appointment-plan-actual-card"><header><div><b>แผนเทียบเวลามาถึงจริง</b><small>มาก่อน = ตรงเวลา · มาช้าภายในเกณฑ์ = ตรงเวลา · เกินเกณฑ์ = มาสาย โดยใช้ผลชุดเดียวกับทุกหน้า</small></div><div class="appointment-plan-actual-actions"><label><span>ช่วงที่ดู</span><select id="appointmentPlanActualDays"><option value="1">วันนี้</option><option value="7">7 วัน</option><option value="30">30 วัน</option><option value="90">90 วัน</option></select></label><button id="appointmentPlanActualReload" class="outline-button" type="button">โหลดใหม่</button>${showProcess?`<button id="appointmentPlanActualProcess" class="outline-button" type="button">ประมวลผลรถล่าสุด</button><button id="appointmentPlanActualRecalculate" class="outline-button appointment-recalculate-button" type="button">คำนวณใหม่ด้วยเกณฑ์ปัจจุบัน</button>`:""}</div></header><div id="appointmentPlanActualBody" class="appointment-plan-actual-body"><div class="appointment-plan-actual-loading">กำลังอ่านผลเทียบเวลาที่บันทึกไว้</div></div></section>`
}
...
function appointmentPlanActualBodyHtml(data){
  ...
  return`${mixed}<div class="appointment-plan-actual-summary"><span><small>รถมีเลขนัดหมาย</small><b>${Number(s.eligible||0)}</b></span><span><small>เทียบได้</small><b>${Number(s.matched||0)}</b></span><span class="${tone}"><small>จับคู่ได้</small><b>${Number(s.coveragePercent||0).toFixed(1)}%</b></span><span><small>มาก่อน (ตรงเวลา)</small><b>${Number(s.early||0)}</b></span><span><small>ตรงเวลาในเกณฑ์</small><b>${Number(s.onTime||0)}</b></span><span class="good"><small>ตรงเวลารวม</small><b>${onTimePercent.toFixed(1)}%</b></span><span><small>มาสาย</small><b>${Number(s.late||0)}</b></span><span><small>ยังไม่มีผลเทียบ</small><b>${Number(s.unmatched||0)}</b></span></div><div class="appointment-plan-actual-meta"><span>จุดเริ่มคำนวณ <b>${escapeHtml(policy.referenceMode||data.referenceMode||"-")}</b></span><span>อนุโลมมาช้า <b>${grace} นาที</b></span><span>ช่วงค้นหาคู่เลขนัดหมาย <b>${Number(policy.searchWindowHours||data.searchWindowHours||0)} ชม.</b></span><span>มาก่อนเฉลี่ย <b>${Number(s.avgEarlyMinutes||0)?appointmentPlanActualDeltaText(-Math.abs(Number(s.avgEarlyMinutes))):"–"}</b></span><span>รถที่มาสาย ช้าจากนัดเฉลี่ย <b>${Number(s.avgLateMinutes||0)?appointmentPlanActualDeltaText(Math.abs(Number(s.avgLateMinutes))):"–"}</b></span></div>${rows?`<div class="appointment-plan-actual-table-wrap"><table class="appointment-plan-actual-table"><thead><tr><th>เลขนัดหมาย / บริษัท</th><th>เวลานัด</th><th>Gate In จริง</th><th>สถานะ KPI</th><th>ข้อเท็จจริงเทียบเวลานัด</th></tr></thead><tbody>${rows}</tbody></table></div>`:`<div class="appointment-plan-actual-empty compact"><b>ยังไม่มีผลเทียบในช่วงนี้</b><span>กด “ประมวลผลรถล่าสุด” เพื่อเติมผลของรถที่เข้ามาแล้ว</span></div>`}`
}
...
function appointmentPlanVolumeHtml(){
  return`<section class="appointment-plan-volume-card"><header><div><b>แผนเทียบจำนวนรถจริง</b><small>ดูว่าช่วงเวลาใดรถเข้ามากหรือน้อยกว่าแผน เพื่อช่วยจัดคนและประตู</small></div><div class="appointment-plan-volume-actions"><label><span>ช่วงที่ดู</span><select id="appointmentPlanVolumeDays"><option value="1">วันนี้</option><option value="7">7 วัน</option><option value="30">30 วัน</option></select></label><button id="appointmentPlanVolumeReload" class="outline-button" type="button">โหลดใหม่</button></div></header><div id="appointmentPlanVolumeBody" class="appointment-plan-volume-body"><div class="appointment-plan-volume-loading">กำลังอ่านแผนและจำนวนรถจริง</div></div></section>`
}
...
function appointmentPlanVolumeBodyHtml(data){
  ...
  return`<div class="appointment-plan-volume-summary"><span><small>ตามแผน</small><b>${Number(s.planned||0)}</b></span><span><small>เข้าจริง</small><b>${Number(s.actual||0)}</b></span><span class="good"><small>จับคู่แผนได้</small><b>${Number(s.matched||0)}</b></span><span><small>ยังไม่จับคู่</small><b>${Number(s.unmatched||0)}</b></span><span class="${Number(s.difference||0)>0?"warn":""}"><small>ต่างจากแผน</small><b>${Number(s.difference||0)>0?"+":""}${Number(s.difference||0)}</b></span><span><small>จับคู่ได้</small><b>${Number(s.matchPercent||0).toFixed(1)}%</b></span></div><div class="appointment-plan-volume-legend"><span><i></i>แผน</span><span><i></i>รถเข้าจริง</span><b>เวลาอ้างอิง ${escapeHtml(data.referenceMode||"-")}</b></div>${hourRows?`<div class="appointment-plan-volume-hours">${hourRows}</div>`:`<div class="appointment-plan-volume-empty compact">ยังไม่มีข้อมูลในช่วงนี้</div>`}${Number(data.days||1)>1&&daily?`<div class="appointment-plan-volume-daily"><table><thead><tr><th>วันที่</th><th>แผน</th><th>เข้าจริง</th><th>จับคู่แผน</th><th>ต่าง</th></tr></thead><tbody>${daily}</tbody></table></div>`:""}`
}
...
function appointmentPartnerBodyHtml(data){
  ...
  return`<div class="appointment-partner-summary"><span><small>${data.type==="po"?"PO ที่พบ":"จำนวนกลุ่ม"}</small><b>${Number(s.groups||0)}</b></span><span><small>${data.type==="po"?"รถที่ผูกกับ PO":"รถที่นำมาวัด"}</small><b>${Number(s.vehicles||0)}</b></span><span class="good"><small>ตรงเวลารวม</small><b>${Number(s.onTimePercent||0).toFixed(1)}%</b></span><span class="bad"><small>มาสาย</small><b>${Number(s.latePercent||0).toFixed(1)}%</b></span><span><small>สายเฉลี่ย</small><b>${appointmentPartnerMinutes(s.avgLateMinutes)}</b></span><span><small>ปฏิเสธ</small><b>${Number(s.rejected||0)}</b></span></div><div class="appointment-partner-note">ตรงเวลารวม = มาก่อน + มาช้าภายในเกณฑ์ Admin · ${escapeHtml(data.note||"")} · ${escapeHtml(data.storage||"")} · อ่าน ${Number(data.durationMs||0).toLocaleString("th-TH")} ms</div>${table?`<div class="appointment-partner-table-wrap"><table class="appointment-partner-table"><thead><tr><th>${nameHead}</th><th>รถ</th><th>มาก่อน (ตรงเวลา)</th><th>ตรงเวลารวม</th><th>มาสาย</th><th>สายเฉลี่ย</th><th>รอยื่นเอกสาร</th><th>รอตรวจรับ</th><th>ตรวจรับสินค้า</th><th>อยู่ในพื้นที่</th><th>ปฏิเสธ</th></tr></thead><tbody>${table}</tbody></table></div>`:`<div class="appointment-partner-empty compact"><b>ยังไม่มีข้อมูลในช่วงนี้</b><span>ต้องมีรถที่จับคู่กับข้อมูลนัดหมายและมีผลเทียบเวลาแล้ว</span></div>`}`
}
