...
const FRONTEND_BUILD="2026.08.19-round204-dashboard-datatable-accuracy";
...
function dashboardAppointmentPlanHtml(data){
  ...
  const metrics=[`<span><small>มีเลขนัดหมาย</small><b>${withNo.toLocaleString("th-TH")}</b></span>`,`<span><small>จับคู่ได้</small><b>${matched.toLocaleString("th-TH")}</b><em>${coverage.toFixed(1)}%</em></span>`,...(showTiming?[`<span><small>มาก่อน</small><b>${Number(p.early||0).toLocaleString("th-TH")}</b></span>`,`<span><small>ตรงเวลา</small><b>${Number(p.onTime||0).toLocaleString("th-TH")}</b></span>`,`<span><small>มาสาย</small><b>${Number(p.late||0).toLocaleString("th-TH")}</b><em>${lateAvg?`เฉลี่ย ${lateAvg.toFixed(1)} นาที`:""}</em></span>`]:[]),`<span><small>ยังไม่จับคู่</small><b>${Number(p.unmatched??p.notMatched??0).toLocaleString("th-TH")}</b></span>`];
...
async function showDashboardAppointmentLineage(data){
  ...
  await Swal.fire({title:"ที่มาของตัวเลขนัดหมาย",html:`<div class="appointment-lineage-swal"><div><b>ช่วงข้อมูล</b><span>${escapeHtml(l.scope||"ช่วงเดียวกับ Dashboard")}</span></div><div><b>ตามแผน</b><span>${escapeHtml(l.planned||"-")}</span></div><div><b>เข้าจริง</b><span>${escapeHtml(l.actual||"-")}</span></div><div><b>จับคู่ได้</b><span>${escapeHtml(l.matched||"-")}</span></div><div><b>ก่อน / ตรง / สาย</b><span>${escapeHtml(l.timing||"-")}</span></div><div><b>% จับคู่</b><span>${escapeHtml(l.coverage||"-")}</span></div><div><b>ผลต่าง</b><span>${escapeHtml(l.difference||"-")}</span></div><div><b>ตรวจยอดภายใน</b><span>${p.integrity?escapeHtml(i.ok?"ผ่าน · ยอดรายชั่วโมงและยอดสถานะตรงกับยอดรวม":"ต้องตรวจสอบ · มียอดย่อยไม่ตรงกับยอดรวม"):"-"}</span></div><div><b>ความสดของผลเทียบ</b><span>${escapeHtml(freshness)}</span></div><div><b>เกณฑ์ย้อนหลัง</b><span>${escapeHtml(audit)}</span></div><footer>ข้อมูล ณ ${escapeHtml(formatDate(data.generatedAt))} · เวลาอ้างอิง ${escapeHtml(p.referenceMode||"PERIOD")} · ${escapeHtml(l.timeZone||"Asia/Bangkok")} · สูตรรุ่น ${escapeHtml(l.version||"R204-1")}</footer></div>`,confirmButtonText:"ปิด",customClass:swalClasses(),buttonsStyling:false,width:580})
}
    ...
    <main class="dt-main dt-main-full">
      <section class="dt-filter-shell dt-filter-shell-compact">
        <div class="dt-filterbar dt-filterbar-r159">
          ...
          ${appointmentData.enabled&&appointmentData.showTiming!==false?`<select id="dtAppointmentTiming"><option value="ALL">ทุกผลเทียบเวลานัด</option><option value="EARLY">มาก่อน</option><option value="ON_TIME">ตรงเวลา</option><option value="LATE">มาสาย</option></select>`:""}
...
async function showDatatableLineage(){
  ...
  await Swal.fire({title:"ที่มาของตัวเลข Datatable",html:`<div class="appointment-lineage-swal"><div><b>ขอบเขตที่กำลังนับ</b><span>${escapeHtml(scope)}</span></div><div><b>ทั้งหมด</b><span>${escapeHtml(b.total||"นับหลังใช้ตัวกรองทั้งหมด")}</span></div><div><b>กำลังดำเนินการ</b><span>${escapeHtml(b.active||"-")}</span></div><div><b>เสร็จสิ้น</b><span>${escapeHtml(b.completed||"-")}</span></div><div><b>ระดับแจ้งเตือน</b><span>${escapeHtml(b.warnings||"-")}</span></div>${appointment.enabled?`<div><b>ก่อน / หลังนัด</b><span>${escapeHtml(b.appointmentTiming||"-")}</span></div><div><b>Vendor / Carrier / PO</b><span>${escapeHtml(b.appointmentDetails||"-")}</span></div>`:""}<div><b>ลำดับการคำนวณ</b><span>ใช้ช่วงวันที่และกะ → ใช้ตัวกรอง → คำนวณจำนวนทั้งหมด → แบ่งหน้าแสดงผล ดังนั้นจำนวนรวมไม่ขึ้นกับหน้าที่กำลังเปิด</span></div><div><b>การอ่านข้อมูลนัดหมาย</b><span>${escapeHtml(p.appointmentSource==="STORED_METRIC"?"ใช้ผลที่บันทึกไว้ของรถปัจจุบัน ไม่จับคู่เวลาซ้ำเมื่อเปิด Datatable":"ไม่ใช้ข้อมูลนัดหมายในคำขอนี้")}</span></div><footer>ข้อมูล ณ ${escapeHtml(formatDate(data.generatedAt))} · อ่านฐานข้อมูล ${Number(p.rowsRead||0).toLocaleString("th-TH")} แถว · ใช้เวลา query ${Number(p.durationMs||0).toFixed(1)} ms · รุ่น R204</footer></div>`,confirmButtonText:"ปิด",customClass:swalClasses(),buttonsStyling:false,width:620})
}
...
async function appointmentParseFile(file){
  ...
  try{const buffer=await file.arrayBuffer(),worker=new Worker("./appointment-excel-worker.js?v=20260819-r200");appointmentUploadState.worker=worker;worker.onmessage=async event=>{const m=event.data||{};if(m.type==="PROGRESS")appointmentSetBusy(true,m.message||"กำลังตรวจข้อมูล");if(m.type==="ERROR"){appointmentStopWorker();appointmentSetBusy(false);await showNotice("error",m.message||"อ่านไฟล์ไม่สำเร็จ")}if(m.type==="MAPPING_REQUIRED"){appointmentStopWorker();appointmentSetBusy(false);appointmentRenderMappingNeeded(m.result||{});window.scrollTo({top:0,behavior:"smooth"})}if(m.type==="RESULT"){appointmentStopWorker();appointmentSetBusy(false);appointmentUploadState.result=m.result;appointmentUploadState.preview=null;appointmentRenderMappingNeeded(null);appointmentRenderResult()}};worker.onerror=async event=>{appointmentStopWorker();appointmentSetBusy(false);await showNotice("error",event.message||"อ่านไฟล์ไม่สำเร็จ")};worker.postMessage({type:"PARSE",buffer,fileName:file.name,config:appointmentUploadState.config?.parserConfig||{}},[buffer])}catch(error){appointmentSetBusy(false);await showNotice("error",error.message||"เปิดไฟล์ไม่สำเร็จ")}
}
...
function appointmentPlanActualBodyHtml(data){
  ...
  const s=data.summary||{},policy=data.policy||{},recent=Array.isArray(data.recent)?data.recent:[],tone=s.coveragePercent>=90?"good":s.coveragePercent>=70?"warn":"bad",mixed=s.mixedPolicy?`<div class="appointment-plan-actual-warning">ช่วงที่เลือกมีผลจากเกณฑ์เวลาเดิมและปัจจุบันร่วมกัน ระบบเก็บประวัติเดิมไว้และไม่เขียนทับอัตโนมัติ</div>`:"";
  const rows=recent.map(row=>`<tr><td><b>${escapeHtml(row.appointmentNo||"-")}</b><small>${escapeHtml(row.companyName||"")}</small></td><td>${escapeHtml(row.plannedAtDisplay||"-")}</td><td>${escapeHtml(row.gateInAtDisplay||"-")}</td><td><span class="appointment-plan-status ${String(row.status||"").toLowerCase()}">${escapeHtml(row.statusLabel||"-")}</span></td><td><b>${escapeHtml(appointmentPlanActualDeltaText(row.deltaMinutes))}</b></td></tr>`).join("");
  return`${mixed}<div class="appointment-plan-actual-summary"><span><small>รถมีเลขนัดหมาย</small><b>${Number(s.eligible||0)}</b></span><span><small>เทียบได้</small><b>${Number(s.matched||0)}</b></span><span class="${tone}"><small>จับคู่ได้</small><b>${Number(s.coveragePercent||0).toFixed(1)}%</b></span><span><small>มาก่อน</small><b>${Number(s.early||0)}</b></span><span><small>ตรงเวลา</small><b>${Number(s.onTime||0)}</b></span><span><small>มาสาย</small><b>${Number(s.late||0)}</b></span><span><small>ยังไม่มีผลเทียบ</small><b>${Number(s.unmatched||0)}</b></span></div><div class="appointment-plan-actual-meta"><span>ใช้เวลาอ้างอิง <b>${escapeHtml(policy.referenceMode||data.referenceMode||"-")}</b></span><span>ช่วงค้นหา <b>${Number(policy.searchWindowHours||data.searchWindowHours||0)} ชม.</b></span><span>มาก่อนเฉลี่ย <b>${Number(s.avgEarlyMinutes||0)?appointmentPlanActualDeltaText(-Math.abs(Number(s.avgEarlyMinutes))):"–"}</b></span><span>มาสายเฉลี่ย <b>${Number(s.avgLateMinutes||0)?appointmentPlanActualDeltaText(Math.abs(Number(s.avgLateMinutes))):"–"}</b></span></div>${rows?`<div class="appointment-plan-actual-table-wrap"><table class="appointment-plan-actual-table"><thead><tr><th>เลขนัดหมาย / บริษัท</th><th>เวลานัด</th><th>Gate In</th><th>ผล</th><th>ต่างจากนัด</th></tr></thead><tbody>${rows}</tbody></table></div>`:`<div class="appointment-plan-actual-empty compact"><b>ยังไม่มีผลเทียบในช่วงนี้</b><span>กด “ประมวลผลรถล่าสุด” เพื่อเติมผลของรถที่เข้ามาแล้ว</span></div>`}`
}
...
function appointmentPartnerBodyHtml(data){
  ...
  return`<div class="appointment-partner-summary"><span><small>${data.type==="po"?"PO ที่พบ":"จำนวนกลุ่ม"}</small><b>${Number(s.groups||0)}</b></span><span><small>${data.type==="po"?"รถที่ผูกกับ PO":"รถที่นำมาวัด"}</small><b>${Number(s.vehicles||0)}</b></span><span class="good"><small>ตรงเวลา</small><b>${Number(s.onTimePercent||0).toFixed(1)}%</b></span><span class="bad"><small>มาสาย</small><b>${Number(s.latePercent||0).toFixed(1)}%</b></span><span><small>สายเฉลี่ย</small><b>${appointmentPartnerMinutes(s.avgLateMinutes)}</b></span><span><small>ปฏิเสธ</small><b>${Number(s.rejected||0)}</b></span></div><div class="appointment-partner-note">${escapeHtml(data.note||"")} · ${escapeHtml(data.storage||"")} · อ่าน ${Number(data.durationMs||0).toLocaleString("th-TH")} ms</div>${table?`<div class="appointment-partner-table-wrap"><table class="appointment-partner-table"><thead><tr><th>${nameHead}</th><th>รถ</th><th>มาก่อน</th><th>ตรงเวลา</th><th>มาสาย</th><th>สายเฉลี่ย</th><th>รอยื่นเอกสาร</th><th>รอตรวจรับ</th><th>ตรวจรับสินค้า</th><th>อยู่ในพื้นที่</th><th>ปฏิเสธ</th></tr></thead><tbody>${table}</tbody></table></div>`:`<div class="appointment-partner-empty compact"><b>ยังไม่มีข้อมูลในช่วงนี้</b><span>ต้องมีรถที่จับคู่กับข้อมูลนัดหมายและมีผลเทียบเวลาแล้ว</span></div>`}`
}
...
function renderAdminAppointmentData(){
  const cfg=adminState.data?.appointmentModule||{moduleEnabled:false,uploadEnabled:false,useImportedData:false,companySourceMode:"AUTO",timeReference:"PERIOD",searchWindowHours:36,adapterTimeoutMs:250,lifecycle:{},display:{},uploadProfile:{}},schema=adminState.data?.appointmentSchema||{ready:false,tablesReady:0,tablesTotal:5,indexReady:false,latest:null},kpiSchema=adminState.data?.appointmentKpiSchema||{ready:false,tablesReady:0,tablesTotal:3,indexesReady:0,indexesTotal:3,metrics:0,policy:null,lastRun:null},profile=cfg.uploadProfile||{};
...
function appointmentKpiUatHtml(data){
  ...
  const policyHtml=`<div class="appointment-kpi-policy"><b>เกณฑ์เวลา: ${escapeHtml(policy.referenceMode||'-')}</b><span>ช่วงค้นหา ${Number(policy.searchWindowHours||0)} ชม. · เริ่ม ${escapeHtml(policy.effectiveAtDisplay||'-')}</span><small>หากผู้ดูแลเปลี่ยน PERIOD / FROM ระบบจะใช้เกณฑ์ใหม่กับข้อมูลถัดไป แต่ไม่คำนวณประวัติเก่าทับเอง</small></div>`;
