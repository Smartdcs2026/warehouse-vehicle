...
const FRONTEND_BUILD="2026.08.18-round179-production-appointment-import";
...
async function init() {
  ...
  if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js?v=20260818-r179",{updateViaCache:"none"}).catch(() => undefined);
...
function appointmentSetBusy(on,message=""){appointmentUploadState.busy=on;const el=$("appointmentUploadProgress");if(el){el.hidden=!on;el.textContent=on?message:""}const choose=$("appointmentFileInput");if(choose)choose.disabled=on;["appointmentPreviewChanges","appointmentConfirmImport"].forEach(id=>{const b=$(id);if(b&&on)b.disabled=true})}
