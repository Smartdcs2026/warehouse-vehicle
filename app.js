...
const FRONTEND_BUILD="2026.08.19-round200-flexible-appointment-files";
...
function appointmentPlanActualAdminHtml(){
  return`<section class="appointment-plan-actual-card"><header><div><b>แผนเทียบเวลามาถึงจริง</b><small>ใช้เวลานัดเทียบกับ Gate In และเก็บผลไว้สำหรับดูย้อนหลัง โดยไม่แก้ข้อมูล Gate In</small></div><div class="appointment-plan-actual-actions"><label><span>ช่วงที่ดู</span><select id="appointmentPlanActualDays"><option value="1">วันนี้</option><option value="7">7 วัน</option><option value="30">30 วัน</option><option value="90">90 วัน</option></select></label><button id="appointmentPlanActualReload" class="outline-button" type="button">โหลดใหม่</button><button id="appointmentPlanActualProcess" class="outline-button" type="button">ประมวลผลรถล่าสุด</button></div></header><div id="appointmentPlanActualBody" class="appointment-plan-actual-body"><div class="appointment-plan-actual-loading">กำลังอ่านผลเทียบเวลาที่บันทึกไว้</div></div></section>`
}
...
function appointmentPlanActualBodyHtml(data){
  ...
  return`${mixed}<div class="appointment-plan-actual-summary"><span><small>รถมีเลขนัดหมาย</small><b>${Number(s.eligible||0)}</b></span><span><small>เทียบได้</small><b>${Number(s.matched||0)}</b></span><span class="${tone}"><small>ครอบคลุม</small><b>${Number(s.coveragePercent||0).toFixed(1)}%</b></span><span><small>มาก่อน</small><b>${Number(s.early||0)}</b></span><span><small>ตรงเวลา</small><b>${Number(s.onTime||0)}</b></span><span><small>มาสาย</small><b>${Number(s.late||0)}</b></span><span><small>ยังไม่มีผลเทียบ</small><b>${Number(s.unmatched||0)}</b></span></div><div class="appointment-plan-actual-meta"><span>ใช้เวลาอ้างอิง <b>${escapeHtml(policy.referenceMode||data.referenceMode||"-")}</b></span><span>ช่วงค้นหา <b>${Number(policy.searchWindowHours||data.searchWindowHours||0)} ชม.</b></span><span>มาก่อนเฉลี่ย <b>${Number(s.avgEarlyMinutes||0)?appointmentPlanActualDeltaText(-Math.abs(Number(s.avgEarlyMinutes))):"–"}</b></span><span>มาสายเฉลี่ย <b>${Number(s.avgLateMinutes||0)?appointmentPlanActualDeltaText(Math.abs(Number(s.avgLateMinutes))):"–"}</b></span></div>${rows?`<div class="appointment-plan-actual-table-wrap"><table class="appointment-plan-actual-table"><thead><tr><th>เลขนัดหมาย / บริษัท</th><th>เวลานัด</th><th>Gate In</th><th>ผล</th><th>ต่างจากนัด</th></tr></thead><tbody>${rows}</tbody></table></div>`:`<div class="appointment-plan-actual-empty compact"><b>ยังไม่มีผลเทียบในช่วงนี้</b><span>กด “ประมวลผลรถล่าสุด” เพื่อเติมผลของรถที่เข้ามาแล้ว</span></div>`}`
}
async function appointmentLoadPlanActual(){
  ...
  try{const data=await api(`/api/admin/appointment-plan-actual?days=${encodeURIComponent(appointmentPlanActualAdminState.days)}`,{timeoutMs:12000});if($("appointmentPlanActualBody"))$("appointmentPlanActualBody").innerHTML=appointmentPlanActualBodyHtml(data)}
...
function appointmentPartnerAnalyticsAdminHtml(){
  return`<section class="appointment-partner-analytics-card"><header><div><b>ตัวชี้วัดจากไฟล์นัดหมาย</b><small>ดูบริษัทผู้ส่งสินค้า ผู้ขนส่ง และ PO เทียบกับเวลามาถึงและเวลาทำงานจริง โดยใช้ข้อมูลเดิมในระบบ</small></div><div class="appointment-partner-actions"><label><span>ช่วงที่ดู</span><select id="appointmentPartnerDays"><option value="7">7 วัน</option><option value="30">30 วัน</option><option value="90">90 วัน</option></select></label><button id="appointmentPartnerLoad" class="outline-button" type="button">โหลดข้อมูล</button></div></header><div class="appointment-partner-tabs" role="tablist"><button type="button" data-partner-type="vendor" class="active">บริษัทผู้ส่งสินค้า</button><button type="button" data-partner-type="carrier">ผู้ขนส่ง</button><button type="button" data-partner-type="po">เลข PO</button></div><div id="appointmentPartnerBody" class="appointment-partner-body"><div class="appointment-partner-empty"><b>ยังไม่ได้อ่านข้อมูลวิเคราะห์</b><span>กด “โหลดข้อมูล” เมื่อต้องการดู เพื่อไม่เพิ่มภาระฐานข้อมูลโดยไม่จำเป็น</span></div></div></section>`
}
...
async function appointmentLoadPartnerAnalytics(){
  ...
  try{const data=await api(`/api/admin/appointment-partner-analytics?days=${encodeURIComponent(appointmentPartnerAnalyticsState.days)}&type=${encodeURIComponent(appointmentPartnerAnalyticsState.type)}`,{timeoutMs:15000});appointmentPartnerAnalyticsState.loaded=true;if($("appointmentPartnerBody"))$("appointmentPartnerBody").innerHTML=appointmentPartnerBodyHtml(data)}catch(error){if($("appointmentPartnerBody"))$("appointmentPartnerBody").innerHTML=`<div class="appointment-partner-empty"><b>โหลดข้อมูลไม่สำเร็จ</b><span>${escapeHtml(error.message||"กรุณาลองใหม่")}</span></div>`}finally{appointmentPartnerAnalyticsState.busy=false}
}
...
function renderAdminAppointmentData(){
  ...
  $("appointmentPlanActualDays")?.addEventListener("change",event=>{appointmentPlanActualAdminState.days=Number(event.target.value)||7;void appointmentLoadPlanActual()});$("appointmentPlanActualReload")?.addEventListener("click",()=>void appointmentLoadPlanActual());$("appointmentPlanActualProcess")?.addEventListener("click",appointmentProcessPlanActual);void appointmentLoadPlanActual();appointmentBindPartnerAnalytics();
