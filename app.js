...
function actionTable(rows){return`<div class="action-table-head"><span>ระดับ</span><span>เลขนัดหมาย</span><span>ทะเบียนรถ</span><span>ขั้นตอน</span><span>ประตู</span><span>เวลาขั้นตอน</span><span>เวลารวม</span></div>${(rows||[]).map(row=>`<div class="action-table-row" style="--alert-color:${safeColor(row.alert_color)}"><b>${alertLevelLabel(row.alert_level)}</b><strong>${escapeHtml(row.appointment_no||row.auto_id)}</strong><span>${escapeHtml(joinText(row.vehicle_plate,row.province))}</span><span>${statusLabel(row.current_status)}</span><span>${escapeHtml(row.door_code||"–")}</span><b>${formatDuration(row.stage_elapsed_seconds)}</b><b>${formatDuration(row.total_elapsed_seconds)}</b></div>`).join("")||`<div class="empty-state">ไม่มีรายการที่ต้องติดตาม</div>`}`}
