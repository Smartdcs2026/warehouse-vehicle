...
function renderDatatableLayout(){
  ...
  $("pageContent").innerHTML=`<section class="dt-page dt-clean-page dt-r100-page">
    <section class="dt-r100-top">
      <div class="dt-r100-controls">
        <label><span>วันที่เริ่ม</span><input id="dtFrom" type="date" value="${escapeHtml(datatableState.from)}"></label>
...
function datatableActionPolicy(row,status=row.currentStatus){
  ...
  if(stage==="document_review"){
    ...
    return{type:"detail",label:"รายละเอียด"};
  }
  if(stage==="ready")return status==="READY_FOR_RECEIVING"?{type:"ready",label:"จัดการคิว"}:{type:"detail",label:"รายละเอียด"};
  if(stage==="receiving")return status==="RECEIVING_IN_PROGRESS"?{type:"receiving",label:"จัดการรับสินค้า"}:{type:"detail",label:"รายละเอียด"};
  if(stage==="document_return")return ["WAITING_DOCUMENT_RETURN","REJECTED_WAITING_DOCUMENT_RETURN"].includes(status)?{type:"track",label:"ติดตาม"}:{type:"detail",label:"รายละเอียด"};
  if(stage==="gate_out")return ["WAITING_GATE_OUT","REJECTED_WAITING_GATE_OUT"].includes(status)?{type:"track",label:"ติดตาม"}:{type:"detail",label:"รายละเอียด"};
  if(stage==="overview"){
    if(status==="WAITING_DOCUMENT_CHECK"&&role==="ADMIN")return{type:"document_check",label:"ตรวจเอกสารเสร็จ"};
    ...
    if(status==="CLOSED")return{type:"detail",label:"รายละเอียด"};
    return{type:"track",label:"ติดตาม"};
  }
  return{type:"detail",label:"รายละเอียด"};
}
...
function datatableCompareDatasetHtml(compare,setKey,label,tone){
  const set=compare[setKey],singleDate=compare.dimension==="DATE",shiftControl=compare.dimension==="SHIFT"?"":`<label><span>กะ</span><select data-cmp-field="${setKey}.shiftId">${datatableCompareShiftOptions(set.shiftId)}</select></label>`,optional=(["DATE","RANGE"].includes(compare.dimension)?`<div class="dt-compare-set-secondary"><label><span>ประตู</span><select data-cmp-field="${setKey}.door">${datatableCompareDoorOptions(set.door)}</select></label><label><span>ผู้ดำเนินการ</span><select data-cmp-field="${setKey}.actor">${datatableCompareActorOptions(set.actor)}</select></label></div>`:"");
  return`<section class="dt-compare-set ${tone}"><header><span>${label}</span><b>ชุดข้อมูล ${label}</b></header><div class="dt-compare-date-grid ${singleDate?"is-single":""}"><label><span>${singleDate?"วันที่":"วันที่เริ่ม"}</span><input type="date" data-cmp-field="${setKey}.from" value="${escapeHtml(set.from)}"></label>${singleDate?"":`<label><span>วันที่สิ้นสุด</span><input type="date" data-cmp-field="${setKey}.to" value="${escapeHtml(set.to)}"></label>`}</div>${shiftControl}${datatableCompareDatasetPrimary(compare,setKey)}${optional}</section>`
}
function datatableCompareMetricCard(label,unit,key,a,b,difference,kind="count"){
  const format=value=>kind==="average"?datatableCompareMinuteClock(value):kind==="duration"?datatableCompareHourClock(value):Number(value||0).toLocaleString("th-TH"),deltaValue=Number(difference?.[key]||0),percentKey=key.replace(/Count$/,"Percent").replace("avgSeconds","avgPercent").replace("totalSeconds","totalPercent").replace("vehicleCount","vehiclePercent"),percent=difference?.[percentKey],badHigher=["avgSeconds","lateCount","criticalCount","rejectedCount","totalSeconds"].includes(key),tone=deltaValue===0?"neutral":badHigher?(deltaValue>0?"bad":"good"):"neutral",deltaText=kind==="average"?datatableCompareSignedMinuteClock(deltaValue):kind==="duration"?datatableCompareSignedHourClock(deltaValue):`${deltaValue>0?"+":""}${deltaValue.toLocaleString("th-TH")}`,percentText=percent==null?"":` · ${percent>0?"+":""}${Number(percent).toFixed(1)}%`;
  return`<article class="dt-compare-metric ${tone}"><header><b>${label}</b><small>${unit}</small></header><div class="dt-compare-ab"><div><span>A</span><strong>${format(a?.[key])}</strong></div><div><span>B</span><strong>${format(b?.[key])}</strong></div></div><footer><span>ผลต่าง B−A</span><b>${deltaText}${percentText}</b></footer></article>`
}
...
function datatableCompareResultsHtml(result){
  if(!result)return`<section class="dt-compare-result-empty"><b>พร้อมเปรียบเทียบ</b><span>เลือกชุดข้อมูล A และ B แล้วกด “เปรียบเทียบ”</span></section>`;
  const a=result.a||{},b=result.b||{},d=result.difference||{};
  return`<section class="dt-compare-results"><div class="dt-compare-metrics">${datatableCompareMetricCard("จำนวนรถ","คัน","vehicleCount",a,b,d)}${datatableCompareMetricCard("เวลาเฉลี่ย","นาที:วินาที","avgSeconds",a,b,d,"average")}${datatableCompareMetricCard("ล่าช้า","รายการ","lateCount",a,b,d)}${datatableCompareMetricCard("วิกฤต","รายการ","criticalCount",a,b,d)}${datatableCompareMetricCard("ปฏิเสธรับสินค้า","รายการ","rejectedCount",a,b,d)}${datatableCompareMetricCard("เวลารวม","ชม.:นาที","totalSeconds",a,b,d,"duration")}</div>${datatableCompareSummary(result)}</section>`
}
function datatableCompareHtml(compare){
  const dimensions=[["DATE","วันที่"],["RANGE","ช่วงวันที่"],["SHIFT","กะ"],["DOOR","ประตู"],["ACTOR","ผู้ดำเนินการ"],["STAGE","ช่วงงาน"]],stageDisabled=compare.dimension==="STAGE";
  return`<div class="dt-compare-popup"><section class="dt-compare-dimensions"><small>มิติที่ใช้เปรียบเทียบ</small><div>${dimensions.map(([key,label])=>`<button type="button" data-cmp-dimension="${key}" class="${compare.dimension===key?"active":""}">${label}</button>`).join("")}</div></section><div class="dt-compare-sets">${datatableCompareDatasetHtml(compare,"a","A","tone-a")}${datatableCompareDatasetHtml(compare,"b","B","tone-b")}</div><section class="dt-compare-common"><label><span>ช่วงงาน</span><select id="cmpCommonStage" ${stageDisabled?"disabled":""}>${datatableCompareStageOptions(compare.common.stage)}</select></label><label><span>สถานะ</span><select id="cmpCommonStatus"><option value="ALL">ทุกสถานะ</option><option value="ACTIVE" ${compare.common.status==="ACTIVE"?"selected":""}>ระหว่างดำเนินการ</option><option value="CLOSED" ${compare.common.status==="CLOSED"?"selected":""}>เสร็จสิ้น</option><option value="REJECTED" ${compare.common.status==="REJECTED"?"selected":""}>ปฏิเสธรับสินค้า</option>${Object.entries(DATATABLE_STATUS_OPTIONS).filter(([key])=>key!=="CLOSED").map(([key,label])=>`<option value="${key}" ${compare.common.status===key?"selected":""}>${escapeHtml(label)}</option>`).join("")}</select></label><label><span>ระดับแจ้งเตือน</span><select id="cmpCommonSla"><option value="ALL">ทุกระดับ</option>${["NORMAL","WATCH","WARNING","URGENT","CRITICAL"].map(level=>`<option value="${level}" ${compare.common.sla===level?"selected":""}>${alertLevelLabel(level)}</option>`).join("")}</select></label></section><div id="dtCompareResults">${datatableCompareResultsHtml(compare.result)}</div><footer class="dt-compare-footer"><button type="button" id="cmpRun" class="primary">เปรียบเทียบ</button><button type="button" id="cmpReset" class="outline-button">ล้างค่า</button><button type="button" id="cmpClose" class="quiet-button">ปิด</button></footer></div>`
}
