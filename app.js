...
function dataSourceToolbar(data){const sources=Array.isArray(data?.sources)?data.sources:[],current=data?.source||sources.find(item=>item.key===adminDataTools.source)||sources[0]||{key:"main",label:"ฐานงานหลัก",name:"warehouse-vehicle-flow"};return`<section class="data-source-toolbar"><div><small>ฐานที่กำลังดู</small><b>${escapeHtml(current.label||"ฐานข้อมูล")}</b><span>${escapeHtml(current.name||"")}</span></div><label><span>เลือกฐานข้อมูล</span><select id="dataSourceSelect">${sources.map(item=>`<option value="${escapeHtml(item.key)}" ${item.key===current.key?"selected":""} ${item.connected===false?"disabled":""}>${escapeHtml(item.label)}${item.connected===false?" · ยังไม่เชื่อม":""}</option>`).join("")}</select></label></section>`}
function bindDataSourceToolbar(body){$("dataSourceSelect")?.addEventListener("change",async event=>{adminDataTools.source=String(event.target.value||"main");adminDataTools.openTable="";adminDataTools.openSchema="";adminDataTools.openCommand="";await renderAdminDataToolBody({})})}
function renderDisconnectedDataSource(body,data){body.innerHTML=`${dataSourceToolbar(data)}<div class="empty-state data-source-disconnected"><b>ฐานนัดหมายแยกยังไม่เชื่อมเข้าหน้าผู้ดูแล</b><span>เมื่อเชื่อมฐานนี้แล้ว จะสามารถดู คัดลอก และดาวน์โหลดโครงสร้างกับชุดคำสั่งได้เหมือนฐานงานหลัก</span></div>`;bindDataSourceToolbar(body)}
