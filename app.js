...
const datatableState={meta:null,data:null,busy:false,reloadRequested:false,requestSeq:0,detailBusy:false,refreshMetaRequested:false,activity:[],shiftAutoDate:false,stage:"overview",from:"",to:"",shiftId:"",search:"",sla:"ALL",status:"ALL",door:"",actor:"",sort:"start_desc",page:1,limit:25,problemOnly:false,rejectedOnly:false,searchTimer:0,columns:new Set(JSON.parse(localStorage.getItem("wvf_dt_columns_r96")||localStorage.getItem("wvf_dt_columns_r95")||'["company","plate","shift","actor"]'))};
...
function renderDatatableLayout(){
  ...
  $("pageContent").innerHTML=`<section class="dt-page dt-clean-page dt-formal-page">
    <section class="dt-top-panel">
      <div class="dt-top-panel-head">
        <div class="dt-top-panel-copy"><b>ศูนย์ข้อมูล Datatable</b><span>ติดตามสถานะงานและสั่งการได้จากหน้าจอเดียว</span></div>
        <div class="dt-top-panel-status"><i></i><div><b id="dtPanelStatus">พร้อมใช้งาน</b><small id="dtPanelStatusSub">กำลังเชื่อมต่อข้อมูล</small></div></div>
      </div>
      <div class="dt-top-panel-controls">
        <label><span>วันที่เริ่มต้น</span><input id="dtFrom" type="date" value="${escapeHtml(datatableState.from)}"></label>
        <label><span>วันที่สิ้นสุด</span><input id="dtTo" type="date" value="${escapeHtml(datatableState.to)}"></label>
        <label><span>กะ</span><select id="dtShift"><option value="">ทุกกะ</option>${shiftOptions}</select></label>
        <button id="dtRefresh" class="quiet-button" type="button">รีเฟรช</button>
        <button id="dtFullscreen" class="outline-button" type="button">เต็มจอ</button>
        <button id="dtExport" class="primary" type="button">ส่งออก</button>
        <span id="dtShiftHint" class="dt-shift-hint" hidden></span>
      </div>
      <div class="dt-top-panel-body">
        <nav id="dtStageTabs" class="dt-stage-tabs dt-stage-tabs-formal">${stages.map(s=>`<button type="button" data-dt-stage="${s.key}" class="${datatableState.stage===s.key?"active":""}"><span>${escapeHtml(s.label)}</span></button>`).join("")}</nav>
        <section id="dtSummary" class="dt-summary dt-summary-formal"></section>
      </div>
    </section>
