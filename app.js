...
const adminDataTools={tab:"overview",inspector:null,busy:false,openTable:"",openCommand:"",openSchema:"",archiveMonth:"",archivePreview:null,archiveBusy:false,cleanupMonth:"",cleanupPreview:null,cleanupBusy:false,cleanupScript:null};
...
async function renderAdminArchive(body){
  ...
  const data=adminDataTools.archivePreview||{},files=(data.files||[]).filter(item=>item.available!==false),ready=Number(data.readyVehicles||0),open=Number(data.openVehicles||0);
  body.innerHTML=`<section class="archive-head"><div><h3>เก็บข้อมูลย้อนหลัง</h3><p>เลือกเดือน ตรวจสอบ แล้วดาวน์โหลดเก็บไว้ก่อน รอบนี้ยังไม่มีปุ่มลบข้อมูล</p></div><label><span>เดือนที่ต้องการ</span><input id="archiveMonth" type="month" value="${escapeHtml(adminDataTools.archiveMonth)}" min="2020-01" max="2100-12"></label><button id="archiveCheck" class="quiet-button" type="button">ตรวจสอบ</button></section><div class="data-safe-note"><b>ปลอดภัย</b><span>ระบบจะส่งออกเฉพาะรถที่จบงานและออกจากพื้นที่แล้ว รายการที่ยังทำงานอยู่จะไม่รวมในชุดเก็บย้อนหลัง</span></div><section class="archive-summary"><article><small>รถทั้งหมดในเดือน</small><b>${Number(data.totalVehicles||0).toLocaleString("th-TH")}</b></article><article class="ok"><small>พร้อมเก็บย้อนหลัง</small><b>${ready.toLocaleString("th-TH")}</b></article><article class="warn"><small>ยังไม่จบงาน</small><b>${open.toLocaleString("th-TH")}</b></article><article><small>ช่วงข้อมูล</small><b>${escapeHtml(data.periodLabel||adminDataTools.archiveMonth)}</b></article></section>${open?`<div class="archive-warning"><b>มี ${open.toLocaleString("th-TH")} รายการที่ยังไม่จบงาน</b><span>ระบบจะเว้นรายการเหล่านี้ไว้ ไม่รวมในไฟล์ย้อนหลัง</span></div>`:""}<section class="archive-files"><header><div><h3>ไฟล์ที่ดาวน์โหลดได้</h3><p>ดาวน์โหลดเก็บไว้เป็นหลักฐานก่อนทำการเคลียร์ข้อมูลในรอบถัดไป</p></div><button id="archiveSummaryDownload" class="quiet-button" type="button">ดาวน์โหลดสรุป</button></header>${files.map(item=>`<article><div><b>${escapeHtml(item.label)}</b><span>${Number(item.rows||0).toLocaleString("th-TH")} แถว</span></div><button type="button" data-archive-file="${escapeHtml(item.id)}" ${Number(item.rows||0)===0?"disabled":""}>ดาวน์โหลด CSV</button></article>`).join("")||`<div class="empty-state">ไม่พบข้อมูลที่พร้อมเก็บย้อนหลัง</div>`}</section><section class="archive-full-backup"><div><h3>สำรองทั้งฐานข้อมูล</h3><p>ใช้เมื่อต้องการเก็บสำเนาทั้งฐานก่อนทำงานสำคัญ</p></div><button id="copyFullBackupCommand" class="quiet-button" type="button">คัดลอกคำสั่ง</button><pre id="fullBackupCommand">npx wrangler d1 export &lt;ชื่อฐานข้อมูล&gt; --remote --output=./warehouse-vehicle-backup.sql</pre></section>`;
