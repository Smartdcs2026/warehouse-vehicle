  ...
  $("pageContent").innerHTML=`<section class="dt-page dt-clean-page dt-r100-page">
    <section class="dt-r100-top">
      <div class="dt-r100-controls">
        ...
        <div class="dt-r100-actions"><button id="dtRefresh" class="quiet-button" type="button">รีเฟรช</button><button id="dtFullscreen" class="outline-button" type="button">เต็มจอ</button><button id="dtExport" class="primary" type="button">ส่งออก</button></div>
...
async function showDatatableReadyActions(vehicle){if(!window.Swal)return;const called=Number(vehicle.queue_called_at||0)>0,buttons=[[called?"recall":"call",called?"เรียกซ้ำ / เปลี่ยนประตู":"เรียกรถ"],["start","เริ่มตรวจรับ"],["reject","ปฏิเสธรับสินค้า"],["track","ติดตามสถานะ"]];return Swal.fire({title:`พร้อมตรวจรับ ${escapeHtml(vehicle.appointment_no||vehicle.auto_id)}`,html:`<div class="dt-action-menu">${buttons.map(([key,label])=>`<button type="button" data-dt-action="${key}">${escapeHtml(label)}</button>`).join("")}</div>`,showConfirmButton:false,showCancelButton:true,cancelButtonText:"ปิด",customClass:{...swalClasses(),popup:"wfv-swal dt-action-swal"},buttonsStyling:false,width:430,didOpen:()=>document.querySelectorAll("[data-dt-action]").forEach(btn=>btn.addEventListener("click",()=>{Swal.close();const action=btn.dataset.dtAction;if(action==="call")callVehicle(vehicle,false);else if(action==="recall")callVehicle(vehicle,true);else if(action==="start")startReceiving(vehicle);else if(action==="reject")rejectReceiving(vehicle);else if(action==="track")openDatatableTrack(vehicle.auto_id)}))})}
async function showDatatableReceivingActions(vehicle){if(!window.Swal)return;const buttons=[["complete","รับสินค้าเสร็จ"],["reject","ปฏิเสธรับสินค้า"],["track","ติดตามสถานะ"]];return Swal.fire({title:`กำลังตรวจรับ ${escapeHtml(vehicle.appointment_no||vehicle.auto_id)}`,html:`<div class="dt-action-menu">${buttons.map(([key,label])=>`<button type="button" data-dt-action="${key}">${escapeHtml(label)}</button>`).join("")}</div>`,showConfirmButton:false,showCancelButton:true,cancelButtonText:"ปิด",customClass:{...swalClasses(),popup:"wfv-swal dt-action-swal"},buttonsStyling:false,width:430,didOpen:()=>document.querySelectorAll("[data-dt-action]").forEach(btn=>btn.addEventListener("click",()=>{Swal.close();const action=btn.dataset.dtAction;if(action==="complete")completeReceiving(vehicle);else if(action==="reject")rejectReceiving(vehicle);else if(action==="track")openDatatableTrack(vehicle.auto_id)}))})}
