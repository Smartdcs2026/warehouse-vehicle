...
async function renderAdminArchive(body){
  ...
  $("archiveExcelDownload")?.addEventListener("click",()=>downloadArchiveExcelReport($("archiveExcelDownload")));
...
async function downloadArchiveExcelReport(button){
  const original=button?.textContent||"ดาวน์โหลด Excel พร้อมใช้";if(button){button.disabled=true;button.textContent="กำลังเตรียม Excel"}
  try{const url=cfg.apiBaseUrl.replace(/\/$/,"")+`/api/admin/archive-excel?month=${encodeURIComponent(adminDataTools.archiveMonth)}`,headers={};if(state.token)headers.authorization=`Bearer ${state.token}`;const response=await fetch(url,{headers});if(!response.ok){let message="สร้างไฟล์ Excel ไม่สำเร็จ";try{const data=await response.json();message=data.message||message}catch{}throw new Error(message)}const blob=await response.blob(),href=URL.createObjectURL(blob),link=document.createElement("a"),disposition=response.headers.get("content-disposition")||"",match=disposition.match(/filename="?([^";]+)"?/i);link.href=href;link.download=match?.[1]||`Warehouse_Vehicle_${adminDataTools.archiveMonth}_Report.xlsx`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(href),2000);if(button){button.textContent="ดาวน์โหลดแล้ว";setTimeout(()=>button.textContent=original,1200)}}catch(error){if(button)button.textContent=original;await showNotice("error",error.message||"สร้างไฟล์ Excel ไม่สำเร็จ")}finally{if(button)button.disabled=false}
}
