...
async function navigate(view) {
  ...
  stopCamera();
  state.view = view;setDashboardShell(view);if(view==="inbound")inboundLiveState.version=""; const titles = { operations:"งานรับสินค้า", inbound:"แผนก Inbound", dashboard:"ภาพรวมการปฏิบัติงาน", admin:"ตั้งค่าระบบ" };
...
function renderInboundRows(items) { $("inboundRows").innerHTML = items.length ? items.map(v => `<div class="list-row status-row" style="--status-color:${safeColor(v.alert_color)}" data-auto-id="${escapeHtml(v.auto_id)}" role="button" tabindex="0" aria-label="เปิดรายละเอียด ${escapeHtml(v.appointment_no||v.auto_id)}"><b>${escapeHtml(v.appointment_no || v.auto_id)}</b><span>${escapeHtml(v.company_name || "ไม่ระบุ")}</span><span>${escapeHtml(joinText(v.vehicle_plate,v.province))}</span><span>${escapeHtml(v.door_code || "-")}</span><span class="row-status-wrap"><span class="badge status-badge">${statusLabel(v.current_status)}</span><small>${alertLevelLabel(v.alert_level)} · <b data-duration-start="${Number(v.alert_started_at||v.gate_in_at||0)}">${formatDuration(v.stage_elapsed_seconds)}</b></small></span></div>`).join("") : `<div class="empty-state"><b>ไม่พบข้อมูล</b></div>`; }
...
function renderDashboard() {
  ...
  $("pageContent").innerHTML=`<section class="opsdash-head"><div class="opsdash-title"><button id="dashboardMenuButton" class="dashboard-menu-button" aria-label="เปิดเมนู" aria-expanded="false">☰</button><div><span>ภาพรวมคลังสินค้า</span><h2>ศูนย์ควบคุมการปฏิบัติงาน</h2><p id="dashboardRangeLabel">กำลังโหลดข้อมูล</p></div></div><div class="opsdash-filter"><button id="dashboardCalendarButton" class="calendar-button" aria-label="เลือกวันที่"><span>▣</span><b id="dashboardDateButtonLabel">เลือกวันที่</b></button><div class="range-buttons"><button data-dashboard-range="today">วัน</button><button data-dashboard-range="7d">7 วัน</button><button data-dashboard-range="30d">30 วัน</button></div><select id="dashboardShift" aria-label="เลือกกะ"><option value="">ทุกกะ</option></select><button id="dashboardFullscreen" class="outline-button">เต็มหน้าจอ</button></div></section><section id="dashboardCalendarPopover" class="dashboard-calendar-popover" hidden></section><div id="dashboardBody" class="loading">กำลังสรุปข้อมูล</div>`;
  document.querySelectorAll("[data-dashboard-range]").forEach(button=>{button.classList.toggle("active",button.dataset.dashboardRange===dashboardState.range);button.addEventListener("click",()=>{dashboardState.range=button.dataset.dashboardRange;dashboardState.lastLoadedAt=0;loadDashboard(true,true)})});
  $("dashboardMenuButton").addEventListener("click",toggleDashboardMenu);$("dashboardFullscreen").addEventListener("click",toggleFullscreen);$("dashboardCalendarButton").addEventListener("click",toggleDashboardCalendar);loadDashboard(true);
}
...
function renderDashboardData(data){
  ...
  if($("dashboardDateButtonLabel"))$("dashboardDateButtonLabel").textContent=formatDashboardDateKey(data.selectedDate||dashboardState.date);
  ...
  const ctx=data.shiftContext||{},shiftHours=ctx.startMinute==null?"ยังไม่กำหนดเวลา":`${minuteToTime(ctx.startMinute)}–${minuteToTime(ctx.endMinute)}${ctx.crossesMidnight?" · ข้ามวัน":""}`,fresh=dashboardState.error?`<span class="opsdash-stale">ข้อมูลล่าสุด · กำลังเชื่อมต่อใหม่</span>`:`<span class="opsdash-live">● ข้อมูลปัจจุบัน</span>`,businessDay=formatDate(data.businessDate||data.from).split(" ")[0];
  $("dashboardBody").innerHTML=`<section class="opsdash-context"><div class="context-date"><span class="context-icon">▣</span><span><small>วันปฏิบัติงาน</small><b>${businessDay}</b></span></div><div><small>${ctx.selected?"กะที่เลือก":"กะอ้างอิง"}</small><b style="--shift-color:${safeColor(ctx.color)}">${escapeHtml(ctx.shiftName||"ทุกกะ")} · ${shiftHours}</b></div><div><small>ช่วงข้อมูล</small><b>${formatDate(data.from)} – ${formatDate(data.to-1)}</b></div>${fresh}</section><section class="opsdash-kpis">${dashboardKpi("รถเข้า",w.gateIn,deltaNote(data.comparison?.delta?.total,"ช่วงก่อน"),"#2563EB","▣")}${dashboardKpi("รถออก",w.gateOut,`จากช่วงก่อน ${Number(w.priorPeriodClosed||0)} คัน`,"#059669","↗")}${dashboardKpi("คงอยู่",w.carryOut,`ต้นช่วง ${Number(w.carryIn||0)} คัน`,"#7C3AED","▰")}${dashboardKpi("รับเสร็จ",w.receivingCompleted,"ตรวจรับสินค้า","#0D9488","✓")}${dashboardKpi("เวลาเฉลี่ย",formatDuration(m.avgTotalSeconds),deltaNote(data.comparison?.delta?.avgTotalSeconds,"ช่วงก่อน",true),"#0F766E","◷")}${dashboardKpi("P90",formatDuration(m.p90TotalSeconds),"90% ปิดภายใน","#D97706","⌛")}</section><nav class="opsdash-tabs">${[["overview","ภาพรวม"],["handover","กะและส่งต่อ"],["comparison","เปรียบเทียบ"],["performance","ประสิทธิภาพ"],["capacity","ประตู"],["exceptions","ข้อยกเว้น"]].map(([id,label])=>`<button data-dashboard-tab="${id}" class="${dashboardState.tab===id?"active":""}">${label}</button>`).join("")}</nav><div id="dashboardTabContent" class="opsdash-content">${dashboardTabContent(data)}</div>`;
...
function dashboardKpi(label,value,note,color,icon=""){return`<article class="opsdash-kpi" style="--kpi-color:${color}"><div><small>${label}</small><b>${value??0}</b><span>${note}</span></div><i aria-hidden="true">${icon}</i></article>`}
...
function panel(title,note,body,className=""){return`<article class="opsdash-panel ${className}"><header><h3>${title}</h3><span>${note||""}</span></header>${body}</article>`}
function dashboardOverview(data){return`<section class="opsdash-grid overview-grid round11-overview">${panel("การไหลของงาน","ต้นช่วง + เข้า = ออก + ปลายช่วง",flowBalance(data),"flow-panel")}${panel("รถเข้าแต่ละชั่วโมง","คัน",hourChart(data.hours,Math.max(1,...(data.hours||[]).map(row=>Number(row.total)||0))),"hourly-panel")}${panel("คิวปลายช่วง","ตามขั้นตอน",stageQueueChart(data.activeStages||{}),"stage-panel")}${panel("แนวโน้ม 7 วัน","รถเข้าและเวลาเฉลี่ย",dailyComboChart(data.comparison?.daily||[]),"daily-panel")}${panel("เร่งดำเนินการ","ระดับเตือนขึ้นไป ${Number(data.metrics?.warningNow||0)} คัน",actionTable(data.actionItems),"urgent-panel")}</section>`}
function dashboardHandover(data){const w=data.workload||{};return`<section class="opsdash-grid handover-grid">${panel("ยอดรับ–ส่งงาน","ขอบเขตช่วงที่เลือก",flowBalance(data),"wide-panel")}${panel("งานต้นช่วง","รับมาจากช่วงก่อน",stageQueueChart(w.handoverInStages||{}))}${panel("งานปลายช่วง","ส่งต่อช่วงถัดไป",stageQueueChart(w.handoverOutStages||{}))}${panel("ผลการดำเนินงาน","แยกงานเดิมและงานใหม่",handoverSummary(data),"wide-panel")}${panel("รายการส่งต่อ","สถานะ ณ ปลายช่วง",actionTable(data.actionItems),"full-panel")}</section>`}
function flowBalance(data){const w=data.workload||{},left=Number(w.carryIn||0)+Number(w.gateIn||0),right=Number(w.gateOut||0)+Number(w.carryOut||0),balanced=left===right;return`<div class="flow-balance"><div><small>ต้นช่วง</small><b>${Number(w.carryIn||0)}</b></div><i>+</i><div><small>รถเข้า</small><b>${Number(w.gateIn||0)}</b></div><i>=</i><div><small>รถออก</small><b>${Number(w.gateOut||0)}</b></div><i>+</i><div><small>ปลายช่วง</small><b>${Number(w.carryOut||0)}</b></div></div><p class="balance-check ${balanced?"ok":"warn"}">${w.balanceApplicable?(balanced?`ยอดสมดุล ${left} คัน`:`ยอดต่าง ${Math.abs(left-right)} คัน · ตรวจเวลาต้นทาง`):"ช่วงหลายกะจะแสดงยอดรวมแต่ละกะ ไม่ใช้สมการยอดคงเหลือ"}</p><div class="cohort-strip"><span>รุ่นรถเข้า</span><b>${Number(data.metrics?.closed||0)}/${Number(data.metrics?.total||0)}</b><small>ปิดแล้ว ${Number(data.metrics?.completionRate||0).toFixed(1)}%</small></div>`}
function handoverSummary(data){const w=data.workload||{};return`<div class="handover-summary"><div><small>เข้า–ออกช่วงเดียว</small><b>${Number(w.samePeriodClosed||0)}</b></div><div><small>งานเดิมปิดช่วงนี้</small><b>${Number(w.priorPeriodClosed||0)}</b></div><div><small>ตรวจรับเสร็จ</small><b>${Number(w.receivingCompleted||0)}</b></div><div><small>ส่งต่อปลายช่วง</small><b>${Number(w.carryOut||0)}</b></div></div>`}
function dashboardComparison(data){return`<section class="opsdash-grid comparison-grid">${panel("แนวโน้มรถเข้ารายวัน","แท่ง = รถเข้า · เส้น = เวลาเฉลี่ย",dailyComboChart(data.comparison?.daily||[]),"full-panel")}${panel("ผลลัพธ์แต่ละกะ","อ้างอิงชื่อและเวลากะที่ Admin กำหนด",shiftCompareChart(data.shiftPerformance||[]),"wide-panel")}${panel("ช่วงก่อนหน้า","ใช้ช่วงเวลายาวเท่ากัน",comparisonSummary(data))}</section>`}
function comparisonSummary(data){const p=data.comparison?.previous||{},d=data.comparison?.delta||{};return`<div class="comparison-summary"><div><small>รถเข้า</small><b>${Number(p.total||0)}</b><span>${deltaNote(d.total,"เทียบปัจจุบัน")}</span></div><div><small>ปิดงาน</small><b>${Number(p.closed||0)}</b><span>${deltaNote(d.closed,"เทียบปัจจุบัน")}</span></div><div><small>เวลาเฉลี่ย</small><b>${formatDuration(p.avgTotalSeconds)}</b><span>${deltaNote(d.avgTotalSeconds,"เทียบปัจจุบัน",true)}</span></div></div>`}
...
function dashboardPerformance(data){const stages=[["Gate In → ยื่นเอกสาร",data.stageAverages?.gate_to_doc],["ยื่นเอกสาร → เริ่มตรวจรับ",data.stageAverages?.doc_to_start],["ระยะเวลาตรวจรับ",data.stageAverages?.receiving],["รับเสร็จ → รับเอกสารคืน",data.stageAverages?.complete_to_return],["รับเอกสารคืน → Gate Out",data.stageAverages?.return_to_out]];return`<section class="opsdash-grid">${panel("เวลาเฉลี่ยแต่ละช่วง","ชี้จุดสูญเสียเวลา",`<div class="stage-time-grid">${stages.map(([label,value])=>`<div><span>${label}</span><b>${value==null?"–":formatDuration(value)}</b></div>`).join("")}</div>`,"wide-panel")}${panel("เวลารวมตามชั่วโมงรถเข้า","เส้นเป้าหมายมาจากเวลาแจ้งเตือนที่ Admin ตั้ง",cycleTrendChart(data.hours||[],data.comparison?.targetTotalSeconds),"wide-panel")}${panel("การกระจายเวลาปิดงาน","จำนวนรถ",dashboardBars(data.cycleBands||[]))}${panel("ผลลัพธ์แต่ละกะ","ปิดงาน / รถเข้า",performanceRows(data.shiftPerformance))}${panel("งานที่ปิดล่าสุด","ใช้ตรวจสอบความเร็วหน้างาน",recentTable(data.recent),"full-panel recent-table")}</section>`}
...
function dashboardExceptions(data){return`<section class="opsdash-grid exceptions-grid">${panel("ระดับการแจ้งเตือน","รถในพื้นที่",dashboardBars(alertRows(data.alertLevels)))}${panel("คอขวดปัจจุบัน","ใช้สั่งเร่งงาน",`<div class="command-callout"><small>ขั้นตอน</small><b>${escapeHtml(data.bottleneck?.label||"ไม่มี")}</b><span>${Number(data.bottleneck?.queue||0)} คัน · ${formatDuration(data.bottleneck?.avg)}</span></div>`)}${panel("ข้อมูลไม่ครบ","ช่วงที่เลือก",qualityRows(data.dataQuality))}${panel("รายการข้อยกเว้น","ใช้เลขนัดหมายและทะเบียนรถเป็นหลัก",actionTable(data.actionItems),"full-panel action-panel")}</section>`}
function dailyComboChart(rows){if(!rows.length)return`<div class="empty-state">ยังไม่มีข้อมูลในช่วงที่เลือก</div>`;const items=rows.slice(-30),maxTotal=Math.max(1,...items.map(row=>Number(row.total)||0)),maxAvg=Math.max(1,...items.map(row=>Number(row.avg_seconds)||0)),w=720,h=210,pad=28,step=(w-pad*2)/Math.max(1,items.length),points=items.map((row,index)=>`${pad+step*index+step/2},${h-pad-(Number(row.avg_seconds)||0)/maxAvg*(h-pad*2)}`).join(" ");return`<div class="opsdash-chart-scroll"><svg class="opsdash-combo" viewBox="0 0 ${w} ${h}" role="img" aria-label="แนวโน้มรถเข้ารายวัน">${items.map((row,index)=>{const x=pad+step*index+step*.18,height=(Number(row.total)||0)/maxTotal*(h-pad*2);return`<rect x="${x}" y="${h-pad-height}" width="${Math.max(5,step*.64)}" height="${height}" rx="4"/><text x="${pad+step*index+step/2}" y="${h-8}" text-anchor="middle">${escapeHtml(String(row.label||"").slice(0,5))}</text>`}).join("")}<polyline points="${points}"/>${items.map((row,index)=>`<circle cx="${pad+step*index+step/2}" cy="${h-pad-(Number(row.avg_seconds)||0)/maxAvg*(h-pad*2)}" r="3"><title>${escapeHtml(row.label)} · ${Number(row.total)||0} คัน · ${formatDuration(row.avg_seconds)}</title></circle>`).join("")}</svg></div><div class="chart-legend"><span class="bar-key">รถเข้า</span><span class="line-key">เวลาเฉลี่ย</span></div>`}
...
function cycleTrendChart(rows,target){const items=Array.from({length:24},(_,hour)=>(rows||[]).find(row=>Number(row.hour)===hour)||{hour,avg_seconds:0}),max=Math.max(1,Number(target||0),...items.map(row=>Number(row.avg_seconds)||0)),points=items.map((row,index)=>`${24+index*28},${164-(Number(row.avg_seconds)||0)/max*126}`).join(" "),targetY=target?164-Number(target)/max*126:null;return`<div class="opsdash-chart-scroll"><svg class="opsdash-line" viewBox="0 0 700 185" role="img" aria-label="แนวโน้มเวลารวมแต่ละชั่วโมง">${targetY==null?"":`<line class="target" x1="24" y1="${targetY}" x2="668" y2="${targetY}"/><text x="26" y="${targetY-5}">เกณฑ์ ${formatDuration(target)}</text>`}<polyline points="${points}"/>${items.map((row,index)=>`<circle cx="${24+index*28}" cy="${164-(Number(row.avg_seconds)||0)/max*126}" r="3"><title>${String(row.hour).padStart(2,"0")}:00 · ${formatDuration(row.avg_seconds)}</title></circle><text x="${24+index*28}" y="180" text-anchor="middle">${index%3===0?String(row.hour).padStart(2,"0"):""}</text>`).join("")}</svg></div>`}
...
function renderAdminShell(){
  const tabs=[["users","ผู้ใช้งาน"],["workflow","ขั้นตอนงาน"],["doors","ประตู"],["shifts","กะทำงาน"],["alerts","เวลาแจ้งเตือน"]];
  $("pageContent").innerHTML=`<section class="admin-hero"><div><span class="eyebrow">ศูนย์ควบคุมระบบ</span><h2>ตั้งค่าการทำงาน</h2><p>การเปลี่ยนขั้นตอนและกะมีผลกับงานใหม่ ประวัติงานเดิมยังคงเดิม</p></div><span class="admin-build">${escapeHtml(adminState.data.build||"")}</span></section><nav class="admin-tabs">${tabs.map(([id,label])=>`<button data-admin-tab="${id}" class="${adminState.tab===id?"active":""}">${label}</button>`).join("")}</nav><section id="adminPanel" class="admin-panel"></section>`;
...
function renderAdminPanel(){if(adminState.tab==="users")renderAdminUsers();else if(adminState.tab==="workflow")renderAdminWorkflow();else if(adminState.tab==="doors")renderAdminDoors();else if(adminState.tab==="shifts")renderAdminShifts();else renderAdminAlerts()}
...
function renderAdminWorkflow(){const w=adminState.data.workflow||{};$("adminPanel").innerHTML=`<div class="admin-section-head"><div><h3>เส้นทางการทำงาน</h3><p>เปิดหรือข้ามแต่ละช่วงได้ งานใหม่จะเลือกสถานะเริ่มต้นอัตโนมัติ</p></div></div><form id="workflowForm" class="workflow-admin-grid">${workflowSwitch("wfInboundFirst","ยื่นเอกสารที่ Inbound","บันทึกเวลายื่นเอกสารครั้งแรก",w.use_inbound_first)}${workflowSwitch("wfReceiving","ตรวจรับสินค้า","แสดงงานให้แผนกรับสินค้า",w.use_receiving)}${workflowSwitch("wfInboundSecond","รับเอกสารคืน","สแกนครั้งที่สองหลังรับสินค้า",w.use_inbound_second)}${workflowSwitch("wfDoor","เลือกประตูรับสินค้า","ใช้ S/R/SS/RR/SR/RS และตัวเลข",w.use_door)}${workflowSwitch("wfRequireDoor","บังคับระบุประตู","ไม่อนุญาตให้เริ่มตรวจรับโดยเว้นว่าง",w.require_door)}<div class="admin-form-actions"><button class="primary" type="submit">บันทึกขั้นตอนงาน</button></div></form>`;const receiving=$("wfReceiving"),door=$("wfDoor"),sync=()=>{door.disabled=!receiving.checked;if(!receiving.checked)door.checked=false;$("wfRequireDoor").disabled=!door.checked;if(!door.checked)$("wfRequireDoor").checked=false};receiving.addEventListener("change",sync);door.addEventListener("change",sync);sync();$("workflowForm").addEventListener("submit",event=>{event.preventDefault();adminMutation("/api/admin/workflow",{useInboundFirst:$("wfInboundFirst").checked,useReceiving:receiving.checked,useInboundSecond:$("wfInboundSecond").checked,useDoor:door.checked,requireDoor:$("wfRequireDoor").checked})})}
...
function renderAdminDoors(){const doors=adminState.data.doors||[];$("adminPanel").innerHTML=`<div class="admin-section-head"><div><h3>ประตูรับสินค้า</h3><p>รองรับ S07, R55, SS12, RR03, SR8 และ RS20</p></div><button id="addDoorRow" class="primary">เพิ่มประตู</button></div><form id="doorsForm"><div id="doorAdminRows" class="door-admin-grid">${doors.map(door=>doorAdminRow(door.door_code,door.is_active)).join("")}</div><div class="admin-form-actions"><button class="primary" type="submit">บันทึกประตู</button></div></form>`;$("addDoorRow").addEventListener("click",()=>{$("doorAdminRows").insertAdjacentHTML("beforeend",doorAdminRow("",1));bindDoorRemove()});bindDoorRemove();$("doorsForm").addEventListener("submit",event=>{event.preventDefault();const doors=[...document.querySelectorAll(".door-admin-row")].map(row=>({doorCode:row.querySelector("input[type=text]").value.trim().toUpperCase(),isActive:row.querySelector("input[type=checkbox]").checked})).filter(item=>item.doorCode);adminMutation("/api/admin/doors",{doors})})}
function doorAdminRow(code,active){return `<div class="door-admin-row"><input type="text" value="${escapeHtml(code)}" maxlength="5" placeholder="S07"><label><input type="checkbox" ${Number(active)?"checked":""}> ใช้งาน</label><button type="button" data-remove-door>นำออก</button></div>`}
...
function updateClocks() { const now=Math.floor(Date.now()/1000),value=formatDate(now); if($("thaiClock")) $("thaiClock").textContent=value; if($("headerClock")) $("headerClock").textContent=value;document.querySelectorAll("[data-duration-start]").forEach(element=>{const start=Number(element.dataset.durationStart);if(start>0)element.textContent=formatDuration(now-start)}) }
...
function alertLevelLabel(level){return({NORMAL:"ปกติ",WATCH:"เฝ้าระวัง",WARNING:"เตือน",URGENT:"เร่งด่วน",CRITICAL:"วิกฤต"})[level]||"ปกติ"}
