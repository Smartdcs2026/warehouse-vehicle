...
function renderDashboard() {
  ...
  $("pageContent").innerHTML=`<section class="opsdash-head"><div class="opsdash-title"><button id="dashboardMenuButton" class="dashboard-menu-button" aria-label="เปิดเมนู" aria-expanded="false">☰</button><div><span>ภาพรวมคลังสินค้า</span><h2>ศูนย์ควบคุมการปฏิบัติงาน</h2><p id="dashboardRangeLabel">กำลังโหลดข้อมูล</p></div></div><div class="opsdash-filter"><div class="range-buttons"><button data-dashboard-range="today">วันนี้</button><button data-dashboard-range="7d">7 วัน</button><button data-dashboard-range="30d">30 วัน</button></div><select id="dashboardShift"><option value="">ทุกกะ</option></select><button id="dashboardFullscreen" class="outline-button">เต็มหน้าจอ</button></div></section><div id="dashboardBody" class="loading">กำลังสรุปข้อมูล</div>`;
...
function dashboardKpi(label,value,note,color){return`<article class="opsdash-kpi" style="--kpi-color:${color}"><small>${label}</small><b>${value??0}</b><span>${note}</span></article>`}
...
function dashboardOverview(data){const b=data.bottleneck||{};return`<section class="opsdash-grid overview-grid">${panel("จุดที่ควรสั่งการ","ข้อมูลปัจจุบัน",`<div class="command-callout"><small>คอขวดที่มีคิวมากที่สุด</small><b>${escapeHtml(b.label||"ยังไม่มีคิว")}</b><span>${Number(b.queue||0)} คัน · เวลาเฉลี่ย ${formatDuration(b.avg)}</span></div>${dashboardBars(alertRows(data.alertLevels))}`,"attention-panel")}${panel("รถเข้าแต่ละชั่วโมง","ใช้จัดกำลังคนและประตู",hourChart(data.hours,Math.max(1,...(data.hours||[]).map(row=>Number(row.total)||0))),"wide-panel")}${panel("คิวงานขณะนี้",`${Number(data.metrics?.activeNow||0)} คัน`,dashboardBars(Object.entries(data.activeStages||{}).map(([status,total])=>({label:statusLabel(status),total}))))}${panel("แนวโน้มรายวัน","รถเข้าและเวลารวม",dailyComboChart(data.comparison?.daily||[]),"wide-panel")}${panel("เปรียบเทียบแต่ละกะ","จำนวนรถ · อัตราปิด · เวลาเฉลี่ย",shiftCompareChart(data.shiftPerformance||[]),"wide-panel")}${panel("รายการที่ต้องติดตามก่อน","เรียงตามระดับเตือนและเวลารวม",actionTable(data.actionItems),"full-panel action-panel")}</section>`}
