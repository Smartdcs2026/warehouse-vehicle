...
const DASHBOARD_INFO={
  ...
  p90:{title:"90% จบภายใน",meaning:"เวลาที่รถส่วนใหญ่ 90% ใช้ไม่เกินค่านี้ เหมาะสำหรับดูว่างานส่วนใหญ่จบในเวลาเท่าใด",source:"เวลารวม Gate In ถึง Gate Out ของรถที่ปิดงานแล้ว",calculation:"เรียงเวลาจากน้อยไปมาก แล้วเลือกค่าที่ตำแหน่ง 90%"},
  ...
  $("pageContent").innerHTML=`<section class="dt-page dt-clean-page dt-r100-page ${datatableState.mobileFiltersOpen?"dt-mobile-filters-open":""}">
    <section class="dt-r100-top">
      <div class="dt-r100-controls">
        ...
        <div class="dt-r100-actions"><button id="dtCompare" class="dt-compare-open" type="button">เปรียบเทียบ</button><button id="dtRefresh" class="quiet-button" type="button">อัปเดต</button><button id="dtFullscreen" class="outline-button" type="button">เต็มจอ</button><button id="dtExport" class="primary" type="button">ส่งออก</button></div>
      ...
      <div class="dt-mobile-toolbelt"><button id="dtMobileFilterToggle" class="outline-button dt-mobile-tool" type="button">ตัวกรอง</button><button id="dtMobileCompare" class="dt-compare-open dt-mobile-tool" type="button">เปรียบเทียบ</button><button id="dtMobileRefresh" class="quiet-button dt-mobile-tool" type="button">อัปเดต</button><button id="dtMobileExport" class="primary dt-mobile-tool" type="button">ส่งออก</button></div>
...
const DASHBOARD_SNAPSHOT_KEY="wvf_dashboard_snapshot_r147_readability";
...
function dashboardKpiSet(data,tab="overview"){const m=data.metrics||{},w=data.workload||{},a=data.activeStages||{},p=data.comparison?.previous||{},levels=data.alertLevels||{},qTotal=dashboardQualityTotal(data),doors=dashboardDoorStats(data),urgent=dashboardUrgentCount(data),rate=Number(m.total||0)?Number(m.closed||0)*100/Number(m.total||0):0,b=data.bottleneck||dashboardClientBottleneck({},a)||{};let specs=[];
  ...
  else if(tab==="comparison")specs=[["รถเข้า",w.gateIn,comparisonNote(w.gateIn,p.total,"ช่วงก่อน"),"#2563EB","↓"],["รถออก",w.gateOut,comparisonNote(w.gateOut,p.closed,"ช่วงก่อน"),"#059669","↗"],["คงอยู่",w.carryOut,"ปลายช่วงที่เลือก","#7C3AED","▰"],["เวลาเฉลี่ย",durationValue(m.avgTotalSeconds),comparisonNote(m.avgTotalSeconds,p.avgTotalSeconds,"ช่วงก่อน",true),"#0F766E","◷"],["90% จบภายใน",durationValue(m.p90TotalSeconds),"ใช้ดูงานส่วนใหญ่","#D97706","90"],["ต้องเร่ง",urgent,"รายการที่ต้องตาม","#DC2626","!"]];
  else if(tab==="performance")specs=[["เวลาเฉลี่ย",durationValue(m.avgTotalSeconds),"ตั้งแต่รถเข้า ถึง รถออก","#0F766E","◷"],["ครึ่งหนึ่งจบภายใน",durationValue(m.p50TotalSeconds),"50% ของงานที่จบ","#2563EB","50"],["90% จบภายใน",durationValue(m.p90TotalSeconds),"ดูงานส่วนใหญ่จบในเวลาเท่าใด","#D97706","90"],["ปิดงาน",`${Number(m.closed||0)}/${Number(m.total||0)}`,`${rate.toFixed(1)}% ของงานในช่วง`,"#059669","✓"],["รับเสร็จ",w.receivingCompleted,"ตรวจรับสินค้าเสร็จ","#0891B2","✓"],["คงอยู่",w.carryOut,"ยังไม่ Gate Out","#7C3AED","▰"]];
  ...
  else specs=[["รถเข้า (สะสมวันนี้)",w.gateIn,comparisonNote(w.gateIn,p.total,"ช่วงก่อน"),"#2563EB","↓"],["รถออก (สะสมวันนี้)",w.gateOut,comparisonNote(w.gateOut,p.closed,"ช่วงก่อน"),"#059669","↗"],["รถในพื้นที่ (ค้างอยู่)",w.carryOut,`${Number(w.carryIn||0)} คันรับต่อจากก่อนช่วง`,"#7C3AED","▰"],["รับสินค้าเสร็จวันนี้",w.receivingCompleted,"จบขั้นตอนตรวจรับสินค้า","#0891B2","✓"],["เวลาเฉลี่ยทั้งหมด",durationValue(m.avgTotalSeconds),comparisonNote(m.avgTotalSeconds,p.avgTotalSeconds,"จากช่วงก่อน",true),"#0F766E","◷"],["90% จบภายใน",durationValue(m.p90TotalSeconds),m.p90TotalSeconds==null?"กำลังคำนวณ":"90% ของงานที่ปิดใช้เวลาไม่เกินนี้","#D97706","90"]];
