...
function datatableCompareResultsHtml(result){
  ...
  return`<section class="dt-compare-results"><header class="dt-compare-section-title"><b>ผลการเปรียบเทียบ</b><span>แสดงค่า A, B และผลต่างตามหน่วยของแต่ละข้อมูล</span></header><div class="dt-compare-metrics">${datatableCompareMetricCard("จำนวนรถ","คัน","vehicleCount",a,b,d)}${datatableCompareMetricCard("เวลาเฉลี่ย","นาที:วินาที","avgSeconds",a,b,d,"average")}${datatableCompareMetricCard("ล่าช้า","รายการ","lateCount",a,b,d)}${datatableCompareMetricCard("วิกฤต","รายการ","criticalCount",a,b,d)}${datatableCompareMetricCard("ปฏิเสธรับสินค้า","รายการ","rejectedCount",a,b,d)}${datatableCompareMetricCard("เวลารวม","ชม.:นาที","totalSeconds",a,b,d,"duration")}</div>${datatableCompareSummary(result)}</section>`
}
...
function clearSession() { stopCamera(); inboundLiveState.version="";inboundLiveState.checking=false;inboundLiveState.failures=0;inboundLiveState.nextAllowedAt=0;alertSoundState.initialized=false;alertSoundState.levels.clear();sessionStorage.removeItem("wvf_token"); state.token=""; state.user=null; state.display={dashboardEnabled:true,datatableEnabled:true}; $("appView").hidden=true; $("loginView").hidden=false; $("loginPassword").value=""; window.scrollTo(0, 0); }
