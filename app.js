...
const FRONTEND_BUILD="2026.08.24-round207.18-release-candidate";
...
function dashboardAppointmentPlanHtml(data){
  ...
  const compliant=Number(p.compliant??(Number(p.early||0)+Number(p.onTime||0))),onTimePercent=Number(p.onTimePercent??(matched?compliant*100/matched:0)),late=Number(p.late||0),diffText=diff>0?`+${diff.toLocaleString("th-TH")}`:diff.toLocaleString("th-TH");
  return`<section class="appointment-dashboard-r204 r2042-appointment-band ${warnings.length?"has-audit-warning":""}"><div class="r2042-appointment-label"><small>APPOINTMENT PLAN</small><b>แผนเทียบการมาถึง</b><span>อ้างอิง ${escapeHtml(p.referenceMode||"PERIOD")} · อนุโลม ${Number(p.lateGraceMinutes||0)} นาที</span></div><div class="r2042-appointment-metrics"><span><small>ตามแผน</small><b>${planned.toLocaleString("th-TH")}</b></span><span><small>เข้าจริง</small><b>${actual.toLocaleString("th-TH")}</b></span><span><small>จับคู่ได้</small><b>${matched.toLocaleString("th-TH")}</b><em>${coverage.toFixed(1)}%</em></span>${showTiming?`<span class="good"><small>ตรงเวลารวม</small><b>${onTimePercent.toFixed(1)}%</b></span><span class="bad"><small>มาสาย</small><b>${late.toLocaleString("th-TH")}</b></span>`:""}<span class="${diff>0?"warn":diff<0?"info":"good"}"><small>ต่างจากแผน</small><b>${diffText}</b></span></div><button type="button" data-appointment-lineage class="r2042-lineage-button">ที่มาของตัวเลข</button>${warnings.length?`<div class="r2042-appointment-warning" title="${escapeHtml(warnings.join(" · "))}">!</div>`:""}</section>`
}
