...
const FRONTEND_BUILD="2026.08.20-round205.2-simple-operational-export";
...
function productionExportAppointmentText(item){if(!item?.appointmentMatched)return"ไม่พบข้อมูลนัดหมายที่จับคู่";const d=Number(item.deltaSeconds);if(!Number.isFinite(d))return"จับคู่ได้";if(d<0)return`มาก่อน ${formatExportDuration(Math.abs(d))} · ตรงเวลา`;if(item.timingStatus==="LATE")return`มาสาย ${formatExportDuration(d)} · เกินเกณฑ์ ${formatExportDuration(Number(item.lateBySeconds||0))}`;if(d===0)return"ตรงเวลา";return`มาช้า ${formatExportDuration(d)} · อยู่ในเกณฑ์ตรงเวลา`}
function productionExportCheck(item,label){return(item?.stageChecks||[]).find(x=>x.label===label)||null}
function productionExportCheckResult(check){if(!check||check.seconds==null)return"";if(check.issue)return`${check.alertLabel||"เกินเกณฑ์"} · เกิน ${productionExportDurationCell(check.exceededSeconds)}`;if(check.alertLevel==="WATCH")return"เฝ้าระวัง · ยังไม่เกินเกณฑ์เตือน";return"ปกติ"}
function productionExportOperationalTable(items=[]){
  const headers=["วันที่นัด","เวลานัด","เลขนัดหมาย","บริษัท","ผู้ขนส่ง","PO","ทะเบียน","จังหวัด","คนขับ","กะ","ประตู","วันที่ Gate In","Gate In","ผลเทียบนัดหมาย","ยื่นเอกสาร","รอยื่นเอกสาร","ผลรอยื่น","ตรวจเอกสารเสร็จ","ตรวจเอกสาร","ผลตรวจเอกสาร","เรียกรถครั้งแรก","จำนวนครั้งเรียก","เริ่มตรวจรับ","รอตรวจรับ","ผลรอตรวจรับ","รับสินค้าเสร็จ","ตรวจรับสินค้า","ผลตรวจรับ","รับเอกสารคืน","รอคืนเอกสาร","ผลรอคืน","วันที่ Gate Out","Gate Out","รอ Gate Out","ผลรอ Gate Out","เวลารวมในพื้นที่","ผลเวลารวม","ขั้นตอนที่เกินเกณฑ์มากที่สุด","ใช้เวลาจริง","เกณฑ์ Admin","เกินเกณฑ์","สถานะล่าสุด","เหตุผลปฏิเสธ"];
  const rows=items.map(x=>{const gate=productionExportCheck(x,"รอยื่นเอกสาร"),review=productionExportCheck(x,"ตรวจเอกสาร"),wait=productionExportCheck(x,"รอตรวจรับ"),receive=productionExportCheck(x,"ตรวจรับสินค้า"),ret=productionExportCheck(x,"รอคืนเอกสาร"),out=productionExportCheck(x,"รอ Gate Out"),worst=x.longestIssue||null,overall=x.overallCheck||null;return[productionExportSourceDate(x.appointmentDate),productionExportTimeOnly(x.plannedAt),x.appointmentNo||"",x.displayCompany||x.companyName||"",x.appointmentMatched?(x.carrier||""):"",x.appointmentMatched?(x.po||""):"",x.vehiclePlate||"",x.province||"",x.driverName||"",x.shiftName||"",x.doorCode||"",productionExportDateOnly(x.gateInAt),productionExportTimeOnly(x.gateInAt),productionExportAppointmentText(x),productionExportTimeOnly(x.documentSubmittedAt),productionExportDurationCell(gate?.seconds),productionExportCheckResult(gate),productionExportTimeOnly(x.documentCheckedAt),productionExportDurationCell(review?.seconds),productionExportCheckResult(review),productionExportTimeOnly(x.firstCalledAt),Number(x.queueCallCount||0),productionExportTimeOnly(x.receivingStartedAt),productionExportDurationCell(wait?.seconds),productionExportCheckResult(wait),productionExportTimeOnly(x.receivingCompletedAt),productionExportDurationCell(receive?.seconds),productionExportCheckResult(receive),productionExportTimeOnly(x.documentReturnedAt),productionExportDurationCell(ret?.seconds),productionExportCheckResult(ret),productionExportDateOnly(x.gateOutAt),productionExportTimeOnly(x.gateOutAt),productionExportDurationCell(out?.seconds),productionExportCheckResult(out),productionExportDurationCell(x.totalInSiteSeconds),productionExportCheckResult(overall),worst?.label||"ไม่เกินเกณฑ์",productionExportDurationCell(worst?.seconds),productionExportDurationCell(worst?.thresholdSeconds),productionExportDurationCell(worst?.exceededSeconds),monthlyExportStatusLabel(x.currentStatus,x.gateOutAt),x.rejectionReason||""]});
  return{key:"vehicles",name:"ข้อมูลรถและเวลาทำงาน",headers,rows}
}
function productionExportIssuesTable(items=[]){const rows=[];for(const x of items)for(const check of x.stageChecks||[])if(check.issue)rows.push([productionExportSourceDate(x.appointmentDate),productionExportTimeOnly(x.plannedAt),x.appointmentNo||"",x.displayCompany||x.companyName||"",x.vehiclePlate||"",x.province||"",x.shiftName||"",x.doorCode||"",check.label,productionExportDurationCell(check.seconds),productionExportDurationCell(check.thresholdSeconds),productionExportDurationCell(check.exceededSeconds),check.alertLabel||"",productionExportTimeOnly(x.gateInAt),productionExportTimeOnly(x.gateOutAt)]);return{key:"issues",name:"รายการเกินเกณฑ์",headers:["วันที่นัด","เวลานัด","เลขนัดหมาย","บริษัท","ทะเบียน","จังหวัด","กะ","ประตู","ขั้นตอน","ใช้เวลาจริง","เกณฑ์ Admin","เกินเกณฑ์","ระดับ","Gate In","Gate Out"],rows}}
