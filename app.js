...
const adminDataTools={tab:"overview",inspector:null,busy:false,openTable:"",openCommand:"",openSchema:"",archiveMonth:"",archivePreview:null,archiveBusy:false,archiveHistory:null,archiveHistoryBusy:false,archiveStoreBusy:false,cleanupMonth:"",cleanupPreview:null,cleanupBusy:false,cleanupScript:null,cleanupVerify:null};
...
async function renderAdminArchive(body){
  ...
  const data=adminDataTools.archivePreview||{},files=(data.files||[]).filter(item=>item.available!==false),ready=Number(data.readyVehicles||0),open=Number(data.openVehicles||0),connected=Boolean(history?.connected),existing=(history?.items||[]).find(item=>item.month===adminDataTools.archiveMonth);
  body.innerHTML=`<section class="archive-head"><div><h3>เก็บข้อมูลย้อนหลัง</h3><p>เลือกเดือน ตรวจสอบ แล้วเก็บสำเนาไว้ก่อนเคลียร์ข้อมูลเก่า</p></div><label><span>เดือนที่ต้องการ</span><input id="archiveMonth" type="month" value="${escapeHtml(adminDataTools.archiveMonth)}" min="2020-01" max="2100-12"></label><button id="archiveCheck" class="quiet-button" type="button">ตรวจสอบ</button></section><section class="r2-status-card ${connected?"ready":"off"}"><div><small>พื้นที่เก็บข้อมูล R2</small><b>${connected?"พร้อมใช้งาน":"ยังไม่ได้เชื่อม"}</b><span>${connected?(existing?`เดือนนี้เก็บล่าสุด ${formatDate(existing.createdAt)}`:"สามารถเก็บข้อมูลเดือนนี้ได้"):"ยังดาวน์โหลดไฟล์เก็บเองได้ตามปกติ"}</span></div>${connected?`<button id="archiveStoreR2" class="primary" type="button" ${ready?"":"disabled"}>${existing?"เก็บใหม่ใน R2":"เก็บใน R2"}</button>`:""}</section><div class="data-safe-note"><b>ปลอดภัย</b><span>ระบบจะเก็บเฉพาะรถที่จบงานและออกจากพื้นที่แล้ว รายการที่ยังทำงานอยู่จะไม่รวม</span></div><section class="archive-summary"><article><small>รถทั้งหมดในเดือน</small><b>${Number(data.totalVehicles||0).toLocaleString("th-TH")}</b></article><article class="ok"><small>พร้อมเก็บย้อนหลัง</small><b>${ready.toLocaleString("th-TH")}</b></article><article class="warn"><small>ยังไม่จบงาน</small><b>${open.toLocaleString("th-TH")}</b></article><article><small>ช่วงข้อมูล</small><b>${escapeHtml(data.periodLabel||adminDataTools.archiveMonth)}</b></article></section>${open?`<div class="archive-warning"><b>มี ${open.toLocaleString("th-TH")} รายการที่ยังไม่จบงาน</b><span>ระบบจะเว้นรายการเหล่านี้ไว้</span></div>`:""}<section class="archive-files"><header><div><h3>ไฟล์ที่ดาวน์โหลดได้</h3><p>ดาวน์โหลดเก็บไว้ในเครื่องได้เพิ่มเติม</p></div><button id="archiveSummaryDownload" class="quiet-button" type="button">ดาวน์โหลดสรุป</button></header>${files.map(item=>`<article><div><b>${escapeHtml(item.label)}</b><span>${Number(item.rows||0).toLocaleString("th-TH")} แถว</span></div><button type="button" data-archive-file="${escapeHtml(item.id)}" ${Number(item.rows||0)===0?"disabled":""}>ดาวน์โหลด CSV</button></article>`).join("")||`<div class="empty-state">ไม่พบข้อมูลที่พร้อมเก็บย้อนหลัง</div>`}</section><section class="archive-full-backup"><div><h3>สำรองทั้งฐานข้อมูล</h3><p>ใช้เมื่อต้องการเก็บสำเนาทั้งฐานก่อนงานสำคัญ</p></div><button id="copyFullBackupCommand" class="quiet-button" type="button">คัดลอกคำสั่ง</button><pre id="fullBackupCommand">npx wrangler d1 export &lt;ชื่อฐานข้อมูล&gt; --remote --output=./warehouse-vehicle-backup.sql</pre></section>`;
  $("archiveCheck")?.addEventListener("click",()=>{const month=String($("archiveMonth")?.value||"").trim();if(!/^\d{4}-(0[1-9]|1[0-2])$/.test(month))return;adminDataTools.archiveMonth=month;adminDataTools.archivePreview=null;renderAdminArchive(body)});
  ...
  $("archiveStoreR2")?.addEventListener("click",async()=>{const button=$("archiveStoreR2");if(!button||button.disabled||adminDataTools.archiveStoreBusy)return;const ok=await Swal.fire({title:"เก็บข้อมูลใน R2",html:`<div style="text-align:left">เดือน <b>${escapeHtml(data.periodLabel||adminDataTools.archiveMonth)}</b><br>รถที่จบงาน <b>${ready.toLocaleString("th-TH")}</b> รายการ</div>`,showCancelButton:true,confirmButtonText:"ยืนยันเก็บข้อมูล",cancelButtonText:"ยกเลิก",reverseButtons:true});if(!ok.isConfirmed)return;adminDataTools.archiveStoreBusy=true;button.disabled=true;button.textContent="กำลังเก็บข้อมูล";try{const result=await api("/api/admin/archive-store",{method:"POST",body:{month:adminDataTools.archiveMonth}});adminDataTools.archiveHistory=null;adminDataTools.cleanupPreview=null;await showNotice("success",result.message||"เก็บข้อมูลเรียบร้อยแล้ว");await renderAdminArchive(body)}catch(error){await showNotice("error",error.message||"เก็บข้อมูลไม่สำเร็จ")}finally{adminDataTools.archiveStoreBusy=false}});
}
async function renderAdminArchiveHistory(body){
  ...
  body.innerHTML=`<section class="archive-history-head"><div><h3>ประวัติการเก็บข้อมูล</h3><p>แสดงชุดข้อมูลล่าสุดของแต่ละเดือนที่เก็บไว้ใน R2</p></div><button id="archiveHistoryRefresh" class="quiet-button" type="button">โหลดใหม่</button></section><div class="archive-history-list">${items.map(item=>`<article><div class="archive-history-main"><b>${escapeHtml(item.periodLabel||item.month||"-")}</b><span>เก็บ ${formatDate(item.createdAt)} · รถ ${Number(item.readyVehicles||0).toLocaleString("th-TH")} รายการ</span></div><div class="archive-history-meta"><span>${(item.files||[]).length} ไฟล์</span><span>${escapeHtml(item.createdBy||"ระบบ")}</span></div><div class="archive-history-files">${(item.files||[]).map(file=>`<button type="button" data-r2-file="${escapeHtml(file.key||"")}">${escapeHtml(file.label||file.filename||"ไฟล์")}</button>`).join("")}</div></article>`).join("")||`<div class="empty-state">ยังไม่มีข้อมูลที่เก็บใน R2</div>`}</div>`;
...
async function renderAdminCleanup(body){
  ...
  const data=adminDataTools.cleanupPreview||{},safe=Boolean(data.safe),ready=Number(data.readyVehicles||0),open=Number(data.openVehicles||0),related=(data.counts||[]).reduce((sum,item)=>sum+Number(item.rows||0),0),messages=Array.isArray(data.messages)?data.messages:[],r2Connected=Boolean(data.archive?.connected),r2Stored=Boolean(data.archive?.stored);
  body.innerHTML=`<section class="cleanup-head"><div><h3>เตรียมเคลียร์ข้อมูลเก่า</h3><p>ตรวจให้ครบก่อนสร้างชุดคำสั่ง ระบบหน้านี้จะไม่ลบข้อมูลเอง</p></div><label><span>เดือนที่ต้องการ</span><input id="cleanupMonth" type="month" value="${escapeHtml(adminDataTools.cleanupMonth)}" min="2020-01" max="2100-12"></label><button id="cleanupCheck" class="quiet-button" type="button">ตรวจสอบ</button></section><div class="${safe?"data-safe-note":"cleanup-stop-note"}"><b>${safe?"พร้อมตรวจขั้นสุดท้าย":"ยังไม่พร้อม"}</b><span>${safe?"ระบบพบข้อมูลที่จบงานแล้วและตรวจความเกี่ยวข้องครบ":"ยังไม่สร้างชุดคำสั่งจนกว่าจะผ่านทุกข้อ"}</span></div><section class="cleanup-summary"><article><small>รถทั้งหมดในเดือน</small><b>${Number(data.totalVehicles||0).toLocaleString("th-TH")}</b></article><article class="ok"><small>พร้อมเคลียร์</small><b>${ready.toLocaleString("th-TH")}</b></article><article class="keep"><small>ต้องเก็บไว้</small><b>${open.toLocaleString("th-TH")}</b><span>ยังไม่จบงาน</span></article><article><small>ข้อมูลที่เกี่ยวข้อง</small><b>${related.toLocaleString("th-TH")}</b></article></section>${messages.length?`<div class="cleanup-messages">${messages.map(message=>`<span>${escapeHtml(message)}</span>`).join("")}</div>`:""}<section class="cleanup-checks"><header><h3>ตรวจความพร้อม</h3><span>ต้องผ่านทุกข้อก่อน</span></header>${(data.checks||[]).map(item=>`<div class="${item.ok?"pass":"fail"}"><i>${item.ok?"✓":"!"}</i><span>${escapeHtml(item.label)}</span><b>${item.ok?"ผ่าน":"ตรวจสอบ"}</b></div>`).join("")}</section><section class="cleanup-counts"><header><h3>ข้อมูลที่จะเกี่ยวข้อง</h3><span>ใช้ตรวจสอบจำนวนก่อนดำเนินการ</span></header>${(data.counts||[]).map(item=>`<div><span>${escapeHtml(item.label)}</span><b>${Number(item.rows||0).toLocaleString("th-TH")} แถว</b></div>`).join("")}</section>${r2Connected?`<div class="cleanup-r2-state ${r2Stored?"ready":"missing"}"><b>${r2Stored?"เก็บสำเนาใน R2 แล้ว":"ยังไม่มีสำเนาใน R2"}</b><span>${r2Stored?`เก็บล่าสุด ${formatDate(data.archive?.createdAt)}`:"กลับไปที่ “เก็บข้อมูลย้อนหลัง” แล้วเก็บเดือนนี้ก่อน"}</span></div>`:""}<label class="cleanup-confirm"><input id="cleanupArchiveConfirmed" type="checkbox" ${safe?"":"disabled"}><span><b>${r2Connected?"ฉันตรวจสอบสำเนาใน R2 แล้ว":"ฉันดาวน์โหลดข้อมูลย้อนหลังของเดือนนี้ครบแล้ว"}</b><small>ควรสำรองทั้งฐานข้อมูลก่อนนำชุดคำสั่งไปใช้</small></span></label><div class="cleanup-actions"><button id="cleanupVerify" class="quiet-button" type="button">ตรวจผลหลังเคลียร์</button><button id="cleanupBuild" class="primary" type="button" ${safe?"":"disabled"}>สร้างชุดคำสั่ง</button></div><section id="cleanupVerifyArea"></section><section id="cleanupScriptArea"></section>`;
  $("cleanupCheck")?.addEventListener("click",()=>{const month=String($("cleanupMonth")?.value||"").trim();if(!/^\d{4}-(0[1-9]|1[0-2])$/.test(month))return;adminDataTools.cleanupMonth=month;adminDataTools.cleanupPreview=null;adminDataTools.cleanupScript=null;renderAdminCleanup(body)});
  const confirm=$("cleanupArchiveConfirmed"),build=$("cleanupBuild");confirm?.addEventListener("change",()=>{if(build)build.disabled=!safe||!confirm.checked});if(build)build.disabled=!safe||!confirm?.checked;
  build?.addEventListener("click",async()=>{if(!confirm?.checked)return;build.disabled=true;build.textContent="กำลังเตรียม";try{adminDataTools.cleanupScript=await api(`/api/admin/cleanup-script?month=${encodeURIComponent(adminDataTools.cleanupMonth)}`);renderCleanupScriptArea($("cleanupScriptArea"),adminDataTools.cleanupScript)}catch(error){await showNotice("error",error.message||"สร้างชุดคำสั่งไม่สำเร็จ")}finally{build.textContent="สร้างชุดคำสั่ง";build.disabled=!safe||!confirm?.checked}});
  $("cleanupVerify")?.addEventListener("click",async()=>{const button=$("cleanupVerify"),area=$("cleanupVerifyArea");if(button){button.disabled=true;button.textContent="กำลังตรวจ"}try{const result=await api(`/api/admin/cleanup-verify?month=${encodeURIComponent(adminDataTools.cleanupMonth)}`);adminDataTools.cleanupVerify=result;if(area)area.innerHTML=`<div class="cleanup-verify-result ${result.verified?"ok":"warn"}"><b>${result.verified?"ตรวจเรียบร้อย":"ยังไม่เรียบร้อย"}</b><span>${escapeHtml(result.message||"")}</span><small>รถที่จบงานยังเหลือ ${Number(result.remaining||0).toLocaleString("th-TH")} รายการ${result.archiveConnected?` · R2 ${result.archiveStored?"มีสำเนาแล้ว":"ยังไม่มีสำเนา"}`:""}</small></div>`}catch(error){if(area)area.innerHTML=`<div class="cleanup-verify-result warn"><b>ตรวจไม่สำเร็จ</b><span>${escapeHtml(error.message||"กรุณาลองใหม่")}</span></div>`}finally{if(button){button.disabled=false;button.textContent="ตรวจผลหลังเคลียร์"}}});
