...
const FRONTEND_BUILD="2026.08.25-round207.23-production-readiness-gate";
...
function bindProductionReadinessEvents(){const readiness=document.querySelector(".production-readiness");if(readiness&&adminReadinessState.data?.releaseCandidate&&!$("releaseCandidateSummary"))readiness.insertAdjacentHTML("beforebegin",releaseCandidateSummaryHtml(adminReadinessState.data));document.querySelectorAll("[data-uat-id]").forEach(button=>button.addEventListener("click",()=>saveProductionUatItem(button.dataset.uatId,button.dataset.uatPassed==="1")));$("productionUatReset")?.addEventListener("click",resetProductionUat)}
