...
const FRONTEND_BUILD="2026.08.18-round180-production-matching-shadow-uat";
...
async function init() {
  ...
  if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js?v=20260818-r179c",{updateViaCache:"none"}).catch(() => undefined);
...
function datatableMobileCard(row){
  ...
  return `<article class="dtm-record" style="--dtm-alert:${accent}">
    <header class="dtm-record-head"><div><small>เลขนัดหมาย</small><b>${appointment}${appointmentBadge}</b></div><span class="dtm-alert"><i></i>${escapeHtml(level)}</span></header>
...
function renderAdminAppointmentData(){
  const cfg=adminState.data?.appointmentModule||{moduleEnabled:false,uploadEnabled:false,useImportedData:false,timeReference:"PERIOD",searchWindowHours:36,adapterTimeoutMs:250,display:{},uploadProfile:{}},schema=adminState.data?.appointmentSchema||{ready:false,tablesReady:0,tablesTotal:5,indexReady:false,latest:null},profile=cfg.uploadProfile||{};
...
function appointmentShadowUatSummaryHtml(data){
  const s=data?.summary||{},matched=(data?.samples?.matched||[]).slice(0,3),notMatched=(data?.samples?.notMatched||[]).slice(0,3);
  const counts=`<div class="appointment-swal-counts"><span><small>รถที่ตรวจ</small><b>${Number(s.vehiclesChecked||0).toLocaleString()}</b></span><span><small>จับคู่ได้</small><b>${Number(s.matched||0).toLocaleString()}</b></span><span><small>ไม่พบ</small><b>${Number(s.notMatched||0).toLocaleString()}</b></span></div>`;
  const perf=`<div class="appointment-swal-counts"><span><small>Median</small><b>${Number(s.medianLookupMs||0).toLocaleString()} ms</b></span><span><small>P95</small><b>${Number(s.p95LookupMs||0).toLocaleString()} ms</b></span><span><small>เป้าหมาย</small><b>≤ ${Number(s.targetMs||150)} ms</b></span></div>`;
  const timing=`<div class="appointment-swal-counts"><span><small>มาก่อน</small><b>${Number(s.early||0)}</b></span><span><small>ตรงเวลา</small><b>${Number(s.onTime||0)}</b></span><span><small>ช้า</small><b>${Number(s.late||0)}</b></span></div>`;
  const rows=[...matched.map(x=>`<div style="display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-bottom:1px solid #edf1f7"><span><b>${escapeHtml(x.appointmentNo||"-")}</b><small style="display:block;color:#68758a">${escapeHtml(x.gateInAtDisplay||"-")}</small></span><b style="color:#17864b">${escapeHtml(x.timing?.deltaLabel||"พบ")}</b></div>`),...notMatched.map(x=>`<div style="display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-bottom:1px solid #edf1f7"><span><b>${escapeHtml(x.appointmentNo||"-")}</b><small style="display:block;color:#68758a">${escapeHtml(x.gateInAtDisplay||"-")}</small></span><b style="color:#9a5a00">${escapeHtml(x.reasonLabel||"ไม่พบ")}</b></div>`)].join("");
  return`${counts}${timing}${perf}<div style="margin-top:10px;padding:9px 11px;border-radius:10px;background:${s.performancePass?"#eef9f2":"#fff5e9"};font-weight:700;color:${s.performancePass?"#17864b":"#a75b00"}">${s.performancePass?"ความเร็วผ่านเป้าหมาย":"ความเร็วสูงกว่าเป้าหมาย ต้องตรวจต่อ"}</div>${rows?`<div style="margin-top:10px;text-align:left">${rows}</div>`:""}<div style="margin-top:10px;padding:9px 11px;border-radius:10px;background:#f5f7fb;color:#68758a">Shadow UAT อ่านข้อมูลจริงเท่านั้น · ไม่เขียน Gate In และไม่เปลี่ยน Workflow</div>`
}
async function appointmentRunShadowUat(){
  ...
  try{
    Swal.fire({title:"กำลังทดสอบการจับคู่",text:"อ่าน Gate In จริงล่าสุดโดยไม่แก้ไขข้อมูล",allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>Swal.showLoading(),customClass:swalClasses(),width:400});
    const data=await api("/api/admin/appointment-shadow-uat",{method:"POST",body:{lookbackHours:48,limit:40},timeoutMs:30000});
    ...
    const s=data.summary||{},icon=Number(s.vehiclesChecked||0)===0?"info":s.performancePass?"success":"warning",title=Number(s.vehiclesChecked||0)===0?"ยังไม่มี Gate In จริงในช่วงทดสอบ":`Shadow UAT ตรวจ ${Number(s.vehiclesChecked||0)} คัน`;
    await Swal.fire({icon,title,html:appointmentShadowUatSummaryHtml(data),confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:440,heightAuto:false});
  }catch(error){try{Swal.close()}catch{}await showNotice("error",error.message||"ทดสอบการจับคู่ไม่สำเร็จ")}finally{adminState.busy=false;if(button)button.disabled=false}
}
