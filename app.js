...
const FRONTEND_BUILD="2026.08.20-round204.1.2-appointment-center-ux";
...
function dashboardAppointmentPlanHtml(data){
  ...
  if(p.ready===false)return`<section class="appointment-dashboard-r204 is-unavailable"><div><b>ข้อมูลนัดหมาย</b><span>${escapeHtml(p.message||"ข้อมูลนัดหมายยังไม่พร้อม")}</span></div></section>`;
  const withNo=Number(p.withAppointment??p.eligible??0),matched=Number(p.matched||0),coverage=Number(p.coveragePercent??(withNo?matched*100/withNo:0)),diff=Number(p.difference||0),lateAvg=Number(p.avgLateMinutes||0),showTiming=p.showTiming!==false,warnings=[];
  if(Number(p.staleMetricCount||0)>0)warnings.push(`พบผลเทียบเวลาที่ไม่ตรงกับข้อมูลรถปัจจุบัน ${Number(p.staleMetricCount||0).toLocaleString("th-TH")} รายการ ระบบไม่นำมาคิด KPI`);
  if(p.integrity&&p.integrity.ok===false&&Number(p.staleMetricCount||0)===0)warnings.push("ตรวจพบยอดย่อยไม่สมดุล ระบบแสดงสถานะให้ตรวจสอบก่อนใช้ตัวเลขตัดสินใจ");
  if(showTiming){if(p.mixedPolicy)warnings.push("ช่วงนี้มีผลเทียบเวลาจากมากกว่า 1 เกณฑ์");else if(p.stalePolicy)warnings.push("ผลย้อนหลังบางรายการบันทึกด้วยเกณฑ์เดิม")}
  const compliant=Number(p.compliant??(Number(p.early||0)+Number(p.onTime||0))),onTimePercent=Number(p.onTimePercent??(matched?compliant*100/matched:0)),metrics=[`<span><small>มีเลขนัดหมาย</small><b>${withNo.toLocaleString("th-TH")}</b></span>`,`<span><small>จับคู่ได้</small><b>${matched.toLocaleString("th-TH")}</b><em>${coverage.toFixed(1)}%</em></span>`,...(showTiming?[`<span><small>มาก่อน (ตรงเวลา)</small><b>${Number(p.early||0).toLocaleString("th-TH")}</b></span>`,`<span><small>ตรงเวลาในเกณฑ์</small><b>${Number(p.onTime||0).toLocaleString("th-TH")}</b></span>`,`<span><small>ตรงเวลารวม</small><b>${compliant.toLocaleString("th-TH")}</b><em>${onTimePercent.toFixed(1)}%</em></span>`,`<span><small>มาสาย</small><b>${Number(p.late||0).toLocaleString("th-TH")}</b><em>${lateAvg?`ช้าจากนัดเฉลี่ย ${lateAvg.toFixed(1)} นาที`:""}</em></span>`]:[]),`<span><small>ยังไม่จับคู่</small><b>${Number(p.unmatched??p.notMatched??0).toLocaleString("th-TH")}</b></span>`];
  return`<section class="appointment-dashboard-r204 ${warnings.length?"has-audit-warning":""}"><header><div><small>แผนเทียบการมาถึง</small><b>แผน ${Number(p.planned||0).toLocaleString("th-TH")} · จริง ${Number(p.actual||0).toLocaleString("th-TH")}</b><span class="${diff>0?"over":diff<0?"under":""}">${diff>0?`จริงมากกว่าแผน +${diff.toLocaleString("th-TH")}`:diff<0?`จริงน้อยกว่าแผน ${Math.abs(diff).toLocaleString("th-TH")}`:"ตรงตามแผน 0"} คัน</span></div><button type="button" data-appointment-lineage>ที่มาของตัวเลข</button></header>${warnings.length?`<div class="appointment-dashboard-audit-warning"><b>ตรวจสอบนิยามข้อมูล</b><span>${escapeHtml(warnings.join(" · "))}</span></div>`:""}<div class="appointment-dashboard-metrics ${showTiming?"":"is-compact"}">${metrics.join("")}</div>${dashboardAppointmentMiniChart(p.hours||[])}</section>`
}
