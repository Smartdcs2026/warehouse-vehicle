...
const state = { token: sessionStorage.getItem("wvf_token") || "", user: null, view: "operations", vehicles: [], activeDoors: [], online: navigator.onLine };
...
function applyVehicleData(data){const items=data.items||[];if(Array.isArray(data.activeDoors))state.activeDoors=data.activeDoors.map(code=>String(code||"").trim().toUpperCase()).filter(Boolean);let shouldSound=false;if(alertSoundState.initialized){for(const vehicle of items){const key=String(vehicle.auto_id),previous=alertSoundState.levels.get(key),current=String(vehicle.alert_level||"NORMAL");if(Number(vehicle.alert_sound_enabled)&&previous&&alertLevelRank(current)>alertLevelRank(previous))shouldSound=true}}state.vehicles=items;alertSoundState.levels=new Map(items.map(vehicle=>[String(vehicle.auto_id),String(vehicle.alert_level||"NORMAL")]));if(alertSoundState.initialized&&shouldSound)playFeedbackSound("warning");alertSoundState.initialized=true}
