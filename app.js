...
const FRONTEND_BUILD="2026.08.19-round199-verified-archive-cleanup";
...
function renderAdminAppointmentData(){
  ...
  const sync=()=>{const on=$("appointmentModuleEnabled").checked;$("appointmentUploadEnabled").disabled=!on;$("appointmentUseImportedData").disabled=!on;document.querySelectorAll("#appointmentModuleForm select,#appointmentModuleForm input[type=number],[data-appointment-page] input,.appointment-admin-file-settings input,.appointment-admin-file-settings select,.appointment-source-filter-card input,.appointment-source-filter-card select,.appointment-company-policy-card select,.appointment-lifecycle-settings input").forEach(el=>el.disabled=!on);if(!on){$("appointmentUploadEnabled").checked=false;$("appointmentUseImportedData").checked=false}};$("appointmentModuleEnabled")?.addEventListener("change",sync);sync();
