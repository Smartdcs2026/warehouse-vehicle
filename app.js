...
function updateFullscreenButton(){
  ...
  [$("fullscreenButton"),$("dashboardFullscreen")].filter(Boolean).forEach(button=>button.textContent=label);
...
function renderDatatableLayout(){
  const meta=datatableState.meta||{},stages=meta.stages||[],shiftOptions=(meta.shifts||[]).map(s=>`<option value="${escapeHtml(s.shift_id)}">${escapeHtml(s.shift_name)}</option>`).join(""),doorOptions=(meta.doors||[]).map(d=>`<option value="${escapeHtml(d.door_code)}">${escapeHtml(d.door_code)}</option>`).join(""),actorOptions=(meta.actors||[]).map(a=>`<option value="${escapeHtml(a.name)}">${escapeHtml(a.name)}</option>`).join("");
  $("pageContent").innerHTML=`<section class="dt-page">
    <header class="dt-page-head"><div><h2>Datatable</h2><span>ศูนย์ข้อมูลติดตามทุกช่วงเวลาและจุดควบคุมงาน</span></div><div class="dt-range-controls"><label><span>จาก</span><input id="dtFrom" type="date" value="${escapeHtml(datatableState.from)}"></label><label><span>ถึง</span><input id="dtTo" type="date" value="${escapeHtml(datatableState.to)}"></label><label><span>กะ</span><select id="dtShift"><option value="">ทั้งหมด</option>${shiftOptions}</select></label><button id="dtRefresh" class="quiet-button" type="button">รีเฟรช</button><button id="dtExport" class="primary" type="button">ส่งออก</button></div></header>
...
function bindDatatableEvents(){
  ...
  $("dtProblem")?.addEventListener("click",()=>{datatableState.problemOnly=!datatableState.problemOnly;datatableState.page=1;$("dtProblem").classList.toggle("is-active",datatableState.problemOnly);loadDatatable(true)});$("dtReset")?.addEventListener("click",resetDatatableFilters);$("dtRefresh")?.addEventListener("click",()=>loadDatatable(true));$("dtExport")?.addEventListener("click",downloadDatatableExport);$("dtColumns")?.addEventListener("click",chooseDatatableColumns);
...
function renderDatatableData(data){renderDatatableSummary(data.summary||{});renderDatatableRows(data.items||[]);renderDatatablePager(data);renderDatatableActivity(data.activity||[],data.generatedAt);}
...
function renderDatatableSide(){const guide=$("dtStageGuide"),rules=$("dtRules");if(guide){guide.innerHTML=(datatableState.meta?.stages||[]).map(stage=>`<button type="button" data-guide-stage="${stage.key}" class="${stage.key===datatableState.stage?"active":""}"><b>${escapeHtml(stage.label)}</b><span>${escapeHtml(stage.description||"")}</span></button>`).join("");guide.querySelectorAll("[data-guide-stage]").forEach(button=>button.addEventListener("click",()=>document.querySelector(`[data-dt-stage="${button.dataset.guideStage}"]`)?.click()))}if(rules)rules.innerHTML=renderDatatableRules(datatableState.meta?.rules||[])}
