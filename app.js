...
function flowBalance(data){const w=data.workload||{},metrics=data.metrics||{},left=Number(w.carryIn||0)+Number(w.gateIn||0),right=Number(w.gateOut||0)+Number(w.carryOut||0),balanced=left===right,total=Number(metrics.total||0),closed=Number(metrics.closed||0),rate=total?Math.min(100,Math.max(0,closed*100/total)):0;return`<div class="flow-balance"><div><small>ต้นช่วง</small><b>${Number(w.carryIn||0)}</b></div><i>+</i><div><small>รถเข้า</small><b>${Number(w.gateIn||0)}</b></div><i>=</i><div><small>รถออก</small><b>${Number(w.gateOut||0)}</b></div><i>+</i><div><small>ปลายช่วง</small><b>${Number(w.carryOut||0)}</b></div></div><p class="balance-check ${balanced?"ok":"warn"}">${w.balanceApplicable?(balanced?`ยอดสมดุล ${left} คัน`:`ยอดต่าง ${Math.abs(left-right)} คัน · ตรวจเวลาต้นทาง`):"ช่วงหลายกะจะแสดงยอดรวมแต่ละกะ ไม่ใช้สมการยอดคงเหลือ"}</p><div class="cohort-strip"><div class="completion-ring" style="--completion:${rate}%"><span><b>${rate.toFixed(0)}%</b><small>ปิดงาน</small></span></div><div><span>รุ่นรถเข้า</span><b>${closed}/${total}</b><small>ปิดแล้ว ${rate.toFixed(1)}%</small></div></div>`}
...
function dailyComboChart(rows){if(!rows.length)return`<div class="empty-state">ยังไม่มีข้อมูลในช่วงที่เลือก</div>`;const items=rows.slice(-30),maxTotal=Math.max(1,...items.map(row=>Number(row.total)||0)),maxAvg=Math.max(1,...items.map(row=>Number(row.avg_seconds)||0)),w=720,h=210,pad=28,step=(w-pad*2)/Math.max(1,items.length),points=items.map((row,index)=>`${pad+step*index+step/2},${h-pad-(Number(row.avg_seconds)||0)/maxAvg*(h-pad*2)}`).join(" ");return`<div class="opsdash-chart-scroll"><svg class="opsdash-combo" viewBox="0 0 ${w} ${h}" role="img" aria-label="แนวโน้มรถเข้ารายวัน">${items.map((row,index)=>{const total=Number(row.total)||0,x=pad+step*index+step*.18,height=total/maxTotal*(h-pad*2),center=pad+step*index+step/2;return`<rect x="${x}" y="${h-pad-height}" width="${Math.max(5,step*.64)}" height="${height}" rx="4"/>${total?`<text class="bar-value" x="${center}" y="${Math.max(12,h-pad-height-5)}" text-anchor="middle">${total}</text>`:""}<text x="${center}" y="${h-8}" text-anchor="middle">${escapeHtml(String(row.label||"").slice(0,5))}</text>`}).join("")}<polyline points="${points}"/>${items.map((row,index)=>`<circle cx="${pad+step*index+step/2}" cy="${h-pad-(Number(row.avg_seconds)||0)/maxAvg*(h-pad*2)}" r="3"><title>${escapeHtml(row.label)} · ${Number(row.total)||0} คัน · ${formatDuration(row.avg_seconds)}</title></circle>`).join("")}</svg></div><div class="chart-legend"><span class="bar-key">รถเข้า</span><span class="line-key">เวลาเฉลี่ย</span></div>`}
...
function hourChart(rows,max){return`<div class="hour-chart">${Array.from({length:24},(_,hour)=>{const row=(rows||[]).find(item=>Number(item.hour)===hour),value=Number(row?.total||0);return`<div title="${String(hour).padStart(2,"0")}:00 · ${value} คัน">${value?`<b>${value}</b>`:""}<i style="height:${Math.max(3,value/max*100)}%"></i><small>${hour%3===0?String(hour).padStart(2,"0"):""}</small></div>`}).join("")}</div>`}
