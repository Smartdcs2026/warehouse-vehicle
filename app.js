...
const FRONTEND_BUILD="2026.08.18-round185a-post-activation-dashboard-recovery";
...
function appointmentFinalUatHtml(data){
  ...
  const freezeBox=freeze.recorded?`<div style="margin-top:10px;padding:10px 12px;border-radius:11px;background:#eef9f2;color:#17864b"><b>Release Freeze พร้อม</b><small style="display:block;margin-top:3px">${escapeHtml(freeze.frozenAtDisplay||'-')} · การใช้งานจริงยังไม่ถูกเปิด</small></div>`:`<div style="margin-top:10px;padding:10px 12px;border-radius:11px;background:#fff7e9;color:#915600"><b>ยังไม่ Freeze</b><small style="display:block;margin-top:3px">แก้รายการที่ไม่ผ่านแล้วทดสอบใหม่</small></div>`;
  return`<div>${head}${baseline}<div style="margin-top:6px;max-height:46vh;overflow:auto;padding-right:3px">${rows}</div>${freezeBox}<div style="margin-top:10px;padding:9px 11px;border-radius:10px;background:#f5f7fb;color:#68758a">Final UAT ไม่เปิดสวิตช์งานจริงอัตโนมัติ · ไม่แก้ Gate In / รถ · วันที่เวลา ${escapeHtml(data?.dateTimeStandard||'dd/MM/yyyy HH:mm:ss')}</div></div>`;
}
async function appointmentRunFinalUat(){
  ...
  try{
    Swal.fire({title:"กำลังตรวจ Final Production UAT",text:"ตรวจทุกชั้นและบันทึก Release Freeze เมื่อผ่านครบ",allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>Swal.showLoading(),customClass:swalClasses(),width:430});
    ...
    await Swal.fire({icon:pass?"success":"warning",title:pass?"Final UAT ผ่าน · Release Freeze พร้อม":"Final UAT มีรายการต้องตรวจ",html:appointmentFinalUatHtml(data),confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:550,heightAuto:false});
