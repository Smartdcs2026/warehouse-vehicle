...
const state = { token: sessionStorage.getItem("wvf_token") || "", user: null, view: "operations", vehicles: [], activeDoors: [], trackingEnabled: true, online: navigator.onLine };
...
function renderJobCards(items) {
  $("jobGrid").innerHTML = items.length ? items.map(v => {
    ...
    const actions=inProgress
      ...
      : called
        ? `<button class="outline-button recall-button" data-receiving-action="recall" data-auto-id="${escapeHtml(v.auto_id)}" ${busy?"disabled":""}>เรียกซ้ำ / เปลี่ยนประตู</button><button class="complete-button start-receiving-button" data-receiving-action="start" data-auto-id="${escapeHtml(v.auto_id)}" ${busy?"disabled":""}>${busy?"กำลังบันทึก":"เริ่มตรวจรับ"}</button>`
        : `<button class="primary call-vehicle-button" data-receiving-action="call" data-auto-id="${escapeHtml(v.auto_id)}" ${busy?"disabled":""}>${busy?"กำลังบันทึก":"เรียกรถ"}</button><button class="outline-button start-receiving-button" data-receiving-action="start" data-auto-id="${escapeHtml(v.auto_id)}" ${busy?"disabled":""}>เริ่มตรวจรับ</button>`;
...
function queueReasonOptions(selected="GENERAL"){
  return [["NO_SHOW","รถยังไม่เข้าจุดตรวจรับ"],["DRIVER_NOT_FOUND","ไม่พบคนขับ"],["WRONG_DOOR","รถเข้าผิดประตู"],["DOOR_CHANGE","เปลี่ยนประตู"],["GENERAL","เรียกซ้ำทั่วไป"],["OTHER","อื่น ๆ"]].map(([value,label])=>`<option value="${value}" ${value===selected?"selected":""}>${label}</option>`).join("");
}
...
async function callVehicle(vehicle,isRecall=false){
  ...
  if(window.Swal){
    ...
    const reasonHtml=isRecall?`<div class="recall-reason-box"><label><span>เหตุผลที่เรียกอีกครั้ง</span><select id="queueReason">${queueReasonOptions()}</select></label><label><span>หมายเหตุ (ไม่บังคับ)</span><input id="queueReasonNote" type="text" maxlength="160" placeholder="ระบุเพิ่มเติมเมื่อจำเป็น"></label></div>`:"";
    const result=await Swal.fire({title:isRecall?"เรียกรถอีกครั้ง":"เรียกรถเข้าตรวจรับ",html:`${vehicleDetailsHtml(vehicle,autoId)}${reasonHtml}${doorHtml}`,showCancelButton:true,confirmButtonText:isRecall?"เรียกอีกครั้ง":"เรียกรถ",cancelButtonText:"ยกเลิก",reverseButtons:true,focusCancel:true,customClass:swalClasses(),buttonsStyling:false,width:460,didOpen:()=>{
      ...
      return{doorCode,reasonCode:isRecall?String($("queueReason")?.value||"GENERAL"):"FIRST",note:isRecall?String($("queueReasonNote")?.value||"").trim():""};
    }});
...
function openPublicQueue(){window.open(new URL("./queue.html?v=20260809-r76",location.href).href,"_blank","noopener")}
...
function handoverSummary(data){const w=data.workload||{},terms=dashboardHandoverTerms(data);return`<div class="handover-summary"><div><small>${terms.sameClosedLabel}</small><b>${Number(w.samePeriodClosed||0)}</b></div><div><small>${terms.inheritedClosedLabel}</small><b>${Number(w.priorPeriodClosed||0)}</b></div><div><small>${terms.receivingLabel}</small><b>${Number(w.receivingCompleted||0)}</b></div><div><small>${terms.outgoingSummaryLabel}</small><b>${Number(w.carryOut||0)}</b></div></div>`}
...
function renderAdminShell(){
  const tabs=[["users","ผู้ใช้งาน"],["workflow","ขั้นตอนงาน"],["doors","ประตู"],["shifts","กะทำงาน"],["alerts","เวลาแจ้งเตือน"],["data","การใช้ข้อมูล"],["export","ส่งออกข้อมูล"],["tracking","ติดตามคนขับ"],["voice","เสียงประกาศ"],["queue","จอคิว"]];
...
function renderAdminVoice(){
  const panel=$("adminPanel");if(!panel)return;
  const v=adminState.data.voice||{enabled:false,volume:80,repeatCount:1,repeatDelaySeconds:7,readDoor:true,readDoorLeadingZero:false,playDing:true,playThanks:true,packId:"th-TH-standard-01"};
  panel.innerHTML=`<div class="admin-section-head clean-admin-head"><div><h3>เสียงประกาศคิว</h3><p>เสียงทำงานแยกจากขั้นตอนหลัก การบันทึกงานจะเสร็จก่อนเริ่มประกาศทุกครั้ง</p></div><span class="tracking-saved-pill"><span>สถานะ</span><b>${v.enabled?"เปิดใช้งาน":"ปิดใช้งาน"}</b></span></div>
  ...
  <form id="voiceSettingsForm" class="admin-form-grid voice-admin-form">
    <label class="admin-toggle-row"><span><b>เปิดเสียงประกาศคิว</b><small>ปิดเสียงได้โดยไม่กระทบการเรียกคิวหรือเวลาของงาน</small></span><input id="voiceEnabled" type="checkbox" ${v.enabled?"checked":""}></label>
    <label><span>ชุดเสียง</span><select id="voicePack"><option value="th-TH-standard-01" selected>เสียงไทย ชุดมาตรฐาน 01</option></select></label>
    ...
    <label><span>เว้นก่อนประกาศซ้ำ (วินาที)</span><input id="voiceRepeatDelay" type="number" min="3" max="30" value="${Number(v.repeatDelaySeconds||7)}"></label>
    <label class="admin-toggle-row"><span><b>เสียงเตือนก่อนประกาศ</b><small>เล่นเสียงสั้นก่อนอ่านหมายเลขนัดหมาย</small></span><input id="voicePlayDing" type="checkbox" ${v.playDing!==false?"checked":""}></label>
    <label class="admin-toggle-row"><span><b>อ่านหมายเลขประตู</b><small>อ่านเฉพาะเมื่อระบบใช้ประตูและรายการมีหมายเลขประตู หากปิดการใช้ประตู ระบบจะข้ามส่วนนี้อัตโนมัติ</small></span><input id="voiceReadDoor" type="checkbox" ${v.readDoor!==false?"checked":""}></label>
    ...
    <label class="admin-toggle-row"><span><b>กล่าวขอบคุณท้ายประกาศ</b><small>เล่นคำขอบคุณหลังประกาศเสร็จ</small></span><input id="voicePlayThanks" type="checkbox" ${v.playThanks!==false?"checked":""}></label>
    <div class="admin-form-actions"><button id="voiceSaveButton" class="primary" type="submit">บันทึกการตั้งค่าเสียง</button><button id="voiceTestButton" class="outline-button" type="button">ทดลองแบบมีประตู R07</button><button id="voiceTestNoDoorButton" class="outline-button" type="button">ทดลองแบบไม่มีประตู</button></div>
  </form>
  <section class="admin-queue-guide"><article><b>ไม่รบกวนเวลางาน</b><p>การกด “เรียกรถ” บันทึกเฉพาะประวัติการเรียก แล้วจอคิวรับรายการไปประกาศภายหลัง โดยไม่แตะเวลาเริ่มตรวจรับ</p></article><article><b>เรียกหลายคันได้</b><p>จอคิวจัดคิวเสียงตามเวลาที่เรียกและไม่พูดทับกัน</p></article><article><b>จอคิวต้องเปิดเสียงครั้งแรก</b><p>หลังเปิดหน้าจอ ให้กด “เปิดเสียง” หนึ่งครั้งเพื่อเตรียมลำโพงและชุดเสียง</p></article></section>`;
  $("voiceSettingsForm")?.addEventListener("submit",async e=>{e.preventDefault();if(adminState.busy)return;const body={enabled:$("voiceEnabled").checked,packId:$("voicePack").value,volume:Number($("voiceVolume").value),repeatCount:Number($("voiceRepeatCount").value),repeatDelaySeconds:Number($("voiceRepeatDelay").value),playDing:$("voicePlayDing").checked,readDoor:$("voiceReadDoor").checked,readDoorLeadingZero:$("voiceDoorZero").checked,playThanks:$("voicePlayThanks").checked};adminState.busy=true;try{const r=await api("/api/admin/voice",{method:"POST",body});adminState.data=await api("/api/admin/settings");renderAdminShell();await showNotice("success",r.message||"บันทึกการตั้งค่าเสียงแล้ว")}catch(error){await showNotice("error",error.message||"บันทึกการตั้งค่าเสียงไม่สำเร็จ")}finally{adminState.busy=false}});
  const testSettings=()=>({enabled:true,packId:$("voicePack").value,volume:Number($("voiceVolume").value),repeatCount:1,repeatDelaySeconds:Number($("voiceRepeatDelay").value),playDing:$("voicePlayDing").checked,readDoor:$("voiceReadDoor").checked,readDoorLeadingZero:$("voiceDoorZero").checked,playThanks:$("voicePlayThanks").checked});
  $("voiceTestButton")?.addEventListener("click",()=>testAdminQueueVoice(testSettings(),{appointmentNo:"2006988",doorCode:"R07",useDoor:true},"voiceTestButton"));
  $("voiceTestNoDoorButton")?.addEventListener("click",()=>testAdminQueueVoice(testSettings(),{appointmentNo:"2006988",doorCode:null,useDoor:false},"voiceTestNoDoorButton"));
  updateAdminVoiceReadyStatus();
}
async function updateAdminVoiceReadyStatus(){const box=$("voiceReadyStatus");if(!box)return;try{const base=String(cfg.apiBaseUrl||"").replace(/\/$/,"");const response=await fetch(base+"/api/voice/status",{cache:"no-store"}),data=await response.json();if(!response.ok||!data.ready)throw new Error(data.message||"ยังไม่พร้อม");box.classList.add("ready");box.innerHTML=`<b>ชุดเสียงพร้อมใช้งาน</b><span>เชื่อมต่อเสียงส่วนกลางเรียบร้อย</span>`}catch(error){box.classList.remove("ready");box.innerHTML=`<b>ชุดเสียงยังไม่พร้อม</b><span>${escapeHtml(error.message||"กรุณาตรวจสอบชุดเสียง")}</span>`}}
async function ensureQueueVoiceEngine(){if(window.SmartQueueVoice)return window.SmartQueueVoice;await new Promise((resolve,reject)=>{const existing=document.querySelector('script[data-queue-voice-engine]');if(existing){existing.addEventListener("load",resolve,{once:true});existing.addEventListener("error",reject,{once:true});return}const script=document.createElement("script");script.src="./voice-engine.js?v=20260809-r76";script.dataset.queueVoiceEngine="1";script.onload=resolve;script.onerror=()=>reject(new Error("โหลดระบบเสียงไม่สำเร็จ"));document.head.appendChild(script)});if(!window.SmartQueueVoice)throw new Error("ระบบเสียงยังไม่พร้อม");return window.SmartQueueVoice}
async function testAdminQueueVoice(settings,item={appointmentNo:"2006988",doorCode:"R07",useDoor:true},buttonId="voiceTestButton"){const button=$(buttonId),original=button?.textContent||"ทดลองเสียง";if(button){button.disabled=true;button.textContent="กำลังเตรียมเสียง"}try{const engine=await ensureQueueVoiceEngine();engine.configure({...settings,enabled:true,assetBasePath:"/voice/queue/th-TH/standard-01/",apiBaseUrl:cfg.apiBaseUrl,repeatCount:1});await engine.unlockAndPrepare();await engine.announceNow(item);await showNotice("success",item?.useDoor===false?"ทดสอบเสียงแบบไม่มีประตูแล้ว":"ทดสอบเสียงแบบมีประตูแล้ว")}catch(error){await showNotice("error",error.message||"ทดลองเสียงไม่สำเร็จ")}finally{if(button){button.disabled=false;button.textContent=original}}}

function renderAdminQueue(){
  const panel=$("adminPanel");if(!panel)return;
  const queueUrl=new URL("./queue.html?v=20260809-r76",location.href).href;
  panel.innerHTML=`<div class="admin-section-head clean-admin-head"><div><h3>จอแสดงสถานะคิว</h3><p>ใช้สำหรับจอส่วนกลาง อ่านข้อมูลอย่างเดียว ไม่เปลี่ยนสถานะงาน</p></div><a class="primary admin-queue-open" href="./queue.html?v=20260809-r76" target="_blank" rel="noopener">เปิดจอคิว</a></div>
...
function applyVehicleData(data){const items=data.items||[];if(typeof data.trackingEnabled!=="undefined")state.trackingEnabled=Boolean(data.trackingEnabled);if(Array.isArray(data.activeDoors))state.activeDoors=data.activeDoors.map(code=>String(code||"").trim().toUpperCase()).filter(Boolean);let shouldSound=false;if(alertSoundState.initialized){for(const vehicle of items){const key=String(vehicle.auto_id),previous=alertSoundState.levels.get(key),current=String(vehicle.alert_level||"NORMAL");if(Number(vehicle.alert_sound_enabled)&&previous&&alertLevelRank(current)>alertLevelRank(previous))shouldSound=true}}state.vehicles=items;alertSoundState.levels=new Map(items.map(vehicle=>[String(vehicle.auto_id),String(vehicle.alert_level||"NORMAL")]));if(alertSoundState.initialized&&shouldSound)playFeedbackSound("warning");alertSoundState.initialized=true}
