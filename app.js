...
const DASHBOARD_THEME_OPTIONS=[
  {id:"blue",name:"Executive Blue",label:"ฟ้าองค์กร",note:"มาตรฐานผู้บริหาร ดูสะอาด ชัดเจน"},
  {id:"slate",name:"Slate Office",label:"เทาองค์กร",note:"สุภาพ เรียบง่าย เหมาะงานธุรกิจ"},
  {id:"green",name:"Emerald Operations",label:"เขียวปฏิบัติการ",note:"สบายตา เหมาะเปิดดูต่อเนื่อง"},
  {id:"indigo",name:"Indigo Insight",label:"ครามวิเคราะห์",note:"เน้นภาพรวมและตัวเลขสำคัญ"},
  {id:"dark",name:"Graphite Command",label:"เข้มห้องควบคุม",note:"เหมาะจอควบคุมและจอใหญ่"},
  {id:"warm",name:"Sandstone Neutral",label:"ครีมสุภาพ",note:"อ่อนตา อ่านง่าย ดูเป็นมิตร"}
];
...
function dashboardThemeMenuHtml(){return `<b>เลือกธีม</b>${DASHBOARD_THEME_OPTIONS.map(item=>`<button type="button" data-dashboard-theme="${item.id}" class="${normalizeDashboardTheme(dashboardState.theme)===item.id?"active":""}"><i></i><span><strong>${item.label}</strong><small>${item.note}</small></span></button>`).join("")}`}
...
async function showDashboardThemeDialog(){const result=await Swal.fire({title:"เลือกธีม Dashboard",html:`<div class="dashboard-theme-dialog">${DASHBOARD_THEME_OPTIONS.map(item=>`<button type="button" data-dialog-theme="${item.id}" class="${normalizeDashboardTheme(dashboardState.theme)===item.id?"active":""}"><i class="theme-${item.id}"></i><span><strong>${item.name}</strong><small>${item.note}</small></span></button>`).join("")}</div>`,showConfirmButton:false,showCloseButton:true,customClass:swalClasses(),width:560,didOpen:()=>document.querySelectorAll("[data-dialog-theme]").forEach(button=>button.addEventListener("click",()=>{setDashboardTheme(button.dataset.dialogTheme);Swal.close()}))});return result}
function applyDashboardTheme(){const theme=normalizeDashboardTheme(dashboardState.theme),meta=dashboardThemeMeta(theme),shell=$("appView");dashboardState.theme=theme;if(shell)shell.dataset.dashboardTheme=theme;document.querySelectorAll("[data-dashboard-theme]").forEach(button=>button.classList.toggle("active",button.dataset.dashboardTheme===theme));const button=$("dashboardThemeButton");if(button){button.dataset.activeTheme=theme;const label=button.querySelector("span");if(label)label.textContent=meta.label}const mobileButton=$("dashboardMobileTheme");if(mobileButton){const span=mobileButton.querySelector("span");if(span)span.textContent=`ธีม · ${meta.label}`}}
