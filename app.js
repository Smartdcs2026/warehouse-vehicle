...
const datatableState={meta:null,data:null,busy:false,reloadRequested:false,requestSeq:0,detailBusy:false,refreshMetaRequested:false,activity:[],shiftAutoDate:false,stage:"overview",from:"",to:"",shiftId:"",search:"",sla:"ALL",status:"ALL",door:"",actor:"",sort:"start_desc",page:1,limit:25,problemOnly:false,rejectedOnly:false,missingAppointmentOnly:false,searchTimer:0,immersive:false,nativeFullscreen:false,mobileFiltersOpen:false,viewportMode:"",viewportTimer:0,columns:new Set(JSON.parse(localStorage.getItem("wvf_dt_columns_r100")||localStorage.getItem("wvf_dt_columns_r96")||localStorage.getItem("wvf_dt_columns_r95")||'["company","plate","shift","actor"]'))};
...
function renderDatatableLayout(){
  const meta=datatableState.meta||{},exclusionAllowed=state.user?.accessRights==="ADMIN"||meta.vehicleExclusion?.userEnabled!==false,stages=(meta.stages||[]).filter(s=>meta.documentCheckEnabled||s.key!=="document_review"),shiftOptions=(meta.shifts||[]).map(s=>`<option value="${escapeHtml(s.shift_id)}">${escapeHtml(s.shift_name)} · ${minuteToTime(s.start_minute)}–${minuteToTime(s.end_minute)}</option>`).join(""),doorOptions=(meta.doors||[]).map(d=>`<option value="${escapeHtml(d.door_code)}">${escapeHtml(d.door_code)}</option>`).join(""),actorOptions=(meta.actors||[]).map(a=>`<option value="${escapeHtml(a.name)}">${escapeHtml(a.name)}</option>`).join("");
...
function bindDatatableEvents(){
  ...
  $("dtProblem")?.addEventListener("click",()=>{datatableState.problemOnly=!datatableState.problemOnly;datatableState.page=1;$("dtProblem").classList.toggle("is-active",datatableState.problemOnly);loadDatatable(true)});$("dtMissingAppointment")?.addEventListener("click",()=>{datatableState.missingAppointmentOnly=!datatableState.missingAppointmentOnly;datatableState.page=1;$("dtMissingAppointment").classList.toggle("is-active",datatableState.missingAppointmentOnly);loadDatatable(true)});$("dtExcludedList")?.addEventListener("click",showVehicleExclusions);$("dtReset")?.addEventListener("click",resetDatatableFilters);$("dtCompare")?.addEventListener("click",showDatatableCompare);$("dtRefresh")?.addEventListener("click",refreshDatatableAll);$("dtFullscreen")?.addEventListener("click",toggleFullscreen);$("dtExport")?.addEventListener("click",downloadDatatableExport);$("dtColumns")?.addEventListener("click",chooseDatatableColumns);$("dtAllRules")?.addEventListener("click",showDatatableAllRules);$("dtActivityButton")?.addEventListener("click",showDatatableActivity);
