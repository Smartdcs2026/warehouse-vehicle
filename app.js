...
async function init() {
  ...
  if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js?v=20260816-r145",{updateViaCache:"none"}).catch(() => undefined);
...
function renderDashboard() {
  ...
  $("pageContent").innerHTML=`<section class="opsdash-head r145-dashboard-head"><div class="opsdash-title"><button id="dashboardMenuButton" class="dashboard-menu-button" aria-label="เปิดเมนู" aria-expanded="false">☰</button><div><h2 id="dashboardPageTitle">ภาพรวมคลังสินค้า</h2><p id="dashboardRangeLabel">กำลังโหลดข้อมูล</p></div></div><div class="opsdash-filter">${["ADMIN","USER"].includes(state.user?.accessRights)?`<button id="dashboardQueueButton" class="dashboard-queue-button" type="button" aria-label="เปิดจอคิว"><span aria-hidden="true">▣</span><b>จอคิว</b></button>`:""}<button id="dashboardCalendarButton" class="calendar-button" aria-label="เลือกวันที่"><span class="calendar-icon" aria-hidden="true"><i></i><i></i><strong>31</strong></span><b id="dashboardDateButtonLabel" data-mobile-label="วันที่">เลือกวันที่</b></button><div class="range-buttons"><button data-dashboard-range="today" data-mobile-label="1วัน" aria-label="แสดงข้อมูลรายวัน">วัน</button><button data-dashboard-range="7d" data-mobile-label="7วัน" aria-label="แสดงข้อมูล 7 วัน">7 วัน</button><button data-dashboard-range="30d" data-mobile-label="30วัน" aria-label="แสดงข้อมูล 30 วัน">30 วัน</button></div><select id="dashboardShift" aria-label="เลือกกะ"><option value="">ทุกกะ</option></select><button id="dashboardRefresh" class="outline-button dashboard-refresh-button" type="button">อัปเดต</button><button id="dashboardFullscreen" class="outline-button">เต็มจอ</button><button id="dashboardMoreButton" class="dashboard-more-button" type="button" aria-label="ตัวเลือกเพิ่มเติม" aria-expanded="false">⋮</button></div><div id="dashboardMobileMenu" class="dashboard-mobile-menu" hidden>${["ADMIN","USER"].includes(state.user?.accessRights)?`<button id="dashboardMobileQueue" type="button"><b>▣</b><span>เปิดจอคิว</span></button>`:""}<button id="dashboardMobileRefresh" type="button"><b>↻</b><span>อัปเดตข้อมูล</span></button><button id="dashboardMobileFullscreen" type="button"><b>⛶</b><span>เต็มจอ</span></button><button id="dashboardMobileInfo" type="button"><b>i</b><span>คำอธิบายข้อมูล</span></button></div></section><section id="dashboardCalendarPopover" class="dashboard-calendar-popover" hidden></section><div id="dashboardBody" class="loading">กำลังสรุปข้อมูล</div>`;
...
function renderDashboardData(data){
  ...
  $("dashboardBody").innerHTML=`<section class="opsdash-context r145-dashboard-tabs"><nav class="opsdash-tabs opsdash-tabs-inline" aria-label="เลือกหน้า Dashboard">${dashboardTabs().map(([id,label])=>`<button data-dashboard-tab="${id}" class="${dashboardState.tab===id?"active":""}">${label}</button>`).join("")}</nav>${fresh}</section><section id="dashboardKpis" class="opsdash-kpis r145-kpis">${dashboardKpiSet(data,dashboardState.tab)}</section><div id="dashboardTabContent" class="opsdash-content r145-content">${dashboardTabContent(data)}</div>`;
...
function dashboardTabs(){return[["overview","ภาพรวม"],["handover","งานค้าง"],["comparison","เทียบช่วงก่อน"],["performance","เวลาทำงาน"],["capacity","ประตู"],["exceptions","จุดที่ต้องรีบแก้"]]}
...
function dashboardUpdatePageTitle(){const title=$("dashboardPageTitle");if(title)title.textContent=dashboardPageTitle()}
...
function dashboardKpiSet(data,tab="overview"){const m=data.metrics||{},w=data.workload||{},a=data.activeStages||{},p=data.comparison?.previous||{},levels=data.alertLevels||{},qTotal=dashboardQualityTotal(data),doors=dashboardDoorStats(data),urgent=dashboardUrgentCount(data),rate=Number(m.total||0)?Number(m.closed||0)*100/Number(m.total||0):0;let specs=[];
  if(tab==="handover")specs=[["ค้างก่อนช่วง",w.carryIn,"งานที่รับต่อมา","#7C3AED","↙"],["รถเข้า",w.gateIn,"งานใหม่ในช่วง","#2563EB","↓"],["ระบายงานเก่าได้",w.priorPeriodClosed,"งานค้างที่ปิดได้","#059669","✓"],["จบงาน",w.gateOut,dashboardFlowMessage(data),"#0F9F74","↗"],["ค้างปลายช่วง",w.carryOut,"ต้องส่งต่อหรือทำต่อ","#8B5CF6","▰"],["งานเร่งด่วน",urgent,"ต้องตามก่อน","#DC2626","!"]];
  else if(tab==="comparison")specs=[["รถเข้า",w.gateIn,comparisonNote(w.gateIn,p.total,"ช่วงก่อน"),"#2563EB","↓"],["จบงาน",w.gateOut,comparisonNote(w.gateOut,p.closed,"ช่วงก่อน"),"#059669","✓"],["งานค้าง",w.carryOut,"ปลายช่วงที่เลือก","#7C3AED","▰"],["เวลาเฉลี่ย",durationValue(m.avgTotalSeconds),comparisonNote(m.avgTotalSeconds,p.avgTotalSeconds,"ช่วงก่อน",true),"#0F766E","◷"],["งานเร่งด่วน",urgent,"ต้องตามก่อน","#DC2626","!"],["รับเสร็จ",w.receivingCompleted,"ตรวจรับสินค้าเสร็จ","#0891B2","✓"]];
  else if(tab==="performance")specs=[["เวลาเฉลี่ย",durationValue(m.avgTotalSeconds),"Gate In ถึง Gate Out","#0F766E","◷"],["ครึ่งหนึ่งจบภายใน",durationValue(m.p50TotalSeconds),"50% ของงานที่จบ","#2563EB","½"],["90% จบภายใน",durationValue(m.p90TotalSeconds),"ดูงานที่ใช้เวลานาน","#D97706","90"],["ปิดงานแล้ว",`${Number(m.closed||0)}/${Number(m.total||0)}`,`${rate.toFixed(1)}% ของงานในช่วง`,"#059669","✓"],["รับเสร็จ",w.receivingCompleted,"ตรวจรับสินค้าเสร็จ","#0891B2","✓"],["งานค้าง",w.carryOut,"ยังไม่ Gate Out","#7C3AED","▰"]];
  else if(tab==="capacity")specs=[["ประตูมีงาน",doors.used,"ประตูที่มีรถในช่วง","#2563EB","▣"],["งานมากสุด",doors.busiest?.label||"–",doors.busiest?`${Number(doors.busiest.total||0)} คัน`:"ยังไม่มีข้อมูล","#0F9F74","↑"],["ใช้เวลานานสุด",doors.slowest?.label||"–",doors.slowest?`เฉลี่ย ${formatDuration(doors.slowest.avg_seconds)}`:"ยังไม่มีข้อมูล","#D97706","◷"],["ข้อมูลประตูขาด",Number(data.dataQuality?.missing_required_door||0),"รายการที่ต้องตรวจ","#DC2626","!"],["กำลังตรวจรับ",Number(a.RECEIVING_IN_PROGRESS||0),"กำลังทำงานอยู่","#0891B2","●"],["รถเข้า",w.gateIn,"ช่วงที่เลือก","#2563EB","↓"]];
  else if(tab==="exceptions")specs=[["วิกฤต",Number(levels.CRITICAL||0),"ต้องติดตามทันที","#DC2626","!"],["ล่าช้า",Number(levels.URGENT||0),"เกินเกณฑ์เวลา","#F97316","!"],["เตือน",Number(levels.WARNING||0),"ใกล้เกินเวลา","#F59E0B","!"],["ข้อมูลไม่ครบ",qTotal,"ควรตรวจแก้","#7C3AED","?"],["คอขวด",data.bottleneck?.label||"ไม่มี",`${Number(data.bottleneck?.queue||0)} คัน`,"#0F766E","▰"],["งานเร่งด่วน",urgent,"รวมเตือนขึ้นไป","#B91C1C","!"]];
  else specs=[["รถเข้า",w.gateIn,comparisonNote(w.gateIn,p.total,"ช่วงก่อน"),"#2563EB","↓"],["จบงาน",w.gateOut,dashboardFlowMessage(data),"#059669","✓"],["งานค้าง",w.carryOut,`${Number(w.carryIn||0)} คันมาจากก่อนช่วง`,"#7C3AED","▰"],["กำลังตรวจรับ",Number(a.RECEIVING_IN_PROGRESS||0),"กำลังทำงานอยู่","#0891B2","●"],["เวลาเฉลี่ย",durationValue(m.avgTotalSeconds),comparisonNote(m.avgTotalSeconds,p.avgTotalSeconds,"ช่วงก่อน",true),"#0F766E","◷"],["งานเร่งด่วน",urgent,"ต้องตามก่อน","#DC2626","!"]];
...
function dashboardOverview(data){const terms=dashboardHandoverTerms(data),maxHour=Math.max(1,...(data.hours||[]).map(row=>Number(row.total)||0));return`<section class="opsdash-grid r145-grid r145-overview">${panel("สมดุลงาน","ค้างก่อน + รถเข้า = จบงาน + ค้างปลายช่วง",flowBalance(data),"r145-flow","flow")}${panel("งานค้างตามขั้นตอน","ดูว่าค้างตรงไหนมากที่สุด",stageQueueChart(data.activeStages||{}),"r145-stage","stageQueue")}${panel("งานที่ต้องรีบทำ",`${dashboardUrgentCount(data)} คันต้องตามก่อน`,dashboardPriorityList(data.actionItems,5),"r145-urgent","urgent")}${panel("รถเข้าแต่ละชั่วโมง","ดูช่วงที่รถเข้าหนาแน่น",hourChart(data.hours,maxHour),"r145-hour","hourly")}${panel("อายุงานค้าง",terms.agingNote,backlogAgingChart(data.backlogAging||[]),"r145-aging","aging")}</section>`}
function dashboardHandover(data){const w=data.workload||{},terms=dashboardHandoverTerms(data),count=Number(data.handoverItemCount||0);return`<section class="opsdash-grid r145-grid r145-handover">${panel("ภาพรวมงานค้าง","เห็นงานที่รับต่อและงานที่ยังไม่จบ",flowBalance(data),"r145-flow","handover")}${panel("งานค้างตามขั้นตอน","สถานะเมื่อสิ้นสุดช่วง",stageQueueChart(w.handoverOutStages||data.activeStages||{}),"r145-stage","handover")}${panel("ผลการเคลียร์งาน","งานที่ปิดได้และงานที่เหลือ",handoverSummary(data),"r145-summary","handover")}${panel("รายการที่ยังไม่จบ",`${Math.min(count,5)} จาก ${count} รายการที่ต้องตาม`,actionTable(data.handoverItems||data.actionItems,5),"r145-list","handover")}</section>`}
function dashboardComparison(data){return`<section class="opsdash-grid r145-grid r145-comparison">${panel("แนวโน้มรายวัน","รถเข้า · จบงาน · เวลาเฉลี่ย",dailyComboChart(data.comparison?.daily||[]),"r145-trend","comparison")}${panel("สรุปการเปลี่ยนแปลง","เทียบกับช่วงก่อนที่ยาวเท่ากัน",comparisonSummary(data),"r145-compare-summary","comparison")}${panel("เทียบแต่ละกะ","ดูจำนวนงานและเวลาเฉลี่ย",shiftCompareChart(data.shiftPerformance||[]),"r145-shifts","comparison")}</section>`}
...
function dashboardPerformance(data){const stages=[["รถเข้า → ยื่นเอกสาร",data.stageAverages?.gate_to_doc],["ยื่นเอกสาร → ตรวจเสร็จ",data.stageAverages?.document_review],["ตรวจเสร็จ → เริ่มตรวจรับ",data.stageAverages?.checked_to_start],["เริ่มตรวจรับ → รับเสร็จ",data.stageAverages?.receiving],["รับเสร็จ → คืนเอกสาร",data.stageAverages?.complete_to_return],["คืนเอกสาร → ออกจากพื้นที่",data.stageAverages?.return_to_out]];return`<section class="opsdash-grid r145-grid r145-performance">${panel("เวลาเฉลี่ยแต่ละช่วง","ดูว่าช้าตรงขั้นตอนไหน",stageTimeChart(stages),"r145-stage-time","performance")}${panel("เวลารวมตามช่วงรถเข้า","ดูช่วงเวลาที่มักใช้เวลานาน",cycleTrendChart(data.hours||[],data.comparison?.targetTotalSeconds),"r145-cycle-trend","performance")}${panel("การกระจายเวลาปิดงาน","จำนวนรถแยกตามเวลาที่ใช้",cycleBandChart(data.cycleBands||[]),"r145-cycle-bands","performance")}${panel("ผลลัพธ์แต่ละกะ","จบงาน / รถเข้า และเวลาเฉลี่ย",performanceRows(data.shiftPerformance),"r145-shift-performance","performance")}</section>`}
...
function dashboardCapacity(data){const maxHour=Math.max(1,...(data.hours||[]).map(row=>Number(row.total)||0));return`<section class="opsdash-grid r145-grid r145-capacity">${panel("ภาระงานตามประตู","จำนวนรถและเวลาเฉลี่ย",doorRows(data.doors),"r145-doors","capacity")}${panel("ข้อมูลที่ต้องแก้","รายการที่ข้อมูลยังไม่ครบ",qualityRows(data.dataQuality),"r145-quality","capacity")}${panel("ช่วงรถเข้าหนาแน่น","ใช้วางคนและประตู",hourChart(data.hours,maxHour),"r145-door-hours","capacity")}${panel("ความหนาแน่นประตูและเวลา","สีเข้มคือมีรถมาก",doorHeatmap(data.comparison?.doorHeatmap||[]),"r145-heatmap","capacity")}</section>`}
function dashboardExceptions(data){return`<section class="opsdash-grid r145-grid r145-exceptions">${panel("สัดส่วนการแจ้งเตือน","รถที่ยังอยู่ในพื้นที่",alertDonut(data.alertLevels),"r145-alerts","exceptions")}${panel("คอขวดตอนนี้","ขั้นตอนที่มีงานค้างมากที่สุด",`<div class="command-callout r145-bottleneck"><small>ขั้นตอน</small><b>${escapeHtml(data.bottleneck?.label||"ไม่มีงานค้าง")}</b><span>${Number(data.bottleneck?.queue||0)} คัน · เวลาเฉลี่ย ${formatDuration(data.bottleneck?.avg)}</span></div>`,"r145-bottleneck-panel","exceptions")}${panel("ข้อมูลที่ยังไม่ครบ","ควรแก้ก่อนปิดงาน",qualityRows(data.dataQuality),"r145-quality","exceptions")}${panel("รายการที่ต้องตามทันที",`เรียงจากงานเร่งด่วนก่อน`,actionTable(data.actionItems,5),"r145-list","exceptions")}</section>`}
...
function dashboardPriorityList(rows,limit=5){const shown=(rows||[]).slice(0,Math.max(1,Number(limit)||5));return`<div class="r145-priority-list">${shown.map(row=>`<div style="--alert-color:${safeColor(row.alert_color)}"><i></i><strong><b>${escapeHtml(row.appointment_no||row.auto_id)}</b><small>${escapeHtml(row.company_name||"ไม่ระบุบริษัท")}</small></strong><span>${statusLabel(row.current_status)}</span><em>${formatDuration(row.stage_elapsed_seconds)}</em><mark>${alertLevelLabel(row.alert_level)}</mark></div>`).join("")||`<div class="empty-state">ไม่มีงานเร่งด่วน</div>`}</div>`}
