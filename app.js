...
const FRONTEND_BUILD="2026.08.29-round207.41-readiness-no-double-count";
...
const AUTH_TOKEN_KEY="wvf_token";
function readStoredAuthToken(){
  let token="";
  try{token=String(localStorage.getItem(AUTH_TOKEN_KEY)||"").trim()}catch{}
  if(!token){try{token=String(sessionStorage.getItem(AUTH_TOKEN_KEY)||"").trim()}catch{}}
  if(token){
...
const inboundMobileUiState={section:(()=>{try{return localStorage.getItem("wvf_inbound_mobile_section")==="list"?"list":"scan"}catch{return"scan"}})()};
...
const adminDataTools={tab:"overview",source:"main",inspectorBySource:{},busy:false,openTable:"",openCommand:"",openSchema:"",retention:null,retentionBusy:false,retentionSaving:false,retentionPreview:null,retentionPreviewBusy:false,retentionPreviewError:"",archiveMonth:"",archivePreview:null,archiveBusy:false,archiveHistory:null,archiveHistoryBusy:false,archiveStoreBusy:false,archiveVerify:null,archiveVerifyBusy:false,cleanupMonth:"",cleanupPreview:null,cleanupBusy:false,cleanupScript:null,cleanupVerify:null,cleanupExecuteBusy:false,cleanupHistory:null};
...
const datatableState={meta:null,data:null,busy:false,reloadRequested:false,requestSeq:0,detailBusy:false,refreshMetaRequested:false,activity:[],shiftAutoDate:false,stage:"overview",from:"",to:"",shiftId:"",search:"",sla:"ALL",status:"ALL",appointmentTiming:"ALL",door:"",actor:"",sort:"start_desc",page:1,limit:25,problemOnly:false,rejectedOnly:false,invalidAppointmentOnly:false,searchTimer:0,immersive:false,nativeFullscreen:false,mobileFiltersOpen:false,viewportMode:"",viewportTimer:0,filterBack:[],filterForward:[],filterHistoryReady:false,filterApplyingHistory:false,columns:new Set(JSON.parse(localStorage.getItem("wvf_dt_columns_r204")||localStorage.getItem("wvf_dt_columns_r100")||localStorage.getItem("wvf_dt_columns_r96")||'["company","plate","shift","actor","planned","timing"]'))};
...
async function init() {
  ...
  if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js?v=20260829-r20740",{updateViaCache:"none"}).catch(() => undefined);
...
async function login(event) {
  ...
  try {
    ...
    state.token = result.token; state.user = result.user; state.display=normalizeDisplaySettings(result.display); state.appointment=normalizeAppointmentAccess(result.appointment); persistAuthToken(state.token); openApp();
...
function appointmentDurationSeconds(seconds){const raw=Math.max(0,Math.round(Math.abs(Number(seconds)||0))),h=Math.floor(raw/3600),m=Math.floor((raw%3600)/60),sec=raw%60,parts=[];if(h)parts.push(`${h} ชม.`);if(m)parts.push(`${m} นาที`);if(sec||!parts.length)parts.push(`${sec} วินาที`);return parts.join(" ")}
function appointmentDeltaSeconds(projection){if(Number.isFinite(Number(projection?.deltaSeconds)))return Number(projection.deltaSeconds);if(Number.isFinite(Number(projection?.deltaMinutes)))return Number(projection.deltaMinutes)*60;return null}
function appointmentCompactGateTime(gateDisplay,plannedDisplay){const gate=String(gateDisplay||"").trim(),planned=String(plannedDisplay||"").trim();if(!gate)return"";return gate.length>=19&&planned.length>=19&&gate.slice(0,10)===planned.slice(0,10)?gate.slice(11):gate}
function appointmentTimingFactText(projection,includeGate=true){if(!projection)return"";const delta=appointmentDeltaSeconds(projection);if(delta==null)return"";const grace=Math.max(0,Number(projection.lateGraceMinutes||0))*60,status=String(projection.timingStatus||"").toUpperCase()||(delta<0?"EARLY":delta<=grace?"ON_TIME":"LATE"),parts=[];if(includeGate){const gate=appointmentCompactGateTime(projection.gateInAtDisplay,projection.plannedAtDisplay);if(gate)parts.push(`เข้า ${gate}`)}if(delta<0){parts.push(`มาก่อน ${appointmentDurationSeconds(-delta)}`);parts.push("ตรงเวลา")}else if(delta===0){parts.push("ตรงเวลา")}else if(status==="LATE"||delta>grace){parts.push(`มาสาย ${appointmentDurationSeconds(delta)}`);parts.push(`เกินเกณฑ์ ${appointmentDurationSeconds(Math.max(0,Number(projection.lateBySeconds??(delta-grace))))}`)}else{parts.push(`มาช้า ${appointmentDurationSeconds(delta)}`);parts.push("อยู่ในเกณฑ์ตรงเวลา")}return parts.join(" · ")}
function appointmentDeltaLabel(minutes){const n=Number(minutes);if(!Number.isFinite(n))return"";return n===0?"ตรงเวลา":`${n<0?"มาก่อน":"ช้า"} ${appointmentDurationSeconds(Math.abs(n)*60)}`}
function appointmentProjectionBits(projection,item=null){if(!projection)return[];const bits=[];if(projection.plannedAtDisplay)bits.push(`นัด ${projection.plannedAtDisplay}`);const timingFact=appointmentTimingFactText(projection,true);if(timingFact)bits.push(timingFact);const vendorText=Array.isArray(projection.vendors)?projection.vendors.join(" / "):"",effectiveCompany=String(item?.company_name??item?.companyName??"").trim(),policyMode=String(item?.company_policy_mode??item?.companyPolicyMode??"").toUpperCase();if(vendorText&&(!effectiveCompany||vendorText!==effectiveCompany||policyMode==="BOTH"))bits.push(vendorText);if(policyMode==="BOTH"){const gateName=String(item?.company_name_gate_in??item?.companyNameGateIn??"").trim(),planName=String(item?.company_name_appointment??item?.companyNameAppointment??"").trim();if(gateName&&planName&&gateName!==planName)bits.push(`Gate In: ${gateName}`)}if(Array.isArray(projection.carriers)&&projection.carriers.length)bits.push(`Carrier ${projection.carriers.join(" / ")}`);if(Array.isArray(projection.pos)&&projection.pos.length)bits.push(`PO ${projection.pos.join(", ")}`);for(const extra of Array.isArray(projection.extraFields)?projection.extraFields:[]){const values=Array.isArray(extra?.values)?extra.values.filter(Boolean):[];if(values.length)bits.push(`${extra.label||"ข้อมูลเพิ่มเติม"} ${values.join(" / ")}`)}return bits.filter(Boolean)}
function appointmentOperationalStrip(item,page,compact=false){const projection=appointmentProjectionOf(item);if(!projection)return"";const bits=appointmentProjectionBits(projection,item);if(!bits.length)return"";return`<div class="appointment-operational-strip ${compact?"is-compact":""}" data-appointment-page="${escapeHtml(page)}"><b>แผนนัดหมาย</b><span>${bits.map(escapeHtml).join(" · ")}</span></div>`}
function appointmentInline(item,page){const projection=appointmentProjectionOf(item);if(!projection)return"";const bits=appointmentProjectionBits(projection,item);return bits.length?`<small class="appointment-inline">${bits.slice(0,4).map(escapeHtml).join(" · ")}</small>`:""}
function dashboardAppointmentMiniChart(rows){const items=(rows||[]).filter(r=>Number(r.planned||0)>0||Number(r.actual||0)>0),max=Math.max(1,...items.flatMap(r=>[Number(r.planned||0),Number(r.actual||0)]));if(!items.length)return`<div class="appointment-dashboard-empty">ยังไม่มีแผนหรือรถเข้าในช่วงนี้</div>`;return`<div class="appointment-dashboard-hourly" aria-label="แผนเทียบรถจริงรายชั่วโมง">${items.map(r=>`<div title="${escapeHtml(r.label)} · แผน ${Number(r.planned||0)} · จริง ${Number(r.actual||0)}"><small>${escapeHtml(String(r.label||"").slice(0,5))}</small><i><em style="height:${Math.max(Number(r.planned||0)?5:0,Number(r.planned||0)/max*100)}%"></em><strong style="height:${Math.max(Number(r.actual||0)?5:0,Number(r.actual||0)/max*100)}%"></strong></i></div>`).join("")}</div>`}
function dashboardAppointmentPlanHtml(data){
  const p=data?.appointmentDashboard||data?.appointmentPlan;if(!p?.enabled)return"";
  if(p.ready===false)return`<section class="appointment-dashboard-r204 r2042-appointment-band is-unavailable"><div><b>ข้อมูลนัดหมาย</b><span>${escapeHtml(p.message||"ข้อมูลนัดหมายยังไม่พร้อม")}</span></div></section>`;
  const withNo=Number(p.withAppointment??p.eligible??0),matched=Number(p.matched||0),coverage=Number(p.coveragePercent??(withNo?matched*100/withNo:0)),planned=Number(p.planned||0),actual=Number(p.actual||0),diff=Number(p.difference??(actual-planned)),showTiming=p.showTiming!==false,warnings=[];
  if(Number(p.staleMetricCount||0)>0)warnings.push(`ผลเทียบเวลาไม่ตรงกับข้อมูลรถปัจจุบัน ${Number(p.staleMetricCount||0).toLocaleString("th-TH")} รายการถูกตัดออกจาก KPI`);
  if(p.integrity&&p.integrity.ok===false&&Number(p.staleMetricCount||0)===0)warnings.push("ยอดย่อยไม่สมดุล กรุณาตรวจที่มาของตัวเลขก่อนใช้ตัดสินใจ");
  if(showTiming){if(p.mixedPolicy)warnings.push("ช่วงนี้มีผลเทียบเวลามากกว่า 1 เกณฑ์");else if(p.stalePolicy)warnings.push("ผลย้อนหลังบางรายการใช้เกณฑ์เดิม")}
  const compliant=Number(p.compliant??(Number(p.early||0)+Number(p.onTime||0))),onTimePercent=Number(p.onTimePercent??(matched?compliant*100/matched:0)),late=Number(p.late||0),diffText=diff>0?`+${diff.toLocaleString("th-TH")}`:diff.toLocaleString("th-TH"),referenceLabel=String(p.referenceMode||"PERIOD").toUpperCase()==="FROM"?"เวลาเริ่มนัด":"ช่วงเวลานัด";
  return`<section class="appointment-dashboard-r204 r2042-appointment-band ${warnings.length?"has-audit-warning":""}"><div class="r2042-appointment-label"><small>ข้อมูลนัดหมาย</small><b>แผนเทียบรถที่มาถึง</b><span>นับจาก${escapeHtml(referenceLabel)} · ยอมให้ช้าได้ ${Number(p.lateGraceMinutes||0)} นาที</span></div><div class="r2042-appointment-metrics"><span><small>ตามแผน</small><b>${planned.toLocaleString("th-TH")}</b></span><span><small>เข้าจริง</small><b>${actual.toLocaleString("th-TH")}</b></span><span><small>มีข้อมูลตรงกัน</small><b>${matched.toLocaleString("th-TH")}</b><em>${coverage.toFixed(1)}%</em></span>${showTiming?`<span class="good"><small>มาตามเวลา</small><b>${onTimePercent.toFixed(1)}%</b></span><span class="bad"><small>มาสาย</small><b>${late.toLocaleString("th-TH")}</b></span>`:""}<span class="${diff>0?"warn":diff<0?"info":"good"}"><small>ต่างจากแผน</small><b>${diffText}</b></span></div><button type="button" data-appointment-lineage class="r2042-lineage-button">ตัวเลขมาจากไหน</button>${warnings.length?`<div class="r2042-appointment-warning" title="${escapeHtml(warnings.join(" · "))}">!</div>`:""}</section>`
}
async function showDashboardAppointmentLineage(data){
  const p=data?.appointmentDashboard;if(!p)return;const l=p.lineage||{},i=p.integrity||{},staleCount=Number(p.staleMetricCount||0),audit=p.mixedPolicy?"หลายเกณฑ์ในช่วงที่เลือก":p.stalePolicy?"มีผลย้อนหลังจากเกณฑ์เดิม":"เกณฑ์สอดคล้องกับข้อมูลที่บันทึก",freshness=staleCount?`พบผลเดิมที่ Gate In/เลขนัดหมายไม่ตรงกับรถปัจจุบัน ${staleCount.toLocaleString("th-TH")} รายการ และตัดออกจาก KPI แล้ว`:"ผลที่นำมาคิดตรงกับ Gate In และเลขนัดหมายปัจจุบัน";
  await Swal.fire({title:"ที่มาของตัวเลขนัดหมาย",html:`<div class="appointment-lineage-swal"><div><b>ช่วงข้อมูล</b><span>${escapeHtml(l.scope||"ช่วงเดียวกับ Dashboard")}</span></div><div><b>ตามแผน</b><span>${escapeHtml(l.planned||"-")}</span></div><div><b>เข้าจริง</b><span>${escapeHtml(l.actual||"-")}</span></div><div><b>จับคู่ได้</b><span>${escapeHtml(l.matched||"-")}</span></div><div><b>มาก่อน / ในเกณฑ์ / สาย</b><span>${escapeHtml(l.timing||"-")}</span></div><div><b>อนุโลมมาช้า</b><span>${Number(p.lateGraceMinutes||0)} นาที</span></div><div><b>ตรงเวลารวม</b><span>${escapeHtml(l.compliance||`มาก่อน + ตรงเวลาในเกณฑ์ = ${Number(p.compliant||0).toLocaleString("th-TH")} คัน (${Number(p.onTimePercent||0).toFixed(1)}%)`)}</span></div><div><b>% จับคู่</b><span>${escapeHtml(l.coverage||"-")}</span></div><div><b>ผลต่าง</b><span>${escapeHtml(l.difference||"-")}</span></div><div><b>ตรวจยอดภายใน</b><span>${p.integrity?escapeHtml(i.ok?"ผ่าน · ยอดรายชั่วโมงและยอดสถานะตรงกับยอดรวม":"ต้องตรวจสอบ · มียอดย่อยไม่ตรงกับยอดรวม"):"-"}</span></div><div><b>ความสดของผลเทียบ</b><span>${escapeHtml(freshness)}</span></div><div><b>เกณฑ์ย้อนหลัง</b><span>${escapeHtml(audit)}</span></div><footer>ข้อมูล ณ ${escapeHtml(formatDate(data.generatedAt))} · เวลาอ้างอิง ${escapeHtml(p.referenceMode||"PERIOD")} · ${escapeHtml(l.timeZone||"Asia/Bangkok")} · สูตรรุ่น ${escapeHtml(l.version||"R204.1-1")}</footer></div>`,confirmButtonText:"ปิด",customClass:swalClasses(),buttonsStyling:false,width:580})
}
...
function renderInbound() {
  ...
  const kioskLogout=`<span class="inbound-user-chip" title="ผู้ใช้งาน"><i aria-hidden="true">U</i><b>${escapeHtml(state.user?.name||"ผู้ใช้งาน")}</b></span><button id="kioskLogout" class="quiet-button inbound-logout-button" type="button" aria-label="ออกจากระบบ">↪ <span>ออกจากระบบ</span></button>`;
  ...
  $("startCamera").addEventListener("click",startCamera);$("stopCamera").addEventListener("click",stopCamera);$("fullscreenButton").addEventListener("click",toggleFullscreen);$("kioskLogout")?.addEventListener("click",confirmLogout);
  $("inboundMobileLogout")?.addEventListener("click",confirmLogout);
  document.querySelectorAll("[data-inbound-mobile-section]").forEach(button=>button.addEventListener("click",()=>{const section=button.dataset.inboundMobileSection==="list"?"list":"scan";inboundMobileUiState.section=section;try{localStorage.setItem("wvf_inbound_mobile_section",section)}catch{}const workspace=document.querySelector(".inbound-workspace");if(workspace)workspace.dataset.mobileSection=section;document.querySelectorAll("[data-inbound-mobile-section]").forEach(item=>{const active=item.dataset.inboundMobileSection===section;item.classList.toggle("active",active);item.setAttribute("aria-selected",String(active))});if(section==="list"&&scannerState.active)stopCamera();if(section==="scan")setTimeout(()=>$('autoSearch')?.focus({preventScroll:true}),40)}));
...
function trackingPageUrl(token){return new URL(`./track.html?t=${encodeURIComponent(token)}&v=20260818-r187`,location.href).href}
function showInboundTracking(tracking,vehicle,seconds){
  ...
  body.innerHTML=`<div class="driver-qr-code">${qr}</div><div class="driver-qr-caption"><div class="driver-qr-meta"><span><small>หมายเลขนัดหมาย</small><b>${escapeHtml(appointment)}</b></span><span><small>บริษัท</small><strong>${escapeHtml(company)}</strong></span><span><small>ทะเบียนรถ</small><strong>${escapeHtml(plate)}</strong></span></div></div>`;
...
function renderDatatableLayout(){
  ...
  $("pageContent").innerHTML=`<section class="dt-page dt-clean-page dt-r100-page ${datatableState.mobileFiltersOpen?"dt-mobile-filters-open":""}">
    <section class="dt-r100-top">
      <div class="dt-r100-controls">
        ...
        <div class="dt-r100-actions"><button id="dtCompare" class="dt-compare-open" type="button">เปรียบเทียบ</button><button id="dtLineage" class="quiet-button" type="button">ที่มาของตัวเลข</button><button id="dtRefresh" class="quiet-button" type="button">รีเฟรช</button><button id="dtFullscreen" class="outline-button" type="button">เต็มจอ</button><button id="dtExport" class="primary" type="button">ส่งออก</button></div>
      ...
      <div class="dt-mobile-toolbelt"><button id="dtMobileFilterToggle" class="outline-button dt-mobile-tool" type="button">ตัวกรอง</button><button id="dtMobileCompare" class="dt-compare-open dt-mobile-tool" type="button">เปรียบเทียบ</button><button id="dtMobileLineage" class="quiet-button dt-mobile-tool" type="button">ที่มาตัวเลข</button><button id="dtMobileRefresh" class="quiet-button dt-mobile-tool" type="button">รีเฟรช</button><button id="dtMobileExport" class="primary dt-mobile-tool" type="button">ส่งออก</button></div>
    ...
    <main class="dt-main dt-main-full">
      <section class="dt-filter-shell dt-filter-shell-compact">
        <div class="dt-filterbar dt-filterbar-r159">
          <div class="dt-search"><input id="dtSearch" value="${escapeHtml(datatableState.search)}" placeholder="ค้นหาเลขนัดหมาย / บริษัท / Carrier / PO / ทะเบียน / คนขับ"><span aria-hidden="true">⌕</span></div>
          ...
          ${appointmentData.enabled&&appointmentData.showTiming!==false?`<select id="dtAppointmentTiming"><option value="ALL">ทุกผลเทียบเวลานัด</option><option value="EARLY">มาก่อน (ตรงเวลา)</option><option value="ON_TIME">ตรงเวลาในเกณฑ์</option><option value="LATE">มาสาย</option></select>`:""}
  ...
  $("dtShift").value=datatableState.shiftId;$("dtSla").value=datatableState.sla;$("dtStatus").value=datatableState.status;if($("dtAppointmentTiming"))$("dtAppointmentTiming").value=datatableState.appointmentTiming;$("dtDoor").value=datatableState.door;$("dtActor").value=datatableState.actor;$("dtSort").value=datatableState.sort;
...
function datatableFilterActiveCount(){let count=0;if(datatableState.search)count++;if(datatableState.sla!=="ALL")count++;if(datatableState.status!=="ALL")count++;if(datatableState.appointmentTiming!=="ALL")count++;if(datatableState.shiftId)count++;if(datatableState.door)count++;if(datatableState.actor)count++;if(datatableState.problemOnly)count++;if(datatableState.rejectedOnly)count++;if(datatableState.invalidAppointmentOnly)count++;return count}
...
function bindDatatableEvents(){
  ...
  $("dtAppointmentTiming")?.addEventListener("change",()=>datatableCommitFilterChange(()=>{datatableState.appointmentTiming=$("dtAppointmentTiming").value}));
...
async function showDatatableLineage(){
  const data=datatableState.data;if(!data||!window.Swal)return;const b=data.calculationBasis||{},p=data.performance||{},appointment=datatableState.meta?.appointmentData||{},filters=datatableFilterDescriptions(),range=data.range||{},shift=data.shiftContext?.shiftName||"ทุกกะ",dateScope=range.fromDate&&range.toDate?(range.fromDate===range.toDate?range.fromDate:`${range.fromDate} ถึง ${range.toDate}`):"ช่วงวันที่ที่เลือก",scope=`${dateScope} · ${shift}${filters.length?` · ${filters.join(" · ")}`:" · ไม่มีตัวกรองเพิ่มเติม"}`;
  await Swal.fire({title:"ที่มาของตัวเลข Datatable",html:`<div class="appointment-lineage-swal"><div><b>ขอบเขตที่กำลังนับ</b><span>${escapeHtml(scope)}</span></div><div><b>ทั้งหมด</b><span>${escapeHtml(b.total||"นับหลังใช้ตัวกรองทั้งหมด")}</span></div><div><b>กำลังดำเนินการ</b><span>${escapeHtml(b.active||"-")}</span></div><div><b>เสร็จสิ้น</b><span>${escapeHtml(b.completed||"-")}</span></div><div><b>ระดับแจ้งเตือน</b><span>${escapeHtml(b.warnings||"-")}</span></div>${appointment.enabled?`<div><b>ผลเทียบเวลานัด</b><span>${escapeHtml(b.appointmentTiming||"-")}</span></div><div><b>Vendor / Carrier / PO</b><span>${escapeHtml(b.appointmentDetails||"-")}</span></div>`:""}<div><b>ลำดับการคำนวณ</b><span>ใช้ช่วงวันที่และกะ → ใช้ตัวกรอง → คำนวณจำนวนทั้งหมด → แบ่งหน้าแสดงผล ดังนั้นจำนวนรวมไม่ขึ้นกับหน้าที่กำลังเปิด</span></div><div><b>การอ่านข้อมูลนัดหมาย</b><span>${escapeHtml(p.appointmentSource==="STORED_METRIC"?"ใช้ผลที่บันทึกไว้ของรถปัจจุบัน ไม่จับคู่เวลาซ้ำเมื่อเปิด Datatable":"ไม่ใช้ข้อมูลนัดหมายในคำขอนี้")}</span></div><footer>ข้อมูล ณ ${escapeHtml(formatDate(data.generatedAt))} · อ่านฐานข้อมูล ${Number(p.rowsRead||0).toLocaleString("th-TH")} แถว · ใช้เวลา query ${Number(p.durationMs||0).toFixed(1)} ms · รุ่น R204.1</footer></div>`,confirmButtonText:"ปิด",customClass:swalClasses(),buttonsStyling:false,width:620})
}
...
function datatableMobileCard(row){
  ...
  return `<article class="dtm-record" style="--dtm-alert:${accent}">
    <header class="dtm-record-head"><div><small>เลขนัดหมาย</small><b>${appointment}${appointmentBadge}</b>${datatableAppointmentInline(row)}</div><span class="dtm-alert"><i></i>${escapeHtml(level)}</span></header>
...
function datatableAppointmentProjection(row){const live=appointmentProjectionOf(row)||{},metric=row?.appointmentMetric||null,policy=datatableState.meta?.appointmentData||{};if(!policy.enabled||!metric||policy.showTiming===false)return live;return{...live,plannedAt:metric.plannedAt??live.plannedAt,plannedAtDisplay:metric.plannedAtDisplay||live.plannedAtDisplay,gateInAt:metric.gateInAt??live.gateInAt,gateInAtDisplay:metric.gateInAtDisplay||live.gateInAtDisplay,deltaMinutes:metric.deltaMinutes??live.deltaMinutes,deltaSeconds:metric.deltaSeconds??live.deltaSeconds,lateGraceMinutes:metric.lateGraceMinutes??live.lateGraceMinutes,lateBySeconds:metric.lateBySeconds??live.lateBySeconds,onTimeByPolicy:metric.onTimeByPolicy??live.onTimeByPolicy,timingStatus:metric.timingStatus||live.timingStatus,referenceType:metric.referenceMode||live.referenceType,metricPolicyId:metric.policyId||null,timingSource:"STORED_METRIC"}}
function datatableAppointmentInline(row){const p=datatableAppointmentProjection(row),bits=appointmentProjectionBits(p,row);return bits.length?`<small class="appointment-inline">${bits.slice(0,4).map(escapeHtml).join(" · ")}</small>`:""}
function datatableAppointmentTimingText(row){const p=datatableAppointmentProjection(row);if(appointmentDeltaSeconds(p)==null)return"-";return appointmentTimingFactText(p,false)||({EARLY:"มาก่อน · ตรงเวลา",ON_TIME:"อยู่ในเกณฑ์ตรงเวลา",LATE:"มาสาย"}[String(p.timingStatus||"")]||"-")}
function datatableAppointmentOptionalHeads(){const a=datatableState.meta?.appointmentData||{};if(!a.enabled)return"";return[datatableColumnOn("planned")&&a.showTiming!==false?"<span>เวลานัด</span>":"",datatableColumnOn("timing")&&a.showTiming!==false?"<span>ผลเทียบเวลานัด</span>":"",datatableColumnOn("vendor")&&a.showCompany!==false?"<span>บริษัทจากไฟล์</span>":"",datatableColumnOn("carrier")&&a.showCarrier!==false?"<span>Carrier</span>":"",datatableColumnOn("po")&&a.showPo!==false?"<span>PO</span>":""].join("")}
function datatableAppointmentOptionalCells(row){const a=datatableState.meta?.appointmentData||{},p=datatableAppointmentProjection(row);if(!a.enabled)return"";const vendor=Array.isArray(p.vendors)?p.vendors.join(" / "):"",carrier=Array.isArray(p.carriers)?p.carriers.join(" / "):"",po=Array.isArray(p.pos)?p.pos.join(", "):"";return[datatableColumnOn("planned")&&a.showTiming!==false?`<span class="dt-cell dt-plan-cell" data-label="เวลานัด">${escapeHtml(p.plannedAtDisplay||"-")}</span>`:"",datatableColumnOn("timing")&&a.showTiming!==false?`<span class="dt-cell dt-timing-cell ${escapeHtml(String(p.timingStatus||"").toLowerCase())}" data-label="ผลเทียบเวลานัด">${escapeHtml(datatableAppointmentTimingText(row))}</span>`:"",datatableColumnOn("vendor")&&a.showCompany!==false?`<span class="dt-cell" data-label="บริษัทจากไฟล์">${escapeHtml(vendor||"-")}</span>`:"",datatableColumnOn("carrier")&&a.showCarrier!==false?`<span class="dt-cell" data-label="Carrier">${escapeHtml(carrier||"-")}</span>`:"",datatableColumnOn("po")&&a.showPo!==false?`<span class="dt-cell" data-label="PO">${escapeHtml(po||"-")}</span>`:""].join("")}
function datatableAppointmentOptionalCount(){const a=datatableState.meta?.appointmentData||{};if(!a.enabled)return 0;return Number(datatableColumnOn("planned")&&a.showTiming!==false)+Number(datatableColumnOn("timing")&&a.showTiming!==false)+Number(datatableColumnOn("vendor")&&a.showCompany!==false)+Number(datatableColumnOn("carrier")&&a.showCarrier!==false)+Number(datatableColumnOn("po")&&a.showPo!==false)}
...
async function chooseDatatableColumns(){if(!window.Swal)return;const a=datatableState.meta?.appointmentData||{},options=[["company","บริษัท"],["plate","ทะเบียนรถ"],["shift","กะ"],["actor","ผู้ดำเนินการ"],...(a.enabled&&a.showTiming!==false?[["planned","เวลานัด"],["timing","ผลเทียบเวลานัด"]]:[]),...(a.enabled&&a.showCompany!==false?[["vendor","บริษัทจากไฟล์"]]:[]),...(a.enabled&&a.showCarrier!==false?[["carrier","Carrier"]]:[]),...(a.enabled&&a.showPo!==false?[["po","PO"]]:[])],html=`<div class="dt-column-chooser">${options.map(([key,label])=>`<label><input type="checkbox" data-dt-column="${key}" ${datatableColumnOn(key)?"checked":""}><span>${label}</span></label>`).join("")}</div>`;const result=await Swal.fire({title:"เลือกคอลัมน์ที่ต้องการแสดง",html,showCancelButton:true,confirmButtonText:"บันทึก",cancelButtonText:"ยกเลิก",customClass:swalClasses(),buttonsStyling:false,width:420,preConfirm:()=>[...document.querySelectorAll("[data-dt-column]:checked")].map(el=>el.dataset.dtColumn)});if(!result.isConfirmed)return;datatableState.columns=new Set(result.value||[]);localStorage.setItem("wvf_dt_columns_r204",JSON.stringify([...datatableState.columns]));renderDatatableRows(datatableState.data?.items||[])}
...
function openPublicQueue(){window.open(new URL("./queue.html?v=20260824-r20712",location.href).href,"_blank","noopener")}
...
function clearDashboardStaleRecovery(resetAttempts=true){if(dashboardState.staleRecoveryTimer){clearTimeout(dashboardState.staleRecoveryTimer);dashboardState.staleRecoveryTimer=0}if(resetAttempts)dashboardState.staleRecoveryAttempts=0}
function scheduleDashboardStaleRecovery(cacheState){
  if(state.view!=="dashboard"||!navigator.onLine)return;
  if(!["STALE_REFRESHING","STALE_FALLBACK"].includes(String(cacheState||""))){clearDashboardStaleRecovery(true);return}
  if(dashboardState.staleRecoveryTimer||dashboardState.staleRecoveryAttempts>=3)return;
  const delays=[900,1600,2800],delay=delays[Math.min(dashboardState.staleRecoveryAttempts,delays.length-1)];dashboardState.staleRecoveryAttempts+=1;
  dashboardState.staleRecoveryTimer=setTimeout(()=>{dashboardState.staleRecoveryTimer=0;if(state.view!=="dashboard"||document.hidden||dashboardState.busy)return;void loadDashboard(false,true)},delay);
}
...
function cancelDashboardRequest(){
  ...
  clearDashboardRetry();clearDashboardStaleRecovery(true);
...
async function loadDashboard(showLoading=true,force=false){
  ...
  try{
    ...
    const displayData=mergeDashboardCoreData(data,dashboardState.dataIdentity===requestIdentity?dashboardState.data:null);dashboardState.data=displayData;dashboardState.dataIdentity=requestIdentity;dashboardState.date=data.selectedDate||requestDate;dashboardState.error="";dashboardState.analyticsError="";dashboardState.cacheState=String(data.dashboardCache?.state||"LIVE");dashboardState.lastLoadedAt=Date.now();dashboardState.failures=0;dashboardState.snapshotLoaded=false;clearDashboardRetry();if(["FRESH","LIVE"].includes(dashboardState.cacheState))clearDashboardStaleRecovery(true);else scheduleDashboardStaleRecovery(dashboardState.cacheState);saveDashboardSnapshot(displayData,requestIdentity);renderDashboardData(displayData);void loadDashboardAnalytics(requestRange,requestDate,requestShiftId,requestIdentity)
  }catch(error){
...
function renderDashboardData(data,forceAfterScroll=false){
  ...
  const selectedKey=data.selectedDate||dashboardState.date,isHistory=selectedKey&&selectedKey!==dashboardTodayKey(),cacheState=String(dashboardState.cacheState||data.dashboardCache?.state||"");
  let fresh;if(isHistory)fresh=`<span class="opsdash-history">● ข้อมูลย้อนหลัง</span>`;else if(dashboardState.error||cacheState==="STALE_FALLBACK")fresh=`<span class="opsdash-stale">● ใช้ข้อมูลล่าสุดที่มี · รอลองใหม่</span>`;else if(Boolean(data.__localSnapshot)||["LOCAL_SNAPSHOT","LOCAL_FALLBACK"].includes(cacheState))fresh=`<span class="opsdash-stale">● ใช้ข้อมูลล่าสุดที่มี · กำลังเชื่อมต่อ</span>`;else if(cacheState==="STALE_REFRESHING")fresh=`<span class="opsdash-stale">● กำลังอัปเดตข้อมูล</span>`;else fresh=`<span class="opsdash-live">● อัปเดตใกล้เวลาจริง</span>`;
  const appointmentPlanHtml=dashboardAppointmentPlanHtml(data);$("dashboardBody").classList.toggle("has-appointment-plan",Boolean(appointmentPlanHtml));
  $("dashboardBody").innerHTML=`<section class="r146-command-strip">${dashboardDecisionRail(data)}${dashboardDataHealth(data,fresh)}</section>${appointmentPlanHtml}<section id="dashboardKpis" class="opsdash-kpis r145-kpis r146-kpis r146-generated-kpis">${dashboardKpiSet(data,dashboardState.tab)}</section><div id="dashboardTabContent" class="opsdash-content r145-content r146-content r146-generated-content">${dashboardTabContent(data)}</div><nav class="r146-bottom-tabs" aria-label="เลือกหน้า Dashboard">${dashboardTabs().map(([id,label])=>`<button data-dashboard-tab="${id}" class="${dashboardState.tab===id?"active":""}">${dashboardTabIcon(id)}<span>${label}</span></button>`).join("")}</nav>`;
...
function dashboardDecisionRail(data){
  ...
  return`<article class="r146-command-card ${urgent>0?"danger":"good"}"><i class="r146-command-symbol">!</i><div><small>ต้องเร่งจัดการ</small><b>${urgent} <em>คัน</em></b><span>${urgent?"มีรายการเกินระดับเตือน":"ยังไม่มีงานเกินเกณฑ์"}</span></div></article><article class="r146-command-card ${Number(b.queue||0)>0?"focus":"neutral"}"><i class="r146-command-symbol">▲</i><div><small>จุดที่งานติดวันนี้</small><b>${escapeHtml(b.label||"ไม่มีงานค้าง")}</b><span>${Number(b.queue||0)} คัน${Number(b.avg||0)>0?` · เฉลี่ย ${formatDuration(b.avg)}`:""}</span></div></article><article class="r146-command-card ${flowTone}"><i class="r146-command-symbol">↗</i><div><small>งานค้างเพิ่ม–ลด</small><b>${flowLabel}</b><span>${flowNote}</span></div></article>`
}
function dashboardDataHealth(data,fresh){const guide=data.dataGuide||{},qTotal=Number(guide.missingMainFields??dashboardQualityTotal(data)),generated=Number(data.generatedAt||0),updated=generated?new Intl.DateTimeFormat("th-TH",{timeZone:"Asia/Bangkok",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false}).format(new Date(generated*1000)):"–",quality=data.analyticsReady?(qTotal?`ขาด ${qTotal} จุด`:"ข้อมูลหลักครบ"):"กำลังตรวจ",sample=data.analyticsReady?`${Number(guide.closedTimeSamples||0)} คัน`:"กำลังนับ";return`<article class="r146-data-health"><div class="r146-health-fresh">${fresh}</div><div><small>ข้อมูลที่ขาด</small><b>${quality}</b></div><div><small>เวลาคำนวณจาก</small><b>${sample}</b></div><div><small>อัปเดตล่าสุด</small><b>${updated}</b></div></article>`}
function dashboardKpiSet(data,tab="overview"){const m=data.metrics||{},w=data.workload||{},a=data.activeStages||{},p=data.comparison?.previous||{},levels=data.alertLevels||{},qTotal=dashboardQualityTotal(data),doors=dashboardDoorStats(data),urgent=dashboardUrgentCount(data),rate=Number(m.total||0)?Number(m.closed||0)*100/Number(m.total||0):0,b=data.bottleneck||dashboardClientBottleneck({},a)||{};let specs=[];
  ...
  else if(tab==="exceptions")specs=[["วิกฤต",Number(levels.CRITICAL||0),"ต้องจัดการทันที","#DC2626","!"],["ล่าช้า",Number(levels.URGENT||0),"เกินเกณฑ์เวลา","#F97316","!"],["เตือน",Number(levels.WARNING||0),"ใกล้เกินเกณฑ์","#F59E0B","!"],["ข้อมูลไม่ครบ",qTotal,"ต้องตรวจแก้","#7C3AED","?"],["จุดงานติด",b.label||"ไม่มี",`${Number(b.queue||0)} คัน`,"#0F766E","▰"],["ต้องเร่งรวม",urgent,"เตือนขึ้นไป","#B91C1C","!"]];
  ...
  const timeSamples=Number(data.dataGuide?.closedTimeSamples??m.closed??0);specs=specs.map(spec=>{const label=String(spec[0]||""),sampleNote=timeSamples?`คำนวณจากรถที่ออกแล้ว ${timeSamples} คัน`:"ยังไม่มีรถออกครบขั้นตอน";if(label.includes("P50")||label.includes("P90")){spec[2]=timeSamples?`คำนวณจากรถที่ออกแล้ว ${timeSamples} คัน`:"ข้อมูลยังไม่พอคำนวณ";spec[4]="◷"}else if(label.includes("เวลาเฉลี่ย"))spec[2]=tab==="comparison"?`${spec[2]} · ${sampleNote}`:sampleNote;return spec});
  return specs.map(([label,value,note,color,icon])=>dashboardKpi(label,value,note,color,icon)).join("")}
function dashboardKpi(label,value,note,color,icon=""){return`<article class="opsdash-kpi opsdash-kpi-static r2042-kpi" style="--kpi-color:${color}"><div><small>${label}</small><b>${value??0}</b><span>${note||""}</span></div><i aria-hidden="true">${icon}</i></article>`}
...
function dashboardPageGuide(data,tab=dashboardState.tab){const urgent=dashboardUrgentCount(data),carry=Number(data.workload?.carryOut||0),missing=Number(data.dataGuide?.missingMainFields??dashboardQualityTotal(data)),b=data.bottleneck||dashboardClientBottleneck(data.stageAverages||{},data.activeStages||{})||{},guides={overview:["ดูอะไร","งานที่ต้องเร่งและรถที่ยังอยู่","ทำอะไรต่อ",urgent?`ตามรถเตือน ${urgent} คันก่อน`:"ยังไม่มีรถเกินเวลา","ดูต่อ",carry?`รถยังอยู่ ${carry} คัน · จุดติดมากสุด ${escapeHtml(b.label||"ไม่มี")}`:"ไม่มีรถค้างในช่วงนี้"],handover:["ดูอะไร","งานที่รับต่อและต้องส่งต่อ","ทำอะไรต่อ",carry?`เตรียมส่งต่องาน ${carry} คัน`:"ไม่มีงานค้างส่งต่อ","ดูต่อ","ตรวจรายการรถและขั้นตอนที่ค้าง"],comparison:["ดูอะไร","ช่วงนี้ดีขึ้นหรือแย่ลง","ทำอะไรต่อ","ดูรถเข้า รถออก และเวลาเทียบช่วงก่อน","ดูต่อ","หากเวลาสูงขึ้น ให้เปิดหน้าเวลาทำงาน"],performance:["ดูอะไร","ขั้นตอนไหนใช้เวลานาน","ทำอะไรต่อ",Number(b.queue||0)?`ตรวจจุด ${escapeHtml(b.label||"งานค้าง")}`:"ดูช่วงเวลาที่รถหนาแน่น","ข้อมูลเวลา",`${Number(data.dataGuide?.closedTimeSamples||0)} คันที่ออกครบขั้นตอน`],capacity:["ดูอะไร","ประตูใดมีงานมากหรือช้า","ทำอะไรต่อ","เตรียมประตูและคนตามช่วงรถหนาแน่น","ข้อมูลที่ขาด",missing?`${missing} จุดที่ควรแก้`:"ข้อมูลหลักครบ"],exceptions:["ดูอะไร","รถเกินเวลาและข้อมูลที่ขาด","ทำอะไรต่อ",urgent?`จัดการรถเตือน ${urgent} คัน`:"ยังไม่มีรถเกินเวลา","ข้อมูลที่ขาด",missing?`${missing} จุดที่ควรแก้`:"ข้อมูลหลักครบ"]},g=guides[tab]||guides.overview;return`<section class="r20720-page-guide" aria-label="คำแนะนำการอ่านหน้านี้">${[0,2,4].map(i=>`<div><small>${g[i]}</small><b>${g[i+1]}</b></div>`).join("")}</section>`}
function dashboardTabContent(data){let body;if(["handover","comparison","performance","capacity","exceptions"].includes(dashboardState.tab)&&!data?.analyticsReady)body=dashboardAnalyticsPending();else if(dashboardState.tab==="handover")body=dashboardHandover(data);else if(dashboardState.tab==="comparison")body=dashboardComparison(data);else if(dashboardState.tab==="performance")body=dashboardPerformance(data);else if(dashboardState.tab==="capacity")body=dashboardCapacity(data);else if(dashboardState.tab==="exceptions")body=dashboardExceptions(data);else body=dashboardOverview(data);return`${dashboardPageGuide(data)}<div class="r20720-page-body">${body}</div>`}
function panel(title,note,body,className="",infoKey=""){const kind=String(infoKey||"general").replace(/[^a-z0-9_-]/gi,"");return`<article class="opsdash-panel r2042-panel ${className}" data-panel-kind="${kind}"><header><div class="r2042-panel-title"><i class="r2042-panel-mark" aria-hidden="true"></i><h3>${title}</h3></div>${note?`<div class="opsdash-panel-meta"><span>${note}</span></div>`:""}</header><div class="opsdash-panel-body">${body}</div></article>`}
...
function dashboardOverview(data){const maxHour=Math.max(1,...(data.hours||[]).map(row=>Math.max(Number(row.total)||0,Number(row.gate_out)||0))),urgent=dashboardUrgentCount(data);return`<section class="r146-generated-overview">${panel("รถที่อยู่ในแต่ละขั้นตอน","จำนวนรถที่ยังอยู่ในระบบตอนนี้",dashboardStageFlowBoard(data),"r146-stage-flow-board","stageQueue")}${panel("รถเข้า–ออกแต่ละชั่วโมง","ดูช่วงที่รถเข้ามากและช่วงที่ระบายรถออก",hourlyInOutChart(data.hours||[],maxHour),"r146-hourly-board","hourly")}${panel("รถค้างแยกตามระดับ","ดูว่ามีรถเกินเวลาเท่าไร",alertDonut(data.alertLevels),"r146-alert-board","exceptions")}${panel("รถที่ต้องรีบจัดการ 5 อันดับ","เรียงรถที่ควรจัดการก่อน",actionTable(data.actionItems,5),"r146-priority-table","urgent")}${panel("จุดที่งานติดมากที่สุด","ขั้นตอนที่มีรถรอมากที่สุดตอนนี้",dashboardBottleneckCard(data),"r146-bottleneck-board","exceptions")}${panel("สรุปงานของกะ","ดูงานรับต่อ งานเข้า งานออก และงานส่งต่อ",dashboardShiftHandoverSummary(data),"r146-handover-board","handover")}</section>`}
...
function dashboardExceptions(data){const b=data.bottleneck||dashboardClientBottleneck({},data.activeStages||{})||{};return`<section class="opsdash-grid r145-grid r145-exceptions r146-exceptions">${panel("รถแยกตามระดับเตือน","รถที่ยังอยู่ในพื้นที่",alertDonut(data.alertLevels),"r145-alerts","exceptions")}${panel("จุดที่งานติด","ขั้นตอนที่มีรถรอมากที่สุด",`<div class="command-callout r145-bottleneck"><small>จุดที่ควรเข้าไปดู</small><b>${escapeHtml(b.label||"ไม่มีงานค้าง")}</b><span>${Number(b.queue||0)} คัน${Number(b.avg||0)>0?` · เวลาเฉลี่ย ${formatDuration(b.avg)}`:""}</span></div>`,"r145-bottleneck-panel","exceptions")}${panel("ข้อมูลที่ขาดหรือผิดปกติ","ควรแก้ก่อนใช้ตัดสินใจ",qualityRows(data.dataQuality),"r145-quality","exceptions")}${panel("รถที่ต้องจัดการทันที","เรียงจากเร่งด่วนที่สุด",actionTable(data.actionItems,5),"r145-list","exceptions")}</section>`}
...
function appointmentImportItem(a){return{dc:a.dc,appointment:a.appointment,date:a.date,period:a.period,from:a.from,to:a.to,referenceType:a.referenceType,referenceMinute:a.referenceMinute,vendors:a.vendors,carriers:a.carriers,lines:a.lines||[],extras:a.extras||{},raw:a.raw||{}}}
...
function appointmentCenterAllowedTabs(){
  return[["overview","ภาพรวม"],["plan","แผนและเวลา"],["partner","คู่ค้า / PO"],["files","ไฟล์นำเข้า"]];
}
function appointmentCenterSafeTab(tab){
  const allowed=new Set(appointmentCenterAllowedTabs().map(x=>x[0]));
  return allowed.has(tab)?tab:"overview";
}
function appointmentCenterSetPlanSub(tab){
  appointmentCenterState.planSub=tab==="volume"?"volume":"timing";
  try{localStorage.setItem("wvf_appointment_plan_sub",appointmentCenterState.planSub)}catch{}
  appointmentRenderPlanWorkspace();
}
function appointmentCenterSetPartnerSub(tab){
  appointmentCenterState.partnerSub=["vendor","carrier","po"].includes(tab)?tab:"vendor";
  appointmentPartnerAnalyticsState.type=appointmentCenterState.partnerSub;
  try{localStorage.setItem("wvf_appointment_partner_sub",appointmentCenterState.partnerSub)}catch{}
  appointmentRenderPartnerWorkspace();
}

function appointmentCenterSetTab(tab){
  appointmentCenterState.tab=appointmentCenterSafeTab(tab);
  try{localStorage.setItem("wvf_appointment_center_tab",appointmentCenterState.tab)}catch{}
  appointmentRenderCenterTab();
  document.querySelectorAll("[data-appointment-center-tab]").forEach(btn=>btn.classList.toggle("active",btn.dataset.appointmentCenterTab===appointmentCenterState.tab));
}
async function renderAppointmentUpload(){
  if(!["ADMIN","USER"].includes(state.user?.accessRights))return navigate(state.user?.accessRights==="INBOUND"?"inbound":"operations");
  appointmentStopWorker();appointmentUploadState.result=null;appointmentUploadState.preview=null;appointmentUploadState.config=null;
  $("pageContent").innerHTML=`<div class="loading">กำลังเตรียมศูนย์ข้อมูลนัดหมาย</div>`;
  try{
    const data=await api("/api/appointment/status");appointmentCenterState.status=data;state.appointment=normalizeAppointmentAccess(data.appointmentModule);renderNavigation();
    if(!viewEnabled("appointmentUpload"))return navigate("operations");
    appointmentCenterState.tab=appointmentCenterSafeTab(appointmentCenterState.tab);
    renderAppointmentCenterShell();
  }catch(error){$("pageContent").innerHTML=`<div class="empty-state"><b>เปิดข้อมูลนัดหมายไม่ได้</b><span>${escapeHtml(error.message)}</span><button id="retryAppointmentUpload" class="primary">ลองใหม่</button></div>`;$("retryAppointmentUpload")?.addEventListener("click",renderAppointmentUpload)}
}

function renderAppointmentCenterShell(){
  const data=appointmentCenterState.status||{},latest=data.schema?.latest,module=data.appointmentModule||{},tabs=appointmentCenterAllowedTabs(),snapshot=latest?appointmentDisplaySnapshot(latest.snapshotDate,latest.snapshotTime):"ยังไม่มีข้อมูล";
  $("pageContent").innerHTML=`<section class="appointment-center-head appointment-center-head-r20412"><div class="appointment-center-title"><h2>ข้อมูลนัดหมาย</h2><p>ดูแผน เวลา และข้อมูลคู่ค้า โดยใช้ข้อมูลชุดเดียวกับ Dashboard และ Datatable</p></div><div class="appointment-center-context"><span class="dataset"><small>ชุดล่าสุด</small><b>${escapeHtml(snapshot)}</b></span><span class="state-chip ${module.useImportedData?"is-on":"is-off"}">${module.useImportedData?"ใช้งานจริง":"ยังไม่ใช้กับงานจริง"}</span><span class="state-chip ${module.uploadEnabled?"is-upload":"is-off"}">${module.uploadEnabled?"รับไฟล์ได้":"ปิดรับไฟล์"}</span>${state.user?.accessRights==="ADMIN"?`<button id="appointmentCenterExport" class="appointment-center-settings-button" type="button" title="ส่งออกข้อมูล">ส่งออก</button><button id="appointmentCenterSettings" class="appointment-center-settings-button" type="button" title="ตั้งค่าข้อมูลนัดหมาย">ตั้งค่า</button>`:""}</div></section><nav class="appointment-center-tabs appointment-center-tabs-r20412">${tabs.map(([key,label])=>`<button type="button" data-appointment-center-tab="${key}" class="${appointmentCenterState.tab===key?"active":""}">${label}</button>`).join("")}</nav><section id="appointmentCenterBody" class="appointment-center-body"></section>`;
  ...
  $("appointmentCenterExport")?.addEventListener("click",openProductionExportPage);$("appointmentCenterSettings")?.addEventListener("click",()=>{adminState.tab="appointmentData";try{localStorage.setItem("wvf_admin_tab","appointmentData")}catch{}navigate("admin")});
  appointmentRenderCenterTab();
}

function appointmentOverviewHtml(){
  return`<section class="appointment-overview-r20412"><header><div><b>วันนี้</b><span>สรุปเฉพาะตัวเลขที่ต้องดูบ่อย รายละเอียดแยกอยู่ในเมนูด้านบน</span></div></header><div class="appointment-overview-grid"><article class="appointment-overview-panel timing"><div class="appointment-overview-panel-head"><div><small>เวลาเทียบจริง</small><b>ความตรงต่อเวลา</b></div><button id="appointmentOverviewOpenTiming" type="button">ดูรายละเอียด</button></div><div id="appointmentOverviewTiming" class="appointment-overview-summary-body"><div class="appointment-overview-loading">กำลังอ่านผลเวลา</div></div></article><article class="appointment-overview-panel volume"><div class="appointment-overview-panel-head"><div><small>จำนวนรถ</small><b>แผนเทียบรถเข้าจริง</b></div><button id="appointmentOverviewOpenVolume" type="button">ดูรายละเอียด</button></div><div id="appointmentOverviewVolume" class="appointment-overview-summary-body"><div class="appointment-overview-loading">กำลังอ่านจำนวนรถ</div></div></article></div></section>`
}

function appointmentBindCenterRoleLinks(){
  $("appointmentOverviewOpenTiming")?.addEventListener("click",()=>{appointmentCenterState.planSub="timing";appointmentCenterSetTab("plan")});
  $("appointmentOverviewOpenVolume")?.addEventListener("click",()=>{appointmentCenterState.planSub="volume";appointmentCenterSetTab("plan")});
}

function appointmentOverviewTimingSummaryHtml(data){
  if(!data?.enabled)return`<div class="appointment-overview-empty">ยังไม่ได้เปิดใช้ข้อมูลนัดหมายกับงานจริง</div>`;
  if(!data?.ready)return`<div class="appointment-overview-empty">${escapeHtml(data?.message||"ยังอ่านผลเวลาไม่ได้")}</div>`;
  const s=data.summary||{},policy=data.policy||{},compliant=Number(s.compliant??(Number(s.early||0)+Number(s.onTime||0))),pct=Number(s.onTimePercent??(Number(s.matched||0)?compliant*100/Number(s.matched):0)),grace=Number(policy.lateGraceMinutes??data.lateGraceMinutes??0);
  return`<div class="appointment-overview-kpis"><span class="neutral"><small>มีเลขนัดหมาย</small><b>${Number(s.eligible||0)}</b></span><span class="info"><small>เทียบได้</small><b>${Number(s.matched||0)}</b></span><span class="good"><small>ตรงเวลารวม</small><b>${pct.toFixed(1)}%</b></span><span class="bad"><small>มาสาย</small><b>${Number(s.late||0)}</b></span><span class="warn"><small>ยังไม่เทียบ</small><b>${Number(s.unmatched||0)}</b></span></div><div class="appointment-overview-rule">จุดเริ่ม <b>${escapeHtml(policy.referenceMode||data.referenceMode||"-")}</b> · อนุโลม <b>${grace} นาที</b></div>`
}
function appointmentOverviewVolumeSummaryHtml(data){
  if(!data?.enabled)return`<div class="appointment-overview-empty">ยังไม่ได้เปิดใช้ข้อมูลนัดหมายกับงานจริง</div>`;
  if(!data?.ready)return`<div class="appointment-overview-empty">${escapeHtml(data?.message||"ยังอ่านจำนวนรถไม่ได้")}</div>`;
  const s=data.summary||{},diff=Number(s.difference||0);
  return`<div class="appointment-overview-kpis four"><span class="neutral"><small>ตามแผน</small><b>${Number(s.planned||0)}</b></span><span class="info"><small>เข้าจริง</small><b>${Number(s.actual||0)}</b></span><span class="good"><small>จับคู่แผนได้</small><b>${Number(s.matched||0)}</b></span><span class="${diff>0?"bad":diff<0?"warn":"neutral"}"><small>ต่างจากแผน</small><b>${diff>0?"+":""}${diff}</b></span></div><div class="appointment-overview-rule">จับคู่ได้ <b>${Number(s.matchPercent||0).toFixed(1)}%</b> · เวลาอ้างอิง <b>${escapeHtml(data.referenceMode||"-")}</b></div>`
}
async function appointmentLoadOverview(){
  if(appointmentCenterState.overviewBusy)return;appointmentCenterState.overviewBusy=true;
  try{
    const [timing,volume]=await Promise.allSettled([
      api("/api/appointment/plan-actual?days=1",{timeoutMs:12000}),
      api("/api/appointment/plan-volume?days=1",{timeoutMs:12000})
    ]);
    if($("appointmentOverviewTiming"))$("appointmentOverviewTiming").innerHTML=timing.status==="fulfilled"?appointmentOverviewTimingSummaryHtml(timing.value):`<div class="appointment-overview-empty">โหลดผลเวลาไม่สำเร็จ</div>`;
    if($("appointmentOverviewVolume"))$("appointmentOverviewVolume").innerHTML=volume.status==="fulfilled"?appointmentOverviewVolumeSummaryHtml(volume.value):`<div class="appointment-overview-empty">โหลดจำนวนรถไม่สำเร็จ</div>`;
  }finally{appointmentCenterState.overviewBusy=false}
}
function appointmentPlanWorkspaceHtml(){
  const tab=appointmentCenterState.planSub==="volume"?"volume":"timing";
  return`<section class="appointment-workspace-r20412"><nav class="appointment-center-subtabs"><button type="button" data-appointment-plan-sub="timing" class="${tab==="timing"?"active":""}">เทียบเวลา</button><button type="button" data-appointment-plan-sub="volume" class="${tab==="volume"?"active":""}">เทียบจำนวนรถ</button></nav><div id="appointmentPlanWorkspaceBody"></div></section>`
}
function appointmentRenderPlanWorkspace(){
  const body=$("appointmentCenterBody");if(!body)return;body.innerHTML=appointmentPlanWorkspaceHtml();
  document.querySelectorAll("[data-appointment-plan-sub]").forEach(btn=>btn.addEventListener("click",()=>appointmentCenterSetPlanSub(btn.dataset.appointmentPlanSub)));
  const pane=$("appointmentPlanWorkspaceBody");if(!pane)return;
  if(appointmentCenterState.planSub==="volume"){pane.innerHTML=appointmentPlanVolumeHtml();appointmentBindPlanVolume(true)}
  else{pane.innerHTML=appointmentPlanActualAdminHtml(state.user?.accessRights==="ADMIN");appointmentBindPlanActualCenter(true)}
}
function appointmentPartnerWorkspaceHtml(){
  const tab=["vendor","carrier","po"].includes(appointmentCenterState.partnerSub)?appointmentCenterState.partnerSub:"vendor";
  return`<section class="appointment-workspace-r20412"><nav class="appointment-center-subtabs three"><button type="button" data-appointment-partner-sub="vendor" class="${tab==="vendor"?"active":""}">บริษัทผู้ส่งสินค้า</button><button type="button" data-appointment-partner-sub="carrier" class="${tab==="carrier"?"active":""}">ผู้ขนส่ง</button><button type="button" data-appointment-partner-sub="po" class="${tab==="po"?"active":""}">เลข PO</button></nav><div id="appointmentPartnerWorkspaceBody"></div></section>`
}
function appointmentRenderPartnerWorkspace(){
  const body=$("appointmentCenterBody");if(!body)return;body.innerHTML=appointmentPartnerWorkspaceHtml();
  document.querySelectorAll("[data-appointment-partner-sub]").forEach(btn=>btn.addEventListener("click",()=>appointmentCenterSetPartnerSub(btn.dataset.appointmentPartnerSub)));
  appointmentPartnerAnalyticsState.type=appointmentCenterState.partnerSub;appointmentPartnerAnalyticsState.loaded=false;
  const pane=$("appointmentPartnerWorkspaceBody");if(pane)pane.innerHTML=appointmentPartnerAnalyticsAdminHtml(false);
  appointmentBindPartnerAnalytics();void appointmentLoadPartnerAnalytics();
}
...
function appointmentRenderCenterTab(){
  ...
  if(tab==="overview"){
    body.innerHTML=appointmentOverviewHtml();appointmentBindCenterRoleLinks();void appointmentLoadOverview();return;
  }
  if(tab==="plan"){appointmentRenderPlanWorkspace();return}
  if(tab==="partner"){appointmentRenderPartnerWorkspace();return}
  if(tab==="files"){void appointmentRenderFileImportTab();return}
}
function appointmentBindPlanActualCenter(autoLoad=false){
  $("appointmentPlanActualDays")?.addEventListener("change",event=>{appointmentPlanActualAdminState.days=Number(event.target.value)||7;void appointmentLoadPlanActual()});
  $("appointmentPlanActualReload")?.addEventListener("click",()=>void appointmentLoadPlanActual());
  $("appointmentPlanActualProcess")?.addEventListener("click",appointmentProcessPlanActual);
  $("appointmentPlanActualRecalculate")?.addEventListener("click",appointmentRecalculatePlanActual);
  if(autoLoad)void appointmentLoadPlanActual();
}
...
async function appointmentRenderFileImportTab(){
  const body=$("appointmentCenterBody");if(!body)return;body.innerHTML=`<div class="loading">กำลังอ่านประวัติไฟล์</div>`;
  let history={ready:false,message:"ยังอ่านประวัติไม่ได้"};try{history=await api("/api/appointment/import-history",{timeoutMs:10000})}catch(error){history={ready:false,message:error.message}}
  const historyHtml=appointmentImportHistoryHtml(history);
  if(state.appointment?.uploadEnabled!==true){body.innerHTML=historyHtml+`<div class="appointment-center-empty compact"><b>ปิดการรับไฟล์อยู่</b><span>ประวัติและข้อมูลวิเคราะห์เดิมยังใช้งานได้ตามปกติ</span></div>`;return}
  try{const data=await api("/api/appointment/upload-config");appointmentUploadState.config=data;appointmentRenderFileImportPane(historyHtml)}catch(error){body.innerHTML=historyHtml+`<div class="empty-state"><b>เปิดหน้ารับไฟล์ไม่ได้</b><span>${escapeHtml(error.message)}</span></div>`}
}
function appointmentRenderFileImportPane(historyHtml=""){
  const data=appointmentUploadState.config||{},latest=data.schema?.latest,body=$("appointmentCenterBody");if(!body)return;
  body.innerHTML=historyHtml+`<section class="appointment-upload-head appointment-upload-head-in-center"><div><h3>รับไฟล์นัดหมาย</h3><p>เลือกไฟล์จากส่วนกลาง ระบบไม่ยึดชื่อไฟล์ และจะตรวจหัวคอลัมน์ก่อนนำเข้า</p></div><label class="appointment-file-button">เลือกไฟล์<input id="appointmentFileInput" type="file" accept=".xlsb,.xlsx,application/vnd.ms-excel.sheet.binary.macroEnabled.12,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"></label></section><section class="appointment-latest-card"><div><small>ชุดข้อมูลล่าสุด</small><b id="appointmentLatestSnapshot">${latest?escapeHtml(appointmentDisplaySnapshot(latest.snapshotDate,latest.snapshotTime)):"ยังไม่มีข้อมูล"}</b><span id="appointmentLatestFile">${latest?.fileName?escapeHtml(latest.fileName):"รอนำเข้าไฟล์นัดหมาย"}</span></div><div><small>สถานะการใช้กับงานจริง</small><b>${data.appointmentModule?.useImportedData?"เปิด":"ปิด"}</b><span>${data.appointmentModule?.useImportedData?"ระบบงานเรียกข้อมูลนัดหมายได้":"การนำเข้าไม่เปลี่ยน Gate In"}</span></div></section><div id="appointmentUploadProgress" class="appointment-upload-progress" hidden></div><section id="appointmentMappingNeededCard" class="appointment-mapping-needed" hidden></section><section id="appointmentUploadResult" hidden><div class="appointment-upload-summary"><article><small>ข้อมูลทั้งหมด</small><b id="appointmentSourceRows">0</b></article><article><small>ของคลังนี้</small><b id="appointmentWarehouseRows">0</b></article><article><small>นัดหมาย</small><b id="appointmentTotalCount">0</b></article><article><small>ต้องตรวจ</small><b id="appointmentIssueTotal">0</b></article></div><section class="appointment-file-summary"><div><b id="appointmentSelectedFile">-</b><span id="appointmentFileRange">-</span></div><span id="appointmentFileStatus">พร้อมตรวจสอบ</span></section><section class="appointment-preview-card"><header><div><h3>รายการนัดหมาย</h3><small id="appointmentShownCount"></small></div><input id="appointmentSearchInput" placeholder="ค้นหา Appointment / บริษัท / Carrier / PO"></header><div class="appointment-table-wrap"><table><thead><tr><th>Appointment</th><th>วันที่</th><th>PERIOD</th><th>FROM</th><th>TO</th><th>บริษัท</th><th>Carrier</th><th>PO</th></tr></thead><tbody id="appointmentPreviewBody"></tbody></table></div></section><section id="appointmentIssuePanel" class="appointment-issue-card" hidden><header><b>รายการที่ต้องตรวจ</b><span id="appointmentIssueSummary"></span></header><div id="appointmentIssueRows"></div></section><section id="appointmentImportPanel" class="appointment-import-card"><div><h3>นำเข้าข้อมูล</h3><p>วันที่และเวลาของชุดข้อมูลต้องเป็นรูปแบบ dd/MM/yyyy HH:mm:ss</p></div><label>วันที่เวลาชุดข้อมูล<input id="appointmentSnapshotInput" inputmode="numeric" maxlength="19" placeholder="18/08/2026 18:00:00"></label><div class="appointment-import-actions"><button id="appointmentPreviewChanges" class="outline-button">ตรวจการเปลี่ยนแปลง</button><button id="appointmentConfirmImport" class="primary" disabled>ยืนยันนำเข้า</button></div></section></section>`;
...
function appointmentCoreFieldLabel(key){return({dc:"รหัสคลัง",date:"วันที่นัด",period:"รอบนัดหมาย",from:"เวลาเริ่ม",to:"เวลาสิ้นสุด",po:"เลข PO",appointment:"เลขนัดหมาย",vendor:"บริษัท / Vendor",carrier:"ผู้ขนส่ง"})[key]||key}
function appointmentRenderMappingNeeded(data){const card=$("appointmentMappingNeededCard");if(!card)return;appointmentUploadState.mappingNeeded=data||null;if(!data){card.hidden=true;card.innerHTML="";return}const missing=(data.missingCritical||[]).map(appointmentCoreFieldLabel),sheet=data.sourceSheet||data.recommendedSheet||data.sheets?.[0]?.name||"-",headers=(data.headers||data.sheets?.find(s=>s.name===sheet)?.headers||[]).filter(Boolean).slice(0,24);card.hidden=false;card.innerHTML=`<div><b>รูปแบบไฟล์เปลี่ยนจากค่าที่บันทึกไว้</b><span>${escapeHtml(data.reason||"ต้องจับคู่หัวคอลัมน์ใหม่ก่อนนำเข้า")}</span></div><div class="appointment-mapping-summary"><span><small>ชีทที่พบ</small><b>${escapeHtml(sheet)}</b></span><span><small>ข้อมูลที่ต้องจับคู่</small><b>${escapeHtml(missing.join(" / ")||"ตรวจชื่อหัวคอลัมน์")}</b></span></div>${headers.length?`<p>หัวคอลัมน์ที่พบ: ${headers.map(escapeHtml).join(" · ")}</p>`:""}<div class="appointment-mapping-actions">${state.user?.accessRights==="ADMIN"?`<button id="appointmentOpenFileSettings" class="primary" type="button">เปิดการตั้งค่าไฟล์</button>`:`<span>กรุณาแจ้งผู้ดูแลระบบให้ตั้งค่าหัวคอลัมน์ใหม่</span>`}</div>`;$("appointmentOpenFileSettings")?.addEventListener("click",()=>{adminState.tab="appointmentData";try{localStorage.setItem("wvf_admin_tab","appointmentData")}catch{}navigate("admin")})}

async function appointmentParseFile(file){
  if(!file)return;if(!/\.(xlsb|xlsx)$/i.test(file.name)){await showNotice("warning","รองรับเฉพาะไฟล์ XLSB และ XLSX");return}appointmentClearResult();appointmentRenderMappingNeeded(null);appointmentStopWorker();appointmentSetBusy(true,"กำลังอ่านข้อมูล");
  try{const buffer=await file.arrayBuffer(),worker=new Worker("./appointment-excel-worker.js?v=20260820-r2041");appointmentUploadState.worker=worker;worker.onmessage=async event=>{const m=event.data||{};if(m.type==="PROGRESS")appointmentSetBusy(true,m.message||"กำลังตรวจข้อมูล");if(m.type==="ERROR"){appointmentStopWorker();appointmentSetBusy(false);await showNotice("error",m.message||"อ่านไฟล์ไม่สำเร็จ")}if(m.type==="MAPPING_REQUIRED"){appointmentStopWorker();appointmentSetBusy(false);appointmentRenderMappingNeeded(m.result||{});window.scrollTo({top:0,behavior:"smooth"})}if(m.type==="RESULT"){appointmentStopWorker();appointmentSetBusy(false);appointmentUploadState.result=m.result;appointmentUploadState.preview=null;appointmentRenderMappingNeeded(null);appointmentRenderResult()}};worker.onerror=async event=>{appointmentStopWorker();appointmentSetBusy(false);await showNotice("error",event.message||"อ่านไฟล์ไม่สำเร็จ")};worker.postMessage({type:"PARSE",buffer,fileName:file.name,config:appointmentUploadState.config?.parserConfig||{}},[buffer])}catch(error){appointmentSetBusy(false);await showNotice("error",error.message||"เปิดไฟล์ไม่สำเร็จ")}
}
...
const ADMIN_SETTING_CATEGORIES=[
  ...
  {id:"appointmentData",label:"ข้อมูลนัดหมาย",group:"ข้อมูลและรายงาน",keywords:"appointment excel upload นำเข้า แผน นัดหมาย carrier po period from dc code 906 906xxx starts_with ขึ้นต้นด้วย รหัสคลัง raw_data mapping คอลัมน์"},
...
function appointmentAdminList(value){return [...new Set(String(value||"").split(/[\n,;|]+/).map(v=>v.trim()).filter(Boolean))]}
function appointmentWarehousePreviewLabel(mode,values){const list=appointmentAdminList(values),exact=String(mode||"STARTS_WITH").toUpperCase()==="EXACT";if(!list.length)return"ยังไม่ได้กำหนดรหัสคลัง";if(exact)return`รับเฉพาะ DC CODE ที่ตรงกับ ${list.join(", ")}`;const sample=list[0]?`${list[0]}xxx`:"";return`รับ DC CODE ที่ขึ้นต้นด้วย ${list.join(", ")}${sample?` · ตัวอย่าง ${sample}`:""}`}

function appointmentExtraKey(){return`extra_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,7)}`}
function appointmentExtraFieldRowHtml(field={},index=0){const key=String(field.key||appointmentExtraKey()),pages=new Set(Array.isArray(field.displayPages)?field.displayPages:[]),pageDefs=[["inbound","Inbound"],["receiving","Receiving"],["datatable","Datatable"],["dashboard","Dashboard"],["queue","จอคิว"],["track","Track"]];return`<article class="appointment-extra-field-row" data-extra-key="${escapeHtml(key)}"><div class="appointment-extra-main"><label><span>ชื่อที่แสดง</span><input class="appointment-extra-label" value="${escapeHtml(field.label||"")}" placeholder="เช่น จำนวนพาเลท"></label><label><span>หัวคอลัมน์</span><input class="appointment-extra-header" value="${escapeHtml(field.header||"")}" placeholder="เช่น PALLET"></label><label><span>ชื่อหัวสำรอง</span><input class="appointment-extra-aliases" value="${escapeHtml((field.aliases||[]).join(", "))}" placeholder="คั่นด้วย ,"></label><button class="appointment-extra-remove" type="button" title="นำข้อมูลนี้ออก">นำออก</button></div><div class="appointment-extra-options"><div><b>แสดงที่หน้า</b>${pageDefs.map(([page,label])=>`<label class="clean-check"><input type="checkbox" data-extra-page="${page}" ${pages.has(page)?"checked":""}><span>${label}</span></label>`).join("")}</div><div><b>นำไปใช้ต่อ</b><label class="clean-check"><input type="checkbox" data-extra-flag="export" ${field.exportEnabled?"checked":""}><span>ส่งออกไฟล์</span></label><label class="clean-check"><input type="checkbox" data-extra-flag="filter" ${field.filterEnabled?"checked":""}><span>ใช้ค้นหา/กรอง</span></label><label class="clean-check"><input type="checkbox" data-extra-flag="analysis" ${field.analysisEnabled?"checked":""}><span>ใช้วิเคราะห์</span></label></div></div></article>`}
function appointmentAddExtraField(field={}){const list=$("appointmentExtraFieldsList");if(!list)return;list.querySelector(".appointment-extra-empty")?.remove();if(list.querySelectorAll(".appointment-extra-field-row").length>=12){void showNotice("warning","เพิ่มข้อมูลจากไฟล์ได้สูงสุด 12 รายการ");return}const wrap=document.createElement("div");wrap.innerHTML=appointmentExtraFieldRowHtml(field,list.children.length);const row=wrap.firstElementChild;list.appendChild(row);appointmentBindExtraFieldRow(row)}
function appointmentBindExtraFieldRow(row){row?.querySelector(".appointment-extra-remove")?.addEventListener("click",()=>row.remove())}
function appointmentCollectExtraFields(){return[...document.querySelectorAll("#appointmentExtraFieldsList .appointment-extra-field-row")].map((row,index)=>{const key=String(row.dataset.extraKey||appointmentExtraKey()),label=String(row.querySelector(".appointment-extra-label")?.value||"").trim(),header=String(row.querySelector(".appointment-extra-header")?.value||"").trim(),aliases=appointmentAdminList(row.querySelector(".appointment-extra-aliases")?.value),displayPages=[...row.querySelectorAll("[data-extra-page]:checked")].map(x=>x.dataset.extraPage);return{key,label:label||header||`ข้อมูลเพิ่มเติม ${index+1}`,header,aliases,enabled:true,displayPages,exportEnabled:row.querySelector('[data-extra-flag="export"]')?.checked===true,filterEnabled:row.querySelector('[data-extra-flag="filter"]')?.checked===true,analysisEnabled:row.querySelector('[data-extra-flag="analysis"]')?.checked===true}}).filter(x=>x.header||x.aliases.length)}
function appointmentCurrentParserConfig(fieldRows){const fields={};for(const[key,,def]of fieldRows)fields[key]={header:String($("appointmentField_"+key)?.value||def).trim()||def,aliases:appointmentAdminList($("appointmentAlias_"+key)?.value)};return{sheetName:String($("appointmentSheetName")?.value||"raw_data").trim()||"raw_data",sheetAliases:appointmentAdminList($("appointmentSheetAliases")?.value),warehouse:{matchMode:$("appointmentWarehouseMode")?.value||"STARTS_WITH",values:appointmentAdminList($("appointmentWarehouseValues")?.value)},timeReference:$("appointmentTimeReference")?.value||"PERIOD",date:{dateSystem:"AUTO",acceptedTextFormats:["DD.MM.YYYY","DD/MM/YYYY","YYYY-MM-DD"]},fields,extraFields:appointmentCollectExtraFields()}}
function appointmentProbeBestSheet(){const result=appointmentAdminFileProbe.result||{},selected=$("appointmentProbeSheet")?.value;return(result.sheets||[]).find(x=>x.name===selected)||(result.sheets||[])[0]||null}
function appointmentHeaderOptions(headers,currentCandidates=[]){const normalized=new Set((currentCandidates||[]).map(v=>String(v).trim().toUpperCase()).filter(Boolean)),current=(headers||[]).find(h=>normalized.has(String(h).trim().toUpperCase()))||"";return`<option value="">— เลือก —</option>${(headers||[]).filter(Boolean).map(h=>`<option value="${escapeHtml(h)}" ${h===current?"selected":""}>${escapeHtml(h)}</option>`).join("")}`}
function appointmentRenderAdminProbe(fieldRows){const panel=$("appointmentFileProbePanel"),result=appointmentAdminFileProbe.result;if(!panel)return;if(!result){panel.hidden=true;panel.innerHTML="";return}const sheets=result.sheets||[];panel.hidden=false;panel.innerHTML=`<header><div><b>ตรวจไฟล์ตัวอย่าง</b><small>${escapeHtml(appointmentAdminFileProbe.fileName||"")} · เลือกว่าหัวคอลัมน์ใดหมายถึงข้อมูลอะไร แล้วนำค่าไปใส่ในแบบฟอร์ม</small></div><label>ชีท<select id="appointmentProbeSheet">${sheets.map(x=>`<option value="${escapeHtml(x.name)}" ${x.name===result.recommendedSheet?"selected":""}>${escapeHtml(x.name)} · แถวหัว ${Number(x.headerRow||1)}</option>`).join("")}</select></label></header><div id="appointmentProbeMappings"></div><div class="appointment-probe-actions"><button id="appointmentProbeAddUnused" class="outline-button" type="button">เพิ่มข้อมูลจากหัวที่พบ</button><button id="appointmentProbeApply" class="primary" type="button">ใช้ค่าที่เลือก</button></div>`;const render=()=>{const sheet=appointmentProbeBestSheet(),headers=sheet?.headers||[],profile=appointmentCurrentParserConfig(fieldRows),core=fieldRows.map(([key,label,def])=>{const f=profile.fields[key]||{header:def,aliases:[]};return`<label><span>${label}</span><select data-probe-core="${key}">${appointmentHeaderOptions(headers,[f.header,...(f.aliases||[])])}</select></label>`}).join(""),extras=appointmentCollectExtraFields().map(f=>`<label><span>${escapeHtml(f.label||f.header)}</span><select data-probe-extra="${escapeHtml(f.key)}">${appointmentHeaderOptions(headers,[f.header,...(f.aliases||[])])}</select></label>`).join("");$("appointmentProbeMappings").innerHTML=`<div class="appointment-probe-grid">${core}${extras}</div><p>ถ้าชื่อหัวคอลัมน์เปลี่ยน ให้เลือกชื่อใหม่ ระบบจะเก็บชื่อเดิมเป็นชื่อสำรองให้อัตโนมัติ</p>`};$("appointmentProbeSheet")?.addEventListener("change",render);render();$("appointmentProbeApply")?.addEventListener("click",()=>appointmentApplyAdminProbe(fieldRows));$("appointmentProbeAddUnused")?.addEventListener("click",()=>appointmentAddUnusedProbeHeader(fieldRows))}
function appointmentMergeAliasInput(input,values){const all=appointmentAdminList([input?.value||"",...(values||[])].join(","));if(input)input.value=all.join(", ")}
function appointmentApplyAdminProbe(fieldRows){const sheet=appointmentProbeBestSheet();if(!sheet)return;const oldSheet=String($("appointmentSheetName")?.value||"").trim(),sheetInput=$("appointmentSheetName");if(sheetInput)sheetInput.value=sheet.name;if(oldSheet&&oldSheet.toUpperCase()!==String(sheet.name).toUpperCase())appointmentMergeAliasInput($("appointmentSheetAliases"),[oldSheet]);for(const[key,,def]of fieldRows){const selected=document.querySelector(`[data-probe-core="${key}"]`)?.value;if(!selected)continue;const header=$("appointmentField_"+key),alias=$("appointmentAlias_"+key),old=String(header?.value||def).trim();if(header)header.value=selected;if(old&&old.toUpperCase()!==selected.toUpperCase())appointmentMergeAliasInput(alias,[old])}for(const row of document.querySelectorAll("#appointmentExtraFieldsList .appointment-extra-field-row")){const key=row.dataset.extraKey,selected=document.querySelector(`[data-probe-extra="${CSS.escape(key)}"]`)?.value;if(!selected)continue;const header=row.querySelector(".appointment-extra-header"),alias=row.querySelector(".appointment-extra-aliases"),old=String(header?.value||"").trim();header.value=selected;if(old&&old.toUpperCase()!==selected.toUpperCase())appointmentMergeAliasInput(alias,[old])}void showNotice("success","ใส่ค่าจากไฟล์ให้แล้ว กรุณาตรวจและกดบันทึกการตั้งค่า")}
async function appointmentAddUnusedProbeHeader(fieldRows){const sheet=appointmentProbeBestSheet();if(!sheet)return;const used=new Set([...document.querySelectorAll("[data-probe-core],[data-probe-extra]")].map(x=>String(x.value||"").trim().toUpperCase()).filter(Boolean)),headers=(sheet.headers||[]).filter(h=>h&&!used.has(String(h).toUpperCase()));if(!headers.length){await showNotice("info","ไม่มีหัวคอลัมน์ที่ยังไม่ได้ใช้");return}const options={};headers.forEach(h=>options[h]=h);const result=await Swal.fire({title:"เพิ่มข้อมูลจากไฟล์",input:"select",inputOptions:options,inputPlaceholder:"เลือกหัวคอลัมน์",showCancelButton:true,confirmButtonText:"เพิ่ม",cancelButtonText:"ยกเลิก",customClass:swalClasses(),buttonsStyling:false,width:420});if(result.isConfirmed&&result.value){appointmentAddExtraField({key:appointmentExtraKey(),label:result.value,header:result.value,aliases:[],displayPages:[],exportEnabled:true});appointmentRenderAdminProbe(fieldRows)}}
async function appointmentAdminDiscoverFile(file,fieldRows){if(!file)return;if(!/\.(xlsb|xlsx)$/i.test(file.name)){await showNotice("warning","รองรับเฉพาะไฟล์ XLSB และ XLSX");return}try{appointmentAdminFileProbe.worker?.terminate()}catch{}appointmentAdminFileProbe.busy=true;const button=$("appointmentProbeFileButton");if(button)button.classList.add("is-busy");try{const buffer=await file.arrayBuffer(),worker=new Worker("./appointment-excel-worker.js?v=20260820-r2041");appointmentAdminFileProbe.worker=worker;worker.onmessage=async event=>{const m=event.data||{};if(m.type==="ERROR"){appointmentAdminFileProbe.busy=false;worker.terminate();if(button)button.classList.remove("is-busy");await showNotice("error",m.message||"อ่านไฟล์ไม่สำเร็จ")}if(m.type==="DISCOVERY"){appointmentAdminFileProbe.busy=false;worker.terminate();appointmentAdminFileProbe.worker=null;appointmentAdminFileProbe.result=m.result;appointmentAdminFileProbe.fileName=file.name;if(button)button.classList.remove("is-busy");appointmentRenderAdminProbe(fieldRows)}};worker.onerror=async event=>{appointmentAdminFileProbe.busy=false;try{worker.terminate()}catch{}if(button)button.classList.remove("is-busy");await showNotice("error",event.message||"อ่านไฟล์ไม่สำเร็จ")};worker.postMessage({type:"DISCOVER",buffer,fileName:file.name,config:appointmentCurrentParserConfig(fieldRows)},[buffer])}catch(error){appointmentAdminFileProbe.busy=false;if(button)button.classList.remove("is-busy");await showNotice("error",error.message||"เปิดไฟล์ไม่สำเร็จ")}}


const appointmentPlanActualAdminState={days:7,busy:false};

function appointmentPlanActualAdminHtml(showProcess=true){
  return`<section class="appointment-plan-actual-card appointment-plan-actual-card-r20412"><header><div><b>เทียบเวลานัดกับ Gate In</b><small>มาก่อน = ตรงเวลา · มาช้าภายในเกณฑ์ = ตรงเวลา · เกินเกณฑ์ = มาสาย</small></div><div class="appointment-plan-actual-actions"><select id="appointmentPlanActualDays" aria-label="ช่วงที่ดู"><option value="1">วันนี้</option><option value="7">7 วัน</option><option value="30">30 วัน</option><option value="90">90 วัน</option></select><button id="appointmentPlanActualReload" class="appointment-refresh-button" type="button">รีเฟรช</button>${showProcess?`<details class="appointment-admin-menu"><summary>จัดการ</summary><div><button id="appointmentPlanActualProcess" type="button">เติมผลรถล่าสุด</button><button id="appointmentPlanActualRecalculate" class="danger" type="button">คำนวณใหม่ตามเกณฑ์</button></div></details>`:""}</div></header><div id="appointmentPlanActualBody" class="appointment-plan-actual-body"><div class="appointment-plan-actual-loading">กำลังอ่านผลเทียบเวลาที่บันทึกไว้</div></div></section>`
}
function appointmentPlanActualDeltaText(minutes){const n=Number(minutes);if(!Number.isFinite(n)||n===0)return n===0?"ตรงเวลา":"–";const abs=Math.abs(Math.round(n)),h=Math.floor(abs/60),m=abs%60,text=`${h?`${h} ชม. `:""}${m||!h?`${m} นาที`:""}`.trim();return`${n<0?"มาก่อน":"ช้า"} ${text}`}
function appointmentPlanActualBodyHtml(data){
  if(!data?.enabled)return`<div class="appointment-plan-actual-empty"><b>ยังไม่ได้เปิดใช้ข้อมูลนัดหมายกับงานจริง</b><span>เมื่อเปิดใช้งาน ระบบจะบันทึกผลเทียบเวลาของรถใหม่ให้อัตโนมัติ</span></div>`;
  if(!data?.ready)return`<div class="appointment-plan-actual-empty"><b>ยังอ่านผลเทียบเวลาไม่ได้</b><span>${escapeHtml(data?.message||"กรุณาตรวจฐานข้อมูลนัดหมาย")}</span></div>`;
  const s=data.summary||{},policy=data.policy||{},recent=Array.isArray(data.recent)?data.recent:[],tone=s.coveragePercent>=90?"good":s.coveragePercent>=70?"warn":"bad",mixed=s.mixedPolicy?`<div class="appointment-plan-actual-warning">ช่วงที่เลือกมีผลจากเกณฑ์เวลาเดิมและปัจจุบันร่วมกัน ระบบเก็บประวัติเดิมไว้และไม่เขียนทับอัตโนมัติ</div>`:"",compliant=Number(s.compliant??(Number(s.early||0)+Number(s.onTime||0))),onTimePercent=Number(s.onTimePercent??(Number(s.matched||0)?compliant*100/Number(s.matched):0)),grace=Number(policy.lateGraceMinutes??data.lateGraceMinutes??0);
  const rows=recent.map(row=>{const p={...row,timingStatus:row.status,lateGraceMinutes:row.lateGraceMinutes??grace};return`<tr><td><b>${escapeHtml(row.appointmentNo||"-")}</b><small>${escapeHtml(row.companyName||"")}</small></td><td>${escapeHtml(row.plannedAtDisplay||"-")}</td><td>${escapeHtml(row.gateInAtDisplay||"-")}</td><td><span class="appointment-plan-status ${String(row.status||"").toLowerCase()}">${escapeHtml(row.statusLabel||"-")}</span></td><td><b>${escapeHtml(appointmentTimingFactText(p,false)||"-")}</b></td></tr>`}).join("");
  return`${mixed}<div class="appointment-plan-actual-summary appointment-plan-actual-summary-r20412"><span><small>รถมีเลขนัดหมาย</small><b>${Number(s.eligible||0)}</b></span><span class="${tone}"><small>จับคู่ได้</small><b>${Number(s.coveragePercent||0).toFixed(1)}%</b></span><span class="good"><small>ตรงเวลารวม</small><b>${onTimePercent.toFixed(1)}%</b></span><span class="bad"><small>มาสาย</small><b>${Number(s.late||0)}</b></span><span class="warn"><small>ยังไม่มีผล</small><b>${Number(s.unmatched||0)}</b></span></div><div class="appointment-plan-actual-meta"><span>เทียบได้ <b>${Number(s.matched||0)}</b> คัน</span><span>มาก่อน <b>${Number(s.early||0)}</b></span><span>มาช้าภายในเกณฑ์ <b>${Number(s.onTime||0)}</b></span><span>จุดเริ่ม <b>${escapeHtml(policy.referenceMode||data.referenceMode||"-")}</b></span><span>อนุโลม <b>${grace} นาที</b></span><span>มาก่อนเฉลี่ย <b>${Number(s.avgEarlyMinutes||0)?appointmentPlanActualDeltaText(-Math.abs(Number(s.avgEarlyMinutes))):"–"}</b></span><span>สายเฉลี่ย <b>${Number(s.avgLateMinutes||0)?appointmentPlanActualDeltaText(Math.abs(Number(s.avgLateMinutes))):"–"}</b></span></div>${rows?`<div class="appointment-plan-actual-table-wrap"><table class="appointment-plan-actual-table"><thead><tr><th>เลขนัดหมาย / บริษัท</th><th>เวลานัด</th><th>Gate In จริง</th><th>สถานะ KPI</th><th>ข้อเท็จจริงเทียบเวลานัด</th></tr></thead><tbody>${rows}</tbody></table></div>`:`<div class="appointment-plan-actual-empty compact"><b>ยังไม่มีผลเทียบในช่วงนี้</b><span>ใช้เมนู “จัดการ” หากต้องเติมผลรถล่าสุด</span></div>`}`
}
async function appointmentLoadPlanActual(){
  const body=$("appointmentPlanActualBody"),select=$("appointmentPlanActualDays");if(!body)return;
  if(select){select.value=String(appointmentPlanActualAdminState.days);appointmentPlanActualAdminState.days=Number(select.value)||7}
  body.innerHTML=`<div class="appointment-plan-actual-loading">กำลังอ่านผลเทียบเวลาที่บันทึกไว้</div>`;
  try{const data=await api(`/api/appointment/plan-actual?days=${encodeURIComponent(appointmentPlanActualAdminState.days)}`,{timeoutMs:12000});if($("appointmentPlanActualBody"))$("appointmentPlanActualBody").innerHTML=appointmentPlanActualBodyHtml(data)}
  catch(error){if($("appointmentPlanActualBody"))$("appointmentPlanActualBody").innerHTML=`<div class="appointment-plan-actual-empty"><b>โหลดผลเทียบเวลาไม่สำเร็จ</b><span>${escapeHtml(error.message||"กรุณาลองใหม่")}</span></div>`}
}
async function appointmentProcessPlanActual(){
  if(appointmentPlanActualAdminState.busy)return;appointmentPlanActualAdminState.busy=true;const button=$("appointmentPlanActualProcess");if(button)button.disabled=true;
  try{
    Swal.fire({title:"กำลังประมวลผลรถล่าสุด",text:"อ่านเฉพาะข้อมูลนัดหมายและ Gate In โดยไม่แก้ข้อมูลรถ",allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>Swal.showLoading(),customClass:swalClasses(),width:410});
    const data=await api("/api/admin/appointment-backfill",{method:"POST",body:{limit:2000,recalculate:false},timeoutMs:30000,retries:0}),r=data?.result||{};
    try{Swal.close()}catch{}
    if(!r.ready){await showNotice("warning",r.message||"ยังประมวลผลไม่ได้")}else await Swal.fire({icon:"success",title:"ประมวลผลเสร็จแล้ว",html:`<div class="appointment-plan-process-result"><span><small>ตรวจ</small><b>${Number(r.scanned||0)}</b></span><span><small>เทียบได้</small><b>${Number(r.matched||0)}</b></span><span><small>เพิ่มใหม่</small><b>${Number(r.inserted||0)}</b></span><span><small>ข้อมูลเดิม</small><b>${Number(r.unchanged||0)}</b></span></div>`,confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:430});
    await appointmentLoadPlanActual();
  }catch(error){try{Swal.close()}catch{}await showNotice("error",error.message||"ประมวลผลไม่สำเร็จ")}
  finally{appointmentPlanActualAdminState.busy=false;if(button)button.disabled=false}
}


async function appointmentRecalculatePlanActual(){
  if(appointmentPlanActualAdminState.busy)return;
  const confirm=await Swal.fire({icon:"warning",title:"คำนวณผลย้อนหลังใหม่?",html:`<div class="appointment-recalc-confirm"><p>ระบบจะใช้ <b>จุดเริ่มคำนวณและเกณฑ์อนุโลมปัจจุบัน</b> คำนวณผลเทียบเวลาของรถล่าสุดใหม่</p><p>ไม่แก้ Gate In ไม่แก้ข้อมูลรถ และไม่แก้ไฟล์นัดหมาย แต่ผล KPI ที่บันทึกไว้ของรถที่เข้าเงื่อนไขอาจเปลี่ยนตามเกณฑ์ใหม่</p></div>`,showCancelButton:true,confirmButtonText:"ยืนยันคำนวณใหม่",cancelButtonText:"ยกเลิก",customClass:swalClasses(),buttonsStyling:false,width:500});
  if(!confirm.isConfirmed)return;appointmentPlanActualAdminState.busy=true;const buttons=[$("appointmentPlanActualProcess"),$("appointmentPlanActualRecalculate")].filter(Boolean);buttons.forEach(b=>b.disabled=true);
  try{Swal.fire({title:"กำลังคำนวณใหม่",text:"ใช้เกณฑ์ปัจจุบัน โดยไม่แก้ Gate In",allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>Swal.showLoading(),customClass:swalClasses(),width:410});const data=await api("/api/admin/appointment-backfill",{method:"POST",body:{limit:2000,recalculate:true},timeoutMs:30000,retries:0}),r=data?.result||{};try{Swal.close()}catch{}if(!r.ready){await showNotice("warning",r.message||"ยังคำนวณใหม่ไม่ได้")}else await Swal.fire({icon:"success",title:"คำนวณด้วยเกณฑ์ปัจจุบันแล้ว",html:`<div class="appointment-plan-process-result"><span><small>ตรวจ</small><b>${Number(r.scanned||0)}</b></span><span><small>เทียบได้</small><b>${Number(r.matched||0)}</b></span><span><small>ปรับผล</small><b>${Number(r.updated||0)}</b></span><span><small>เดิม</small><b>${Number(r.unchanged||0)}</b></span></div>`,confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:430});await appointmentLoadPlanActual()}catch(error){try{Swal.close()}catch{}await showNotice("error",error.message||"คำนวณใหม่ไม่สำเร็จ")}finally{appointmentPlanActualAdminState.busy=false;buttons.forEach(b=>b.disabled=false)}
}


function appointmentPlanVolumeHtml(){
  return`<section class="appointment-plan-volume-card appointment-plan-volume-card-r20412"><header><div><b>แผนเทียบจำนวนรถจริง</b><small>ดูช่วงเวลาที่รถเข้ามากหรือน้อยกว่าแผน เพื่อช่วยจัดคนและประตู</small></div><div class="appointment-plan-volume-actions"><select id="appointmentPlanVolumeDays" aria-label="ช่วงที่ดู"><option value="1">วันนี้</option><option value="7">7 วัน</option><option value="30">30 วัน</option></select><button id="appointmentPlanVolumeReload" class="appointment-refresh-button" type="button">รีเฟรช</button></div></header><div id="appointmentPlanVolumeBody" class="appointment-plan-volume-body"><div class="appointment-plan-volume-loading">กำลังอ่านแผนและจำนวนรถจริง</div></div></section>`
}
function appointmentPlanVolumeDate(day){const m=String(day||"").match(/^(\d{4})-(\d{2})-(\d{2})$/);return m?`${m[3]}/${m[2]}/${m[1]}`:String(day||"-")}
function appointmentPlanVolumeBodyHtml(data){
  if(!data?.enabled)return`<div class="appointment-plan-volume-empty"><b>ยังไม่ได้เปิดใช้ข้อมูลนัดหมายกับงานจริง</b><span>เปิดใช้งานจากตั้งค่าระบบเมื่อพร้อม</span></div>`;
  if(!data?.ready)return`<div class="appointment-plan-volume-empty"><b>ยังอ่านแผนเทียบจำนวนไม่ได้</b><span>${escapeHtml(data?.message||"กรุณาลองใหม่")}</span></div>`;
  const s=data.summary||{},rows=(data.hours||[]).filter(r=>Number(r.planned||0)>0||Number(r.actual||0)>0),max=Math.max(1,...rows.flatMap(r=>[Number(r.planned||0),Number(r.actual||0)])),hourRows=rows.map(r=>`<div class="appointment-volume-hour"><b>${escapeHtml(r.label)}</b><div class="appointment-volume-bars"><span><i style="width:${Math.max(Number(r.planned||0)?5:0,Number(r.planned||0)/max*100)}%"></i></span><span><i style="width:${Math.max(Number(r.actual||0)?5:0,Number(r.actual||0)/max*100)}%"></i></span></div><div><em>แผน ${Number(r.planned||0)}</em><strong>จริง ${Number(r.actual||0)}</strong><small class="${Number(r.difference||0)>0?"over":Number(r.difference||0)<0?"under":""}">${Number(r.difference||0)>0?"+":""}${Number(r.difference||0)} คัน</small></div></div>`).join(""),daily=(data.daily||[]).map(r=>`<tr><td>${appointmentPlanVolumeDate(r.day)}</td><td>${Number(r.planned||0)}</td><td>${Number(r.actual||0)}</td><td>${Number(r.matched||0)}</td><td class="${Number(r.difference||0)>0?"over":Number(r.difference||0)<0?"under":""}">${Number(r.difference||0)>0?"+":""}${Number(r.difference||0)}</td></tr>`).join(""),diff=Number(s.difference||0);
  return`<div class="appointment-plan-volume-summary appointment-plan-volume-summary-r20412"><span><small>ตามแผน</small><b>${Number(s.planned||0)}</b></span><span><small>เข้าจริง</small><b>${Number(s.actual||0)}</b></span><span class="${diff>0?"warn":""}"><small>ต่างจากแผน</small><b>${diff>0?"+":""}${diff}</b></span><span class="good"><small>จับคู่ได้</small><b>${Number(s.matchPercent||0).toFixed(1)}%</b></span></div><div class="appointment-plan-volume-legend"><span><i></i>แผน</span><span><i></i>รถเข้าจริง</span><b>จับคู่แผน ${Number(s.matched||0)} · ยังไม่จับคู่ ${Number(s.unmatched||0)} · อ้างอิง ${escapeHtml(data.referenceMode||"-")}</b></div>${hourRows?`<div class="appointment-plan-volume-hours">${hourRows}</div>`:`<div class="appointment-plan-volume-empty compact">ยังไม่มีข้อมูลในช่วงนี้</div>`}${Number(data.days||1)>1&&daily?`<div class="appointment-plan-volume-daily"><table><thead><tr><th>วันที่</th><th>แผน</th><th>เข้าจริง</th><th>จับคู่แผน</th><th>ต่าง</th></tr></thead><tbody>${daily}</tbody></table></div>`:""}`
}
async function appointmentLoadPlanVolume(){
  const body=$("appointmentPlanVolumeBody"),days=$("appointmentPlanVolumeDays");if(!body||appointmentCenterState.volumeBusy)return;
  if(days){days.value=String(appointmentCenterState.volumeDays);appointmentCenterState.volumeDays=Number(days.value)||1}
  appointmentCenterState.volumeBusy=true;body.innerHTML=`<div class="appointment-plan-volume-loading">กำลังอ่านแผนและจำนวนรถจริง</div>`;
  try{const data=await api(`/api/appointment/plan-volume?days=${encodeURIComponent(appointmentCenterState.volumeDays)}`,{timeoutMs:12000});appointmentCenterState.volumeData=data;if($("appointmentPlanVolumeBody"))$("appointmentPlanVolumeBody").innerHTML=appointmentPlanVolumeBodyHtml(data)}
  catch(error){if($("appointmentPlanVolumeBody"))$("appointmentPlanVolumeBody").innerHTML=`<div class="appointment-plan-volume-empty"><b>โหลดข้อมูลไม่สำเร็จ</b><span>${escapeHtml(error.message||"กรุณาลองใหม่")}</span></div>`}
  finally{appointmentCenterState.volumeBusy=false}
}
function appointmentBindPlanVolume(autoLoad=false){
  $("appointmentPlanVolumeDays")?.addEventListener("change",event=>{appointmentCenterState.volumeDays=Number(event.target.value)||1;void appointmentLoadPlanVolume()});
  $("appointmentPlanVolumeReload")?.addEventListener("click",()=>void appointmentLoadPlanVolume());
  if(autoLoad)void appointmentLoadPlanVolume();
}

const appointmentPartnerAnalyticsState={days:7,type:"vendor",loaded:false,busy:false};

function appointmentPartnerAnalyticsAdminHtml(showTabs=true){
  const typeLabel=appointmentPartnerAnalyticsState.type==="carrier"?"ผู้ขนส่ง":appointmentPartnerAnalyticsState.type==="po"?"เลข PO":"บริษัทผู้ส่งสินค้า";
  return`<section class="appointment-partner-analytics-card appointment-partner-card-r20412"><header><div><b>${showTabs?"ตัวชี้วัดจากไฟล์นัดหมาย":typeLabel}</b><small>อ่านเฉพาะข้อมูลช่วงที่เลือก และไม่สร้างข้อมูลวิเคราะห์ซ้ำ</small></div><div class="appointment-partner-actions"><select id="appointmentPartnerDays" aria-label="ช่วงที่ดู"><option value="7">7 วัน</option><option value="30">30 วัน</option><option value="90">90 วัน</option></select><button id="appointmentPartnerLoad" class="appointment-refresh-button" type="button">รีเฟรช</button></div></header>${showTabs?`<div class="appointment-partner-tabs" role="tablist"><button type="button" data-partner-type="vendor" class="${appointmentPartnerAnalyticsState.type==="vendor"?"active":""}">บริษัทผู้ส่งสินค้า</button><button type="button" data-partner-type="carrier" class="${appointmentPartnerAnalyticsState.type==="carrier"?"active":""}">ผู้ขนส่ง</button><button type="button" data-partner-type="po" class="${appointmentPartnerAnalyticsState.type==="po"?"active":""}">เลข PO</button></div>`:""}<div id="appointmentPartnerBody" class="appointment-partner-body"><div class="appointment-partner-empty"><b>กำลังเตรียมข้อมูล</b><span>ระบบจะอ่านเฉพาะช่วงที่เลือก</span></div></div></section>`
}
function appointmentPartnerMinutes(value){const n=Number(value);if(!Number.isFinite(n)||n<=0)return"–";const r=Math.round(n),h=Math.floor(r/60),m=r%60;return h?`${h} ชม.${m?` ${m} นาที`:""}`:`${m} นาที`}
function appointmentPartnerBodyHtml(data){
  if(!data?.enabled)return`<div class="appointment-partner-empty"><b>ยังไม่ได้เปิดใช้ข้อมูลนัดหมายกับงานจริง</b><span>ส่วนนี้จะเริ่มอ่านข้อมูลเมื่อเปิดใช้งานและกดโหลดเท่านั้น</span></div>`;
  if(!data?.ready)return`<div class="appointment-partner-empty"><b>ยังอ่านตัวชี้วัดไม่ได้</b><span>${escapeHtml(data?.message||"กรุณาลองใหม่")}</span></div>`;
  const s=data.summary||{},rows=Array.isArray(data.rows)?data.rows:[],nameHead=data.type==="carrier"?"ผู้ขนส่ง":data.type==="po"?"เลข PO":"บริษัทผู้ส่งสินค้า",table=rows.map(row=>`<tr><td><b>${escapeHtml(row.name||"ไม่ระบุ")}</b></td><td>${Number(row.vehicles||0).toLocaleString("th-TH")}</td><td>${Number(row.early||0)}</td><td><strong>${Number(row.onTimePercent||0).toFixed(1)}%</strong></td><td><strong class="late">${Number(row.latePercent||0).toFixed(1)}%</strong></td><td>${appointmentPartnerMinutes(row.avgLateMinutes)}</td><td>${appointmentPartnerMinutes(row.avgGateToDocumentMinutes)}</td><td>${appointmentPartnerMinutes(row.avgWaitReceivingMinutes)}</td><td>${appointmentPartnerMinutes(row.avgReceivingMinutes)}</td><td>${appointmentPartnerMinutes(row.avgCycleMinutes)}</td><td>${Number(row.rejected||0)}</td></tr>`).join("");
  return`<div class="appointment-partner-summary appointment-partner-summary-r20412"><span><small>${data.type==="po"?"PO ที่พบ":"จำนวนกลุ่ม"}</small><b>${Number(s.groups||0)}</b></span><span><small>${data.type==="po"?"รถที่ผูกกับ PO":"รถที่นำมาวัด"}</small><b>${Number(s.vehicles||0)}</b></span><span class="good"><small>ตรงเวลารวม</small><b>${Number(s.onTimePercent||0).toFixed(1)}%</b></span><span class="bad"><small>มาสาย</small><b>${Number(s.latePercent||0).toFixed(1)}%</b></span></div><div class="appointment-partner-note">สายเฉลี่ย <b>${appointmentPartnerMinutes(s.avgLateMinutes)}</b> · ปฏิเสธ <b>${Number(s.rejected||0)}</b> · ตรงเวลารวม = มาก่อน + มาช้าภายในเกณฑ์ Admin · ${escapeHtml(data.note||"")} · อ่าน ${Number(data.durationMs||0).toLocaleString("th-TH")} ms</div>${table?`<div class="appointment-partner-table-wrap"><table class="appointment-partner-table"><thead><tr><th>${nameHead}</th><th>รถ</th><th>มาก่อน (ตรงเวลา)</th><th>ตรงเวลารวม</th><th>มาสาย</th><th>สายเฉลี่ย</th><th>รอยื่นเอกสาร</th><th>รอตรวจรับ</th><th>ตรวจรับสินค้า</th><th>อยู่ในพื้นที่</th><th>ปฏิเสธ</th></tr></thead><tbody>${table}</tbody></table></div>`:`<div class="appointment-partner-empty compact"><b>ยังไม่มีข้อมูลในช่วงนี้</b><span>ต้องมีรถที่จับคู่กับข้อมูลนัดหมายและมีผลเทียบเวลาแล้ว</span></div>`}`
}
async function appointmentLoadPartnerAnalytics(){
  const body=$("appointmentPartnerBody"),days=$("appointmentPartnerDays");if(!body||appointmentPartnerAnalyticsState.busy)return;if(days){days.value=String(appointmentPartnerAnalyticsState.days);appointmentPartnerAnalyticsState.days=Number(days.value)||7}appointmentPartnerAnalyticsState.busy=true;body.innerHTML=`<div class="appointment-partner-loading">กำลังอ่านข้อมูลที่บันทึกไว้</div>`;
  try{const data=await api(`/api/appointment/partner-analytics?days=${encodeURIComponent(appointmentPartnerAnalyticsState.days)}&type=${encodeURIComponent(appointmentPartnerAnalyticsState.type)}`,{timeoutMs:15000});appointmentPartnerAnalyticsState.loaded=true;if($("appointmentPartnerBody"))$("appointmentPartnerBody").innerHTML=appointmentPartnerBodyHtml(data)}catch(error){if($("appointmentPartnerBody"))$("appointmentPartnerBody").innerHTML=`<div class="appointment-partner-empty"><b>โหลดข้อมูลไม่สำเร็จ</b><span>${escapeHtml(error.message||"กรุณาลองใหม่")}</span></div>`}finally{appointmentPartnerAnalyticsState.busy=false}
}
function appointmentBindPartnerAnalytics(){
  $("appointmentPartnerDays")?.addEventListener("change",event=>{appointmentPartnerAnalyticsState.days=Number(event.target.value)||7;if(appointmentPartnerAnalyticsState.loaded)void appointmentLoadPartnerAnalytics()});$("appointmentPartnerLoad")?.addEventListener("click",()=>void appointmentLoadPartnerAnalytics());document.querySelectorAll("[data-partner-type]").forEach(button=>button.addEventListener("click",()=>{document.querySelectorAll("[data-partner-type]").forEach(x=>x.classList.toggle("active",x===button));appointmentPartnerAnalyticsState.type=button.dataset.partnerType||"vendor";if(appointmentPartnerAnalyticsState.loaded)void appointmentLoadPartnerAnalytics()}))
}

function renderAdminAppointmentData(){
  const cfg=adminState.data?.appointmentModule||{moduleEnabled:false,uploadEnabled:false,useImportedData:false,companySourceMode:"AUTO",timeReference:"PERIOD",lateGraceMinutes:0,searchWindowHours:36,adapterTimeoutMs:250,lifecycle:{},display:{},uploadProfile:{}},schema=adminState.data?.appointmentSchema||{ready:false,tablesReady:0,tablesTotal:5,indexReady:false,latest:null},kpiSchema=adminState.data?.appointmentKpiSchema||{ready:false,graceReady:false,tablesReady:0,tablesTotal:3,indexesReady:0,indexesTotal:3,metrics:0,policy:null,lastRun:null},profile=cfg.uploadProfile||{};
  const pages=[["inbound","Inbound"],["receiving","Receiving"],["datatable","Datatable"],["dashboard","Dashboard"],["queue","จอคิว"],["track","Track"]],latest=schema.latest,fields=profile.fields||{},fieldRows=[["dc","รหัสคลัง","DC CODE"],["date","วันที่นัด","DATE"],["period","รอบนัดหมาย","PERIOD"],["from","เวลาเริ่ม","FROM"],["to","เวลาสิ้นสุด","TO"],["po","เลข PO","PO NBR"],["appointment","Appointment","APPTNBR"],["vendor","บริษัท / Vendor","VEND NAME"],["carrier","Carrier / ผู้ขนส่ง","CARR"]],life=cfg.lifecycle||{},extraFields=Array.isArray(profile.extraFields)?profile.extraFields:[],extraRows=extraFields.map(appointmentExtraFieldRowHtml).join("");
  const pageCards=pages.map(([key,label])=>{const p=cfg.display?.[key]||{};return`<section class="display-setting-card appointment-display-page" data-appointment-page="${key}"><div><div><b>${label}</b><small>กำหนดข้อมูลนัดหมายที่อนุญาตให้แสดงในหน้านี้</small></div></div><div class="appointment-display-checks"><label class="clean-check"><input data-ap-field="enabled" type="checkbox" ${p.enabled!==false?"checked":""}><span>แสดง</span></label><label class="clean-check"><input data-ap-field="timing" type="checkbox" ${p.timing!==false?"checked":""}><span>ผลเทียบเวลานัด</span></label><label class="clean-check"><input data-ap-field="vendor" type="checkbox" ${p.vendor?"checked":""}><span>บริษัท</span></label><label class="clean-check"><input data-ap-field="carrier" type="checkbox" ${p.carrier?"checked":""}><span>Carrier</span></label><label class="clean-check"><input data-ap-field="po" type="checkbox" ${p.po?"checked":""}><span>PO</span></label></div></section>`}).join("");
  ...
  $("adminPanel").innerHTML=`<div class="admin-section-head clean-admin-head"><div><h3>ข้อมูลนัดหมาย</h3><p>ควบคุมการรับไฟล์และการใช้ข้อมูลกับงานจริงแยกจากกัน</p></div>${cfg.moduleEnabled?`<button id="openAppointmentUpload" class="outline-button" type="button">เปิดข้อมูลนัดหมาย</button>`:""}</div><section class="system-health-summary ${schema.ready?"pass":"warn"}"><div><small>ฐานข้อมูลนัดหมาย</small><b>${schema.ready?"พร้อม":"ยังไม่พร้อม"}</b><span>ตาราง ${Number(schema.tablesReady||0)}/${Number(schema.tablesTotal||5)} · ส่วนช่วยค้นหา ${schema.indexReady?"พร้อม":"ยังไม่มี"}</span></div><div><small>ชุดข้อมูลล่าสุด</small><b>${latest?escapeHtml(appointmentDisplaySnapshot(latest.snapshotDate,latest.snapshotTime)):"ยังไม่มีข้อมูล"}</b><span>${latest?.fileName?escapeHtml(latest.fileName):"รอนำเข้าไฟล์นัดหมาย"}</span></div><div><small>ผลเทียบเวลา / ประมวลผลย้อนหลัง</small><b>${kpiSchema.ready?"พร้อม":"ยังไม่พร้อม"}</b><span>${kpiSchema.ready?`บันทึกแล้ว ${Number(kpiSchema.metrics||0)} คัน · ${escapeHtml(kpiSchema.policy?.referenceMode||cfg.timeReference)} · อนุโลม ${Number(kpiSchema.policy?.lateGraceMinutes??cfg.lateGraceMinutes??0)} นาที`:`ตาราง ${Number(kpiSchema.tablesReady||0)}/${Number(kpiSchema.tablesTotal||3)} · ส่วนช่วยค้นหา ${Number(kpiSchema.indexesReady||0)}/${Number(kpiSchema.indexesTotal||3)}`}</span></div><div><small>ข้อมูลคอลัมน์เพิ่มเติม</small><b>${schema.extraDataReady?"พร้อม":"ยังไม่เพิ่ม"}</b><span>${schema.extraDataReady?`รองรับ ${extraFields.length} รายการที่ Admin เลือกใช้`:"หากต้องใช้คอลัมน์เพิ่มเติม ให้ติดตั้งไฟล์ฐานข้อมูล Round 200 หนึ่งครั้ง"}</span></div></section><form id="appointmentModuleForm" class="display-settings-form"><section class="display-setting-card"><div><span class="display-setting-icon dashboard-icon">A</span><div><b>เปิดโมดูลข้อมูลนัดหมาย</b><small>เมื่อปิด ระบบหลักทำงานแบบเดิมและไม่ค้นข้อมูลนัดหมาย</small></div></div><label class="clean-toggle"><input id="appointmentModuleEnabled" type="checkbox" ${cfg.moduleEnabled?"checked":""}><span></span></label></section><section class="display-setting-card"><div><span class="display-setting-icon datatable-icon">↑</span><div><b>เปิดการอัปโหลดไฟล์</b><small>ADMIN และ USER จะสามารถรับไฟล์ใหม่ได้ ส่วนหน้าวิเคราะห์ยังดูได้เมื่อปิดการรับไฟล์</small></div></div><label class="clean-toggle"><input id="appointmentUploadEnabled" type="checkbox" ${cfg.uploadEnabled?"checked":""}><span></span></label></section><section class="display-setting-card"><div><span class="display-setting-icon dashboard-icon">↔</span><div><b>ใช้ข้อมูลนัดหมายกับงานจริง</b><small>ควรเปิดหลังตรวจการนำเข้าข้อมูลจริงผ่านการตรวจแล้วเท่านั้น</small></div></div><label class="clean-toggle"><input id="appointmentUseImportedData" type="checkbox" ${cfg.useImportedData?"checked":""}><span></span></label></section><div class="appointment-rule-grid appointment-timing-rule-grid"><label><span>จุดเริ่มคำนวณเวลานัด</span><select id="appointmentTimeReference"><option value="PERIOD">PERIOD</option><option value="FROM">FROM</option></select><small>เลือกจุดเดียวสำหรับทุกหน้า · ผลย้อนหลังเดิมไม่ถูกเขียนทับอัตโนมัติ</small></label><label><span>อนุโลมมาช้าไม่เกิน</span><input id="appointmentLateGrace" type="number" min="0" max="1440" step="1" list="appointmentLateGracePresets" value="${Number(cfg.lateGraceMinutes||0)}"><datalist id="appointmentLateGracePresets"><option value="0"><option value="15"><option value="30"><option value="60"></datalist><small>นาที · เช่น 15 / 30 / 60 หรือกำหนดเอง</small></label><label><span>ช่วงค้นหาคู่เลขนัดหมาย</span><input id="appointmentSearchWindow" type="number" min="1" max="168" value="${Number(cfg.searchWindowHours||36)}"><small>ชั่วโมง · ใช้ค้นหาคู่เท่านั้น ไม่ใช่เวลาผ่อนผัน</small></label><label><span>เวลารอข้อมูลสูงสุด</span><input id="appointmentAdapterTimeout" type="number" min="50" max="2000" step="10" value="${Number(cfg.adapterTimeoutMs||250)}"><small>มิลลิวินาที</small></label></div><section id="appointmentTimingRulePreview" class="appointment-timing-rule-preview"></section>${!kpiSchema.graceReady?`<section class="appointment-timing-migration-warning"><b>ต้องติดตั้งฐานเกณฑ์เวลา Round 204.1</b><span>รัน SQL Round 204.1 ในฐาน warehouse-vehicle-flow ก่อนบันทึกหรือเปิดใช้เกณฑ์อนุโลม เพื่อให้ผลย้อนหลังตรวจสอบได้ว่าคำนวณด้วยเกณฑ์ใด</span></section>`:""}<section class="appointment-center-link-card"><div><b>ผลการดำเนินงานและการวิเคราะห์</b><small>แผนเทียบเวลาจริง แผนเทียบจำนวน บริษัท ผู้ขนส่ง และ PO ย้ายไปอยู่ในเมนู “ข้อมูลนัดหมาย” เพื่อให้หน้าตั้งค่ามีเฉพาะส่วนควบคุม</small></div><button id="openAppointmentCenter" class="outline-button" type="button">เปิดข้อมูลนัดหมาย</button></section><section class="appointment-company-policy-card"><div><b>ชื่อบริษัทที่ใช้ในระบบ</b><small>ไม่แก้ชื่อบริษัทต้นฉบับที่คนขับกรอก ระบบเปลี่ยนเฉพาะชื่อที่ใช้แสดงผล</small></div><label><span>แหล่งชื่อบริษัท</span><select id="appointmentCompanySourceMode"><option value="AUTO">อัตโนมัติ (แนะนำ) · จับคู่ได้ใช้ไฟล์ ไม่พบใช้ Gate In</option><option value="GATE_IN">ใช้ชื่อที่คนขับกรอกตอน Gate In เสมอ</option><option value="BOTH">ใช้ไฟล์เมื่อจับคู่ และแสดงชื่อ Gate In เทียบด้วย</option></select></label><p>หากเลขนัดหมายไม่ตรงกับไฟล์ หรือข้อมูลนัดหมายใช้ไม่ได้ ระบบจะใช้ชื่อบริษัทจาก Gate In และงานดำเนินต่อทันที</p></section><section class="appointment-source-filter-card"><header><div><b>การกรองไฟล์นำเข้า</b><small>ชื่อไฟล์เปลี่ยนได้ ระบบตรวจจากข้อมูลภายในไฟล์และหัวคอลัมน์</small></div><div class="appointment-source-head-actions"><strong id="appointmentWarehousePreview">${escapeHtml(appointmentWarehousePreviewLabel(profile.warehouse?.matchMode,(profile.warehouse?.values||["906"]).join(", ")))}</strong><label id="appointmentProbeFileButton" class="outline-button appointment-probe-file">ตรวจไฟล์ตัวอย่าง<input id="appointmentProbeFileInput" type="file" accept=".xlsb,.xlsx" hidden></label></div></header><div class="appointment-source-filter-grid"><label><span>ชื่อชีท</span><input id="appointmentSheetName" value="${escapeHtml(profile.sheetName||"raw_data")}"><small>ค่าเริ่มต้น raw_data</small></label><label><span>ชื่อชีทสำรอง</span><input id="appointmentSheetAliases" value="${escapeHtml((profile.sheetAliases||["RAW_DATA","Raw Data"]).join(", "))}"><small>คั่นหลายชื่อด้วยเครื่องหมายจุลภาค</small></label><label><span>วิธีกรอง DC CODE</span><select id="appointmentWarehouseMode"><option value="STARTS_WITH">ขึ้นต้นด้วย</option><option value="EXACT">ตรงกัน</option></select><small>ใช้ STARTS_WITH เมื่อต้องการรับรหัสที่ต่อท้ายเลขคลัง</small></label><label><span>รหัสคลัง</span><input id="appointmentWarehouseValues" value="${escapeHtml((profile.warehouse?.values||["906"]).join(", "))}" placeholder="เช่น 906"><small>ตัวอย่าง STARTS_WITH 906 จะรับ 906xxx · รองรับหลายค่า</small></label></div></section><section id="appointmentFileProbePanel" class="appointment-file-probe-panel" hidden></section><details class="appointment-admin-file-settings"><summary><b>ชื่อคอลัมน์และชื่อสำรอง</b><span>จับคู่ข้อมูลจากไฟล์ส่วนกลาง</span></summary><div class="appointment-admin-file-body"><div class="appointment-map-head"><b>ข้อมูล</b><b>ชื่อคอลัมน์</b><b>ชื่อสำรอง</b></div>${mapping}</div></details><section class="appointment-extra-fields-card"><header><div><b>ข้อมูลเพิ่มเติมจากไฟล์</b><small>เลือกเฉพาะคอลัมน์ที่ต้องใช้ แล้วกำหนดว่าจะให้แสดงหน้าใดหรือใช้ต่ออย่างไร</small></div><div class="appointment-extra-head-actions"><button id="appointmentChooseExtraFromFile" class="outline-button" type="button">เลือกจากไฟล์ตัวอย่าง</button><button id="appointmentAddExtraField" class="outline-button" type="button">เพิ่มข้อมูลเอง</button></div></header><div class="appointment-extra-guide"><span><b>1</b><em>ตรวจไฟล์</em><small>ดูหัวคอลัมน์จริง</small></span><i>›</i><span><b>2</b><em>เลือกข้อมูล</em><small>กำหนดชื่อและชื่อสำรอง</small></span><i>›</i><span><b>3</b><em>เลือกจุดใช้งาน</em><small>หน้าแสดงผล / ส่งออก / กรอง / วิเคราะห์</small></span></div><div id="appointmentExtraFieldsList">${extraRows||`<div class="appointment-extra-empty">ยังไม่ได้เพิ่มข้อมูลเพิ่มเติม</div>`}</div><p>${schema.extraDataReady?"พื้นที่เก็บข้อมูลเพิ่มเติมพร้อมใช้งาน":"ก่อนบันทึกข้อมูลเพิ่มเติมครั้งแรก ให้ติดตั้งไฟล์ฐานข้อมูล Round 200 หนึ่งครั้ง"}</p><div class="appointment-extra-import-note"><b>เมื่อเพิ่มคอลัมน์ใหม่ภายหลัง</b><span>ให้บันทึกการตั้งค่านี้ก่อน แล้วนำเข้าไฟล์ที่มีคอลัมน์ดังกล่าวอีกครั้ง ระบบจะเก็บเฉพาะข้อมูลที่ Admin เลือกใช้ ไม่ดึงคอลัมน์เก่าที่ยังไม่เคยเก็บย้อนหลังเอง</span></div></section><section class="appointment-retention-central-card"><div><b>อายุการเก็บข้อมูล</b><small>ย้ายการตั้งค่าอายุข้อมูลทั้งหมดไปไว้ที่ “การใช้ข้อมูล” เพื่อให้ควบคุมจากจุดเดียว</small></div><button id="openCentralRetentionSettings" class="outline-button" type="button">เปิดการตั้งค่าอายุข้อมูล</button></section><div class="admin-section-head clean-admin-head"><div><h3>การแสดงข้อมูลแต่ละหน้า</h3><p>เมื่อปิดช่องใด หน้านั้นจะไม่แสดงข้อมูลส่วนนั้น</p></div></div>${pageCards}<section class="display-setting-card"><div><span class="display-setting-icon dashboard-icon">✓</span><div><b>ตรวจความครอบคลุมและการจับคู่</b><small>ตรวจว่ารถจริงอยู่ในชุดนัดหมายหรือไม่ และทดสอบการจับคู่ด้วยข้อมูลจริงแบบอ่านอย่างเดียว</small></div></div><button id="appointmentShadowUatButton" class="outline-button" type="button">ทดสอบการจับคู่</button></section><section class="display-setting-card"><div><span class="display-setting-icon datatable-icon">↔</span><div><b>ตรวจการส่งข้อมูลไปยังหน้างาน</b><small>ตรวจว่าข้อมูลนัดหมายส่งไปยัง Inbound / Receiving ได้ถูกต้อง โดยยังไม่ต้องเปิดใช้กับงานจริง</small></div></div><button id="appointmentLiveWiringUatButton" class="outline-button" type="button">ทดสอบการส่งข้อมูล</button></section><section class="display-setting-card"><div><span class="display-setting-icon dashboard-icon">▦</span><div><b>ตรวจการแสดงผล 6 หน้า</b><small>ตรวจสิทธิ์การแสดง เวลา บริษัท Carrier และ PO ตามค่าที่ Admin กำหนด โดยไม่ต้องเปิดใช้งานกับงานจริง</small></div></div><button id="appointmentDisplayUatButton" class="outline-button" type="button">ทดสอบการแสดงผล 6 หน้า</button></section><section class="display-setting-card"><div><span class="display-setting-icon dashboard-icon">Σ</span><div><b>ตรวจแผนเทียบเวลาจริง</b><small>ตรวจ PERIOD / FROM, ขอบเขตอนุโลมถึงระดับวินาที, TO ข้ามวัน และการประมวลผลย้อนหลังโดยไม่แก้ Gate In</small></div></div><button id="appointmentKpiUatButton" class="outline-button" type="button">ทดสอบแผนเทียบจริง</button></section><section class="display-setting-card"><div><span class="display-setting-icon datatable-icon">⚙</span><div><b>ตรวจความเร็วและความทนต่อปัญหา</b><small>ทดสอบระบบหลักและข้อมูลนัดหมายพร้อมกัน รวมการใช้งานหลายส่วนพร้อมกัน ข้อมูลซ้ำ และการนำเข้าที่ขาดกลางทาง โดยไม่แก้ Gate In</small></div></div><button id="appointmentStressUatButton" class="outline-button" type="button">ทดสอบความเสถียร</button></section><section class="display-setting-card"><div><span class="display-setting-icon datatable-icon">↔</span><div><b>ตรวจชื่อบริษัทและอายุข้อมูล</b><small>ตรวจการเลือกบริษัทจาก Gate In / ไฟล์ การใช้ชื่อ Gate In เมื่อไม่พบเลขนัดหมาย และตรวจอายุข้อมูลแบบอ่านอย่างเดียว</small></div></div><button id="appointmentRound186UatButton" class="outline-button" type="button">ทดสอบรอบ 186</button></section><section class="display-setting-card"><div><span class="display-setting-icon dashboard-icon">✓</span><div><b>ตรวจขั้นสุดท้ายก่อนใช้งานจริง</b><small>ตรวจได้ทั้งก่อนและหลังเปิดใช้งานจริง และบันทึกสภาพการตั้งค่าปัจจุบันโดยไม่เปลี่ยนสวิตช์</small></div></div><button id="appointmentFinalUatButton" class="primary" type="button">ตรวจขั้นสุดท้าย</button></section><div class="display-setting-note"><b>ความปลอดภัยของการเปิดใช้งาน</b><span>เปิดโมดูลและอัปโหลดได้โดยยังคง “ใช้ข้อมูลนัดหมายกับงานจริง” เป็นปิด การนำเข้าไฟล์จะไม่เปลี่ยน Gate In</span></div><div class="admin-form-actions"><button class="primary" type="submit">บันทึกการตั้งค่า</button></div></form>`;
  $("appointmentTimeReference").value=cfg.timeReference==="FROM"?"FROM":"PERIOD";$("appointmentCompanySourceMode").value=["AUTO","GATE_IN","BOTH"].includes(String(cfg.companySourceMode||"AUTO"))?String(cfg.companySourceMode||"AUTO"):"AUTO";$("appointmentWarehouseMode").value=profile.warehouse?.matchMode==="EXACT"?"EXACT":"STARTS_WITH";
  const updateWarehousePreview=()=>{const preview=$("appointmentWarehousePreview");if(preview)preview.textContent=appointmentWarehousePreviewLabel($("appointmentWarehouseMode")?.value,$("appointmentWarehouseValues")?.value)};$("appointmentWarehouseMode")?.addEventListener("change",updateWarehousePreview);$("appointmentWarehouseValues")?.addEventListener("input",updateWarehousePreview);updateWarehousePreview();
  document.querySelectorAll("#appointmentExtraFieldsList .appointment-extra-field-row").forEach(appointmentBindExtraFieldRow);$("appointmentAddExtraField")?.addEventListener("click",()=>{const empty=$("appointmentExtraFieldsList")?.querySelector(".appointment-extra-empty");empty?.remove();appointmentAddExtraField()});$("appointmentChooseExtraFromFile")?.addEventListener("click",()=>$("appointmentProbeFileInput")?.click());$("appointmentProbeFileInput")?.addEventListener("change",()=>appointmentAdminDiscoverFile($("appointmentProbeFileInput").files?.[0],fieldRows));if(appointmentAdminFileProbe.result)appointmentRenderAdminProbe(fieldRows);
  const sync=()=>{const on=$("appointmentModuleEnabled").checked;$("appointmentUploadEnabled").disabled=!on;$("appointmentUseImportedData").disabled=!on;document.querySelectorAll("#appointmentModuleForm select,#appointmentModuleForm input[type=number],[data-appointment-page] input,.appointment-admin-file-settings input,.appointment-admin-file-settings select,.appointment-source-filter-card input,.appointment-source-filter-card select,.appointment-company-policy-card select,.appointment-extra-fields-card input,.appointment-extra-fields-card select,.appointment-extra-fields-card button,.appointment-lifecycle-settings input").forEach(el=>el.disabled=!on);if(!on){$("appointmentUploadEnabled").checked=false;$("appointmentUseImportedData").checked=false}};$("appointmentModuleEnabled")?.addEventListener("change",sync);sync();
    const updateTimingPreview=()=>{const node=$("appointmentTimingRulePreview");if(!node)return;const ref=$("appointmentTimeReference")?.value||"PERIOD",grace=Math.max(0,Number($("appointmentLateGrace")?.value||0));node.innerHTML=`<b>กติกาที่ใช้กับทุกหน้า</b><span>ก่อน ${escapeHtml(ref)} = <strong>มาก่อน · ตรงเวลา</strong></span><span>${escapeHtml(ref)} ถึง ${escapeHtml(ref)} + ${grace} นาที = <strong>อยู่ในเกณฑ์ตรงเวลา</strong></span><span>เกิน ${grace} นาที = <strong>มาสาย</strong></span><small>ตัดสินสถานะจากเวลาจริงระดับวินาที · TO ใช้เป็นขอบเขตนัดและรองรับข้ามวัน แต่ไม่ใช่จุดเริ่มคำนวณความล่าช้า</small>`};
  $("appointmentTimeReference")?.addEventListener("change",updateTimingPreview);$("appointmentLateGrace")?.addEventListener("input",updateTimingPreview);updateTimingPreview();
  ...
  $("appointmentLiveWiringUatButton")?.addEventListener("click",appointmentRunLiveWiringUat);$("appointmentDisplayUatButton")?.addEventListener("click",appointmentRunDisplayUat);$("appointmentKpiUatButton")?.addEventListener("click",appointmentRunKpiUat);$("appointmentStressUatButton")?.addEventListener("click",appointmentRunStressUat);$("openCentralRetentionSettings")?.addEventListener("click",()=>{adminState.tab="data";adminDataTools.tab="retention";try{localStorage.setItem("wvf_admin_tab","data")}catch{}renderAdminShell()});$("appointmentRound186UatButton")?.addEventListener("click",appointmentRunRound186Uat);$("appointmentFinalUatButton")?.addEventListener("click",appointmentRunFinalUat);
  $("openAppointmentCenter")?.addEventListener("click",()=>navigate("appointmentUpload"));
  $("appointmentModuleForm")?.addEventListener("submit",async event=>{event.preventDefault();if(adminState.busy)return;const warehouseValues=appointmentAdminList($("appointmentWarehouseValues")?.value);if(!warehouseValues.length){await showNotice("warning","กรุณากำหนดรหัสคลังอย่างน้อย 1 ค่า เช่น 906");$("appointmentWarehouseValues")?.focus();return}const lateGrace=Math.max(0,Number($("appointmentLateGrace")?.value||0)),searchWindow=Math.max(0,Number($("appointmentSearchWindow")?.value||0));if(lateGrace>searchWindow*60){await showNotice("warning","ช่วงค้นหาคู่เลขนัดหมายต้องไม่น้อยกว่าเวลาอนุโลมมาช้า");$("appointmentSearchWindow")?.focus();return}const display={};document.querySelectorAll("[data-appointment-page]").forEach(card=>{const key=card.dataset.appointmentPage,read=f=>card.querySelector(`[data-ap-field="${f}"]`)?.checked===true;display[key]={enabled:read("enabled"),timing:read("timing"),vendor:read("vendor"),carrier:read("carrier"),po:read("po")}});const uploadFields={};for(const[key,,def]of fieldRows)uploadFields[key]={header:String($("appointmentField_"+key)?.value||def).trim()||def,aliases:appointmentAdminList($("appointmentAlias_"+key)?.value)};const body={moduleEnabled:$("appointmentModuleEnabled").checked,uploadEnabled:$("appointmentUploadEnabled").checked,useImportedData:$("appointmentUseImportedData").checked,companySourceMode:$("appointmentCompanySourceMode").value,timeReference:$("appointmentTimeReference").value,lateGraceMinutes:lateGrace,searchWindowHours:Number($("appointmentSearchWindow").value),adapterTimeoutMs:Number($("appointmentAdapterTimeout").value),lifecycle:life,display,uploadProfile:{sheetName:String($("appointmentSheetName").value||"raw_data").trim()||"raw_data",sheetAliases:appointmentAdminList($("appointmentSheetAliases").value),warehouse:{matchMode:$("appointmentWarehouseMode").value,values:warehouseValues},fields:uploadFields,extraFields:appointmentCollectExtraFields()}};adminState.busy=true;try{const result=await api("/api/admin/appointment-module",{method:"POST",body});state.appointment=normalizeAppointmentAccess(result.appointmentModule);renderNavigation();adminState.data=await api("/api/admin/settings");renderAdminShell();await Swal.fire({icon:"success",title:result.message||"บันทึกแล้ว",timer:1600,showConfirmButton:false,customClass:swalClasses(),width:380})}catch(error){await showNotice("error",error.message||"บันทึกข้อมูลนัดหมายไม่สำเร็จ")}finally{adminState.busy=false}})
}
...
function appointmentKpiUatHtml(data){
  ...
  const head=`<div class="appointment-swal-counts"><span><small>ผ่าน</small><b>${Number(s.passed||0)}/${Number(s.total||0)}</b></span><span><small>ประมวลผลย้อนหลัง</small><b>${Number(b.matched||0)} คัน</b></span><span><small>Gate In ถูกแก้</small><b>${Number(data?.gateInWriteCount||0)}</b></span></div>`;
  const policyHtml=`<div class="appointment-kpi-policy"><b>จุดเริ่ม: ${escapeHtml(policy.referenceMode||'-')} · อนุโลม ${Number(policy.lateGraceMinutes||0)} นาที</b><span>ช่วงค้นหาคู่ ${Number(policy.searchWindowHours||0)} ชม. · เริ่ม ${escapeHtml(policy.effectiveAtDisplay||'-')}</span><small>มาก่อนนับว่าตรงเวลา · มาช้าภายในเกณฑ์นับว่าตรงเวลา · เกินเกณฑ์จึงมาสาย และไม่คำนวณประวัติเก่าทับเองโดยอัตโนมัติ</small></div>`;
  ...
  const backfill=`<div class="appointment-kpi-backfill"><b>ประมวลผลข้อมูลย้อนหลัง</b><span>ตรวจ ${Number(b.scanned||0)} · จับคู่ ${Number(b.matched||0)} · เพิ่ม ${Number(b.inserted||0)} · ปรับ ${Number(b.updated||0)} · เดิม ${Number(b.unchanged||0)}</span>${Number(b.policySkipped||0)>0?`<small>ข้าม ${Number(b.policySkipped||0)} รายการ เพราะเป็นผลจากเกณฑ์เวลาเดิม</small>`:''}</div>`;
  const kpiHtml=`<div class="appointment-swal-counts" style="margin-top:10px"><span><small>มาก่อน (ตรงเวลา)</small><b>${Number(k.early||0)}</b></span><span><small>ตรงเวลาในเกณฑ์</small><b>${Number(k.onTime||0)}</b></span><span><small>มาสาย</small><b>${Number(k.late||0)}</b></span></div>`;
  return`<div class="appointment-kpi-uat">${head}${policyHtml}<div class="appointment-kpi-math">${mathRows}</div>${testRows}${backfill}${kpiHtml}<div class="appointment-kpi-note">วันที่เวลา ${escapeHtml(data?.dateTimeStandard||'dd/MM/yyyy HH:mm:ss')} · บันทึกเฉพาะผลเทียบเวลา ไม่แก้ข้อมูลรถหรือ Gate In</div></div>`;
}
async function appointmentRunKpiUat(){if(adminState.busy)return;const button=$("appointmentKpiUatButton");adminState.busy=true;if(button)button.disabled=true;try{Swal.fire({title:"กำลังตรวจผลเทียบเวลา",text:"ใช้ข้อมูล Production จริง แต่ไม่แก้ Gate In",allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>Swal.showLoading(),customClass:swalClasses(),width:410});const data=await api("/api/admin/appointment-kpi-uat",{method:"POST",body:{}});if(!data.ready){await Swal.fire({icon:"info",title:"ยังทดสอบไม่ได้",text:data.message||"ข้อมูลยังไม่พร้อม",confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:410});return}const pass=data?.summary?.allPassed===true;await Swal.fire({icon:pass?"success":"warning",title:pass?"ผลเทียบเวลาผ่าน":"มีรายการต้องตรวจ",html:appointmentKpiUatHtml(data),confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:500,heightAuto:false});adminState.data=await api("/api/admin/settings");renderAdminAppointmentData();}catch(error){try{Swal.close()}catch{}await showNotice("error",error.message||"ทดสอบผลเทียบเวลาไม่สำเร็จ")}finally{adminState.busy=false;if(button)button.disabled=false}}


function appointmentStressUatHtml(data){
  const summary=data?.summary||{},tests=Array.isArray(data?.tests)?data.tests:[],imp=data?.importGuards||{},bulk=data?.bulk||{};
  const head=`<div class="appointment-swal-counts"><span><small>ผ่าน</small><b>${Number(summary.passed||0)}/${Number(summary.total||0)}</b></span><span><small>Core P95</small><b>${Math.round(Number(summary.coreP95Ms||0))} ms</b></span><span><small>Match P95</small><b>${Math.round(Number(summary.matchP95Ms||0))} ms</b></span></div>`;
  const targets=`<div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:10px 0"><span style="padding:9px;border:1px solid #e3e9f3;border-radius:10px"><small style="display:block;color:#68758a">Core เป้าหมาย</small><b>≤ ${Number(summary.coreTargetMs||500)} ms</b></span><span style="padding:9px;border:1px solid #e3e9f3;border-radius:10px"><small style="display:block;color:#68758a">Match เป้าหมาย</small><b>≤ ${Number(summary.matchTargetMs||150)} ms</b></span><span style="padding:9px;border:1px solid #e3e9f3;border-radius:10px"><small style="display:block;color:#68758a">Bulk</small><b>${Math.round(Number(summary.bulkMs||0))} ms</b></span></div>`;
  const rows=tests.map(t=>`<div style="display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px solid #edf1f7;text-align:left"><div><b>${escapeHtml(t.label||t.id||'-')}</b><small style="display:block;color:#68758a;margin-top:2px">${escapeHtml(t.detail||'')}</small></div><strong style="flex:0 0 auto;color:${t.passed?'#17864b':'#b42318'}">${t.passed?'ผ่าน':'ต้องแก้'}</strong></div>`).join('');
  const guards=`<div style="margin-top:10px;padding:10px 12px;border-radius:10px;background:#f5f7fb;text-align:left"><b>Import Guard</b><small style="display:block;margin-top:4px;color:#68758a">ไฟล์ซ้ำ ${Number(imp.duplicateFileGroups||0)} · Batch ซ้ำ ${Number(imp.duplicateBatchGroups||0)} · งานค้าง ${Number(imp.openImports||0)} · ข้อมูลเกินจำนวน ${Number(imp.invalidProcessed||0)}</small><small style="display:block;margin-top:4px;color:#68758a">Bulk จับคู่ ${Number(bulk.matched||0)}/${Number(bulk.eligible||0)} · ${Number(bulk.queryChunks||0)} ชุด Query</small></div>`;
  return`<div>${head}${targets}${rows}${guards}<div style="margin-top:10px;padding:9px 11px;border-radius:10px;background:#eef9f2;color:#17864b">ทดสอบแบบอ่านข้อมูลจริง · จำลองเฉพาะ Timeout/Error · ไม่เขียน Gate In · ไม่แก้รถ · วันที่เวลา ${escapeHtml(data?.dateTimeStandard||'dd/MM/yyyy HH:mm:ss')}</div></div>`;
}
async function appointmentRunStressUat(){
  if(adminState.busy)return;adminState.busy=true;const button=$("appointmentStressUatButton");if(button)button.disabled=true;
  try{
    Swal.fire({title:"กำลังทดสอบความเสถียร",text:"ตรวจคำขอพร้อมกัน ความเร็ว Fail-open และ Import Guard",allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>Swal.showLoading(),customClass:swalClasses(),width:420});
    const data=await api("/api/admin/appointment-stress-uat",{method:"POST",body:{},timeoutMs:45000,retries:0});
    if(!data.ready){await Swal.fire({icon:"info",title:"ยังทดสอบไม่ได้",text:data.message||"ข้อมูลยังไม่พร้อม",confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:410});return}
    const pass=data?.summary?.allPassed===true;
    await Swal.fire({icon:pass?"success":"warning",title:pass?"Performance และ Resilience ผ่าน":"มีรายการต้องตรวจ",html:appointmentStressUatHtml(data),confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:530,heightAuto:false});
  }catch(error){try{Swal.close()}catch{}await showNotice("error",error.message||"ทดสอบความเสถียรไม่สำเร็จ")}finally{adminState.busy=false;if(button)button.disabled=false}
}



function appointmentLifecycleHtml(data,title="ตรวจพื้นที่ข้อมูล"){
  const c=data?.current||{},x=data?.candidates||{},r=data?.retention||{},total=Number(data?.totalCandidates||Object.values(x).reduce((s,v)=>s+Number(v||0),0));
  return`<div class="appointment-lifecycle-swal"><div class="appointment-swal-counts"><span><small>นัดหมายปัจจุบัน</small><b>${Number(c.appointments||0).toLocaleString()}</b></span><span><small>ประวัตินำเข้า</small><b>${Number(c.imports||0).toLocaleString()}</b></span><span><small>พร้อมล้าง</small><b>${total.toLocaleString()}</b></span></div><div class="appointment-lifecycle-summary"><div><span>ข้อมูลชั่วคราวหลังนำเข้าสำเร็จ</span><b>${Number(x.completedBatches||0).toLocaleString()}</b></div><div><span>งานนำเข้าที่ค้าง (เก็บไว้ Resume)</span><b>${Number(x.staleIncompleteImports||0).toLocaleString()}</b></div><div><span>ประวัตินำเข้าเก่า</span><b>${Number(x.oldImportHistory||0).toLocaleString()}</b></div><div><span>ประวัติการเปลี่ยนแปลงเก่า</span><b>${Number(x.oldChangeHistory||0).toLocaleString()}</b></div><div><span>นัดหมายที่หมดอายุ</span><b>${Number(x.expiredAppointments||0).toLocaleString()}</b></div><div><span>ผลเทียบเวลา / ประวัติการคำนวณเก่า</span><b>${(Number(x.oldMetrics||0)+Number(x.oldBackfillRuns||0)).toLocaleString()}</b></div></div><div class="appointment-lifecycle-note">ไฟล์ Excel ต้นฉบับไม่ถูกเก็บใน D1 · งานนำเข้าที่ยังค้างจะไม่ถูกลบอัตโนมัติ · Gate In / รถ / Workflow ไม่ถูกล้าง</div></div>`
}
async function appointmentRunLifecyclePreview(){if(adminState.busy)return;adminState.busy=true;const b=$("appointmentLifecyclePreviewButton");if(b)b.disabled=true;try{const data=await api("/api/admin/appointment-lifecycle");await Swal.fire({icon:"info",title:"พื้นที่ข้อมูลนัดหมาย",html:appointmentLifecycleHtml(data),confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:500,heightAuto:false})}catch(error){await showNotice("error",error.message||"ตรวจพื้นที่ข้อมูลไม่สำเร็จ")}finally{adminState.busy=false;if(b)b.disabled=false}}
async function appointmentRunLifecycleCleanup(){if(adminState.busy)return;adminState.busy=true;const b=$("appointmentLifecycleCleanupButton");if(b)b.disabled=true;try{const preview=await api("/api/admin/appointment-lifecycle"),total=Number(preview.totalCandidates||0);if(!total){await Swal.fire({icon:"success",title:"ยังไม่มีข้อมูลเก่าที่ต้องล้าง",html:appointmentLifecycleHtml(preview),confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:500,heightAuto:false});return}const confirm=await Swal.fire({icon:"question",title:"ล้างข้อมูลเก่าตามอายุที่กำหนด?",html:appointmentLifecycleHtml(preview),showCancelButton:true,confirmButtonText:"ล้างข้อมูลเก่า",cancelButtonText:"ยกเลิก",customClass:swalClasses(),buttonsStyling:false,width:500,heightAuto:false});if(!confirm.isConfirmed)return;Swal.fire({title:"กำลังล้างข้อมูลเก่า",text:"ไม่แตะ Gate In / รถ / Workflow",allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>Swal.showLoading(),customClass:swalClasses(),width:400});const done=await api("/api/admin/appointment-lifecycle/cleanup",{method:"POST",body:{},timeoutMs:45000,retries:0});await Swal.fire({icon:"success",title:"ล้างข้อมูลเก่าเรียบร้อย",html:`<div class="appointment-lifecycle-cleaned"><b>นัดหมายลบ ${Number(done.removed?.appointments||0).toLocaleString()} รายการ</b><span>ประวัตินำเข้า ${Number(done.removed?.importHistory||0).toLocaleString()} · ประวัติเปลี่ยนแปลง ${Number(done.removed?.changeHistory||0).toLocaleString()} · Batch ชั่วคราว ${Number(done.removed?.completedBatches||0).toLocaleString()}</span><small>Gate In / รถ ถูกแก้ 0 รายการ</small></div>`,confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:450})}catch(error){try{Swal.close()}catch{}await showNotice("error",error.message||"ล้างข้อมูลเก่าไม่สำเร็จ")}finally{adminState.busy=false;if(b)b.disabled=false}}
function appointmentRound186UatHtml(data){const s=data?.summary||{},tests=Array.isArray(data?.tests)?data.tests:[],life=data?.lifecycle||{};return`<div><div class="appointment-swal-counts"><span><small>ผ่าน</small><b>${Number(s.passed||0)}/${Number(s.total||0)}</b></span><span><small>บริษัท</small><b>${escapeHtml(String(data?.companySourceMode||"AUTO"))}</b></span><span><small>ไฟล์ต้นฉบับ</small><b>${life.rawExcelStored?"เก็บ":"ไม่เก็บ"}</b></span></div><div class="appointment-round186-tests">${tests.map(t=>`<div><span><b>${escapeHtml(t.label||t.id)}</b>${t.detail?`<small>${escapeHtml(t.detail)}</small>`:""}</span><strong class="${t.passed?"pass":"fail"}">${t.passed?"ผ่าน":"ต้องแก้"}</strong></div>`).join("")}</div><div class="appointment-lifecycle-note">บริษัทใน vehicles ไม่ถูกเขียนทับ · ถ้าไม่พบ Appointment ใช้ชื่อ Gate In และ Workflow ไปต่อ</div></div>`}
async function appointmentRunRound186Uat(){if(adminState.busy)return;adminState.busy=true;const b=$("appointmentRound186UatButton");if(b)b.disabled=true;try{Swal.fire({title:"กำลังตรวจรอบ 186",text:"ตรวจชื่อบริษัท การ fallback และการดูแลพื้นที่ข้อมูล",allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>Swal.showLoading(),customClass:swalClasses(),width:420});const data=await api("/api/admin/appointment-round186-uat",{method:"POST",body:{},timeoutMs:45000,retries:0}),pass=data?.summary?.allPassed===true;await Swal.fire({icon:pass?"success":"warning",title:pass?"Round 186 ผ่าน":"มีรายการต้องตรวจ",html:appointmentRound186UatHtml(data),confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:500,heightAuto:false})}catch(error){try{Swal.close()}catch{}await showNotice("error",error.message||"ทดสอบรอบ 186 ไม่สำเร็จ")}finally{adminState.busy=false;if(b)b.disabled=false}}

function appointmentFinalUatHtml(data){
  const summary=data?.summary||{},tests=Array.isArray(data?.tests)?data.tests:[],freeze=data?.freeze||{},latest=data?.latest||{},coverage=data?.coverage||{};
  const head=`<div class="appointment-swal-counts"><span><small>ผ่าน</small><b>${Number(summary.passed||0)}/${Number(summary.total||0)}</b></span><span><small>Core P95</small><b>${Math.round(Number(summary.coreP95Ms||0))} ms</b></span><span><small>Match P95</small><b>${Math.round(Number(summary.matchP95Ms||0))} ms</b></span></div>`;
  const baseline=`<div style="margin:10px 0;padding:10px 12px;border-radius:11px;background:#f5f7fb;text-align:left"><b>Baseline Production</b><small style="display:block;margin-top:4px;color:#68758a">${escapeHtml(latest.fileName||'-')} · ${escapeHtml(latest.snapshotDate&&latest.snapshotTime?`${appointmentDisplayDate(latest.snapshotDate)} ${latest.snapshotTime}`:'-')} · ${Number(summary.appointmentCount||0)} นัดหมาย</small><small style="display:block;margin-top:3px;color:#68758a">Coverage รถจริง ${Number(coverage.vehiclesInPlan||0)}/${Number(coverage.vehiclesChecked||0)} · จับคู่จริง ${Number(coverage.actualMatched||0)}</small></div>`;
  const rows=tests.map(t=>`<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;padding:7px 0;border-bottom:1px solid #edf1f7;text-align:left"><div style="min-width:0"><b>${escapeHtml(t.label||t.id||'-')}</b><small style="display:block;color:#68758a;margin-top:2px;overflow-wrap:anywhere">${escapeHtml(t.detail||'')}</small></div><strong style="flex:0 0 auto;white-space:nowrap;color:${t.passed?'#17864b':'#b42318'}">${t.passed?'ผ่าน':'ต้องแก้'}</strong></div>`).join('');
  const activationOn=String(data?.activationState||"OFF")==="ON";
  const freezeLabel=freeze.status==="FROZEN_ACTIVE"?"Production Freeze หลังเปิดใช้งานพร้อม":"Release Freeze ก่อนเปิดใช้งานพร้อม";
  const freezeBox=freeze.recorded?`<div style="margin-top:10px;padding:10px 12px;border-radius:11px;background:#eef9f2;color:#17864b"><b>${freezeLabel}</b><small style="display:block;margin-top:3px">${escapeHtml(freeze.frozenAtDisplay||'-')} · ${activationOn?"ใช้งานจริงเปิดอยู่":"ใช้งานจริงยังปิด"}</small></div>`:`<div style="margin-top:10px;padding:10px 12px;border-radius:11px;background:#fff7e9;color:#915600"><b>ยังไม่ Freeze</b><small style="display:block;margin-top:3px">แก้รายการที่ไม่ผ่านแล้วทดสอบใหม่</small></div>`;
  return`<div>${head}${baseline}<div style="margin-top:6px;max-height:46vh;overflow:auto;padding-right:3px">${rows}</div>${freezeBox}<div style="margin-top:10px;padding:9px 11px;border-radius:10px;background:#f5f7fb;color:#68758a">Final UAT ตรวจตามสถานะจริงโดยไม่เปลี่ยนสวิตช์ · ไม่แก้ Gate In / รถ · วันที่เวลา ${escapeHtml(data?.dateTimeStandard||'dd/MM/yyyy HH:mm:ss')}</div></div>`;
}
async function appointmentRunFinalUat(){
  if(adminState.busy)return;adminState.busy=true;const button=$("appointmentFinalUatButton");if(button)button.disabled=true;
  try{
    Swal.fire({title:"กำลังตรวจ Final Production UAT",text:"ตรวจตามสถานะก่อน/หลังเปิดใช้งาน และบันทึก Freeze เมื่อผ่านครบ",allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>Swal.showLoading(),customClass:swalClasses(),width:430});
    const data=await api("/api/admin/appointment-final-uat",{method:"POST",body:{frontendBuild:FRONTEND_BUILD},timeoutMs:60000,retries:0});
    if(!data.ready){await Swal.fire({icon:"info",title:"ยังทดสอบไม่ได้",text:data.message||"ข้อมูลยังไม่พร้อม",confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:410});return}
    const pass=data?.summary?.allPassed===true&&data?.freeze?.recorded===true;
    await Swal.fire({icon:pass?"success":"warning",title:pass?"Final UAT ผ่าน · Production Freeze พร้อม":"Final UAT มีรายการต้องตรวจ",html:appointmentFinalUatHtml(data),confirmButtonText:"ตกลง",customClass:swalClasses(),buttonsStyling:false,width:550,heightAuto:false});
    adminState.data=await api("/api/admin/settings");renderAdminAppointmentData();
  }catch(error){try{Swal.close()}catch{}await showNotice("error",error.message||"Final Production UAT ไม่สำเร็จ")}finally{adminState.busy=false;if(button)button.disabled=false}
}
...
async function updateAdminVoiceReadyStatus(){const box=$("voiceReadyStatus");if(!box)return;const packId=$("voicePack")?.value||adminState.data.voice?.packId||"th-TH-standard-01",controller=new AbortController(),timer=setTimeout(()=>controller.abort(),10000);try{const base=String(cfg.apiBaseUrl||"").replace(/\/$/,""),response=await fetch(base+"/api/voice/status?packId="+encodeURIComponent(packId),{cache:"no-store",signal:controller.signal}),data=await response.json();if(!response.ok||!data.ready)throw new Error(data.message||"ยังไม่พร้อม");box.classList.add("ready");const c=data.components||{};box.innerHTML=data.supportsPlateProvince?`<b>${escapeHtml(data.packLabel||"ชุดเสียง")} พร้อมใช้งาน</b><span>ตัวเลข ${Number(c.digits||0)}/10 · พยัญชนะ ${Number(c.thaiLetters||0)}/44 · จังหวัด ${Number(c.provinces||0)}/77 · R/S ${Number(c.doorLetters||0)}/2</span>`:`<b>${escapeHtml(data.packLabel||"ชุดเสียง")} พร้อมใช้งาน</b><span>ชุดนี้ใช้การประกาศแบบเดิม ไม่อ่านทะเบียนและจังหวัด</span>`}catch(error){box.classList.remove("ready");box.innerHTML=`<b>ชุดเสียงนี้ยังไม่พร้อม</b><span>${escapeHtml(error?.name==="AbortError"?"ตรวจชุดเสียงใช้เวลานานเกินไป กรุณาลองใหม่":error.message||"กรุณาตรวจสอบชุดเสียง")}</span>`}finally{clearTimeout(timer)}}
...
function currentQueueDisplayBody(overrides={}){
  ...
  return{
    ...
    videoTrafficEnabled:$("queueVideoTrafficEnabled")?.checked??current.videoTrafficEnabled!==false,
    ...overrides
  };
}
async function saveQueueDisplayFromAdmin(overrides={},options={}){
  ...
  if(body.videoEnabled&&!body.videoClassicEnabled&&!body.videoVisualEnabled&&!body.videoTrafficEnabled){await showNotice("warning","กรุณาเลือกหน้าจอที่จะแสดงวิดีโออย่างน้อย 1 แบบ");return null}
...
function renderAdminQueue(){
  ...
  const queueUrl=new URL("./queue.html?v=20260824-r20712",location.href).href;
  const queueDisplay=adminState.data?.queueDisplay||{showDoorPanel:true,displayMode:"CLASSIC",visualTheme:"DARK",fontFamily:"NOTO_SANS_THAI",videoEnabled:false,videoUrl:"",videoSize:"STANDARD",videoSoundEnabled:true,videoVolume:70,videoLoop:true,videoAutoplay:true,videoClassicEnabled:true,videoVisualEnabled:true,videoTrafficEnabled:true};
  const rawDisplayMode=String(queueDisplay.displayMode||"CLASSIC").toUpperCase(),displayMode=["CLASSIC","VISUAL","TRAFFIC"].includes(rawDisplayMode)?rawDisplayMode:"CLASSIC";
  const showDoorPanel=queueDisplay.showDoorPanel!==false,visualTheme=["LIGHT","DARK","NEON"].includes(String(queueDisplay.visualTheme||"").toUpperCase())?String(queueDisplay.visualTheme).toUpperCase():"DARK",fontFamily=["NOTO_SANS_THAI","SARABUN","PROMPT","NOTO_SANS_THAI_LOOPED"].includes(String(queueDisplay.fontFamily||"").toUpperCase())?String(queueDisplay.fontFamily).toUpperCase():"NOTO_SANS_THAI",videoEnabled=queueDisplay.videoEnabled===true,videoUrl=String(queueDisplay.videoUrl||""),videoSize=String(queueDisplay.videoSize||"STANDARD").toUpperCase()==="LARGE"?"LARGE":"STANDARD",videoSoundEnabled=queueDisplay.videoSoundEnabled!==false,videoVolume=Math.max(0,Math.min(100,Number(queueDisplay.videoVolume??70))),videoLoop=queueDisplay.videoLoop!==false,videoAutoplay=queueDisplay.videoAutoplay!==false,videoClassicEnabled=queueDisplay.videoClassicEnabled!==false,videoVisualEnabled=queueDisplay.videoVisualEnabled!==false,videoTrafficEnabled=queueDisplay.videoTrafficEnabled!==false;
  panel.innerHTML=`<div class="admin-section-head clean-admin-head"><div><h3>จอแสดงสถานะคิว</h3><p>กำหนดรูปแบบหน้าจอ สถานะประตู และวิดีโอที่ใช้บนจอคิว</p></div><a class="primary admin-queue-open" href="./queue.html?v=20260824-r20712" target="_blank" rel="noopener">เปิดจอคิว</a></div>
  <section class="admin-form-card admin-queue-display-card queue-display-mode-card"><header><div><b>รูปแบบจอคิว</b><small>เลือกแบบที่เหมาะกับพื้นที่ใช้งาน และสลับกลับได้ทุกเวลา</small></div></header><div class="queue-display-mode-options">
    ...
    <label class="queue-display-choice queue-display-traffic ${displayMode==="TRAFFIC"?"is-selected":""}"><input type="radio" name="queueDisplayMode" value="TRAFFIC" ${displayMode==="TRAFFIC"?"checked":""}><span><b>แบบสัญญาณไฟ</b><small>ใช้ไฟจราจรสื่อขั้นตอนงานและระดับเวลาตามเกณฑ์ที่ตั้งไว้</small></span></label>
  </div></section>
  <section id="queueVisualThemeCard" class="admin-form-card admin-queue-display-card ${["VISUAL","TRAFFIC"].includes(displayMode)?"":"is-muted"}"><header><div><b>ธีมจอ Visual / สัญญาณไฟ</b><small>ใช้ร่วมกับจอ Visual และจอสัญญาณไฟ</small></div></header><div class="queue-theme-choices">
  ...
  <section class="admin-form-card admin-queue-display-card"><header><div><b>การแสดงสถานะประตู</b><small>ใช้ได้กับจอเดิม จอ Visual และจอสัญญาณไฟ</small></div></header><label class="admin-toggle-row"><span><b>แสดงสถานะประตูในจอคิว</b><small>ปิดได้เมื่อต้องการพื้นที่แสดงรถมากขึ้น</small></span><input id="queueDoorPanelEnabled" type="checkbox" ${showDoorPanel?"checked":""}></label></section>
  ...
  const syncQueueDisplayControls=()=>{const mode=document.querySelector('input[name="queueDisplayMode"]:checked')?.value||"CLASSIC",card=$("queueVisualThemeCard");card?.classList.toggle("is-muted",!["VISUAL","TRAFFIC"].includes(mode));document.querySelectorAll(".queue-display-choice").forEach(label=>label.classList.toggle("is-selected",label.querySelector("input")?.checked));document.querySelectorAll(".queue-theme-choice").forEach(label=>label.classList.toggle("is-selected",label.querySelector("input")?.checked));document.querySelectorAll(".queue-font-choice").forEach(label=>label.classList.toggle("is-selected",label.querySelector("input")?.checked));syncQueueVideoAdminControls()};
...
function releaseCandidateSummaryHtml(data){const rc=data?.releaseCandidate;if(!rc)return"";const blockers=Array.isArray(rc.blockers)?rc.blockers:[],cfg=rc.configuration||{},buildMatch=String(rc.build||data.build||"")===String(FRONTEND_BUILD||""),go=rc.decision==="GO"&&rc.ready===true&&buildMatch,tone=go?"pass":"fail",shownBlockers=buildMatch?blockers:[{label:"รุ่นหน้าเว็บและระบบกลางไม่ตรงกัน",action:"นำไฟล์รอบเดียวกันขึ้นระบบให้ครบ"},...blockers];return`<section id="releaseCandidateSummary" class="production-readiness ${tone}"><header><div><b>${escapeHtml(rc.name||"รอบตรวจความพร้อม")}</b><small>ผลสรุปจากการตรวจระบบและการทดสอบหน้างาน</small></div><span class="health-mini ${tone}">${go?"เปิดใช้ได้":"ห้ามเปิด"}</span></header><div class="production-readiness-summary"><span><small>รายการที่ยังไม่ผ่าน</small><b>${Number(shownBlockers.length)}</b></span><span><small>รถปัจจุบัน</small><b>${Number(cfg.activeVehicles||0)}/${Number(cfg.activeVehicleLimit||500)}</b></span><span><small>Dashboard / Datatable</small><b>${cfg.dashboardEnabled?"เปิด":"ปิด"} / ${cfg.datatableEnabled?"เปิด":"ปิด"}</b></span></div>${shownBlockers.length?`<div class="production-readiness-warning"><b>ห้ามเปิดใช้งานจริงจนกว่าจะจัดการครบ</b><span>${shownBlockers.slice(0,6).map(item=>`${escapeHtml(item.label)} — ${escapeHtml(item.action||item.detail||"")}`).join(" · ")}${shownBlockers.length>6?` · และอีก ${shownBlockers.length-6} รายการ`:""}</span></div>`:`<div class="production-readiness-warning"><b>ผ่านเงื่อนไขรอบนี้</b><span>ผลตรวจระบบผ่านและมีการยืนยันการทดสอบหน้างานครบแล้ว</span></div>`}</section>`}
function productionApprovalHtml(data,buildMatch){const approval=data?.approval||{},approved=approval.approved===true&&buildMatch,canApprove=data?.readyForApproval===true&&buildMatch;return`<section class="production-approval ${approved?"approved":canApprove?"waiting":"locked"}"><div><small>การยืนยันขั้นสุดท้าย</small><b>${approved?"ผู้ดูแลอนุมัติเปิดใช้งานแล้ว":canApprove?"พร้อมให้ผู้ดูแลอนุมัติ":"ยังอนุมัติไม่ได้"}</b><span>${approved?`${escapeHtml(approval.approvedByName||"ผู้ดูแล")} · ${approval.approvedAt?escapeHtml(formatDate(approval.approvedAt)):"-"}`:canApprove?"ตรวจสรุป ผู้รับผิดชอบ และแผนรับมือก่อนยืนยัน":"ต้องแก้รายการและทดสอบจริงให้ครบก่อน"}</span>${approved&&approval.note?`<em>${escapeHtml(approval.note)}</em>`:""}</div>${approved?`<button id="productionApprovalRevoke" class="quiet-button" type="button">ยกเลิกการอนุมัติ</button>`:`<button id="productionApprovalConfirm" class="primary" type="button" ${canApprove?"":"disabled"}>ยืนยันเปิดใช้งาน</button>`}</section>`}
function productionReadinessHtml(data){
  if(!data)return"";const summary=data.summary||{},buildMatch=String(data.build||"")===String(FRONTEND_BUILD||""),ready=Boolean(data.releaseReady&&buildMatch),tone=ready?"pass":data.status==="FAIL"?"fail":"warn",manual=Array.isArray(data.manual)?data.manual:[],automated=Array.isArray(data.automated)?data.automated:[],baselineBanner=data.uat?.baselineAt?`<div class="production-readiness-warning"><b>จุดเริ่มตรวจใช้งานจริง</b><span>${escapeHtml(formatDate(data.uat.baselineAt))} · ตรวจรถที่ยังทำงานทั้งหมดและงานที่ปิดหลังเวลานี้</span></div>`:"";
  return `<section class="production-readiness ${tone}"><header><div><b>ความพร้อมก่อนใช้งานจริง</b><small>ผลตรวจอัตโนมัติ + รายการ UAT ที่ต้องยืนยันจากการทดสอบจริง</small></div><span class="health-mini ${tone}">${ready?"พร้อมใช้งานจริง":"ยังไม่พร้อม"}</span></header><div class="production-readiness-summary"><span><small>ตรวจอัตโนมัติ</small><b>${Number(summary.autoPassed||0)}/${Number(summary.autoTotal||0)}</b></span><span><small>UAT ที่ยืนยันแล้ว</small><b>${Number(summary.manualPassed||0)}/${Number(summary.manualTotal||0)}</b></span><span><small>รุ่นหน้าเว็บ / Worker</small><b>${buildMatch?"ตรงกัน":"ไม่ตรงกัน"}</b><em>${escapeHtml(FRONTEND_BUILD)}</em></span></div>${data.uat?.staleBuild?`<div class="production-readiness-warning"><b>รายการ UAT เดิมเป็นคนละรุ่น</b><span>${escapeHtml(data.uat.previousBuild||"")} · ต้องทดสอบใหม่สำหรับรุ่นนี้</span></div>`:""}<div class="production-auto-grid">${automated.map(item=>`<article class="${adminHealthTone(item.status)}"><i></i><div><b>${escapeHtml(item.label)}</b><small>${escapeHtml(item.detail||"")}</small></div><strong>${item.status==="PASS"?"ผ่าน":item.status==="WARN"?"ตรวจดู":"ต้องแก้"}</strong></article>`).join("")}<article class="${buildMatch?"pass":"fail"}"><i></i><div><b>รุ่นหน้าเว็บตรงกับ Worker</b><small>${buildMatch?"ใช้รุ่นเดียวกัน":"หน้าเว็บและ Worker เป็นคนละรุ่น กรุณา Deploy ให้ครบ"}</small></div><strong>${buildMatch?"ผ่าน":"ต้องแก้"}</strong></article></div><div class="production-uat-head"><div><b>รายการทดสอบจริง</b><small>กดผ่านเฉพาะหัวข้อที่ได้ทดสอบหน้างานจริงแล้ว</small></div><button id="productionUatReset" class="quiet-button" type="button">เริ่ม UAT ใหม่</button></div><div class="production-uat-grid">${manual.map(item=>`<button type="button" class="production-uat-item ${item.passed?"passed":""}" data-uat-id="${escapeHtml(item.id)}" data-uat-passed="${item.passed?"1":"0"}"><span class="uat-check">${item.passed?"✓":""}</span><span><small>${escapeHtml(item.group||"")}</small><b>${escapeHtml(item.label)}</b><em>${escapeHtml(item.detail||"")}</em>${item.passed?`<i>${item.byName?escapeHtml(item.byName):"ผู้ดูแล"}${item.at?` · ${escapeHtml(formatDate(item.at))}`:""}</i>`:""}</span></button>`).join("")}</div>${productionApprovalHtml(data,buildMatch)}<footer><b>${ready?"ผ่านเกณฑ์รอบนี้":"ยังไม่ควรเปิดใช้งานจริง"}</b><span>${ready?"ตรวจครบ ทดสอบจริงครบ และผู้ดูแลอนุมัติแล้ว":data.readyForApproval?"รอผู้ดูแลยืนยันขั้นสุดท้าย":"แก้จุดที่ไม่ผ่านและทดสอบจริงให้ครบก่อน"}</span></footer></section>`
}
...
async function saveProductionUatItem(itemId,currentPassed){
  ...
  if(next){const result=await Swal.fire({icon:"question",title:"ยืนยันผลทดสอบ",text:"ระบุสิ่งที่ตรวจ เช่น หมายเลขรถ ช่วงเวลา เครื่องที่ใช้ หรือผลที่เห็น",input:"text",inputPlaceholder:"สิ่งที่ทดสอบหรือหลักฐาน",inputValidator:value=>String(value||"").trim().length<3?"กรุณาระบุสิ่งที่ทดสอบก่อนยืนยัน":undefined,showCancelButton:true,confirmButtonText:"ยืนยันว่าผ่าน",cancelButtonText:"ยกเลิก",reverseButtons:true,customClass:swalClasses(),width:480});if(!result.isConfirmed)return;note=String(result.value||"").trim()}
...
async function resetProductionUat(){const result=await Swal.fire({icon:"warning",title:"เริ่ม UAT ใหม่",html:'<p style="font-size:13px">ผลที่ยืนยันไว้สำหรับรุ่นนี้จะถูกล้าง และเวลาปัจจุบันจะเป็นจุดเริ่มตรวจใช้งานจริง</p><p style="font-size:12px;color:#68758a">รถที่ยังทำงานจะถูกตรวจทั้งหมด ส่วนงานที่ปิดก่อนจุดเริ่มจะเก็บไว้เป็นประวัติและไม่ขวาง UAT รอบนี้</p>',showCancelButton:true,confirmButtonText:"บันทึกจุดเริ่มและเริ่ม UAT",cancelButtonText:"ยกเลิก",reverseButtons:true,customClass:swalClasses(),width:480});if(!result.isConfirmed)return;try{adminReadinessState.data=await api("/api/admin/production-readiness/checklist",{method:"POST",body:{action:"RESET"},timeoutMs:30000,retries:0});renderAdminDiagnosticsContent(adminHealthState.data);await showNotice("success","บันทึกจุดเริ่มและเริ่ม UAT ใหม่แล้ว")}catch(error){await showNotice("error",error.message||"เริ่ม UAT ใหม่ไม่สำเร็จ")}}
async function approveProductionRelease(){const result=await Swal.fire({icon:"warning",title:"ยืนยันเปิดใช้งานจริง",html:'<p style="font-size:13px">ระบุผู้รับผิดชอบ เวลาเปิดใช้ และวิธีรับมือหากมีปัญหา</p><textarea id="approvalNote" class="swal2-textarea" maxlength="300" placeholder="เช่น สมชายดูแล 08:00–17:00 หากผิดปกติหยุดใช้และกลับรุ่นก่อนหน้า"></textarea><input id="approvalPhrase" class="swal2-input" maxlength="30" placeholder="พิมพ์: ยืนยันเปิดใช้งาน">',showCancelButton:true,confirmButtonText:"อนุมัติ",cancelButtonText:"ยกเลิก",reverseButtons:true,customClass:swalClasses(),width:520,preConfirm:()=>{const note=String(document.getElementById("approvalNote")?.value||"").trim(),confirmation=String(document.getElementById("approvalPhrase")?.value||"").trim();if(note.length<10)return Swal.showValidationMessage("กรุณาระบุรายละเอียดอย่างน้อย 10 ตัวอักษร");if(confirmation!=="ยืนยันเปิดใช้งาน")return Swal.showValidationMessage("กรุณาพิมพ์ ยืนยันเปิดใช้งาน ให้ตรงกัน");return{note,confirmation}}});if(!result.isConfirmed)return;try{adminReadinessState.data=await api("/api/admin/production-readiness/approval",{method:"POST",body:result.value,timeoutMs:30000,retries:0});renderAdminDiagnosticsContent(adminHealthState.data);await showNotice("success","บันทึกการอนุมัติแล้ว")}catch(error){await showNotice("error",error.message||"อนุมัติไม่สำเร็จ")}}
async function revokeProductionRelease(){const result=await Swal.fire({icon:"warning",title:"ยกเลิกการอนุมัติ",text:"สถานะจะกลับเป็นห้ามเปิดใช้ จนกว่าจะตรวจและอนุมัติใหม่",showCancelButton:true,confirmButtonText:"ยกเลิกการอนุมัติ",cancelButtonText:"กลับ",reverseButtons:true,customClass:swalClasses(),width:440});if(!result.isConfirmed)return;try{adminReadinessState.data=await api("/api/admin/production-readiness/approval",{method:"POST",body:{action:"REVOKE"},timeoutMs:30000,retries:0});renderAdminDiagnosticsContent(adminHealthState.data);await showNotice("success","ยกเลิกการอนุมัติแล้ว")}catch(error){await showNotice("error",error.message||"ยกเลิกการอนุมัติไม่สำเร็จ")}}
function bindProductionReadinessEvents(){const readiness=document.querySelector(".production-readiness");if(readiness&&adminReadinessState.data?.releaseCandidate&&!$("releaseCandidateSummary"))readiness.insertAdjacentHTML("beforebegin",releaseCandidateSummaryHtml(adminReadinessState.data));const byId=new Map((adminReadinessState.data?.manual||[]).map(item=>[String(item.id),item]));document.querySelectorAll("[data-uat-id]").forEach(button=>{const item=byId.get(String(button.dataset.uatId));if(item?.passed&&item.note){const evidence=document.createElement("i");evidence.className="uat-evidence";evidence.textContent=`หลักฐาน: ${item.note}`;button.lastElementChild?.append(evidence)}button.addEventListener("click",()=>saveProductionUatItem(button.dataset.uatId,button.dataset.uatPassed==="1"))});$("productionUatReset")?.addEventListener("click",resetProductionUat);$("productionApprovalConfirm")?.addEventListener("click",approveProductionRelease);$("productionApprovalRevoke")?.addEventListener("click",revokeProductionRelease)}

function renderAdminDiagnosticsContent(data){const panel=$("adminPanel");if(!panel||adminState.tab!=="health")return;if(!data){panel.innerHTML=`<div class="empty-state"><b>ตรวจระบบไม่สำเร็จ</b><span>${escapeHtml(adminHealthState.error||"กรุณาลองใหม่")}</span><button id="healthRetry" class="primary">ลองใหม่</button></div>`;$("healthRetry")?.addEventListener("click",()=>renderAdminDiagnostics(true));return}const tone=adminHealthTone(data.status),checks=Array.isArray(data.checks)?data.checks:[],repairable=Number(data.repairable?.total||0),latest=data.latestSync,watchdog=data.syncWatchdog;panel.innerHTML=`<div class="admin-section-head clean-admin-head system-health-head"><div><h3>ตรวจระบบ</h3><p>ตรวจฐานข้อมูล การรับข้อมูล ความสัมพันธ์ และค่าที่จำเป็น โดยไม่แก้ข้อมูลปฏิบัติงาน</p></div><div class="system-health-actions"><button id="healthRefresh" class="quiet-button" type="button">ตรวจอีกครั้ง</button><button id="healthPerformance" class="quiet-button" type="button">ทดสอบความเร็ว</button><button id="healthResilience" class="quiet-button" type="button">ทดสอบความทนทาน</button><button id="healthReadiness" class="quiet-button" type="button">ตรวจความพร้อม</button>${repairable?`<button id="healthRepair" class="primary" type="button">ซ่อมส่วนที่ปลอดภัย (${repairable})</button>`:""}</div></div><section class="system-health-summary ${tone}"><div><small>สถานะรวม</small><b>${escapeHtml(data.statusLabel||"-")}</b><span>ใช้เวลา ${Number(data.durationMs||0).toLocaleString("th-TH")} ms</span></div><div><small>Worker</small><b>${escapeHtml(data.build||"-")}</b><span>ตาราง ${Number(data.summary?.tables||0)} · ตัวช่วยค้นหา ${Number(data.summary?.indexes||0)}</span></div><div><small>รับข้อมูล Gate ล่าสุด</small><b>${watchdog?escapeHtml(watchdog.statusLabel||"-"):latest?escapeHtml(String(latest.status||"-")):"ยังไม่มีข้อมูล"}</b><span>${watchdog?.ageLabel?`ผ่านมา ${escapeHtml(watchdog.ageLabel)}`:latest?.finishedAt||latest?.startedAt?formatDate(latest.finishedAt||latest.startedAt):"-"}</span></div></section>${syncWatchdogSettingsHtml(watchdog)}<section class="system-health-grid">${checks.map(item=>`<article class="system-health-check ${adminHealthTone(item.status)}"><span class="system-health-dot"></span><div><b>${escapeHtml(item.label)}</b><p>${escapeHtml(item.detail||"")}</p></div><strong>${item.status==="PASS"?"ผ่าน":item.status==="WARN"?"ตรวจดู":"ต้องแก้"}</strong></article>`).join("")}</section>${adminPerformanceState.data?performanceDiagnosticsHtml(adminPerformanceState.data,adminPerformanceState.client):""}${adminResilienceState.data?resilienceDiagnosticsHtml(adminResilienceState.data,adminResilienceState.client):""}${adminReadinessState.data?productionReadinessHtml(adminReadinessState.data):""}${diagnosticClientHtml()}<section class="system-health-note"><b>ซ่อมส่วนที่ปลอดภัย</b><span>ระบบจัดการเฉพาะรอบรับข้อมูลที่ค้างเกินเวลาที่ผู้ดูแลกำหนด ล้างข้อมูลเข้าสู่ระบบที่หมดอายุตามระยะเวลาที่ผู้ดูแลตั้งไว้ และล้างข้อมูลสรุปชั่วคราว ไม่แก้สถานะรถหรือประวัติขั้นตอนงาน</span></section>`;$("healthRefresh")?.addEventListener("click",()=>renderAdminDiagnostics(true));$("healthPerformance")?.addEventListener("click",runAdminPerformanceDiagnostics);$("healthResilience")?.addEventListener("click",runAdminResilienceDiagnostics);$("healthReadiness")?.addEventListener("click",runProductionReadiness);bindProductionReadinessEvents();$("syncWatchdogSave")?.addEventListener("click",saveSyncWatchdogSettings);$("healthRepair")?.addEventListener("click",async()=>{const ok=await Swal.fire({icon:"warning",title:"ซ่อมส่วนที่ปลอดภัย",text:"จะจัดการเฉพาะรอบรับข้อมูลที่ค้างและข้อมูลเข้าสู่ระบบที่หมดอายุตามค่าที่ผู้ดูแลกำหนด ไม่แก้ข้อมูลรถ",showCancelButton:true,confirmButtonText:"ตรวจและซ่อม",cancelButtonText:"ยกเลิก",reverseButtons:true,customClass:swalClasses(),width:420});if(!ok.isConfirmed)return;const button=$("healthRepair");if(button){button.disabled=true;button.textContent="กำลังตรวจ"}try{const result=await api("/api/admin/diagnostics/repair",{method:"POST",body:{confirmation:"SAFE_REPAIR"},timeoutMs:20000});adminHealthState.data=result.diagnostics;adminHealthState.error="";updateAdminHealthBadge();renderAdminDiagnosticsContent(adminHealthState.data);await showNotice("success",result.message||"ตรวจและซ่อมแล้ว")}catch(error){await showNotice("error",error.message||"ซ่อมไม่สำเร็จ")}})}
...
async function renderAdminDataUsage(){
  ...
  try{
    ...
    panel.innerHTML=`<div class="admin-section-head clean-admin-head data-usage-head"><div><h3>การใช้ข้อมูล</h3><p>ดูพื้นที่ กำหนดอายุข้อมูล ตรวจโครงสร้าง และเก็บข้อมูลย้อนหลังอย่างปลอดภัย</p></div><button id="refreshDataUsage" class="quiet-button" type="button">โหลดใหม่</button></div><nav class="data-tools-tabs" aria-label="ส่วนจัดการข้อมูล"><button type="button" data-data-tool="overview" class="${adminDataTools.tab==="overview"?"active":""}">ภาพรวม</button><button type="button" data-data-tool="retention" class="${adminDataTools.tab==="retention"?"active":""}">อายุข้อมูล</button><button type="button" data-data-tool="structure" class="${adminDataTools.tab==="structure"?"active":""}">โครงสร้าง</button><button type="button" data-data-tool="commands" class="${adminDataTools.tab==="commands"?"active":""}">ชุดคำสั่งตรวจสอบ</button><button type="button" data-data-tool="archive" class="${adminDataTools.tab==="archive"?"active":""}">เก็บข้อมูลย้อนหลัง</button><button type="button" data-data-tool="history" class="${adminDataTools.tab==="history"?"active":""}">ประวัติการเก็บ</button><button type="button" data-data-tool="cleanup" class="${adminDataTools.tab==="cleanup"?"active":""}">เตรียมเคลียร์</button></nav><section id="dataToolsBody"></section>`;
    ...
    $("refreshDataUsage")?.addEventListener("click",()=>{adminDataTools.inspectorBySource={};adminDataTools.retention=null;adminDataTools.retentionPreview=null;adminDataTools.retentionPreviewError="";renderAdminDataUsage()});
...
async function renderAdminDataToolBody(data){
  ...
  if(adminDataTools.tab==="retention"){await renderAdminDataRetention(body);return}
  ...
  const source=adminDataTools.source||"main";
  if(!adminDataTools.inspectorBySource[source]){
    ...
    try{adminDataTools.inspectorBySource[source]=await api(`/api/admin/data-inspector?source=${encodeURIComponent(source)}`)}catch(error){body.innerHTML=`<div class="empty-state"><b>อ่านข้อมูลฐานข้อมูลไม่สำเร็จ</b><span>${escapeHtml(error.message)}</span><button id="retryDataInspector" class="primary">ลองใหม่</button></div>`;$("retryDataInspector")?.addEventListener("click",()=>{delete adminDataTools.inspectorBySource[source];renderAdminDataToolBody(data)});return}finally{adminDataTools.busy=false}
  }
  const inspector=adminDataTools.inspectorBySource[source];
  if(inspector?.connected===false){renderDisconnectedDataSource(body,inspector);return}
  if(adminDataTools.tab==="structure")renderAdminDataStructure(body,inspector);else renderAdminSqlCommands(body,inspector);
}
...
function renderAdminDataOverview(body,data){
  ...
  body.innerHTML=`<section class="data-usage-summary"><article class="data-usage-main ${tone}"><div><small>พื้นที่ฐานข้อมูล</small><b>${formatStorage(used)} <em>/ ${formatStorage(limit)}</em></b><span>${status}</span></div><div class="data-usage-ring" style="--usage:${percent}%"><strong>${percent.toFixed(1)}%</strong></div><div class="data-usage-progress"><i style="width:${percent}%"></i></div><footer><span>ใช้แล้ว <b>${formatStorage(used)}</b></span><span>คงเหลือ <b>${formatStorage(remaining)}</b></span></footer></article><article class="data-plan-card"><small>แพ็กเกจปัจจุบัน</small><b>${data.plan==="PAID"?"แบบชำระเงิน":"แบบฟรี"}</b><span>ขีดจำกัดต่อฐานข้อมูล ${formatStorage(limit)}</span><hr><small>การอ่านจากการตรวจหน้านี้</small><b>${Number(data.readRowsThisCheck||0).toLocaleString("th-TH")} แถว</b><span>เป็นเฉพาะการตรวจครั้งนี้ ไม่ใช่ยอดรวมที่ Cloudflare แสดง</span></article></section><section class="data-count-grid">${dataCountCard("รถทั้งหมด",c.vehicles)}${dataCountCard("รถที่ยังอยู่ในพื้นที่",c.activeVehicles)}${dataCountCard("ประวัติขั้นตอน",c.workflowEvents)}${dataCountCard("ประวัติการเรียกรถ",c.queueCalls)}${dataCountCard("ปฏิเสธรับสินค้า",c.receivingRejections)}${dataCountCard("ประวัติการจัดการ",c.auditLogs)}${dataCountCard("ผู้ใช้งาน",c.users)}${dataCountCard("ประตู",c.doors)}${dataCountCard("รอบรับข้อมูล",c.syncRuns)}</section><div class="data-safe-note data-read-note"><b>รอบ 198 ลดการอ่านซ้ำ</b><span>การตรวจว่าหน้า Inbound มีข้อมูลเปลี่ยนหรือไม่ ถูกลดให้ดูเฉพาะข้อมูลจำเป็น แทนการไล่อ่านประวัติขั้นตอนทุก 5 วินาที</span></div><section class="data-history-card"><header><div><h3>ประวัติการทำงานล่าสุด</h3><p>รายการสำคัญที่ระบบบันทึกไว้</p></div><span>${formatDate(data.generatedAt)}</span></header><div class="data-history-list">${renderDataActivity(data.activity||[])}</div></section>`;
}


function dataRetentionNumber(id,fallback,min,max){const n=Math.round(Number($(id)?.value));return Number.isFinite(n)?Math.min(max,Math.max(min,n)):fallback}
function renderAdminRetentionPreviewArea(){
  const area=$("retentionPreviewArea");if(!area)return;
  if(adminDataTools.retentionPreviewBusy){area.innerHTML=`<section class="retention-preview"><div class="data-inspector-loading"><span></span><b>กำลังตรวจข้อมูลที่ถึงอายุ</b><small>เป็นการนับเพื่อแสดงตัวอย่างเท่านั้น ยังไม่มีการลบข้อมูล</small></div></section>`;return}
  const data=adminDataTools.retentionPreview;
  if(!data){area.innerHTML=`<section class="retention-preview"><header><div><h3>ข้อมูลที่ถึงอายุตามค่าปัจจุบัน</h3><p>แยกการนับออกจากการเปิดหน้าตั้งค่า เพื่อให้หน้าเปิดเร็วและลดการอ่าน D1 ที่ไม่จำเป็น</p></div><button id="loadRetentionPreview" class="quiet-button" type="button">ตรวจข้อมูลที่ถึงอายุ</button></header>${adminDataTools.retentionPreviewError?`<div class="empty-state"><b>ตรวจข้อมูลไม่สำเร็จ</b><span>${escapeHtml(adminDataTools.retentionPreviewError)}</span></div>`:`<div class="data-safe-note"><b>ยังไม่ได้ตรวจ</b><span>ค่าตั้งด้านบนสามารถแก้และบันทึกได้ทันที กดตรวจเฉพาะเมื่อต้องการดูจำนวนข้อมูลที่ถึงอายุ</span></div>`}</section>`;$("loadRetentionPreview")?.addEventListener("click",loadAdminRetentionPreview);return}
  const c=data.candidates||{},a=data.appointment?.candidates||{};
  area.innerHTML=`<section class="retention-preview"><header><div><h3>ข้อมูลที่ถึงอายุตามค่าปัจจุบัน</h3><p>เป็นการนับเพื่อให้ผู้ดูแลเห็นก่อน ยังไม่มีการลบ</p></div><button id="loadRetentionPreview" class="quiet-button" type="button">ตรวจอีกครั้ง</button></header><div>${dataCountCard("รถที่จบงานและถึงอายุ",c.closedVehicles)}${dataCountCard("ประวัติการจัดการที่ถึงอายุ",c.oldAuditLogs)}${dataCountCard("ข้อมูลเข้าสู่ระบบที่ถึงอายุ",c.expiredSessions)}${dataCountCard("ข้อมูลนัดหมายที่พร้อมจัดการ",c.appointment)}</div>${data.appointment?.ready?`<footer><span>นัดหมายหมดอายุ ${Number(a.expiredAppointments||0).toLocaleString("th-TH")}</span><span>ประวัตินำเข้า ${Number(a.oldImportHistory||0).toLocaleString("th-TH")}</span><span>ประวัติเปลี่ยนแปลง ${Number(a.oldChangeHistory||0).toLocaleString("th-TH")}</span><span>ผลเทียบเวลา ${Number(a.oldMetrics||0).toLocaleString("th-TH")}</span></footer>`:`${data.appointment?.error?`<div class="empty-state"><b>ตรวจข้อมูลนัดหมายไม่สำเร็จ</b><span>${escapeHtml(data.appointment.error)}</span></div>`:""}`}</section>`;
  $("loadRetentionPreview")?.addEventListener("click",loadAdminRetentionPreview)
}
async function loadAdminRetentionPreview(){
  if(adminDataTools.retentionPreviewBusy)return;adminDataTools.retentionPreviewBusy=true;adminDataTools.retentionPreviewError="";renderAdminRetentionPreviewArea();
  try{adminDataTools.retentionPreview=await api("/api/admin/data-retention/preview",{timeoutMs:45000,retries:0})}
  catch(error){adminDataTools.retentionPreview=null;adminDataTools.retentionPreviewError=error.message||"ตรวจข้อมูลที่ถึงอายุไม่สำเร็จ"}
  finally{adminDataTools.retentionPreviewBusy=false;renderAdminRetentionPreviewArea()}
}
async function renderAdminDataRetention(body){
  if(!adminDataTools.retention){body.innerHTML=`<div class="data-inspector-loading"><span></span><b>กำลังอ่านค่าการเก็บข้อมูล</b><small>โหลดเฉพาะค่าตั้งก่อน เพื่อให้เปิดหน้าได้รวดเร็ว</small></div>`;if(adminDataTools.retentionBusy)return;adminDataTools.retentionBusy=true;try{adminDataTools.retention=await api("/api/admin/data-retention",{timeoutMs:15000,retries:0})}catch(error){body.innerHTML=`<div class="empty-state"><b>อ่านอายุข้อมูลไม่สำเร็จ</b><span>${escapeHtml(error.message)}</span><button id="retryRetention" class="primary">ลองใหม่</button></div>`;$("retryRetention")?.addEventListener("click",()=>{adminDataTools.retention=null;renderAdminDataRetention(body)});return}finally{adminDataTools.retentionBusy=false}}
  const data=adminDataTools.retention||{},r=data.settings||{};
  body.innerHTML=`<section class="retention-head"><div><h3>อายุการเก็บข้อมูล</h3><p>ผู้ดูแลเป็นผู้กำหนดระยะเวลาเอง การล้างอัตโนมัติปิดเป็นค่าเริ่มต้น</p></div><span class="retention-state ${r.autoCleanupEnabled?"on":"off"}">${r.autoCleanupEnabled?"อนุญาตตามค่าที่ตั้ง":"ปิดการล้างอัตโนมัติ"}</span></section><div class="data-safe-note"><b>กฎความปลอดภัย</b><span>ข้อมูลที่ต้องเก็บย้อนหลังจะต้องผ่าน เก็บสำรอง → ตรวจไฟล์ → ตรวจจำนวนให้ตรง → ผู้ดูแลยืนยัน ก่อนเคลียร์จริง</span></div><form id="dataRetentionForm" class="retention-form"><section class="retention-switches"><label><span><b>อนุญาตการล้างอัตโนมัติ</b><small>ค่าเริ่มต้นปิด การทำงานจะเกิดเฉพาะส่วนที่ผู้ดูแลเปิดไว้</small></span><input id="retentionAutoCleanup" type="checkbox" ${r.autoCleanupEnabled?"checked":""}></label><label><span><b>ล้างข้อมูลการเข้าสู่ระบบที่หมดอายุ</b><small>ทำงานเมื่อเปิดการล้างอัตโนมัติเท่านั้น</small></span><input id="retentionSessionsCleanup" type="checkbox" ${r.cleanupExpiredSessions?"checked":""}></label><label><span><b>อนุญาตจัดการข้อมูลนัดหมายตามอายุ</b><small>ใช้เป็นกฎจากผู้ดูแล แต่จะไม่ลบทันทีหลังอัปโหลด ข้อมูลต้องผ่านขั้นตอนสำรองก่อน</small></span><input id="retentionAppointmentCleanup" type="checkbox" ${r.cleanupAppointmentData?"checked":""}></label></section><section class="retention-grid"><label><span>รถที่จบงานแล้ว</span><div><input id="retentionClosedVehicles" type="number" min="30" max="1095" value="${Number(r.closedVehicleDays||365)}"><em>วัน</em></div><small>ครบอายุแล้วจะรอเก็บย้อนหลัง ไม่ลบเองในรอบนี้</small></label><label><span>ประวัติการจัดการ</span><div><input id="retentionAudit" type="number" min="30" max="1095" value="${Number(r.auditLogDays||365)}"><em>วัน</em></div><small>ใช้เป็นเกณฑ์สำหรับรอบเก็บย้อนหลังถัดไป</small></label><label><span>ข้อมูลการเข้าสู่ระบบที่หมดอายุ</span><div><input id="retentionSessions" type="number" min="1" max="30" value="${Number(r.expiredSessionDays||7)}"><em>วัน</em></div><small>นับหลังหมดอายุแล้ว</small></label><label><span>ข้อมูลนัดหมายที่หมดอายุ</span><div><input id="retentionAppointments" type="number" min="14" max="365" value="${Number(r.appointmentRetentionDays||60)}"><em>วัน</em></div><small>รถที่ยังทำงานอยู่จะถูกเก็บไว้</small></label><label><span>ประวัติการนำเข้าไฟล์</span><div><input id="retentionImports" type="number" min="30" max="730" value="${Number(r.importHistoryRetentionDays||180)}"><em>วัน</em></div><small>เก็บชุดล่าสุดไว้ตามกฎของระบบ</small></label><label><span>ประวัติการเปลี่ยนข้อมูลนัดหมาย</span><div><input id="retentionChanges" type="number" min="30" max="730" value="${Number(r.changeHistoryRetentionDays||90)}"><em>วัน</em></div></label><label><span>ผลเทียบเวลานัดกับเวลามาถึง</span><div><input id="retentionComparison" type="number" min="90" max="1095" value="${Number(r.kpiRetentionDays||365)}"><em>วัน</em></div></label><label><span>ประวัติการคำนวณย้อนหลัง</span><div><input id="retentionBackfill" type="number" min="30" max="730" value="${Number(r.backfillRetentionDays||90)}"><em>วัน</em></div></label><label><span>งานนำเข้าไฟล์ที่ยังไม่เสร็จ</span><div><input id="retentionIncomplete" type="number" min="1" max="30" value="${Number(r.incompleteImportRetentionDays||7)}"><em>วัน</em></div><small>ใช้แจ้งเตือนเท่านั้น ไม่ลบ เพื่อให้ทำต่อได้</small></label></section><section id="retentionPreviewArea"></section><div class="admin-form-actions"><button class="primary" type="submit">บันทึกอายุข้อมูล</button></div></form>`;
  renderAdminRetentionPreviewArea();
  $("dataRetentionForm")?.addEventListener("submit",async event=>{event.preventDefault();if(adminDataTools.retentionSaving)return;adminDataTools.retentionSaving=true;const button=event.submitter||event.currentTarget.querySelector('button[type="submit"]');if(button){button.disabled=true;button.textContent="กำลังบันทึก"}try{const payload={autoCleanupEnabled:Boolean($("retentionAutoCleanup")?.checked),cleanupExpiredSessions:Boolean($("retentionSessionsCleanup")?.checked),cleanupAppointmentData:Boolean($("retentionAppointmentCleanup")?.checked),closedVehicleDays:dataRetentionNumber("retentionClosedVehicles",365,30,1095),auditLogDays:dataRetentionNumber("retentionAudit",365,30,1095),expiredSessionDays:dataRetentionNumber("retentionSessions",7,1,30),appointmentRetentionDays:dataRetentionNumber("retentionAppointments",60,14,365),importHistoryRetentionDays:dataRetentionNumber("retentionImports",180,30,730),changeHistoryRetentionDays:dataRetentionNumber("retentionChanges",90,30,730),kpiRetentionDays:dataRetentionNumber("retentionComparison",365,90,1095),backfillRetentionDays:dataRetentionNumber("retentionBackfill",90,30,730),incompleteImportRetentionDays:dataRetentionNumber("retentionIncomplete",7,1,30),keepLatestImports:Number(r.keepLatestImports||20)};const saved=await api("/api/admin/data-retention",{method:"POST",body:payload});adminDataTools.retention={...data,settings:saved.settings||payload,generatedAt:unixNow(),previewDeferred:true};adminDataTools.retentionPreview=null;adminDataTools.retentionPreviewError="";renderAdminDataRetention(body);await showNotice("success","บันทึกอายุการเก็บข้อมูลแล้ว")}catch(error){await showNotice("error",error.message||"บันทึกอายุข้อมูลไม่สำเร็จ")}finally{adminDataTools.retentionSaving=false;if(button&&document.body.contains(button)){button.disabled=false;button.textContent="บันทึกอายุข้อมูล"}}})
}
function dataSourceToolbar(data){const sources=Array.isArray(data?.sources)?data.sources:[],current=data?.source||sources.find(item=>item.key===adminDataTools.source)||sources[0]||{key:"main",label:"ฐานงานหลัก",name:"warehouse-vehicle-flow"};return`<section class="data-source-toolbar"><div><small>ฐานที่กำลังดู</small><b>${escapeHtml(current.label||"ฐานข้อมูล")}</b><span>${escapeHtml(current.name||"")}${current.mode?` · ${escapeHtml(current.mode)}`:""}</span></div><label><span>เลือกฐานข้อมูล</span><select id="dataSourceSelect">${sources.map(item=>`<option value="${escapeHtml(item.key)}" ${item.key===current.key?"selected":""}>${escapeHtml(item.label)}${item.connected===false?" · ยังไม่เชื่อม":""}</option>`).join("")}</select></label></section>`}
function bindDataSourceToolbar(body){$("dataSourceSelect")?.addEventListener("change",async event=>{adminDataTools.source=String(event.target.value||"main");adminDataTools.openTable="";adminDataTools.openSchema="";adminDataTools.openCommand="";delete adminDataTools.inspectorBySource?.[adminDataTools.source];await renderAdminDataToolBody({})})}
function renderDisconnectedDataSource(body,data){
  const guide=data?.connectionGuide||{},connectionName=guide.connectionName||"APPOINTMENT_DEV_DB",databaseName=guide.databaseName||data?.source?.name||"warehouse-vehicle-appointment-dev",steps=Array.isArray(guide.steps)&&guide.steps.length?guide.steps:["เปิด Worker ที่ระบบนี้ใช้งาน แล้วเข้า Settings > Bindings","เพิ่มการเชื่อม D1 Database","ตั้งชื่อการเชื่อมเป็น APPOINTMENT_DEV_DB",`เลือกฐาน ${databaseName} แล้วบันทึก/Deploy รุ่นใหม่`];
  body.innerHTML=`${dataSourceToolbar(data)}<section class="data-source-connect-card"><header><div><small>ฐานนัดหมายแยก</small><h3>ยังไม่ได้เชื่อมเข้าหน้าผู้ดูแล</h3><p>ฐานงานหลักยังทำงานตามปกติ ส่วนฐานนี้ใช้สำหรับตรวจสอบและสำรองเท่านั้น</p></div><span>ยังไม่เชื่อม</span></header><div class="data-source-connect-grid"><article><small>ชื่อฐาน</small><b>${escapeHtml(databaseName)}</b></article><article><small>ชื่อการเชื่อมที่แนะนำ</small><b>${escapeHtml(connectionName)}</b></article></div><div class="data-source-connect-steps">${steps.map((step,index)=>`<div><i>${index+1}</i><span>${escapeHtml(step)}</span></div>`).join("")}</div><footer><button id="copySecondaryConnectionName" class="quiet-button" type="button">คัดลอกชื่อการเชื่อม</button><button id="retrySecondaryDatabase" class="primary" type="button">ตรวจการเชื่อมอีกครั้ง</button></footer><div class="data-safe-note"><b>ปลอดภัย</b><span>การเชื่อมส่วนนี้ใช้เพื่ออ่านโครงสร้าง คัดลอก และดาวน์โหลดเท่านั้น ไม่ย้าย ไม่รวม และไม่ลบข้อมูลจากสองฐาน</span></div></section>`;
  bindDataSourceToolbar(body);
  $("copySecondaryConnectionName")?.addEventListener("click",event=>copyAdminText(connectionName,event.currentTarget));
  $("retrySecondaryDatabase")?.addEventListener("click",async event=>{const button=event.currentTarget;button.disabled=true;button.textContent="กำลังตรวจ";try{delete adminDataTools.inspectorBySource?.appointment;adminDataTools.source="appointment";await renderAdminDataToolBody({})}finally{if(document.body.contains(button)){button.disabled=false;button.textContent="ตรวจการเชื่อมอีกครั้ง"}}});
}
function downloadAdminSql(text,filename){const value=String(text||"").trim();if(!value)return;const blob=new Blob([value],{type:"text/plain;charset=utf-8"}),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=filename.endsWith(".sql")?filename:`${filename}.sql`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1500)}
...
async function renderAdminArchiveHistory(body){
  ...
  if(!data.connected){body.innerHTML=`<section class="archive-history-head"><div><h3>ประวัติการเก็บข้อมูล</h3><p>ข้อมูลส่วนนี้จะแสดงเมื่อเชื่อมพื้นที่เก็บข้อมูลสำรอง แล้ว</p></div></section><div class="cleanup-stop-note"><b>ยังไม่ได้เชื่อมพื้นที่สำรอง</b><span>ไฟล์ที่ดาวน์โหลดไว้ในเครื่องยังใช้เก็บสำรองได้ตามปกติ</span></div>`;return}
  body.innerHTML=`<section class="archive-history-head"><div><h3>ประวัติการเก็บข้อมูล</h3><p>แสดงชุดข้อมูลล่าสุดของแต่ละเดือนที่เก็บไว้ในพื้นที่สำรอง</p></div><button id="archiveHistoryRefresh" class="quiet-button" type="button">โหลดใหม่</button></section><div class="archive-history-list">${items.map(item=>`<article><div class="archive-history-main"><b>${escapeHtml(item.periodLabel||item.month||"-")}</b><span>เก็บ ${formatDate(item.createdAt)} · รถ ${Number(item.readyVehicles||0).toLocaleString("th-TH")} รายการ</span></div><div class="archive-history-meta"><span>${(item.files||[]).length} ไฟล์</span><span>${escapeHtml(item.createdBy||"ระบบ")}</span><button type="button" data-verify-month="${escapeHtml(item.month||"")}">ตรวจความครบถ้วน</button></div><div class="archive-history-files">${item.report?.key?`<button type="button" class="primary" data-r2-file="${escapeHtml(item.report.key)}">${String(item.report.filename||"").toLowerCase().endsWith(".csv")?"ดาวน์โหลด CSV พร้อมใช้":"ดาวน์โหลดไฟล์พร้อมใช้"}</button>`:""}${(item.files||[]).map(file=>`<button type="button" data-r2-file="${escapeHtml(file.key||"")}">${escapeHtml(file.label||file.filename||"ไฟล์")}</button>`).join("")}</div>${adminDataTools.archiveVerify?.month===item.month?renderArchiveVerifyResult(adminDataTools.archiveVerify):""}</article>`).join("")||`<div class="empty-state">ยังไม่มีข้อมูลที่เก็บสำรอง</div>`}</div>`;
...
function renderArchiveVerifyResult(data){
  const files=Array.isArray(data.files)?data.files:[],complete=Boolean(data.complete),report=data.report||null,totalParts=files.length+(report?1:0),passed=files.filter(x=>x.ok).length+(report?.ok?1:0);
  return `<section class="archive-verify ${complete?"ok":"warn"}"><header><div><small>ผลตรวจข้อมูลสำรอง</small><b>${complete?"ครบและตรงกัน":"ต้องตรวจสอบ"}</b><span>${escapeHtml(data.message||"")}</span></div><strong>${passed}/${totalParts} ส่วน</strong></header>${files.length?`<div class="archive-verify-files">${files.map(file=>`<span class="${file.ok?"ok":"bad"}"><b>${escapeHtml(file.label||file.id||"ไฟล์")}</b><small>${Number(file.rows||0).toLocaleString("th-TH")} แถว · ${Number(file.columnCount||0).toLocaleString("th-TH")} ช่อง ${file.ok?"· ครบ":`· ${escapeHtml(file.reason||"ต้องตรวจ")}`}</small></span>`).join("")}${report?`<span class="${report.ok?"ok":"bad"}"><b>CSV พร้อมใช้</b><small>${Number(report.rows||0).toLocaleString("th-TH")} แถว ${report.ok?"· ครบ":`· ${escapeHtml(report.reason||"ต้องตรวจ")}`}</small></span>`:""}</div>`:""}</section>`;
}
...
async function renderAdminCleanup(body){
  ...
  const data=adminDataTools.cleanupPreview||{},safe=Boolean(data.safe),ready=Number(data.readyVehicles||0),open=Number(data.openVehicles||0),related=(data.counts||[]).reduce((sum,item)=>sum+Number(item.rows||0),0),messages=Array.isArray(data.messages)?data.messages:[],r2Connected=Boolean(data.archive?.connected),r2Stored=Boolean(data.archive?.stored),r2Complete=Boolean(data.archive?.complete),r2Current=Boolean(data.archive?.currentMatches),retentionDays=Number(data.retention?.closedVehicleDays||0),historyItems=Array.isArray(adminDataTools.cleanupHistory?.items)?adminDataTools.cleanupHistory.items:[];
  body.innerHTML=`<section class="cleanup-head"><div><h3>เตรียมเคลียร์ข้อมูลเก่า</h3><p>ทำตามอายุที่ผู้ดูแลกำหนด และเคลียร์เฉพาะรถที่จบงานแล้ว</p></div><label><span>เดือนที่ต้องการ</span><input id="cleanupMonth" type="month" value="${escapeHtml(adminDataTools.cleanupMonth)}" min="2020-01" max="2100-12"></label><button id="cleanupCheck" class="quiet-button" type="button">ตรวจสอบ</button></section><div class="${safe?"data-safe-note":"cleanup-stop-note"}"><b>${safe?"พร้อมตรวจขั้นสุดท้าย":"ยังไม่พร้อม"}</b><span>${safe?"สำเนาครบ และจำนวนข้อมูลปัจจุบันยังตรงกับชุดที่เก็บไว้":"ระบบจะไม่ให้เคลียร์จนกว่าจะผ่านทุกข้อ"}</span></div><section class="cleanup-summary"><article><small>อายุข้อมูลที่กำหนด</small><b>${retentionDays?retentionDays.toLocaleString("th-TH"):"-"} วัน</b></article><article class="ok"><small>พร้อมเคลียร์</small><b>${ready.toLocaleString("th-TH")}</b></article><article class="keep"><small>ต้องเก็บไว้</small><b>${open.toLocaleString("th-TH")}</b><span>ยังไม่จบงาน</span></article><article><small>ข้อมูลที่เกี่ยวข้อง</small><b>${related.toLocaleString("th-TH")}</b></article></section>${messages.length?`<div class="cleanup-messages">${messages.map(message=>`<span>${escapeHtml(message)}</span>`).join("")}</div>`:""}<section class="cleanup-checks"><header><h3>ตรวจความพร้อม</h3><span>ต้องผ่านทุกข้อก่อน</span></header>${(data.checks||[]).map(item=>`<div class="${item.ok?"pass":"fail"}"><i>${item.ok?"✓":"!"}</i><span>${escapeHtml(item.label)}</span><b>${item.ok?"ผ่าน":"ตรวจสอบ"}</b></div>`).join("")}</section><section class="cleanup-counts"><header><h3>ข้อมูลที่จะเกี่ยวข้อง</h3><span>จำนวนนี้ต้องตรงกับชุดสำรองก่อนเคลียร์</span></header>${(data.counts||[]).map(item=>`<div><span>${escapeHtml(item.label)}</span><b>${Number(item.rows||0).toLocaleString("th-TH")} แถว</b></div>`).join("")}</section>${r2Connected?`<div class="cleanup-r2-state ${r2Complete&&r2Current?"ready":"missing"}"><div><b>${r2Complete&&r2Current?"สำเนาครบ และข้อมูลปัจจุบันตรงกัน":r2Complete?"สำเนาครบ แต่ข้อมูลใน D1 เปลี่ยนแล้ว":r2Stored?"มีสำเนา แต่ยังตรวจไม่ผ่าน":"ยังไม่มีชุดสำรอง"}</b><span>${r2Complete&&r2Current?`ชุดสำรอง ${escapeHtml(data.archive?.archiveId||"-")} · ${formatDate(data.archive?.createdAt)}`:r2Complete?"ให้กลับไปเก็บเดือนนี้ใหม่ก่อนเคลียร์":r2Stored?"กลับไปตรวจความครบถ้วนหรือเก็บใหม่ก่อน":"กลับไปที่ “เก็บข้อมูลย้อนหลัง” แล้วเก็บเดือนนี้ก่อน"}</span></div>${r2Complete&&r2Current?`<strong>พร้อม</strong>`:""}</div>`:`<div class="cleanup-r2-state missing"><div><b>ยังไม่ได้เชื่อมพื้นที่เก็บข้อมูลสำรอง</b><span>Round 199 ไม่อนุญาตให้เคลียร์ข้อมูลจริงหากไม่มีสำเนาที่ตรวจผ่าน</span></div></div>`}<label class="cleanup-confirm"><input id="cleanupArchiveConfirmed" type="checkbox" ${safe&&r2Complete&&r2Current?"":"disabled"}><span><b>ฉันตรวจสอบชุดสำรองและจำนวนข้อมูลแล้ว</b><small>หากข้อมูลเปลี่ยนหลังจากนี้ ระบบจะหยุดและให้ตรวจใหม่โดยอัตโนมัติ</small></span></label><div class="cleanup-actions"><button id="cleanupVerify" class="quiet-button" type="button">ตรวจผลหลังเคลียร์</button><button id="cleanupBuild" class="quiet-button" type="button" ${safe?"":"disabled"}>สร้างชุดคำสั่ง</button><button id="cleanupExecute" class="danger-button" type="button" disabled>เคลียร์ข้อมูลจริง</button></div><section id="cleanupVerifyArea"></section><section id="cleanupScriptArea"></section>${renderCleanupHistory(historyItems)}</section>`;
  ...
  const updateCleanupButtons=()=>{if(build)build.disabled=!safe||!confirm?.checked;if(execute)execute.disabled=!safe||!r2Complete||!r2Current||!confirm?.checked||adminDataTools.cleanupExecuteBusy};confirm?.addEventListener("change",updateCleanupButtons);updateCleanupButtons();
...
function renderAdminDataStructure(body,data){
  ...
  $("downloadSchema")?.addEventListener("click",()=>downloadAdminSql(data.schemaSql||"",`${base}_Structure.sql`));
}
...
function renderAdminSqlCommands(body,data){
  const commands=Array.isArray(data.commands)?data.commands:[],groups=[...new Set(commands.map(item=>item.group))],base=String(data.downloadBaseName||"database");
  body.innerHTML=`${dataSourceToolbar(data)}<section class="data-commands-head"><div><h3>ชุดคำสั่งตรวจสอบ</h3><p>คำสั่งชุดนี้ใช้ดูและตรวจข้อมูล สามารถคัดลอกหรือเก็บเป็นไฟล์สำรองได้</p></div><div class="data-structure-actions"><button id="copyAllCommands" class="quiet-button" type="button">คัดลอกทั้งหมด</button><button id="downloadCommands" class="primary" type="button">ดาวน์โหลดไฟล์</button></div></section><div class="data-safe-note"><b>มีเฉพาะคำสั่งดูข้อมูล</b><span>ไม่มีคำสั่งลบ เปลี่ยน หรือสร้างตาราง</span></div><section class="archive-full-backup data-full-backup"><div><h3>สำรองทั้งฐานข้อมูล</h3><p>คัดลอกคำสั่งนี้ไว้สำหรับสำรองข้อมูลทั้งฐานก่อนงานสำคัญ</p></div><button id="copySelectedFullBackup" class="quiet-button" type="button">คัดลอกคำสั่ง</button><pre>${escapeHtml(String(data.fullBackupCommand||""))}</pre></section><div class="data-command-groups">${groups.map(group=>`<section><h4>${escapeHtml(group)}</h4>${commands.filter(item=>item.group===group).map(item=>renderDataCommandCard(item)).join("")}</section>`).join("")}</div>`;
  bindDataSourceToolbar(body);
  ...
  $("copyAllCommands")?.addEventListener("click",event=>copyAdminText(String(data.commandsSql||""),event.currentTarget));
  $("downloadCommands")?.addEventListener("click",()=>downloadAdminSql(data.commandsSql||"",`${base}_Check_Commands.sql`));
  $("copySelectedFullBackup")?.addEventListener("click",event=>copyAdminText(String(data.fullBackupCommand||""),event.currentTarget));
}
...
function downloadAdminSchema(sql){const date=new Intl.DateTimeFormat("en-CA",{timeZone:cfg.timezone,year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date()).replace(/-/g,"");downloadAdminSql(sql,`Warehouse_Vehicle_D1_Structure_${date}.sql`)}


function openProductionExportPage(){adminState.tab="export";try{localStorage.setItem("wvf_admin_tab","export")}catch{}navigate("admin")}
...
function currentBangkokDate(){const parts=new Intl.DateTimeFormat("en-CA",{timeZone:cfg.timezone,year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(new Date());const p=Object.fromEntries(parts.map(x=>[x.type,x.value]));return `${p.year}-${p.month}-${p.day}`}
function productionExportMonthDates(month){const m=String(month||"").match(/^(\d{4})-(\d{2})$/);if(!m)return null;const y=Number(m[1]),mo=Number(m[2]),last=new Date(Date.UTC(y,mo,0)).getUTCDate();return{from:`${m[1]}-${m[2]}-01`,to:`${m[1]}-${m[2]}-${String(last).padStart(2,"0")}`}}
const productionExportState={meta:null,data:null,key:"",busy:false};
const PRODUCTION_EXPORT_DATASETS=[
  ["vehicles","ข้อมูลรถและเวลาทำงาน","สร้างเสมอ · รถหนึ่งคันต่อหนึ่งแถว พร้อมขั้นตอน เวลา และผลเทียบเกณฑ์ Admin"],
  ["events","เหตุการณ์ละเอียด","สำหรับตรวจย้อนหลังเมื่อจำเป็น เช่น เรียกรถซ้ำหรือเปลี่ยนประตู"],
  ["plan","Plan vs Actual","สรุปแผนเทียบรถจริงและความตรงต่อเวลา"],
  ["hourly","รายชั่วโมง","แผน / เข้าจริง / จับคู่ แยกตามชั่วโมง"],
  ["vendor","บริษัทผู้ส่งสินค้า","สรุป KPI ตามบริษัทจากไฟล์นัดหมาย"],
  ["carrier","ผู้ขนส่ง","สรุป KPI ตามผู้ขนส่ง"],
  ["po","เลข PO","สรุป KPI ตาม PO"],
  ["quality","คุณภาพข้อมูล","ใช้ตรวจข้อมูลนัดหมายที่ยังจับคู่ไม่ได้หรือผิดกฎ"]
];
function productionExportSelectedDatasets(){const selected=[...document.querySelectorAll('[data-production-dataset]:checked')].map(x=>x.dataset.productionDataset);return selected.includes("vehicles")?selected:["vehicles",...selected]}
function productionExportInvalidate(){productionExportState.data=null;productionExportState.key="";const b=$("productionExportDownload");if(b)b.disabled=true;const st=$("productionExportStatus");if(st)st.innerHTML='<b>พร้อมตรวจข้อมูล</b><span>กด “ตรวจข้อมูล” ก่อนสร้างไฟล์ เพื่อยืนยันจำนวนและช่วงข้อมูล</span>'}
function productionExportRangeFromUi(){const mode=String($("productionExportRangeMode")?.value||"month"),month=String($("productionExportMonth")?.value||currentBangkokMonth()),from=String($("productionExportFrom")?.value||""),to=String($("productionExportTo")?.value||"");return mode==="month"?productionExportMonthDates(month):{from,to}}
function renderAdminMonthlyExport(){
  const panel=$("adminPanel");if(!panel)return;const month=currentBangkokMonth(),dates=productionExportMonthDates(month);
  const optional=PRODUCTION_EXPORT_DATASETS.filter(x=>x[0]!=="vehicles");
  panel.innerHTML=`<div class="admin-section-head clean-admin-head export-head"><div><h3>ส่งออกข้อมูล</h3><p>ไฟล์หลักเน้นข้อมูลรถและเวลาทำงานที่อ่านง่าย ไม่แสดงข้อมูลเทคนิคของระบบ</p></div></div><section class="production-export-card production-export-simple"><div class="production-export-filter"><label><span>ช่วงข้อมูล</span><select id="productionExportRangeMode"><option value="month">รายเดือน</option><option value="custom">กำหนดวันที่</option></select></label><label id="productionExportMonthWrap"><span>เดือน</span><input id="productionExportMonth" type="month" value="${month}" min="2020-01" max="2100-12"></label><label class="production-export-custom" hidden><span>วันที่เริ่ม</span><input id="productionExportFrom" type="date" value="${dates?.from||currentBangkokDate()}"></label><label class="production-export-custom" hidden><span>วันที่สิ้นสุด</span><input id="productionExportTo" type="date" value="${dates?.to||currentBangkokDate()}"></label><label><span>กะ</span><select id="productionExportShift"><option value="">ทุกกะ</option></select></label><label><span>รูปแบบไฟล์</span><select id="productionExportFormat"><option value="excel">Excel (.xlsx)</option><option value="csv">CSV</option></select></label></div><section class="production-export-main-sheet"><span>ไฟล์หลัก</span><div><b>ข้อมูลรถและเวลาทำงาน</b><small>Gate In จริง · รถ · นัดหมาย · เวลาแต่ละขั้นตอน · ระยะเวลา · จุดที่เกินเกณฑ์ Admin</small></div><em>สร้างเสมอ</em></section><section class="production-export-main-sheet issue"><span>สรุปปัญหา</span><div><b>รายการเกินเกณฑ์</b><small>แสดงเฉพาะรถและขั้นตอนที่ถึงระดับ เตือน / ล่าช้า / วิกฤต พร้อมเวลาที่เกิน</small></div><em>สร้างเสมอ</em></section><details class="production-export-optional"><summary><b>ข้อมูลเพิ่มเติม</b><span>ไม่สร้างหากไม่ได้เลือก</span></summary><div class="production-export-datasets simple-options">${optional.map(([key,label,note])=>`<label><input type="checkbox" data-production-dataset="${key}"><span><b>${label}</b><small>${note}</small></span></label>`).join("")}</div></details><section class="production-export-lineage"><b>ชื่อบริษัท</b><span>ถ้าจับคู่ Appointment ได้ ใช้ชื่อบริษัทจากไฟล์นัดหมายทันที · ถ้าจับคู่ไม่ได้จึงใช้ชื่อบริษัทที่มากับ Gate In · ทะเบียนและจังหวัดรวมคอลัมน์เดียว · เวลา/ระยะเวลาแยกจากข้อความสถานะ</span></section><div class="production-export-actions"><button id="productionExportPreview" class="outline-button" type="button">ตรวจข้อมูล</button><button id="productionExportDownload" class="primary" type="button" disabled>สร้างไฟล์</button></div><div id="productionExportStatus" class="monthly-export-status"><b>กำลังอ่านค่าตั้งต้น</b><span>รอสักครู่</span></div></section>`;
  bindProductionExportUi();loadProductionExportMeta();
}
function bindProductionExportUi(){
  const mode=$("productionExportRangeMode"),toggle=()=>{const custom=mode?.value==="custom";if($("productionExportMonthWrap"))$("productionExportMonthWrap").hidden=custom;document.querySelectorAll(".production-export-custom").forEach(x=>x.hidden=!custom);productionExportInvalidate()};mode?.addEventListener("change",toggle);toggle();
  ["productionExportMonth","productionExportFrom","productionExportTo","productionExportShift","productionExportFormat"].forEach(id=>$(id)?.addEventListener("change",productionExportInvalidate));document.querySelectorAll("[data-production-dataset]").forEach(x=>x.addEventListener("change",productionExportInvalidate));
  $("productionExportPreview")?.addEventListener("click",previewProductionExport);$("productionExportDownload")?.addEventListener("click",downloadProductionExport);
}
async function loadProductionExportMeta(){const st=$("productionExportStatus"),range=productionExportRangeFromUi();if(!range?.from||!range?.to)return;try{const q=new URLSearchParams({from:range.from,to:range.to,mode:"meta"}),data=await api(`/api/admin/production-export?${q}`);productionExportState.meta=data;const shift=$("productionExportShift");if(shift){const value=shift.value;shift.innerHTML='<option value="">ทุกกะ</option>'+((data.shifts||[]).map(s=>`<option value="${escapeHtml(s.shiftId)}">${escapeHtml(s.shiftName)}</option>`).join(""));shift.value=value}if(st)st.innerHTML=`<b>พร้อมตรวจข้อมูล</b><span>เวลานัดอ้างอิง ${escapeHtml(data.referenceMode||"-")} · อนุโลมมาช้า ${Number(data.lateGraceMinutes||0)} นาที · เกณฑ์ขั้นตอนใช้ค่าปัจจุบันที่ Admin ตั้งไว้</span>`}catch(error){if(st)st.innerHTML=`<b>อ่านค่าตั้งต้นไม่สำเร็จ</b><span>${escapeHtml(error.message||"กรุณาลองใหม่")}</span>`}}
function productionExportRequestKey(){const r=productionExportRangeFromUi(),datasets=productionExportSelectedDatasets(),shift=$("productionExportShift")?.value||"";return JSON.stringify({r,datasets,shift})}
async function previewProductionExport(){if(productionExportState.busy)return;const range=productionExportRangeFromUi(),datasets=productionExportSelectedDatasets(),shift=$("productionExportShift")?.value||"",st=$("productionExportStatus"),btn=$("productionExportPreview");if(!range?.from||!range?.to){await showNotice("warning","กรุณาเลือกช่วงวันที่");return}productionExportState.busy=true;if(btn){btn.disabled=true;btn.textContent="กำลังตรวจ"}if(st)st.innerHTML='<b>กำลังตรวจข้อมูล</b><span>กำลังอ่านรถและเกณฑ์เวลาที่ Admin กำหนด</span>';try{const q=new URLSearchParams({from:range.from,to:range.to,shiftId:shift,datasets:datasets.join(",")}),data=await api(`/api/admin/production-export?${q}`);productionExportState.data=data;productionExportState.key=productionExportRequestKey();const count=data.counts||{},parts=[`รถ ${Number(count.vehicles||0).toLocaleString("th-TH")} คัน`,`เกินเกณฑ์ ${Number(count.issues||0).toLocaleString("th-TH")} รายการ`];for(const key of datasets.filter(x=>!["vehicles"].includes(x)))if(count[key]!=null)parts.push(`${productionExportDatasetLabel(key)} ${Number(count[key]||0).toLocaleString("th-TH")}`);if(st)st.innerHTML=`<b>ตรวจแล้ว พร้อมสร้างไฟล์</b><span>${escapeHtml(data.range?.fromDate||range.from)} – ${escapeHtml(data.range?.toDate||range.to)}${data.shift?.shiftName?` · ${escapeHtml(data.shift.shiftName)}`:" · ทุกกะ"} · ${parts.join(" · ")} · ${Number(data.durationMs||0).toLocaleString("th-TH")} ms</span>`;if($("productionExportDownload"))$("productionExportDownload").disabled=false}catch(error){productionExportState.data=null;if(st)st.innerHTML=`<b>ตรวจข้อมูลไม่สำเร็จ</b><span>${escapeHtml(error.message||"กรุณาลองใหม่")}</span>`;await showNotice("error",error.message||"ตรวจข้อมูลไม่สำเร็จ")}finally{productionExportState.busy=false;if(btn){btn.disabled=false;btn.textContent="ตรวจข้อมูล"}}}
function productionExportDatasetLabel(key){if(key==="issues")return"รายการเกินเกณฑ์";return Object.fromEntries(PRODUCTION_EXPORT_DATASETS.map(x=>[x[0],x[1]]))[key]||key}
function productionExportDateOnly(epoch){const n=Number(epoch);if(!Number.isFinite(n)||n<=0)return"";const parts=new Intl.DateTimeFormat("en-GB",{timeZone:cfg.timezone,day:"2-digit",month:"2-digit",year:"numeric"}).formatToParts(new Date(n*1000)),p=Object.fromEntries(parts.map(x=>[x.type,x.value]));return`${p.day}/${p.month}/${p.year}`}
function productionExportTimeOnly(epoch){const n=Number(epoch);if(!Number.isFinite(n)||n<=0)return"";const parts=new Intl.DateTimeFormat("en-GB",{timeZone:cfg.timezone,hour:"2-digit",minute:"2-digit",second:"2-digit",hourCycle:"h23"}).formatToParts(new Date(n*1000)),p=Object.fromEntries(parts.map(x=>[x.type,x.value]));return`${p.hour}:${p.minute}:${p.second}`}
function productionExportSourceDate(value){const m=String(value||"").match(/^(\d{4})-(\d{2})-(\d{2})$/);return m?`${m[3]}/${m[2]}/${m[1]}`:String(value||"")}
function productionExportMinuteTime(value){const n=Number(value);if(!Number.isFinite(n))return"";const x=((Math.trunc(n)%1440)+1440)%1440;return`${String(Math.floor(x/60)).padStart(2,"0")}:${String(x%60).padStart(2,"0")}:00`}
function productionExportDurationCell(value){return value==null||!Number.isFinite(Number(value))?"":formatExportDuration(Number(value))}
function productionExportArrivalStatus(item){if(!item?.appointmentMatched)return"ไม่พบข้อมูลนัดหมาย";return item?.timingStatus==="LATE"?"มาสาย":"ตรงเวลา"}
function productionExportArrivalType(item){if(!item?.appointmentMatched||!Number.isFinite(Number(item?.deltaSeconds)))return"";const d=Number(item.deltaSeconds);return d<0?"มาก่อน":d>0?"มาช้า":"ตรงเวลาพอดี"}
function productionExportArrivalDelta(item){if(!item?.appointmentMatched||!Number.isFinite(Number(item?.deltaSeconds)))return"";return productionExportDurationCell(Math.abs(Number(item.deltaSeconds)))}
function productionExportArrivalLateBy(item){if(!item?.appointmentMatched||item?.timingStatus!=="LATE")return"";const n=Number(item?.lateBySeconds);return Number.isFinite(n)&&n>=0?productionExportDurationCell(n):""}
function productionExportVehicleText(item){return [String(item?.vehiclePlate||"").trim(),String(item?.province||"").trim()].filter(Boolean).join(" ")}
function productionExportValue(item,key){const timeKeys=new Set(["plannedAt","gateInAt","documentSubmittedAt","documentCheckedAt","firstCalledAt","lastCalledAt","receivingStartedAt","receivingCompletedAt","documentReturnedAt","gateOutAt","rejectedAt"]),durationKeys=new Set(["gateToDocumentSeconds","documentReviewSeconds","readyToReceivingSeconds","receivingSeconds","receivingToReturnSeconds","returnToGateOutSeconds","totalInSiteSeconds"]);if(timeKeys.has(key))return productionExportTimeOnly(item[key]);if(durationKeys.has(key))return productionExportDurationCell(item[key]);if(key==="lateGraceMinutes")return productionExportDurationCell(Number(item[key]||0)*60);if(key==="deltaSeconds")return productionExportArrivalDelta(item);return item[key]==null?"":item[key]}
function productionExportCheck(item,label){return(item?.stageChecks||[]).find(x=>x.label===label)||null}
function productionExportCheckStatus(check){if(!check||check.seconds==null)return"";if(check.issue)return check.alertLabel||"เกินเกณฑ์";if(check.alertLevel==="WATCH")return"เฝ้าระวัง";return"ปกติ"}
function productionExportCheckExceeded(check){return check?.issue?productionExportDurationCell(check.exceededSeconds):""}
function productionExportOperationalTable(items=[]){
  const headers=["วันที่ Gate In","Gate In","เลขนัดหมาย","บริษัท","ทะเบียน / จังหวัด","คนขับ","กะ","ประตู","วันที่นัด","เวลานัด","ผลการมาตามนัด","ลักษณะการมา","ต่างจากเวลานัด","เวลาอนุโลม","เกินเวลาอนุโลม","ผู้ขนส่ง","PO","ยื่นเอกสาร","รอยื่นเอกสาร","เกินเกณฑ์รอยื่น","สถานะรอยื่น","ตรวจเอกสารเสร็จ","ตรวจเอกสาร","เกินเกณฑ์ตรวจเอกสาร","สถานะตรวจเอกสาร","เรียกรถครั้งแรก","จำนวนครั้งเรียก","เริ่มตรวจรับ","รอตรวจรับ","เกินเกณฑ์รอตรวจรับ","สถานะรอตรวจรับ","รับสินค้าเสร็จ","ตรวจรับสินค้า","เกินเกณฑ์ตรวจรับ","สถานะตรวจรับ","รับเอกสารคืน","รอคืนเอกสาร","เกินเกณฑ์รอคืน","สถานะรอคืน","วันที่ Gate Out","Gate Out","รอ Gate Out","เกินเกณฑ์รอ Gate Out","สถานะรอ Gate Out","เวลารวมในพื้นที่","เกินเกณฑ์เวลารวม","สถานะเวลารวม","ขั้นตอนที่เกินเกณฑ์มากที่สุด","ใช้เวลาจริง","เกณฑ์ Admin","เกินเกณฑ์","สถานะล่าสุด","เหตุผลปฏิเสธ"];
  const formats=["date","time","text","text","text","text","text","text","date","time","text","text","duration","duration","duration","text","text","time","duration","duration","text","time","duration","duration","text","time","number","time","duration","duration","text","time","duration","duration","text","time","duration","duration","text","date","time","duration","duration","text","duration","duration","text","text","duration","duration","duration","text","text"];
  const rows=items.map(x=>{const gate=productionExportCheck(x,"รอยื่นเอกสาร"),review=productionExportCheck(x,"ตรวจเอกสาร"),wait=productionExportCheck(x,"รอตรวจรับ"),receive=productionExportCheck(x,"ตรวจรับสินค้า"),ret=productionExportCheck(x,"รอคืนเอกสาร"),out=productionExportCheck(x,"รอ Gate Out"),worst=x.longestIssue||null,overall=x.overallCheck||null;return[productionExportDateOnly(x.gateInAt),productionExportTimeOnly(x.gateInAt),x.appointmentNo||"",x.displayCompany||x.companyName||"",productionExportVehicleText(x),x.driverName||"",x.shiftName||"",x.doorCode||"",productionExportSourceDate(x.appointmentDate),productionExportTimeOnly(x.plannedAt),productionExportArrivalStatus(x),productionExportArrivalType(x),productionExportArrivalDelta(x),x.appointmentMatched?productionExportDurationCell(Number(x.lateGraceMinutes||0)*60):"",productionExportArrivalLateBy(x),x.appointmentMatched?(x.carrier||""):"",x.appointmentMatched?(x.po||""):"",productionExportTimeOnly(x.documentSubmittedAt),productionExportDurationCell(gate?.seconds),productionExportCheckExceeded(gate),productionExportCheckStatus(gate),productionExportTimeOnly(x.documentCheckedAt),productionExportDurationCell(review?.seconds),productionExportCheckExceeded(review),productionExportCheckStatus(review),productionExportTimeOnly(x.firstCalledAt),Number(x.queueCallCount||0),productionExportTimeOnly(x.receivingStartedAt),productionExportDurationCell(wait?.seconds),productionExportCheckExceeded(wait),productionExportCheckStatus(wait),productionExportTimeOnly(x.receivingCompletedAt),productionExportDurationCell(receive?.seconds),productionExportCheckExceeded(receive),productionExportCheckStatus(receive),productionExportTimeOnly(x.documentReturnedAt),productionExportDurationCell(ret?.seconds),productionExportCheckExceeded(ret),productionExportCheckStatus(ret),productionExportDateOnly(x.gateOutAt),productionExportTimeOnly(x.gateOutAt),productionExportDurationCell(out?.seconds),productionExportCheckExceeded(out),productionExportCheckStatus(out),productionExportDurationCell(x.totalInSiteSeconds),productionExportCheckExceeded(overall),productionExportCheckStatus(overall),worst?.label||"",productionExportDurationCell(worst?.seconds),productionExportDurationCell(worst?.thresholdSeconds),productionExportDurationCell(worst?.exceededSeconds),monthlyExportStatusLabel(x.currentStatus,x.gateOutAt),x.rejectionReason||""]});
  return{key:"vehicles",name:"ข้อมูลรถและเวลาทำงาน",headers,formats,rows}
}
function productionExportIssuesTable(items=[]){const headers=["วันที่ Gate In","Gate In","เลขนัดหมาย","บริษัท","ทะเบียน / จังหวัด","กะ","ประตู","วันที่นัด","เวลานัด","ขั้นตอน","ใช้เวลาจริง","เกณฑ์ Admin","เกินเกณฑ์","ระดับ","Gate Out"],formats=["date","time","text","text","text","text","text","date","time","text","duration","duration","duration","text","time"],rows=[];for(const x of items)for(const check of x.stageChecks||[])if(check.issue)rows.push([productionExportDateOnly(x.gateInAt),productionExportTimeOnly(x.gateInAt),x.appointmentNo||"",x.displayCompany||x.companyName||"",productionExportVehicleText(x),x.shiftName||"",x.doorCode||"",productionExportSourceDate(x.appointmentDate),productionExportTimeOnly(x.plannedAt),check.label,productionExportDurationCell(check.seconds),productionExportDurationCell(check.thresholdSeconds),productionExportDurationCell(check.exceededSeconds),check.alertLabel||"",productionExportTimeOnly(x.gateOutAt)]);return{key:"issues",name:"รายการเกินเกณฑ์",headers,formats,rows}}
function productionExportEventsTable(items=[],vehicles=[]){const company=new Map((vehicles||[]).map(x=>[String(x.autoId),x.displayCompany||x.companyName||""]));return{key:"events",name:"เหตุการณ์ละเอียด",headers:["Auto ID","เลขนัดหมาย","บริษัท","ทะเบียน","วันที่","เวลา","เหตุการณ์","ประตู","ผู้ดำเนินการ","เหตุผล/หมายเหตุ"],rows:items.map(x=>[x.autoId,x.appointmentNo||"",company.get(String(x.autoId))||x.companyName||"",x.vehiclePlate||"",productionExportDateOnly(x.eventAt),productionExportTimeOnly(x.eventAt),x.eventLabel||"",x.doorCode||"",x.actor||"",[x.reason,x.note].filter(Boolean).join(" · ")])}}
function productionExportTables(data){const tables=[],d=data.data||{},vehicles=Array.isArray(d.vehicles)?d.vehicles:[];if(vehicles.length||Array.isArray(d.vehicles)){tables.push(productionExportOperationalTable(vehicles));tables.push(productionExportIssuesTable(vehicles));if(Array.isArray(d.events))tables.push(productionExportEventsTable(d.events,vehicles))}if(Array.isArray(d.plan))tables.push({key:"plan",name:"Plan vs Actual",headers:["ตามแผน","เข้าจริง","รถมีเลขนัดหมาย","จับคู่ได้","จับคู่ %","ยังไม่จับคู่","ต่างจากแผน","ตรงเวลารวม","ตรงเวลา %","มาสาย","มาสาย %","สายเฉลี่ย"],rows:d.plan.map(x=>[x.planned,x.actual,x.withAppointment,x.matched,x.matchPercent,x.unmatched,x.difference,x.compliant,x.compliancePercent,x.late,x.latePercent,productionExportDurationCell(Number(x.avgLateMinutes||0)*60)])});if(Array.isArray(d.hourly))tables.push({key:"hourly",name:"รายชั่วโมง",headers:["ช่วงเวลา","ตามแผน","เข้าจริง","จับคู่ได้","ต่างจากแผน"],rows:d.hourly.map(x=>[String(x.label||"").replace(/(\d{2}:\d{2})/g,"$1:00"),x.planned,x.actual,x.matched,x.difference])});for(const key of ["vendor","carrier","po"]){if(!Array.isArray(d[key]))continue;tables.push({key,name:productionExportDatasetLabel(key),headers:[key==="po"?"PO":key==="carrier"?"ผู้ขนส่ง":"บริษัทผู้ส่งสินค้า","รถ","ตรงเวลา %","มาสาย %","สายเฉลี่ย","รอยื่นเอกสารเฉลี่ย","รอตรวจรับเฉลี่ย","ตรวจรับเฉลี่ย","อยู่ในพื้นที่เฉลี่ย","ปฏิเสธ"],rows:d[key].map(x=>[x.name,x.vehicles,x.onTimePercent,x.latePercent,productionExportDurationCell(Number(x.avgLateMinutes||0)*60),productionExportDurationCell(Number(x.avgGateToDocumentMinutes||0)*60),productionExportDurationCell(Number(x.avgWaitReceivingMinutes||0)*60),productionExportDurationCell(Number(x.avgReceivingMinutes||0)*60),productionExportDurationCell(Number(x.avgCycleMinutes||0)*60),x.rejected])})}if(Array.isArray(d.quality))tables.push({key:"quality",name:"คุณภาพข้อมูล",headers:["รายการ","จำนวน","คำอธิบาย"],rows:d.quality.map(x=>[x.metric,x.value,x.note])});return tables}
function downloadProductionExport(){const data=productionExportState.data;if(!data||productionExportState.key!==productionExportRequestKey()){showNotice("warning","ช่วงข้อมูลหรือข้อมูลเพิ่มเติมมีการเปลี่ยน กรุณากดตรวจข้อมูลใหม่ก่อนสร้างไฟล์");return}const tables=productionExportTables(data);if(!tables.length){showNotice("info","ไม่มีข้อมูลสำหรับสร้างไฟล์");return}const format=$("productionExportFormat")?.value||"excel",base=`Warehouse_Vehicle_${data.range.fromDate}_${data.range.toDate}${data.shift?.shiftName?`_${safeExportFilename(data.shift.shiftName)}`:""}`;if(format==="csv"){tables.forEach((t,i)=>setTimeout(()=>downloadTextFile("\uFEFF"+[t.headers,...t.rows].map(row=>row.map(csvCell).join(",")).join("\r\n"),`${base}_${safeExportFilename(t.name)}.csv`,"text/csv;charset=utf-8"),i*180));showNotice("success",`สร้าง CSV ${tables.length} ไฟล์แล้ว`);return}const bytes=productionExportXlsx(tables,data);downloadBinaryFile(bytes,`${base}.xlsx`,`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`);showNotice("success",`สร้าง Excel ${tables.length} ชีทแล้ว`)}
function safeExportFilename(value){return String(value||"data").replace(/[\\/:*?"<>|]+/g,"_").replace(/\s+/g,"_").slice(0,60)}
function downloadTextFile(text,name,type){downloadBinaryFile(new TextEncoder().encode(text),name,type)}
function downloadBinaryFile(content,name,type){const blob=new Blob([content],{type}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),2000)}
function xmlEscape(value){return String(value??"").replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g,"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&apos;")}
function xlsxColumnName(index){let n=index+1,out="";while(n){const r=(n-1)%26;out=String.fromCharCode(65+r)+out;n=Math.floor((n-1)/26)}return out}
function xlsxTimeNumber(value){const m=String(value||"").match(/^(\d+):(\d{2}):(\d{2})$/);if(!m)return null;const h=Number(m[1]),min=Number(m[2]),sec=Number(m[3]);if(!Number.isFinite(h)||min>59||sec>59)return null;return(h*3600+min*60+sec)/86400}
function xlsxCellXml(value,row,col,style=0,format="text"){const ref=`${xlsxColumnName(col)}${row}`;if((format==="time"||format==="duration")&&value!==""&&value!=null){const n=xlsxTimeNumber(value);if(n!=null){const s=format==="time"?2:3;return `<c r="${ref}" s="${s}"><v>${n}</v></c>`}}const attr=style?` s="${style}"`:"";if(typeof value==="number"&&Number.isFinite(value))return `<c r="${ref}"${attr}><v>${value}</v></c>`;const text=xmlEscape(value==null?"":value);return `<c r="${ref}" t="inlineStr"${attr}><is><t xml:space="preserve">${text}</t></is></c>`}
function xlsxWorksheetXml(table){const rows=[table.headers,...table.rows],formats=Array.isArray(table.formats)?table.formats:[],sheetData=rows.map((row,i)=>`<row r="${i+1}">${row.map((v,j)=>xlsxCellXml(v,i+1,j,i===0?1:0,i===0?"text":(formats[j]||"text"))).join("")}</row>`).join(""),cols=table.headers.map((_,i)=>`<col min="${i+1}" max="${i+1}" width="${Math.min(34,Math.max(12,String(table.headers[i]||"").length+3))}" customWidth="1"/>`).join("");return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>${cols}</cols><sheetData>${sheetData}</sheetData><autoFilter ref="A1:${xlsxColumnName(Math.max(0,table.headers.length-1))}${Math.max(1,rows.length)}"/></worksheet>`}
function xlsxSafeSheetNames(tables){const used=new Set();return tables.map((t,i)=>{let base=String(t.name||`Sheet ${i+1}`).replace(/[\\/?*\[\]:]/g," ").trim().slice(0,31)||`Sheet ${i+1}`,name=base,n=2;while(used.has(name.toLowerCase())){const suffix=` ${n++}`;name=base.slice(0,31-suffix.length)+suffix}used.add(name.toLowerCase());return name})}
function crc32Bytes(bytes){let crc=0xffffffff;for(const b of bytes){crc^=b;for(let k=0;k<8;k++)crc=(crc>>>1)^((crc&1)?0xedb88320:0)}return(crc^0xffffffff)>>>0}
function zipU16(n){return[n&255,(n>>>8)&255]}function zipU32(n){return[n&255,(n>>>8)&255,(n>>>16)&255,(n>>>24)&255]}
function zipStore(entries){const enc=new TextEncoder(),locals=[],centrals=[];let offset=0;for(const entry of entries){const name=enc.encode(entry.name),data=entry.data instanceof Uint8Array?entry.data:enc.encode(String(entry.data)),crc=crc32Bytes(data),local=new Uint8Array([...zipU32(0x04034b50),...zipU16(20),...zipU16(0),...zipU16(0),...zipU16(0),...zipU16(0),...zipU32(crc),...zipU32(data.length),...zipU32(data.length),...zipU16(name.length),...zipU16(0),...name]),combined=new Uint8Array(local.length+data.length);combined.set(local);combined.set(data,local.length);locals.push(combined);const central=new Uint8Array([...zipU32(0x02014b50),...zipU16(20),...zipU16(20),...zipU16(0),...zipU16(0),...zipU16(0),...zipU16(0),...zipU32(crc),...zipU32(data.length),...zipU32(data.length),...zipU16(name.length),...zipU16(0),...zipU16(0),...zipU16(0),...zipU16(0),...zipU32(0),...zipU32(offset),...name]);centrals.push(central);offset+=combined.length}const centralSize=centrals.reduce((a,b)=>a+b.length,0),end=new Uint8Array([...zipU32(0x06054b50),...zipU16(0),...zipU16(0),...zipU16(entries.length),...zipU16(entries.length),...zipU32(centralSize),...zipU32(offset),...zipU16(0)]),size=offset+centralSize+end.length,out=new Uint8Array(size);let pos=0;for(const b of locals){out.set(b,pos);pos+=b.length}for(const b of centrals){out.set(b,pos);pos+=b.length}out.set(end,pos);return out}
function productionExportXlsx(tables,data){const all=tables,names=xlsxSafeSheetNames(all),sheetEntries=all.map((t,i)=>({name:`xl/worksheets/sheet${i+1}.xml`,data:xlsxWorksheetXml(t)})),contentTypes=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/>${all.map((_,i)=>`<Override PartName="/xl/worksheets/sheet${i+1}.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>`).join("")}</Types>`,rels=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>`,workbook=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>${names.map((n,i)=>`<sheet name="${xmlEscape(n)}" sheetId="${i+1}" r:id="rId${i+1}"/>`).join("")}</sheets></workbook>`,workbookRels=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${all.map((_,i)=>`<Relationship Id="rId${i+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet${i+1}.xml"/>`).join("")}<Relationship Id="rId${all.length+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>`,styles=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="2"><numFmt numFmtId="164" formatCode="hh:mm:ss"/><numFmt numFmtId="165" formatCode="[h]:mm:ss"/></numFmts><fonts count="2"><font><sz val="10"/><name val="Aptos"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="10"/><name val="Aptos"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF2F6FD6"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="4"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1"/><xf numFmtId="164" fontId="0" fillId="0" borderId="0" xfId="0" applyNumberFormat="1"/><xf numFmtId="165" fontId="0" fillId="0" borderId="0" xfId="0" applyNumberFormat="1"/></cellXfs></styleSheet>`;return zipStore([{name:"[Content_Types].xml",data:contentTypes},{name:"_rels/.rels",data:rels},{name:"xl/workbook.xml",data:workbook},{name:"xl/_rels/workbook.xml.rels",data:workbookRels},{name:"xl/styles.xml",data:styles},...sheetEntries])}
...
async function confirmLogout(){
  if(!state.user)return clearSession();
  if(window.Swal){
    const answer=await Swal.fire({icon:"question",title:"ออกจากระบบ?",text:`ผู้ใช้งาน ${state.user?.name||""}`,showCancelButton:true,confirmButtonText:"ออกจากระบบ",cancelButtonText:"ยกเลิก",customClass:swalClasses(),buttonsStyling:false,width:350});
    if(!answer.isConfirmed)return;
  }
  await logout();
}
async function logout() { try { await api("/api/auth/logout",{method:"POST"}); } catch {} clearSession(); }
function clearSession() { stopCamera();cancelDashboardRequest();dashboardState.data=null;dashboardState.dataIdentity="";dashboardState.error="";dashboardState.analyticsError="";dashboardState.analyticsLastLoadedAt=0;dashboardState.lastLoadedAt=0;dashboardState.snapshotLoaded=false;clearDashboardStaleRecovery(true); inboundLiveState.version="";inboundLiveState.checking=false;inboundLiveState.failures=0;inboundLiveState.nextAllowedAt=0;alertSoundState.initialized=false;alertSoundState.levels.clear();clearStoredAuthToken(); sessionStorage.removeItem("wvf_dashboard_snapshot_r118"); state.token=""; state.user=null; state.capacityWarning=null; state.display={dashboardEnabled:true,datatableEnabled:true}; state.appointment={moduleEnabled:false,uploadEnabled:false}; appointmentStopWorker(); appointmentUploadState.config=null;appointmentUploadState.result=null;appointmentUploadState.preview=null;appointmentUploadState.mappingNeeded=null;try{appointmentAdminFileProbe.worker?.terminate()}catch{}appointmentAdminFileProbe.worker=null;appointmentAdminFileProbe.result=null;appointmentAdminFileProbe.fileName=""; if($("mobileAccountName"))$("mobileAccountName").textContent="-"; $("appView").hidden=true; $("loginView").hidden=false; $("loginPassword").value=""; setConnection(navigator.onLine); window.scrollTo(0, 0); }
...
async function api(path, options={}) {
  ...
  while(attempt<=maxRetries){
    ...
    try{
      ...
      if(path.startsWith("/api/vehicles/active")){if(data.capacity?.truncated){const total=Number(data.capacity.total||0),returned=Number(data.capacity.returned||0),limit=Number(data.capacity.limit||500);state.capacityWarning={total,returned,limit}}else state.capacityWarning=null;setConnection(true)}
      return data
    }catch(error){
...
function setConnection(online) { state.online=online;const capacity=online?state.capacityWarning:null,banner=$("connectionBanner");if(banner){banner.hidden=online&&!capacity;banner.textContent=!online?"การเชื่อมต่อขัดข้อง กำลังลองใหม่":capacity?`คำเตือน: มีรถปัจจุบัน ${capacity.total} คัน แต่หน้าจอแสดง ${capacity.returned}/${capacity.limit} คัน กรุณาแจ้งผู้ดูแลระบบ`:""}if($("syncStatus")){ $("syncStatus").textContent=!online?"● รอเชื่อมต่อ":capacity?`● ข้อมูลไม่ครบ ${capacity.returned}/${capacity.total}`:"● พร้อมใช้งาน"; $("syncStatus").style.color=online&&!capacity?"#08783a":"#a82020"; } if($("inboundSyncStatus")){ $("inboundSyncStatus").textContent=!online?"● รอเชื่อมต่อ":capacity?`● ข้อมูลไม่ครบ ${capacity.returned}/${capacity.total}`:"● พร้อมใช้งาน"; $("inboundSyncStatus").classList.toggle("is-online",online&&!capacity); $("inboundSyncStatus").classList.toggle("is-offline",!online||Boolean(capacity));} }
