...
function renderAdminAppointmentData(){
  ...
  const sync=()=>{const on=$("appointmentModuleEnabled").checked;$("appointmentUploadEnabled").disabled=!on;$("appointmentUseImportedData").disabled=!on;document.querySelectorAll("#appointmentModuleForm select,#appointmentModuleForm input[type=number],[data-appointment-page] input,.appointment-admin-file-settings input,.appointment-admin-file-settings select,.appointment-company-policy-card select,.appointment-lifecycle-settings input").forEach(el=>el.disabled=!on);if(!on){$("appointmentUploadEnabled").checked=false;$("appointmentUseImportedData").checked=false}};$("appointmentModuleEnabled")?.addEventListener("change",sync);sync();
