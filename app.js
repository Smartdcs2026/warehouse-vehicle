...
const FRONTEND_BUILD="2026.08.20-round205-production-export";
...
const PRODUCTION_EXPORT_DATASETS=[
  ["vehicles","รายละเอียดรถ","รถรายคัน + ขั้นตอนงาน + ผลเทียบเวลานัด"],
...
const PRODUCTION_VEHICLE_COLUMNS=[
  ...
  ["plannedAt","เวลานัด","นัดหมาย"],["gateInAt","Gate In","เวลา"],["referenceMode","จุดเริ่มคำนวณ","นัดหมาย"],["lateGraceMinutes","อนุโลมมาช้า (นาที)","นัดหมาย"],["deltaMinutes","ต่างจากเวลานัด (นาที)","นัดหมาย"],["timingLabel","ผลตามเกณฑ์","นัดหมาย"],["policyId","รหัสเกณฑ์เวลา","นัดหมาย"],
  ["documentSubmittedAt","ยื่นเอกสาร","เวลา"],["documentCheckedAt","ตรวจเอกสารเสร็จ","เวลา"],["firstCalledAt","เรียกรถครั้งแรก","เวลา"],["lastCalledAt","เรียกรถล่าสุด","เวลา"],["queueCallCount","จำนวนครั้งเรียก","เวลา"],["receivingStartedAt","เริ่มตรวจรับ","เวลา"],["receivingCompletedAt","รับสินค้าเสร็จ","เวลา"],["documentReturnedAt","รับเอกสารคืน","เวลา"],["gateOutAt","Gate Out","เวลา"],
  ["rejectedAt","เวลาปฏิเสธ","ปฏิเสธ"],["rejectionReason","เหตุผลปฏิเสธ","ปฏิเสธ"],["rejectionDetail","รายละเอียดปฏิเสธ","ปฏิเสธ"],["rejectionSupervisor","หัวหน้างานรับทราบ","ปฏิเสธ"],
  ["gateToDocumentSeconds","Gate In → ยื่นเอกสาร (วินาที)","ระยะเวลา"],["documentReviewSeconds","ตรวจเอกสาร (วินาที)","ระยะเวลา"],["readyToReceivingSeconds","พร้อม → เริ่มตรวจรับ (วินาที)","ระยะเวลา"],["receivingSeconds","ตรวจรับสินค้า (วินาที)","ระยะเวลา"],["receivingToReturnSeconds","รับเสร็จ → คืนเอกสาร (วินาที)","ระยะเวลา"],["returnToGateOutSeconds","คืนเอกสาร → Gate Out (วินาที)","ระยะเวลา"],["totalInSiteSeconds","เวลารวมในพื้นที่ (วินาที)","ระยะเวลา"]
];
...
function productionExportDatasetLabel(key){return Object.fromEntries(PRODUCTION_EXPORT_DATASETS.map(x=>[x[0],x[1]]))[key]||key}
function productionExportValue(item,key){const timeKeys=new Set(["plannedAt","gateInAt","documentSubmittedAt","documentCheckedAt","firstCalledAt","lastCalledAt","receivingStartedAt","receivingCompletedAt","documentReturnedAt","gateOutAt","rejectedAt"]);if(timeKeys.has(key))return item[key]?formatDate(item[key]):"";return item[key]==null?"":item[key]}
function productionExportTables(data){const tables=[],selectedCols=new Set(productionExportSelectedVehicleColumns()),extra=data.extraFields||[],d=data.data||{};if(Array.isArray(d.vehicles)){const cols=PRODUCTION_VEHICLE_COLUMNS.filter(x=>selectedCols.has(x[0])),headers=cols.map(x=>x[1]).concat(extra.map(x=>x.label)),rows=d.vehicles.map(item=>cols.map(c=>productionExportValue(item,c[0])).concat(extra.map(x=>item.extras?.[x.key]??"")));tables.push({key:"vehicles",name:"รายละเอียดรถ",headers,rows})}if(Array.isArray(d.plan)){tables.push({key:"plan",name:"Plan vs Actual",headers:["ตามแผน","เข้าจริง","รถมีเลขนัดหมาย","จับคู่ได้","จับคู่ %","ยังไม่จับคู่","ต่างจากแผน","มาก่อน (ตรงเวลา)","มาช้าในเกณฑ์","ตรงเวลารวม","ตรงเวลา %","มาสาย","มาสาย %","สายเฉลี่ย (นาที)","หลายเกณฑ์ย้อนหลัง","เวลาอ้างอิงปัจจุบัน","อนุโลมปัจจุบัน (นาที)"],rows:d.plan.map(x=>[x.planned,x.actual,x.withAppointment,x.matched,x.matchPercent,x.unmatched,x.difference,x.early,x.onTime,x.compliant,x.compliancePercent,x.late,x.latePercent,x.avgLateMinutes,x.mixedPolicy?"ใช่":"ไม่",x.referenceMode,x.lateGraceMinutes])})}if(Array.isArray(d.hourly)){tables.push({key:"hourly",name:"รายชั่วโมง",headers:["ช่วงเวลา","ตามแผน","เข้าจริง","รถมีเลขนัดหมาย","จับคู่ได้","ต่างจากแผน"],rows:d.hourly.map(x=>[x.label,x.planned,x.actual,x.withAppointment,x.matched,x.difference])})}for(const key of ["vendor","carrier","po"]){if(!Array.isArray(d[key]))continue;tables.push({key,name:productionExportDatasetLabel(key),headers:[key==="po"?"PO":key==="carrier"?"ผู้ขนส่ง":"บริษัทผู้ส่งสินค้า","รถ","มาก่อน (ตรงเวลา)","มาช้าในเกณฑ์","ตรงเวลารวม","ตรงเวลา %","มาสาย","มาสาย %","สายเฉลี่ย (นาที)","Gate In → ยื่นเอกสาร เฉลี่ย (นาที)","รอตรวจรับ เฉลี่ย (นาที)","ตรวจรับ เฉลี่ย (นาที)","อยู่ในพื้นที่ เฉลี่ย (นาที)","ปฏิเสธ"],rows:d[key].map(x=>[x.name,x.vehicles,x.early,x.onTime,x.compliant,x.onTimePercent,x.late,x.latePercent,x.avgLateMinutes,x.avgGateToDocumentMinutes,x.avgWaitReceivingMinutes,x.avgReceivingMinutes,x.avgCycleMinutes,x.rejected])})}if(Array.isArray(d.quality))tables.push({key:"quality",name:"คุณภาพข้อมูล",headers:["รายการตรวจ","จำนวน","คำอธิบาย"],rows:d.quality.map(x=>[x.metric,x.value,x.note])});return tables}
