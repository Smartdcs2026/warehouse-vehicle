...
const FRONTEND_BUILD="2026.08.20-round205.1-complete-timeline-export";
...
async function init() {
  ...
  if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js?v=20260819-r204",{updateViaCache:"none"}).catch(() => undefined);
...
function renderAdminAppointmentData(){
  ...
  document.querySelectorAll("#appointmentExtraFieldsList .appointment-extra-field-row").forEach(appointmentBindExtraFieldRow);$("appointmentAddExtraField")?.addEventListener("click",()=>{const empty=$("appointmentExtraFieldsList")?.querySelector(".appointment-extra-empty");empty?.remove();appointmentAddExtraField()});$("appointmentProbeFileInput")?.addEventListener("change",()=>appointmentAdminDiscoverFile($("appointmentProbeFileInput").files?.[0],fieldRows));if(appointmentAdminFileProbe.result)appointmentRenderAdminProbe(fieldRows);
...
const PRODUCTION_EXPORT_DATASETS=[
  ["vehicles","เส้นทางรถครบทุกขั้นตอน","บังคับสร้างเสมอ · รถหนึ่งคันต่อหนึ่งแถว + Appointment ที่จับได้จริง"],
  ["plan","Plan vs Actual","สรุปแผนเทียบรถจริงและความตรงต่อเวลา"],
  ["hourly","รายชั่วโมง","แผน / เข้าจริง / จับคู่ แยก 24 ชั่วโมง"],
  ["vendor","บริษัทผู้ส่งสินค้า","KPI ตาม Vendor"],
  ["carrier","ผู้ขนส่ง","KPI ตาม Carrier"],
  ["po","เลข PO","KPI ตาม PO"],
  ["quality","คุณภาพข้อมูล","จำนวนข้อมูลที่ต้องตรวจและที่มาของปัญหา"]
];
const PRODUCTION_VEHICLE_COLUMNS=[
  ["autoId","Auto ID","หลัก"],["appointmentNo","เลขนัดหมาย","หลัก"],["companyName","บริษัทในระบบ","หลัก"],["vendor","Vendor จากไฟล์","นัดหมาย"],["carrier","Carrier","นัดหมาย"],["po","PO","นัดหมาย"],["driverName","คนขับ","หลัก"],["phone","โทรศัพท์","หลัก"],["vehiclePlate","ทะเบียน","หลัก"],["province","จังหวัด","หลัก"],["vehicleType","ประเภทรถ","หลัก"],["shiftName","กะ","หลัก"],["shiftBusinessDate","วันที่เริ่มกะ","หลัก"],["doorCode","ประตู","หลัก"],["currentStatus","สถานะ","หลัก"],
  ["plannedAt","เวลานัดที่ใช้จริง (HH:mm:ss)","นัดหมาย"],["gateInAt","Gate In (HH:mm:ss)","เวลา"],["referenceMode","จุดเริ่มคำนวณ","นัดหมาย"],["lateGraceMinutes","อนุโลม (HH:mm:ss)","นัดหมาย"],["deltaSeconds","ข้อเท็จจริงเทียบเวลานัด","นัดหมาย"],["timingLabel","ผลตามเกณฑ์","นัดหมาย"],["policyId","รหัสเกณฑ์เวลา","นัดหมาย"],
  ["documentSubmittedAt","ยื่นเอกสาร (HH:mm:ss)","เวลา"],["documentCheckedAt","ตรวจเอกสารเสร็จ (HH:mm:ss)","เวลา"],["firstCalledAt","เรียกรถครั้งแรก (HH:mm:ss)","เวลา"],["lastCalledAt","เรียกรถล่าสุด (HH:mm:ss)","เวลา"],["queueCallCount","จำนวนครั้งเรียก","เวลา"],["receivingStartedAt","เริ่มตรวจรับ (HH:mm:ss)","เวลา"],["receivingCompletedAt","รับสินค้าเสร็จ (HH:mm:ss)","เวลา"],["documentReturnedAt","รับเอกสารคืน (HH:mm:ss)","เวลา"],["gateOutAt","Gate Out (HH:mm:ss)","เวลา"],
  ["rejectedAt","เวลาปฏิเสธ (HH:mm:ss)","ปฏิเสธ"],["rejectionReason","เหตุผลปฏิเสธ","ปฏิเสธ"],["rejectionDetail","รายละเอียดปฏิเสธ","ปฏิเสธ"],["rejectionSupervisor","หัวหน้างานรับทราบ","ปฏิเสธ"],
  ["gateToDocumentSeconds","Gate In → ยื่นเอกสาร (HH:mm:ss)","ระยะเวลา"],["documentReviewSeconds","ตรวจเอกสาร (HH:mm:ss)","ระยะเวลา"],["readyToReceivingSeconds","พร้อม → เริ่มตรวจรับ (HH:mm:ss)","ระยะเวลา"],["receivingSeconds","ตรวจรับสินค้า (HH:mm:ss)","ระยะเวลา"],["receivingToReturnSeconds","รับเสร็จ → คืนเอกสาร (HH:mm:ss)","ระยะเวลา"],["returnToGateOutSeconds","คืนเอกสาร → Gate Out (HH:mm:ss)","ระยะเวลา"],["totalInSiteSeconds","เวลารวมในพื้นที่ (HH:mm:ss)","ระยะเวลา"]
];
function productionExportSelectedDatasets(){return[...document.querySelectorAll('[data-production-dataset]:checked')].map(x=>x.dataset.productionDataset)}
function productionExportSelectedVehicleColumns(){return[...document.querySelectorAll('[data-production-column]:checked')].map(x=>x.dataset.productionColumn)}
...
function renderAdminMonthlyExport(){
  const panel=$("adminPanel");if(!panel)return;const month=currentBangkokMonth(),dates=productionExportMonthDates(month),saved=(()=>{try{return JSON.parse(localStorage.getItem("wvf_production_export_columns")||"null")}catch{return null}})(),selectedCols=new Set(Array.isArray(saved)?saved:PRODUCTION_VEHICLE_COLUMNS.map(x=>x[0]));
  panel.innerHTML=`<div class="admin-section-head clean-admin-head export-head"><div><h3>ส่งออกข้อมูล</h3><p>สร้างไฟล์สำหรับ Excel / CSV จากข้อมูลจริงชุดเดียวกับระบบ โดยตรวจจำนวนก่อนสร้างไฟล์</p></div></div><section class="production-export-card"><div class="production-export-filter"><label><span>ช่วงข้อมูล</span><select id="productionExportRangeMode"><option value="month">รายเดือน</option><option value="custom">กำหนดวันที่</option></select></label><label id="productionExportMonthWrap"><span>เดือน</span><input id="productionExportMonth" type="month" value="${month}" min="2020-01" max="2100-12"></label><label class="production-export-custom" hidden><span>วันที่เริ่ม</span><input id="productionExportFrom" type="date" value="${dates?.from||currentBangkokDate()}"></label><label class="production-export-custom" hidden><span>วันที่สิ้นสุด</span><input id="productionExportTo" type="date" value="${dates?.to||currentBangkokDate()}"></label><label><span>กะ</span><select id="productionExportShift"><option value="">ทุกกะ</option></select></label><label><span>รูปแบบไฟล์</span><select id="productionExportFormat"><option value="excel">Excel Workbook (.xlsx)</option><option value="csv">CSV</option></select></label></div><section class="production-export-datasets"><header><div><b>ชุดข้อมูล</b><span>เลือกเฉพาะข้อมูลที่ต้องใช้ เพื่อลดเวลาประมวลผล</span></div><button id="productionExportAllDatasets" class="quiet-button" type="button">เลือกทั้งหมด</button></header><div>${PRODUCTION_EXPORT_DATASETS.map(([key,label,note])=>`<label><input type="checkbox" data-production-dataset="${key}" ${["vehicles","plan","hourly","vendor","carrier","po","quality"].includes(key)?"checked":""} ${key==="vehicles"?"disabled":""}><span><b>${label}${key==="vehicles"?' <em class="production-export-required">จำเป็น</em>':""}</b><small>${note}</small></span></label>`).join("")}</div></section><details class="production-export-columns"><summary><b>คอลัมน์รายละเอียดรถ</b><span>Admin เลือกคอลัมน์ที่จะใส่ในชีท/ไฟล์รายละเอียดรถ</span></summary><div class="production-export-column-actions"><button id="productionExportCoreColumns" type="button">ข้อมูลหลัก</button><button id="productionExportAllColumns" type="button">ทั้งหมด</button><button id="productionExportNoColumns" type="button">ล้างเลือก</button></div><div class="production-export-column-grid">${PRODUCTION_VEHICLE_COLUMNS.map(([key,label,group])=>`<label><input type="checkbox" data-production-column="${key}" ${selectedCols.has(key)?"checked":""}><span>${label}<small>${group}</small></span></label>`).join("")}</div><div id="productionExportExtraColumns"></div></details><section class="production-export-lineage"><b>หลักการนับ</b><span>ชีทเส้นทางรถและประวัติเหตุการณ์ถูกสร้างอัตโนมัติ · Appointment ใช้ appointment_key ที่จับไว้จริงเท่านั้น · เวลาเป็น HH:mm:ss และวันที่แยก dd/MM/yyyy · ไม่มีการเก็บไฟล์ Export ลง D1</span></section><div class="production-export-actions"><button id="productionExportPreview" class="outline-button" type="button">ตรวจข้อมูล</button><button id="productionExportDownload" class="primary" type="button" disabled>สร้างไฟล์</button></div><div id="productionExportStatus" class="monthly-export-status"><b>กำลังอ่านค่าตั้งต้น</b><span>รอสักครู่</span></div></section>`;
...
function bindProductionExportUi(){
  ...
  document.querySelectorAll("[data-production-column]").forEach(x=>x.addEventListener("change",()=>{try{localStorage.setItem("wvf_production_export_columns",JSON.stringify(productionExportSelectedVehicleColumns()))}catch{}}));
  $("productionExportAllDatasets")?.addEventListener("click",()=>{document.querySelectorAll("[data-production-dataset]").forEach(x=>x.checked=true);productionExportInvalidate()});
  $("productionExportCoreColumns")?.addEventListener("click",()=>{document.querySelectorAll("[data-production-column]").forEach(x=>x.checked=["หลัก","นัดหมาย"].includes(PRODUCTION_VEHICLE_COLUMNS.find(c=>c[0]===x.dataset.productionColumn)?.[2]));productionExportInvalidate()});
  $("productionExportAllColumns")?.addEventListener("click",()=>{document.querySelectorAll("[data-production-column]").forEach(x=>x.checked=true);productionExportInvalidate()});
  $("productionExportNoColumns")?.addEventListener("click",()=>{document.querySelectorAll("[data-production-column]").forEach(x=>x.checked=false);productionExportInvalidate()});
...
async function loadProductionExportMeta(){const st=$("productionExportStatus"),range=productionExportRangeFromUi();if(!range?.from||!range?.to)return;try{const q=new URLSearchParams({from:range.from,to:range.to,mode:"meta"}),data=await api(`/api/admin/production-export?${q}`);productionExportState.meta=data;const shift=$("productionExportShift");if(shift){const value=shift.value;shift.innerHTML='<option value="">ทุกกะ</option>'+((data.shifts||[]).map(s=>`<option value="${escapeHtml(s.shiftId)}">${escapeHtml(s.shiftName)}</option>`).join(""));shift.value=value}const extra=$("productionExportExtraColumns"),fields=data.extraFields||[];if(extra)extra.innerHTML=fields.length?`<div class="production-export-extra-note"><b>คอลัมน์เพิ่มเติมจากไฟล์</b><span>${fields.map(x=>escapeHtml(x.label)).join(" · ")} จะถูกส่งออกตามที่ Admin เปิด “ส่งออกไฟล์” ไว้</span></div>`:"";if(st)st.innerHTML=`<b>พร้อมตรวจข้อมูล</b><span>เวลาอ้างอิง ${escapeHtml(data.referenceMode||"-")} · อนุโลม ${Number(data.lateGraceMinutes||0)} นาที · ไม่เก็บไฟล์ลง D1</span>`}catch(error){if(st)st.innerHTML=`<b>อ่านค่าตั้งต้นไม่สำเร็จ</b><span>${escapeHtml(error.message||"กรุณาลองใหม่")}</span>`}}
...
async function previewProductionExport(){if(productionExportState.busy)return;const range=productionExportRangeFromUi(),datasets=productionExportSelectedDatasets(),shift=$("productionExportShift")?.value||"",st=$("productionExportStatus"),btn=$("productionExportPreview");if(!range?.from||!range?.to){await showNotice("warning","กรุณาเลือกช่วงวันที่");return}if(!datasets.length){await showNotice("warning","กรุณาเลือกชุดข้อมูลอย่างน้อย 1 ชุด");return}productionExportState.busy=true;if(btn){btn.disabled=true;btn.textContent="กำลังตรวจ"}if(st)st.innerHTML='<b>กำลังตรวจข้อมูล</b><span>อ่านเฉพาะชุดที่เลือก และจะใช้ข้อมูลชุดนี้สร้างไฟล์ต่อทันที</span>';try{const q=new URLSearchParams({from:range.from,to:range.to,shiftId:shift,datasets:datasets.join(",")}),data=await api(`/api/admin/production-export?${q}`);productionExportState.data=data;productionExportState.key=productionExportRequestKey();const labels=Object.entries(data.counts||{}).map(([k,v])=>`${productionExportDatasetLabel(k)} ${Number(v).toLocaleString("th-TH")}`).join(" · ");if(st)st.innerHTML=`<b>ตรวจแล้ว พร้อมสร้างไฟล์</b><span>${escapeHtml(data.range?.fromDate||range.from)} – ${escapeHtml(data.range?.toDate||range.to)}${data.shift?.shiftName?` · ${escapeHtml(data.shift.shiftName)}`:" · ทุกกะ"} · ${labels||"ไม่มีข้อมูล"} · ${Number(data.durationMs||0).toLocaleString("th-TH")} ms</span>`;if($("productionExportDownload"))$("productionExportDownload").disabled=false}catch(error){productionExportState.data=null;if(st)st.innerHTML=`<b>ตรวจข้อมูลไม่สำเร็จ</b><span>${escapeHtml(error.message||"กรุณาลองใหม่")}</span>`;await showNotice("error",error.message||"ตรวจข้อมูลไม่สำเร็จ")}finally{productionExportState.busy=false;if(btn){btn.disabled=false;btn.textContent="ตรวจข้อมูล"}}}
function productionExportDatasetLabel(key){if(key==="events")return"ประวัติเหตุการณ์";return Object.fromEntries(PRODUCTION_EXPORT_DATASETS.map(x=>[x[0],x[1]]))[key]||key}
...
function productionExportDeltaText(item){const d=Number(item?.deltaSeconds);if(!Number.isFinite(d))return"";if(d<0)return`มาก่อน ${formatExportDuration(Math.abs(d))}`;if(item?.timingStatus==="LATE")return`มาสาย ${formatExportDuration(d)}`;if(d===0)return"ตรงเวลาพอดี";return`มาช้า ${formatExportDuration(d)} · อยู่ในเกณฑ์`}
function productionExportValue(item,key){const timeKeys=new Set(["plannedAt","gateInAt","documentSubmittedAt","documentCheckedAt","firstCalledAt","lastCalledAt","receivingStartedAt","receivingCompletedAt","documentReturnedAt","gateOutAt","rejectedAt"]),durationKeys=new Set(["gateToDocumentSeconds","documentReviewSeconds","readyToReceivingSeconds","receivingSeconds","receivingToReturnSeconds","returnToGateOutSeconds","totalInSiteSeconds"]);if(timeKeys.has(key))return productionExportTimeOnly(item[key]);if(durationKeys.has(key))return productionExportDurationCell(item[key]);if(key==="lateGraceMinutes")return productionExportDurationCell(Number(item[key]||0)*60);if(key==="deltaSeconds")return productionExportDeltaText(item);return item[key]==null?"":item[key]}
function productionExportJourneyTable(items=[]){const headers=["Auto ID","เลขนัดหมาย","จับคู่ Appointment","Appointment Key","วันที่นัด","PERIOD","FROM","TO","TO วันถัดไป","จุดอ้างอิงที่ใช้","วันที่เวลานัดที่ใช้","เวลานัดที่ใช้ (HH:mm:ss)","อนุโลม (HH:mm:ss)","สถานะตามเกณฑ์","ข้อเท็จจริงเทียบเวลานัด","เกินเกณฑ์ (HH:mm:ss)","บริษัทในระบบ","Vendor จากคู่ที่จับ","Carrier จากคู่ที่จับ","PO จากคู่ที่จับ","คนขับ","โทรศัพท์","ทะเบียน","จังหวัด","ประเภทรถ","กะ","วันที่เริ่มกะ","ประตู","สถานะล่าสุด","วันที่ Gate In","Gate In","วันที่ยื่นเอกสาร","ยื่นเอกสาร","Gate In → ยื่น","วันที่ตรวจเอกสารเสร็จ","ตรวจเอกสารเสร็จ","ยื่น → ตรวจเอกสาร","วันที่พร้อมตรวจรับ","พร้อมตรวจรับ","ยื่นเอกสาร → พร้อมตรวจรับ","วันที่เรียกครั้งแรก","เรียกครั้งแรก","พร้อมตรวจรับ → เรียกครั้งแรก","วันที่เรียกล่าสุด","เรียกล่าสุด","จำนวนครั้งเรียก","วันที่เริ่มตรวจรับ","เริ่มตรวจรับ","เรียกครั้งแรก → เริ่มตรวจรับ","เรียกล่าสุด → เริ่มตรวจรับ","วันที่รับสินค้าเสร็จ","รับสินค้าเสร็จ","ตรวจรับสินค้า","วันที่รับเอกสารคืน","รับเอกสารคืน","รับเสร็จ → คืนเอกสาร","วันที่ Gate Out","Gate Out","คืนเอกสาร → Gate Out","Gate In → Gate Out","วันที่ปฏิเสธ","เวลาปฏิเสธ","เหตุผลปฏิเสธ","รายละเอียดปฏิเสธ","หัวหน้างานรับทราบ","ตำแหน่งหัวหน้างาน","สถานะก่อนปฏิเสธ","Policy ID","Appointment Snapshot","Metric Snapshot","วันที่ชุด Appointment","เวลาชุด Appointment"];
  const rows=items.map(x=>[x.autoId,x.appointmentNo,x.appointmentMatched?"จับคู่ได้":"ไม่พบคู่",x.appointmentKey||"",productionExportSourceDate(x.appointmentDate),productionExportMinuteTime(x.periodMinute),productionExportMinuteTime(x.fromMinute),productionExportMinuteTime(x.toMinute),x.toNextDay?"ใช่":"ไม่",x.referenceMode||"",productionExportDateOnly(x.plannedAt),productionExportTimeOnly(x.plannedAt),productionExportDurationCell(Number(x.lateGraceMinutes||0)*60),x.timingLabel||"",productionExportDeltaText(x),productionExportDurationCell(x.lateBySeconds),x.companyName||"",x.vendor||"",x.carrier||"",x.po||"",x.driverName||"",x.phone||"",x.vehiclePlate||"",x.province||"",x.vehicleType||"",x.shiftName||"",x.shiftBusinessDate?productionExportSourceDate(x.shiftBusinessDate):"",x.doorCode||"",x.currentStatus||"",productionExportDateOnly(x.gateInAt),productionExportTimeOnly(x.gateInAt),productionExportDateOnly(x.documentSubmittedAt),productionExportTimeOnly(x.documentSubmittedAt),productionExportDurationCell(x.gateToDocumentSeconds),productionExportDateOnly(x.documentCheckedAt),productionExportTimeOnly(x.documentCheckedAt),productionExportDurationCell(x.documentReviewSeconds),productionExportDateOnly(x.readyForReceivingAt),productionExportTimeOnly(x.readyForReceivingAt),productionExportDurationCell(x.readyForReceivingAt&&x.documentSubmittedAt&&x.readyForReceivingAt>=x.documentSubmittedAt?x.readyForReceivingAt-x.documentSubmittedAt:null),productionExportDateOnly(x.firstCalledAt),productionExportTimeOnly(x.firstCalledAt),productionExportDurationCell(x.readyToFirstCallSeconds),productionExportDateOnly(x.lastCalledAt),productionExportTimeOnly(x.lastCalledAt),x.queueCallCount??0,productionExportDateOnly(x.receivingStartedAt),productionExportTimeOnly(x.receivingStartedAt),productionExportDurationCell(x.firstCallToReceivingSeconds),productionExportDurationCell(x.lastCallToReceivingSeconds),productionExportDateOnly(x.receivingCompletedAt),productionExportTimeOnly(x.receivingCompletedAt),productionExportDurationCell(x.receivingSeconds),productionExportDateOnly(x.documentReturnedAt),productionExportTimeOnly(x.documentReturnedAt),productionExportDurationCell(x.receivingToReturnSeconds),productionExportDateOnly(x.gateOutAt),productionExportTimeOnly(x.gateOutAt),productionExportDurationCell(x.returnToGateOutSeconds),productionExportDurationCell(x.totalInSiteSeconds),productionExportDateOnly(x.rejectedAt),productionExportTimeOnly(x.rejectedAt),x.rejectionReason||"",x.rejectionDetail||"",x.rejectionSupervisor||"",x.rejectionSupervisorPosition||"",x.rejectionFromStatus||"",x.policyId||"",x.appointmentSnapshotKey??"",x.metricSnapshotKey??"",productionExportSourceDate(x.sourceSnapshotDate),x.sourceSnapshotTime||""]);
  return{key:"journey",name:"เส้นทางรถครบทุกขั้นตอน",headers,rows}}
function productionExportEventsTable(items=[]){return{key:"events",name:"ประวัติเหตุการณ์ทั้งหมด",headers:["ลำดับ","Auto ID","เลขนัดหมาย","บริษัท","ทะเบียน","วันที่เหตุการณ์","เวลาเหตุการณ์ (HH:mm:ss)","ขั้นตอน/เหตุการณ์","รหัสเหตุการณ์","แหล่งข้อมูล","ประตู","ประตูก่อนหน้า","ผู้ดำเนินการ","เหตุผล","หัวหน้างาน","สถานะก่อนหน้า","หมายเหตุ","Metadata"],rows:items.map(x=>[x.sequence,x.autoId,x.appointmentNo||"",x.companyName||"",x.vehiclePlate||"",productionExportDateOnly(x.eventAt),productionExportTimeOnly(x.eventAt),x.eventLabel||"",x.eventCode||"",x.source||"",x.doorCode||"",x.previousDoorCode||"",x.actor||"",x.reason||"",x.supervisor||"",x.fromStatus||"",x.note||"",x.metadata||""])}}
function productionExportTables(data){const tables=[],selectedCols=new Set(productionExportSelectedVehicleColumns()),extra=data.extraFields||[],d=data.data||{};if(Array.isArray(d.vehicles)){tables.push(productionExportJourneyTable(d.vehicles));if(Array.isArray(d.events))tables.push(productionExportEventsTable(d.events));const cols=PRODUCTION_VEHICLE_COLUMNS.filter(x=>selectedCols.has(x[0])),headers=cols.map(x=>x[1]).concat(extra.map(x=>x.label)),rows=d.vehicles.map(item=>cols.map(c=>productionExportValue(item,c[0])).concat(extra.map(x=>item.extras?.[x.key]??"")));if(cols.length||extra.length)tables.push({key:"vehicles",name:"รายละเอียดรถ (เลือกคอลัมน์)",headers,rows})}if(Array.isArray(d.plan)){tables.push({key:"plan",name:"Plan vs Actual",headers:["ตามแผน","เข้าจริง","รถมีเลขนัดหมาย","จับคู่ได้","จับคู่ %","ยังไม่จับคู่","ต่างจากแผน","มาก่อน (ตรงเวลา)","มาช้าในเกณฑ์","ตรงเวลารวม","ตรงเวลา %","มาสาย","มาสาย %","สายเฉลี่ย (HH:mm:ss)","หลายเกณฑ์ย้อนหลัง","เวลาอ้างอิงปัจจุบัน","อนุโลมปัจจุบัน (HH:mm:ss)"],rows:d.plan.map(x=>[x.planned,x.actual,x.withAppointment,x.matched,x.matchPercent,x.unmatched,x.difference,x.early,x.onTime,x.compliant,x.compliancePercent,x.late,x.latePercent,productionExportDurationCell(Number(x.avgLateMinutes||0)*60),x.mixedPolicy?"ใช่":"ไม่",x.referenceMode,productionExportDurationCell(Number(x.lateGraceMinutes||0)*60)])})}if(Array.isArray(d.hourly)){tables.push({key:"hourly",name:"รายชั่วโมง",headers:["ช่วงเวลา","ตามแผน","เข้าจริง","รถมีเลขนัดหมาย","จับคู่ได้","ต่างจากแผน"],rows:d.hourly.map(x=>[String(x.label||"").replace(/(\d{2}:\d{2})/g,"$1:00"),x.planned,x.actual,x.withAppointment,x.matched,x.difference])})}for(const key of ["vendor","carrier","po"]){if(!Array.isArray(d[key]))continue;tables.push({key,name:productionExportDatasetLabel(key),headers:[key==="po"?"PO":key==="carrier"?"ผู้ขนส่ง":"บริษัทผู้ส่งสินค้า","รถ","มาก่อน (ตรงเวลา)","มาช้าในเกณฑ์","ตรงเวลารวม","ตรงเวลา %","มาสาย","มาสาย %","สายเฉลี่ย (HH:mm:ss)","Gate In → ยื่นเอกสาร เฉลี่ย (HH:mm:ss)","รอตรวจรับ เฉลี่ย (HH:mm:ss)","ตรวจรับ เฉลี่ย (HH:mm:ss)","อยู่ในพื้นที่ เฉลี่ย (HH:mm:ss)","ปฏิเสธ"],rows:d[key].map(x=>[x.name,x.vehicles,x.early,x.onTime,x.compliant,x.onTimePercent,x.late,x.latePercent,productionExportDurationCell(Number(x.avgLateMinutes||0)*60),productionExportDurationCell(Number(x.avgGateToDocumentMinutes||0)*60),productionExportDurationCell(Number(x.avgWaitReceivingMinutes||0)*60),productionExportDurationCell(Number(x.avgReceivingMinutes||0)*60),productionExportDurationCell(Number(x.avgCycleMinutes||0)*60),x.rejected])})}if(Array.isArray(d.quality))tables.push({key:"quality",name:"คุณภาพข้อมูล",headers:["รายการตรวจ","จำนวน","คำอธิบาย"],rows:d.quality.map(x=>[x.metric,x.value,x.note])});return tables}
function downloadProductionExport(){const data=productionExportState.data;if(!data||productionExportState.key!==productionExportRequestKey()){showNotice("warning","ช่วงข้อมูลหรือชุดข้อมูลมีการเปลี่ยน กรุณากดตรวจข้อมูลใหม่ก่อนสร้างไฟล์");return}const tables=productionExportTables(data);if(!tables.length){showNotice("info","ไม่มีข้อมูลสำหรับสร้างไฟล์");return}const format=$("productionExportFormat")?.value||"excel",base=`Warehouse_Vehicle_${data.range.fromDate}_${data.range.toDate}${data.shift?.shiftName?`_${safeExportFilename(data.shift.shiftName)}`:""}`;if(format==="csv"){tables.forEach((t,i)=>setTimeout(()=>downloadTextFile("\uFEFF"+[t.headers,...t.rows].map(row=>row.map(csvCell).join(",")).join("\r\n"),`${base}_${safeExportFilename(t.name)}.csv`,"text/csv;charset=utf-8"),i*180));showNotice("success",`สร้าง CSV ${tables.length} ไฟล์แล้ว`);return}const bytes=productionExportXlsx(tables,data);downloadBinaryFile(bytes,`${base}.xlsx`,`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`);showNotice("success",`สร้าง Excel ${tables.length} ชีทแล้ว`)}
...
function xlsxCellXml(value,row,col,style=0){const ref=`${xlsxColumnName(col)}${row}`,attr=style?` s="${style}"`:"";if(typeof value==="number"&&Number.isFinite(value))return `<c r="${ref}"${attr}><v>${value}</v></c>`;const text=xmlEscape(value==null?"":value);return `<c r="${ref}" t="inlineStr"${attr}><is><t xml:space="preserve">${text}</t></is></c>`}
function xlsxWorksheetXml(table){const rows=[table.headers,...table.rows],sheetData=rows.map((row,i)=>`<row r="${i+1}">${row.map((v,j)=>xlsxCellXml(v,i+1,j,i===0?1:0)).join("")}</row>`).join(""),cols=table.headers.map((_,i)=>`<col min="${i+1}" max="${i+1}" width="${Math.min(34,Math.max(12,String(table.headers[i]||"").length+3))}" customWidth="1"/>`).join("");return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>${cols}</cols><sheetData>${sheetData}</sheetData><autoFilter ref="A1:${xlsxColumnName(Math.max(0,table.headers.length-1))}${Math.max(1,rows.length)}"/></worksheet>`}
...
function productionExportXlsx(tables,data){const meta={name:"ข้อมูลไฟล์",headers:["รายการ","ค่า"],rows:[["ช่วงข้อมูล",`${data.range.fromDate} – ${data.range.toDate}`],["กะ",data.shift?.shiftName||"ทุกกะ"],["เวลาอ้างอิงปัจจุบัน",data.referenceMode||"-"],["อนุโลมปัจจุบัน (HH:mm:ss)",productionExportDurationCell(Number(data.lateGraceMinutes||0)*60)],["สร้างวันที่",productionExportDateOnly(data.generatedAt)],["สร้างเวลา",productionExportTimeOnly(data.generatedAt)],["ที่มารถ",data.lineage?.vehicles||""],["ที่มาเส้นทาง",data.lineage?.journey||""],["ที่มาประวัติเหตุการณ์",data.lineage?.events||""],["ที่มาผลเวลา",data.lineage?.timing||""],["ที่มา Appointment",data.lineage?.appointment||""],["ที่มาแผน",data.lineage?.plan||""],["รูปแบบเวลา",data.lineage?.timeFormat||"HH:mm:ss"],["หมายเหตุ","ผลรถรายคันใช้ policy และ appointment_key ที่บันทึกกับรถคันนั้น ส่วน Plan ใช้ค่าปัจจุบันของ Admin"],["การจัดเก็บ","ไฟล์ถูกสร้างใน Browser และไม่ได้เก็บลง D1"]]},all=[meta,...tables],names=xlsxSafeSheetNames(all),sheetEntries=all.map((t,i)=>({name:`xl/worksheets/sheet${i+1}.xml`,data:xlsxWorksheetXml(t)})),contentTypes=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/>${all.map((_,i)=>`<Override PartName="/xl/worksheets/sheet${i+1}.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>`).join("")}</Types>`,rels=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>`,workbook=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>${names.map((n,i)=>`<sheet name="${xmlEscape(n)}" sheetId="${i+1}" r:id="rId${i+1}"/>`).join("")}</sheets></workbook>`,workbookRels=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${all.map((_,i)=>`<Relationship Id="rId${i+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet${i+1}.xml"/>`).join("")}<Relationship Id="rId${all.length+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>`,styles=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="10"/><name val="Aptos"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="10"/><name val="Aptos"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF2F6FD6"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="2"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1"/></cellXfs></styleSheet>`;return zipStore([{name:"[Content_Types].xml",data:contentTypes},{name:"_rels/.rels",data:rels},{name:"xl/workbook.xml",data:workbook},{name:"xl/_rels/workbook.xml.rels",data:workbookRels},{name:"xl/styles.xml",data:styles},...sheetEntries])}
...
function clearSession() { stopCamera();cancelDashboardRequest();dashboardState.data=null;dashboardState.dataIdentity="";dashboardState.error="";dashboardState.analyticsError="";dashboardState.analyticsLastLoadedAt=0;dashboardState.lastLoadedAt=0;dashboardState.snapshotLoaded=false;clearDashboardStaleRecovery(true); inboundLiveState.version="";inboundLiveState.checking=false;inboundLiveState.failures=0;inboundLiveState.nextAllowedAt=0;alertSoundState.initialized=false;alertSoundState.levels.clear();sessionStorage.removeItem("wvf_token"); sessionStorage.removeItem("wvf_dashboard_snapshot_r118"); state.token=""; state.user=null; state.display={dashboardEnabled:true,datatableEnabled:true}; state.appointment={moduleEnabled:false,uploadEnabled:false}; appointmentStopWorker(); appointmentUploadState.config=null;appointmentUploadState.result=null;appointmentUploadState.preview=null;appointmentUploadState.mappingNeeded=null;try{appointmentAdminFileProbe.worker?.terminate()}catch{}appointmentAdminFileProbe.worker=null;appointmentAdminFileProbe.result=null;appointmentAdminFileProbe.fileName=""; $("appView").hidden=true; $("loginView").hidden=false; $("loginPassword").value=""; window.scrollTo(0, 0); }
