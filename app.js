...
const FRONTEND_BUILD="2026.08.24-round207.20-dashboard-action-layout";
...
function releaseCandidateSummaryHtml(data){const rc=data?.releaseCandidate;if(!rc)return"";const blockers=Array.isArray(rc.blockers)?rc.blockers:[],cfg=rc.configuration||{},go=rc.decision==="GO"&&rc.ready===true,tone=go?"pass":"fail";return`<section id="releaseCandidateSummary" class="production-readiness ${tone}"><header><div><b>${escapeHtml(rc.name||"Release Candidate")}</b><small>ผลตัดสินก่อน Deploy จากการตรวจอัตโนมัติและ UAT หน้างาน</small></div><span class="health-mini ${tone}">${go?"GO":"NO-GO"}</span></header><div class="production-readiness-summary"><span><small>Blocker</small><b>${Number(rc.blockerCount||blockers.length)}</b></span><span><small>รถปัจจุบัน</small><b>${Number(cfg.activeVehicles||0)}/${Number(cfg.activeVehicleLimit||500)}</b></span><span><small>Dashboard / Datatable</small><b>${cfg.dashboardEnabled?"เปิด":"ปิด"} / ${cfg.datatableEnabled?"เปิด":"ปิด"}</b></span></div>${blockers.length?`<div class="production-readiness-warning"><b>ห้ามเปิด Production จนกว่าจะจัดการครบ</b><span>${blockers.slice(0,6).map(item=>`${escapeHtml(item.label)} — ${escapeHtml(item.action||item.detail||"")}`).join(" · ")}${blockers.length>6?` · และอีก ${blockers.length-6} รายการ`:""}</span></div>`:`<div class="production-readiness-warning"><b>Release Candidate ผ่านเงื่อนไข</b><span>ผลตรวจอัตโนมัติผ่านและมีการยืนยัน UAT ครบแล้ว</span></div>`}</section>`}
