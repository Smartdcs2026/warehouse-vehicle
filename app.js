...
const datatableState={meta:null,data:null,busy:false,reloadRequested:false,requestSeq:0,detailBusy:false,refreshMetaRequested:false,activity:[],shiftAutoDate:false,stage:"overview",from:"",to:"",shiftId:"",search:"",sla:"ALL",status:"ALL",door:"",actor:"",sort:"start_desc",page:1,limit:25,problemOnly:false,rejectedOnly:false,searchTimer:0,columns:new Set(JSON.parse(localStorage.getItem("wvf_dt_columns_r95")||'["company","plate","shift","actor"]'))};
...
async function init() {
  ...
  if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js?v=20260812-r95",{updateViaCache:"none"}).catch(() => undefined);
...
function renderDatatableLayout(){
  ...
  $("pageContent").innerHTML=`<section class="dt-page dt-clean-page">
    <header class="dt-clean-toolbar"><div class="dt-range-controls"><label><span>วันที่เริ่ม</span><input id="dtFrom" type="date" value="${escapeHtml(datatableState.from)}"></label><label><span>วันที่สิ้นสุด</span><input id="dtTo" type="date" value="${escapeHtml(datatableState.to)}"></label><label><span>กะ</span><select id="dtShift"><option value="">ทุกกะ</option>${shiftOptions}</select></label><span id="dtShiftHint" class="dt-shift-hint" hidden></span><button id="dtRefresh" class="quiet-button" type="button">รีเฟรช</button><button id="dtFullscreen" class="outline-button" type="button">เต็มหน้าจอ</button><button id="dtExport" class="primary" type="button">ส่งออก</button></div></header>
    <section id="dtSummary" class="dt-summary dt-summary-compact"></section>
    <nav id="dtStageTabs" class="dt-stage-tabs dt-stage-tabs-clean">${stages.map(s=>`<button type="button" data-dt-stage="${s.key}" class="${datatableState.stage===s.key?"active":""}"><span>${escapeHtml(s.label)}</span></button>`).join("")}</nav>
    <section class="dt-current-rule"><div><small>เกณฑ์เวลาของช่วงนี้</small><b id="dtRuleStageName">${escapeHtml(datatableStageMeta().label)}</b></div><div id="dtCurrentRules" class="dt-current-rule-levels"></div><div class="dt-current-rule-actions"><button id="dtAllRules" class="quiet-button" type="button">ดูเกณฑ์ทั้งหมด</button><button id="dtActivityButton" class="quiet-button" type="button">กิจกรรมล่าสุด</button></div></section>
    <main class="dt-main dt-main-full"><div class="dt-filters"><div class="dt-search"><input id="dtSearch" value="${escapeHtml(datatableState.search)}" placeholder="ค้นหาเลขนัดหมาย / บริษัท / ทะเบียน / คนขับ"><span aria-hidden="true">⌕</span></div><select id="dtSla"><option value="ALL">ระดับแจ้งเตือน: ทั้งหมด</option>${["NORMAL","WATCH","WARNING","URGENT","CRITICAL"].map(v=>`<option value="${v}">${alertLevelLabel(v)}</option>`).join("")}</select><select id="dtStatus"><option value="ALL">สถานะ: ทั้งหมด</option><option value="ACTIVE">กำลังดำเนินการ</option><option value="CLOSED">เสร็จสิ้น</option><option value="REJECTED">ปฏิเสธรับสินค้า</option>${Object.keys(DATATABLE_STATUS_OPTIONS).map(v=>`<option value="${v}">${DATATABLE_STATUS_OPTIONS[v]}</option>`).join("")}</select><select id="dtDoor"><option value="">ประตู: ทั้งหมด</option>${doorOptions}</select><select id="dtActor"><option value="">ผู้ดำเนินการ: ทั้งหมด</option>${actorOptions}</select></div>
      <div class="dt-tools"><select id="dtSort"><option value="start_desc">ใหม่ → เก่า</option><option value="start_asc">เก่า → ใหม่</option><option value="duration_desc">นาน → สั้น</option><option value="duration_asc">สั้น → นาน</option><option value="appointment_asc">เลขนัดหมาย น้อย → มาก</option><option value="appointment_desc">เลขนัดหมาย มาก → น้อย</option><option value="company_asc">บริษัท A → Z</option></select><button id="dtReset" class="quiet-button" type="button">ล้างตัวกรอง</button><button id="dtProblem" class="outline-button ${datatableState.problemOnly?"is-active":""}" type="button">เฉพาะมีปัญหา</button><button id="dtColumns" class="outline-button" type="button">เลือกคอลัมน์</button></div>
  ...
  renderDatatableSide();syncDatatableShiftHint();bindDatatableEvents();updateFullscreenButton();
}
...
function bindDatatableEvents(){
  ...
  $("dtFrom")?.addEventListener("change",()=>{datatableState.shiftAutoDate=false;datatableState.from=$("dtFrom").value;datatableState.page=1;loadDatatable(true)});$("dtTo")?.addEventListener("change",()=>{datatableState.shiftAutoDate=false;datatableState.to=$("dtTo").value;datatableState.page=1;loadDatatable(true)});$("dtShift")?.addEventListener("change",()=>{datatableState.shiftId=$("dtShift").value;adjustDatatableDateForCrossDayShift();syncDatatableShiftHint();datatableState.page=1;loadDatatable(true)});
  ...
  $("dtProblem")?.addEventListener("click",()=>{datatableState.problemOnly=!datatableState.problemOnly;datatableState.page=1;$("dtProblem").classList.toggle("is-active",datatableState.problemOnly);loadDatatable(true)});$("dtReset")?.addEventListener("click",resetDatatableFilters);$("dtRefresh")?.addEventListener("click",refreshDatatableAll);$("dtFullscreen")?.addEventListener("click",toggleFullscreen);$("dtExport")?.addEventListener("click",downloadDatatableExport);$("dtColumns")?.addEventListener("click",chooseDatatableColumns);$("dtAllRules")?.addEventListener("click",showDatatableAllRules);$("dtActivityButton")?.addEventListener("click",showDatatableActivity);
...
function renderDatatableData(data){renderDatatableSummary(data.summary||{});renderDatatableHeaderStatus(data);renderDatatableRows(data.items||[]);renderDatatablePager(data);renderDatatableActivity(data.activity||[],data.generatedAt);renderDatatableSide();syncDatatableShiftHint(data.shiftContext)}
...
function renderDatatableHeaderStatus(data){const total=Number(data.total||0),button=$("dtRefresh");if(button)button.title=`${total.toLocaleString("th-TH")} รายการ · อัปเดต ${formatDateShort(data.generatedAt)}`;}
function datatableColumnOn(name){return datatableState.columns.has(name)}
function renderDatatableRows(items){const table=$("dtTable");if(!table)return;const overview=datatableState.stage==="overview",labels=datatableStageLabels();const optional=[datatableColumnOn("company")?`<span>บริษัท</span>`:"",datatableColumnOn("plate")?`<span>ทะเบียน</span>`:"",datatableColumnOn("shift")?`<span>กะ</span>`:""].join("");const actorHead=datatableColumnOn("actor")?`<span>ผู้ดำเนินการ</span>`:"";const head=overview?`<div class="dt-row dt-head"><span>เลขนัดหมาย</span>${optional}<span>สถานะ</span><span>เวลารวม</span><span>ระดับแจ้งเตือน</span><span>ประตู</span>${actorHead}<span>สั่งการ</span><span>รายละเอียด</span></div>`:`<div class="dt-row dt-head"><span>เลขนัดหมาย</span>${optional}<span>${escapeHtml(labels[0])}</span><span>${escapeHtml(labels[1])}</span><span>ใช้เวลา</span><span>ระดับแจ้งเตือน</span>${actorHead}<span>สั่งการ</span><span>รายละเอียด</span></div>`;
  const body=items.length?items.map(row=>{const company=datatableColumnOn("company")?`<span class="dt-company" data-label="บริษัท">${escapeHtml(row.companyName||"-")}</span>`:"",plate=datatableColumnOn("plate")?`<span data-label="ทะเบียน">${escapeHtml(joinText(row.vehiclePlate,row.province))}</span>`:"",shift=datatableColumnOn("shift")?`<span data-label="กะ">${escapeHtml(row.shiftName||"-")}${row.shiftBusinessDate?`<small class="dt-shift-date">เริ่ม ${escapeHtml(row.shiftBusinessDate.slice(8,10)+"/"+row.shiftBusinessDate.slice(5,7))}</small>`:""}</span>`:"",actor=datatableColumnOn("actor")?`<span data-label="ผู้ดำเนินการ">${escapeHtml(row.stageActor||"ระบบ")}</span>`:"",sla=`<span class="dt-sla" style="--dt-alert:${safeColor(row.stageAlertColor)}" data-label="ระดับแจ้งเตือน"><i></i><b>${alertLevelLabel(row.stageAlertLevel)}</b></span>`,command=`<button class="dt-command ${alertLevelRank(row.stageAlertLevel)>=3?"urgent":""}" type="button" data-dt-command="${escapeHtml(row.autoId)}">${datatableCommandLabel(row)}</button>`,detail=`<button class="dt-detail" type="button" data-dt-detail="${escapeHtml(row.autoId)}">เปิดข้อมูล</button>`;if(overview)return`<div class="dt-row"><b data-label="เลขนัดหมาย">${escapeHtml(row.appointmentNo||row.autoId)}</b>${company}${plate}${shift}<span data-label="สถานะ"><em class="dt-status ${statusTone(row.currentStatus)}">${statusLabel(row.currentStatus)}</em></span><b data-label="เวลารวม">${formatDuration(row.totalInSiteSeconds)}</b>${sla}<span data-label="ประตู">${escapeHtml(row.doorCode||"-")}</span>${actor}<span data-label="สั่งการ">${command}</span><span data-label="รายละเอียด">${detail}</span></div>`;return`<div class="dt-row"><b data-label="เลขนัดหมาย">${escapeHtml(row.appointmentNo||row.autoId)}</b>${company}${plate}${shift}<span data-label="${escapeHtml(labels[0])}">${formatDateShort(row.stageStartAt)}</span><span data-label="${escapeHtml(labels[1])}">${formatDateShort(row.stageEndAt)}</span><b data-label="ใช้เวลา">${row.stageSeconds==null?"-":formatDuration(row.stageSeconds)}</b>${sla}${actor}<span data-label="สั่งการ">${command}</span><span data-label="รายละเอียด">${detail}</span></div>`}).join(""):`<div class="empty-state"><b>ไม่พบข้อมูลตามตัวกรอง</b><span>ลองเปลี่ยนวันที่ สถานะ หรือระดับแจ้งเตือน</span></div>`;table.innerHTML=head+body;table.dataset.optionalColumns=String((datatableColumnOn("company")?1:0)+(datatableColumnOn("plate")?1:0)+(datatableColumnOn("shift")?1:0)+(datatableColumnOn("actor")?1:0));table.classList.toggle("is-overview",overview);const headRow=table.querySelector(".dt-head");if(headRow)table.style.setProperty("--dt-columns",String(Math.max(1,headRow.children.length-1)))}
...
function syncDatatableShiftHint(context=null){const el=$("dtShiftHint"),shift=context||datatableSelectedShift();if(!el)return;if(!shift){el.hidden=true;el.textContent="";return}const start=Number(shift.startMinute??shift.start_minute),end=Number(shift.endMinute??shift.end_minute),cross=shift.crossesMidnight??start>=end;el.hidden=false;el.textContent=cross?`${shift.shiftName||shift.shift_name} ${minuteToTime(start)}–${minuteToTime(end)} · นับเป็นวันที่เริ่มกะ`:`${shift.shiftName||shift.shift_name} ${minuteToTime(start)}–${minuteToTime(end)}`}
...
async function runDatatableCommand(autoId,sourceButton=null){const row=datatableState.data?.items?.find(item=>String(item.autoId)===String(autoId));if(!row||datatableState.detailBusy)return;const original=sourceButton?.textContent||"";if(sourceButton){sourceButton.disabled=true;sourceButton.textContent="กำลังเปิด"}try{datatableState.detailBusy=true;const context=await api(`/api/datatable/action-context?autoId=${encodeURIComponent(autoId)}`),vehicle=datatableInternalVehicle(context.vehicle||{});state.activeDoors=(context.activeDoors||[]).map(code=>String(code||"").toUpperCase());state.queueRecall=context.queueRecall||state.queueRecall;state.documentCheckEnabled=Boolean(context.documentCheckEnabled);const index=state.vehicles.findIndex(v=>String(v.auto_id)===String(autoId));if(index>=0)state.vehicles[index]=vehicle;else state.vehicles.unshift(vehicle);if(vehicle.current_status==="WAITING_DOCUMENT_CHECK"&&state.user?.accessRights==="ADMIN")return await confirmDocumentChecked(vehicle);if(["READY_FOR_RECEIVING","RECEIVING_IN_PROGRESS"].includes(vehicle.current_status))return await showDatatableReceivingActions(vehicle);return await openDatatableTrack(autoId)}catch(error){await showNotice("error",error.message||"ไม่สามารถสั่งการรายการนี้ได้")}finally{datatableState.detailBusy=false;if(sourceButton?.isConnected){sourceButton.disabled=false;sourceButton.textContent=original}}}
async function showDatatableReceivingActions(vehicle){if(!window.Swal)return;const progress=vehicle.current_status==="RECEIVING_IN_PROGRESS",called=Number(vehicle.queue_called_at||0)>0,buttons=progress?[["complete","รับสินค้าเสร็จ"],["reject","ปฏิเสธรับสินค้า"],["track","ติดตามสถานะ"]]:[[called?"recall":"call",called?"เรียกซ้ำ / เปลี่ยนประตู":"เรียกรถ"],["start","เริ่มตรวจรับ"],["reject","ปฏิเสธรับสินค้า"],["track","ติดตามสถานะ"]];const result=await Swal.fire({title:`สั่งการ ${escapeHtml(vehicle.appointment_no||vehicle.auto_id)}`,html:`<div class="dt-action-menu">${buttons.map(([key,label])=>`<button type="button" data-dt-action="${key}">${escapeHtml(label)}</button>`).join("")}</div>`,showConfirmButton:false,showCancelButton:true,cancelButtonText:"ปิด",customClass:swalClasses(),buttonsStyling:false,width:430,didOpen:()=>document.querySelectorAll("[data-dt-action]").forEach(btn=>btn.addEventListener("click",()=>{Swal.close();const action=btn.dataset.dtAction;if(action==="call")callVehicle(vehicle,false);else if(action==="recall")callVehicle(vehicle,true);else if(action==="start")startReceiving(vehicle);else if(action==="complete")completeReceiving(vehicle);else if(action==="reject")rejectReceiving(vehicle);else if(action==="track")openDatatableTrack(vehicle.auto_id)}))});return result}
async function openDatatableTrack(autoId){try{const result=await api("/api/track/link",{method:"POST",body:{search:autoId}}),token=String(result?.token||"").trim();if(!token)throw new Error("ไม่สามารถสร้างลิงก์ Track ได้");window.open(new URL(`./track.html?t=${encodeURIComponent(token)}&v=20260812-r95`,location.href).href,"_blank","noopener")}catch(error){await showNotice("error",error.message||"เปิด Track ไม่สำเร็จ")}}
async function showDatatableDetail(autoId,sourceButton=null){if(datatableState.detailBusy)return;const original=sourceButton?.textContent||"";if(sourceButton){sourceButton.disabled=true;sourceButton.textContent="กำลังเปิด"}datatableState.detailBusy=true;uiState.detailsOpen=true;try{if(window.Swal)Swal.fire({title:"กำลังเปิดข้อมูล",allowOutsideClick:false,allowEscapeKey:false,didOpen:()=>Swal.showLoading(),showConfirmButton:false,customClass:swalClasses(),width:380});const data=await api(`/api/datatable/detail?autoId=${encodeURIComponent(autoId)}`),v=data.vehicle||{},events=data.events||[],d=data.durations||{},rej=data.rejection;const stages=[["รถเข้า → ยื่นเอกสาร",d.gateToDocumentSeconds],["ตรวจเอกสาร",d.documentReviewSeconds],["รอตรวจรับ",d.readyToReceivingSeconds],["เรียก → เริ่มตรวจรับ",d.calledToReceivingSeconds],["ตรวจรับสินค้า",d.receivingSeconds],["คืนเอกสาร",d.receivingToReturnSeconds],["รอออกจากพื้นที่",d.returnToGateOutSeconds],["เวลารวม",d.totalInSiteSeconds]];const html=`<div class="dt-detail-modal"><section class="dt-detail-identity"><div><small>เลขนัดหมาย</small><b>${escapeHtml(v.appointmentNo||v.autoId||autoId)}</b><span>${escapeHtml(v.companyName||"ไม่ระบุบริษัท")}</span></div><div><small>ทะเบียน</small><b>${escapeHtml(joinText(v.vehiclePlate,v.province))}</b><span>${escapeHtml(statusLabel(v.currentStatus))}${v.shiftName?` · ${escapeHtml(v.shiftName)}${v.shiftBusinessDate?` (${escapeHtml(v.shiftBusinessDate.slice(8,10)+"/"+v.shiftBusinessDate.slice(5,7))})`:""}`:""}</span></div></section><section class="dt-detail-durations">${stages.map(([label,value])=>`<div><small>${label}</small><b>${value==null?"-":formatDuration(value)}</b></div>`).join("")}</section>${rej?`<section class="dt-rejection-detail"><b>ปฏิเสธรับสินค้า</b><span>เหตุผล: ${escapeHtml(rej.reason||"-")}</span><span>ผู้รับทราบ: ${escapeHtml(rej.supervisor||"-")}</span><span>${Number(rej.require_document_return)?"ต้องคืนเอกสาร":"ไม่ต้องคืนเอกสาร"}</span></section>`:""}<section class="dt-timeline"><h3>ประวัติการทำงาน</h3>${events.map(e=>`<div><i></i><time>${formatDate(e.occurred_at)}</time><span><b>${datatableEventLabel(e.event_type)}</b><small>${escapeHtml(e.actor||"ระบบ")}</small></span></div>`).join("")}</section></div>`;if(window.Swal)await Swal.fire({title:"ข้อมูลนัดหมาย",html,confirmButtonText:"ปิด",customClass:{...swalClasses(),popup:"wfv-swal dt-detail-swal"},buttonsStyling:false,width:760})}catch(error){if(window.Swal)Swal.close();await showNotice("error",error.message||"เปิดข้อมูลไม่สำเร็จ")}finally{datatableState.detailBusy=false;uiState.detailsOpen=false;if(sourceButton?.isConnected){sourceButton.disabled=false;sourceButton.textContent=original}}}
